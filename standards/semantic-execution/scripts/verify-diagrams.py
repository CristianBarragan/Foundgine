...
for p in sorted(pumls - svgs):
    errors.append(f"Missing SVG for {p}.puml")
for s in sorted(svgs - pumls):
    errors.append(f"SVG has no PlantUML source: {s}.svg")
for md in sorted(root.glob("*.md")):
    t = md.read_text(encoding="utf-8")
    for rel in re.findall(r"!\[[^\]]*\]\((diagrams/[^)]+\.svg)\)", t):
        if not (root / rel).exists():
            errors.append(f"{md.name}: missing {rel}")
...
if readme.exists():
    ...
    if re.search(r"```text\s*\n\s*Caller\s*\n.*?Evidence\s*\n```", t, re.S):
        errors.append(
            "README.md: canonical architecture diagram must be PlantUML-backed, not an inline text diagram"
        )
    for rel in re.findall(r"!\[[^\]]*\]\((diagrams/[^)]+\.svg)\)", t):
        stem = Path(rel).stem
...
if errors:
    ...
    sys.exit(1)
print(
    f"OK: {len(pumls)} PlantUML sources have matching SVGs and all Markdown embeds resolve."
)
