 ...
 * {@code Semantic/Tests/OpenIntentSupplyChainTests.cs} — the Open Intent
 * (read + mutation) surface exercised directly against the real Supply Chain
...
class OpenIntentSupplyChainParityTest {
    ...
    void productShipmentsIsAnOpenLogicalTraversalOverTheRealSupplyChainPath() {
        ...
        var request = new ReadIntent("Product",
                List.of(new ReadSelection(null, "shipments",
                        List.of(new ReadSelection("Status")))));

        var semanticRequest = new ReadIntentCompiler(model).compile(request);
        var graph = new SemanticRequestResolver(new SemanticContractSnapshot(model.freeze()))
        ...
        var status = model.get(nodes.get(3).entityId()).fields().stream()
    ...
    void openSupplyChainMutationCoversGeneratedIdentityValueFlowAndBranching() {
        ...
        var linePurchaseOrderId = model.resolveEntity("PurchaseOrderLine").fields().stream()
        ...
        assertTrue(plan.dependencies().stream()
                .anyMatch(d -> d.toOperationId().equals("1") && d.targetField().equals(linePurchaseOrderId)));
        assertTrue(plan.dependencies().stream()
                .anyMatch(d -> d.toOperationId().equals("2") && d.targetField().equals(shipmentPurchaseOrderId)));
