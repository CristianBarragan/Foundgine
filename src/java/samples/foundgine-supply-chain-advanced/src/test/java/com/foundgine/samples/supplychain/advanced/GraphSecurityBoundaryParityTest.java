...
class GraphSecurityBoundaryParityTest {
    ...
    void deep_bom_traversal_is_rejected_at_the_configured_graph_depth() {
        ...
        var ex =
                assertThrows(
                        ...
                        () -> compiler.compileOperationGraph(intent, tight));
    ...
    void denied_supplier_incidents_are_pruned_but_authorized_role_keeps_them() {
        ...
        assertTrue(
                denied.graph().nodes().stream()
                        .noneMatch(
                                n ->
                                        n.entityId()
                                                .equals(
                                                        SupplyChainSemanticModel
                                                                .COMPLIANCE_INCIDENT)));

        var allowed =
                new SemanticAuthorizer(
                                SupplyChainAuthorization.create(
                                        "tenant-a", SupplyChainAuthorization.Role.ANALYST))
                        .authorizeGraphWithEvidence(contract, graph);
        assertEquals(2, allowed.graph().nodes().size());
        assertTrue(
                allowed.graph().nodes().stream()
                        .anyMatch(
                                n ->
                                        n.entityId()
                                                .equals(
