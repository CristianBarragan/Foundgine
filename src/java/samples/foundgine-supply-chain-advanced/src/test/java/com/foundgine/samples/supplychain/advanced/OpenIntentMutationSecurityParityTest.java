 ...
 * {@code Semantic/Tests/OpenIntentMutationSecurityTests.cs} — Supply Chain
 * mutation cases that deliberately exercise the open mutation surface beyond
 * CRUD: branching generated-value flow, target filters, upsert conflicts,
...
class OpenIntentMutationSecurityParityTest {
    ...
    void purchaseOrderFanOutPreservesIdentityFlowToLineAndShipment() {
        ...
        plan.dependencies().forEach(dependency -> assertEquals(0, Integer.parseInt(dependency.fromOperationId())));
        var purchaseOrderLineId = model.resolveEntity("PurchaseOrderLine").fields().stream()
                .filter(f -> f.name().equals("PurchaseOrderId")).findFirst().orElseThrow().id();
        var shipmentPurchaseOrderId = model.resolveEntity("Shipment").fields().stream()
                .filter(f -> f.name().equals("PurchaseOrderId")).findFirst().orElseThrow().id();
        plan.dependencies().forEach(dependency -> assertTrue(
                dependency.targetField().equals(purchaseOrderLineId)
                        || dependency.targetField().equals(shipmentPurchaseOrderId)));
    }
    ...
    void upsertRequiresExplicitConflictSemantics() {
        var model = SupplyChainSemanticModel.build();
        var withoutConflict = new SemanticMutationIntentBuilder(model).upsert("PurchaseOrder").set("SupplierId", 1);
    ...
    void mutationFieldAndEntityNamesAreResolvedBeforePlanning() {
        ...
        assertThrows(IllegalStateException.class, () -> new SemanticMutationIntentBuilder(model)
    ...
    void mutationDependenciesCannotReferenceAFutureOperation() {
        ...
        assertThrows(IllegalStateException.class, () -> builder.setFrom("PurchaseOrderId", "order", "Id"));
    }
