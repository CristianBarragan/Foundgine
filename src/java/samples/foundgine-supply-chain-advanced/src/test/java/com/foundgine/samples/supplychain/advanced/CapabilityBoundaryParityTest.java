    ...
    void leafProjectionNeverLeaksBackingOnlyField() {
        ...
        metadata.register(
                new EntityMetadata(
                        ...
                        List.of(
                                new FieldMetadata(
                                        SUPPLIER_ID, "Id", Integer.class, ref(SUPPLIER, 1)),
                                new FieldMetadata(
                                        SUPPLIER_NAME, "Name", String.class, ref(SUPPLIER, 2))),
                        ref(SUPPLIER, 1),
                        null,
                        false,
                        null,
        ...
        var result =
                new InMemoryExecutionProvider(metadata, data)
                        .executeAsync(
                                new InMemoryCompiler(metadata, data).compile(ir),
        ...
        assertFalse(
                row.effectiveCells().keySet().stream().anyMatch(k -> k.fieldId().equals(internal)));
    }
    ...
    void traversalNeverLeaksRelationshipJoinKey() {
        ...
        metadata.register(
                new EntityMetadata(
                        ...
                        List.of(
                                new FieldMetadata(
                                        PO_ID, "Id", Integer.class, ref(PURCHASE_ORDER, 1)),
                                new FieldMetadata(
                                        PO_STATUS, "Status", String.class, ref(PURCHASE_ORDER, 3)),
                                new FieldMetadata(
                                        PO_SUPPLIER_FK,
                                        "SupplierId",
                                        Integer.class,
                                        ref(PURCHASE_ORDER, 2))),
                        ref(PURCHASE_ORDER, 1),
                        null,
                        false,
                        null,
                        null));
        ...
        var data =
                new InMemoryDataSet()
                        .add(
                                new InMemoryRow(
                                        SUPPLIER,
                                        Map.of(SUPPLIER_ID, 1, SUPPLIER_NAME, "Kiwi Components")))
                        .add(
                                new InMemoryRow(
                                        PURCHASE_ORDER,
                                        Map.of(PO_ID, 500, PO_STATUS, "Open", PO_SUPPLIER_FK, 1)));
        var child =
                new ExecutionIRNode(
                        1,
        ...
        assertFalse(
                row.effectiveCells().keySet().stream()
                        .anyMatch(k -> k.fieldId().equals(PO_SUPPLIER_FK)));
    }
