 ...
 * Semantic/Tests/Mcp/AuthorizationMcpPenetrationTests.cs} — MCP-boundary adversarial cases. These
 * intentionally behave like Run 5: the caller is treated as untrusted and attempts to use a valid
 * MCP tool in a way that should cross a semantic authorization boundary.
 */
class AuthorizationMcpPenetrationParityTest {
    ...
    void claimsAttackMatrixDefinesTheRequiredClaimValidationBoundaries() {
        ...
        var claimNarrowingUses =
                List.of(
                        "self-imposed-read-only", // scope=read-only restricts a manager's own
                        // writes
                        ...
                        "valid-reconcile-evidence" // well-formed reason + change_ticket allows
                        // reconcile
                        );
    ...
    void rawMcpRequestHasExpectedJsonRpcShape() throws Exception {
        ...
        if (!"1".equals(System.getenv("RUN_SUPPLYCHAIN_MCP_TESTS"))) return;
