 ...
 * Semantic/Tests/Grounding/SupplyChainAliasWeightTests.cs}.
 *
 * <p>Weights are evidence metadata, not retrieval scores, and never grant authority. The gate is
 * deliberately exercised separately from lexical resolution so a low-confidence signal cannot
 * silently become an authorization decision.
 */
class SupplyChainAliasWeightParityTest {
    ...
    void aotGeneratedSupplyChainContractPreservesWeightedEntityFieldAndRelationshipAliases() {
        ...
        var country =
                supplier.fields().stream()
                        ...
                        .orElseThrow();
        assertEquals(
                List.of("State"), country.effectiveAliases().stream().map(a -> a.name()).toList());
        assertEquals(
                List.of(85), country.effectiveAliases().stream().map(a -> a.weight()).toList());

        var purchaseOrder = contract.resolveEntity("PurchaseOrder");
        assertEquals(
                List.of("PO", "POs", "Buy", "Buys"),
                purchaseOrder.effectiveAliases().stream().map(a -> a.name()).toList());
        assertEquals(
                List.of(100, 95, 90, 85),
                purchaseOrder.effectiveAliases().stream().map(a -> a.weight()).toList());

        var dueDate =
                purchaseOrder.fields().stream()
                        .filter(f -> f.name().equals("ExpectedArrival"))
                        .findFirst()
                        .orElseThrow();
        assertEquals(
                List.of("DueDate"),
                dueDate.effectiveAliases().stream().map(a -> a.name()).toList());
        assertEquals(
                List.of(90), dueDate.effectiveAliases().stream().map(a -> a.weight()).toList());

        var supplierRel =
                purchaseOrder.relationships().stream()
                        .filter(r -> r.name().equals("supplier"))
                        .findFirst()
                        .orElseThrow();
        assertEquals(
                List.of("vendor"),
                supplierRel.effectiveAliases().stream().map(a -> a.name()).toList());
        assertEquals(
                List.of(85), supplierRel.effectiveAliases().stream().map(a -> a.weight()).toList());
    }
    ...
    void supplyChainWeightedAliasEvidenceOnlyCountsTheDeclaredLexicalIdentity() {
        var result =
                AliasWeightEvidenceGate.evaluate(
                        SupplyChainSemanticModel.MODEL,
                        80,
        ...
        assertEquals(
                List.of(SupplyChainSemanticModel.SUPPLIER),
                List.copyOf(result.entityWeights().keySet()));
    ...
    void supplyChainWeightedExampleCoversEntityFieldAndRelationshipAliases() {
        ...
        var purchaseOrder =
                SupplyChainSemanticModel.MODEL.get(SupplyChainSemanticModel.PURCHASE_ORDER);
    ...
    void loweringTheThresholdChangesOnlyTheEvidenceGateNotTheAliasIdentity() {
        ...
        assertTrue(
                supplier.effectiveAliases().stream()
                        .anyMatch(a -> a.name().equals("Seller") && a.weight() == 90));
    }
    ...
    void fieldWeightDoesNotBecomeSupplierEntityWeight() {
        var field =
                SupplyChainSemanticModel.MODEL
                        .get(SupplyChainSemanticModel.SUPPLIER)
                        .fields()
                        .stream()
                        .filter(f -> f.name().equals("Country"))
                        .findFirst()
        ...
        var result =
                AliasWeightEvidenceGate.evaluate(
                        SupplyChainSemanticModel.MODEL,
                        90,
                        lexicalField("State", SupplyChainSemanticModel.SUPPLIER, field.id()),
                        false);
    ...
    private static SemanticLexicalResolution lexicalField(
            String token, EntityId entityId, FieldId fieldId) {
        var candidate =
        ...
        return new SemanticLexicalResolution(
                ...
                List.of(new SemanticLexicalStep(token, candidate, .99, List.of())),
                .99,
                entityId,
                null);
    }
