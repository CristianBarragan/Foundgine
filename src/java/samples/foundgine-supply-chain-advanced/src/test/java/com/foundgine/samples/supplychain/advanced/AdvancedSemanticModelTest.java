...
class AdvancedSemanticModelTest {
    @Test
    void semanticContractContainsAdvancedSupplyChainTopology() {
        ...
        assertTrue(
                model.get(SupplyChainSemanticModel.PRODUCT).relationships().stream()
                        .anyMatch(r -> r.name().equals("components")));
        assertTrue(
                model.get(SupplyChainSemanticModel.SUPPLIER).relationships().stream()
                        .anyMatch(r -> r.name().equals("certifications")));
    }

    @Test
    void productAliasesAndConstraintsArePartOfSemanticContract() {
        var product = SupplyChainSemanticModel.MODEL.get(SupplyChainSemanticModel.PRODUCT);
        assertTrue(
                product.effectiveAliases().stream()
                        .anyMatch(a -> a.name().equalsIgnoreCase("Item")));
        var sku =
                product.fields().stream()
                        ...
                        .orElseThrow();
        assertTrue(
                sku.effectiveAliases().stream()
                        .anyMatch(a -> a.name().equalsIgnoreCase("PartNumber")));
        assertTrue(
    ...
    @Test
    void warehouseClaimNarrowsSemanticAuthorizationPredicate() {
        var policy =
        ...
        var predicate =
                policy.getPredicate(
                        SupplyChainSemanticModel.INVENTORY_LOT, AuthorizationOperation.READ);
        assertNotNull(predicate);
        assertEquals(AuthorizationPredicateKind.EQUAL, predicate.kind());
    }

    @Test
    void sensitiveFieldsRemainRoleBound() {
        var analyst =
                SupplyChainAuthorization.create(
                        "tenant-a", SupplyChainAuthorization.Role.ANALYST, Map.of());
        var customer =
                SupplyChainAuthorization.create(
                        "tenant-a", SupplyChainAuthorization.Role.CUSTOMER, Map.of());
        assertTrue(
                analyst.canAccessField(
                        SupplyChainSemanticModel.SUPPLIER,
                        SupplyChainAuthorization.field("Supplier", "RiskScore")));
        assertFalse(
                customer.canAccessField(
                        SupplyChainSemanticModel.SUPPLIER,
                        SupplyChainAuthorization.field("Supplier", "RiskScore")));
        assertFalse(
                analyst.canAccessField(
    ...
    @Test
    void lexicalAliasCanCommitToProduct() {
        var source =
        ...
        var resolver =
                new SemanticLexicalResolver(
                        SupplyChainSemanticModel.MODEL.createSnapshot(), source);
