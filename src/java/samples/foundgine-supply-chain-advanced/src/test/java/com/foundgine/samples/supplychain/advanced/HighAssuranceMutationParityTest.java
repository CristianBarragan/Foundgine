...
class HighAssuranceMutationParityTest {
        ...
        var result =
                new PlaceOrderService(data)
                        .placeOrder(
                                ...
                                1,
                                List.of(
                                        new PlaceOrderService.OrderLine(4, 1),
                                        new PlaceOrderService.OrderLine(4, 2)),
                                "duplicate-lines");

        assertEquals(new BigDecimal("102.00"), result.total());
        assertEquals(
                1, data.orderItems.stream().filter(x -> x.orderId() == result.orderId()).count());
        assertEquals(
                3,
        ...
        var order =
                new PlaceOrderService(data)
                        .placeOrder(
                                ...
                                placeAuth,
                                1,
                                List.of(new PlaceOrderService.OrderLine(4, 2)),
                                "cancel-boundary-place");
        var before =
    ...
    void cancelled_order_cannot_be_cancelled_again_with_a_new_key() {
        ...
        var order =
                place.placeOrder(
                        ...
                        auth,
                        1,
                        List.of(new PlaceOrderService.OrderLine(4, 1)),
                        "already-cancelled-place");
        var cancel = new CancelOrderService(data);
        cancel.cancelOrder("alice", auth, order.orderId(), "first-cancel");

        assertThrows(
                IllegalStateException.class,
                () -> cancel.cancelOrder("alice", auth, order.orderId(), "second-cancel"));
    }
    ...
    void cancellation_restores_the_exact_allocated_lot() {
        ...
        new CancelOrderService(data)
                .cancelOrder("alice", auth, order.orderId(), "exact-lot-cancel");
        assertEquals(
                before900,
                data.inventory.stream()
                        .filter(x -> x.id() == 900)
