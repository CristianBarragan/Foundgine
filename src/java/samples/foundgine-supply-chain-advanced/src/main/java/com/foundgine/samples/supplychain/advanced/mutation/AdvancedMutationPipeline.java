...
/**
 * End-to-end Advanced sample mutation boundary: Intent -> semantic planning -> authorization ->
 * execution IR -> provider -> domain mutation. The provider is deliberately an application adapter;
 * the runtime remains provider-neutral.
 */
public final class AdvancedMutationPipeline {
    ...
    public AdvancedMutationPipeline(
        ...
        var policy =
                SupplyChainAuthorization.create(
                        auth.tenantId(),
                        SupplyChainAuthorization.Role.valueOf(auth.role().name()),
                        Map.of());
        this.engine =
                new FoundgineMutationEngine(
                        schema,
                        policy,
    ...
    public CompletionStage<MutationExecutionResult> placeOrder(
            String actor, int customerId, int productId, int quantity, String idempotencyKey) {
        var op =
                new SemanticMutationOperation(
                        PLACE_ORDER,
                        SemanticMutationKind.CREATE,
                        List.of(
                                value(PLACE_ORDER, "Actor", actor),
                                value(PLACE_ORDER, "CustomerId", customerId),
                                value(PLACE_ORDER, "ProductId", productId),
                                value(PLACE_ORDER, "Quantity", quantity),
                                value(PLACE_ORDER, "IdempotencyKey", idempotencyKey)),
                        null,
                        List.of(),
                        List.of(field(PLACE_ORDER, "OrderId")),
                        List.of(),
                        List.of());
        return engine.executeAsync(
                new SemanticMutationRequest(new SemanticMutationOperationGraph(List.of(op))),
                ExecutionContext.EMPTY,
                CancellationToken.NONE);
    }

    public CompletionStage<MutationExecutionResult> cancelOrder(
            String actor, int orderId, String idempotencyKey) {
        var op =
                new SemanticMutationOperation(
                        CANCEL_ORDER,
                        SemanticMutationKind.CREATE,
                        List.of(
                                value(CANCEL_ORDER, "Actor", actor),
                                value(CANCEL_ORDER, "OrderId", orderId),
                                value(CANCEL_ORDER, "IdempotencyKey", idempotencyKey)),
                        null,
                        List.of(),
                        List.of(field(CANCEL_ORDER, "OrderId")),
                        List.of(),
                        List.of());
        return engine.executeAsync(
                new SemanticMutationRequest(new SemanticMutationOperationGraph(List.of(op))),
    ...
    private static FieldId field(EntityId entity, String name) {
        return FieldId.create(
                entity == PLACE_ORDER ? "PlaceOrderCommand" : "CancelOrderCommand", name);
    }

    private static FieldId field(EntityId entity, String name, boolean ignored) {
        return field(entity, name);
    }

    /**
     * Command capabilities are semantic entities, but they are not part of the generated
     * supply-chain/domain topology or the manual semantic overlay.
     *
     * <p>Keep this model local to the mutation boundary so the command contract cannot contaminate
    ...
    private static SemanticModel mutationModel() {
        ...
        builder.entity(
                ...
                e -> {
                    ...
                    e.field(
                            FieldId.create("PlaceOrderCommand", "Actor"),
                            "Actor",
                            String.class,
                            null,
                            (byte)
                    ...
                    e.field(
                            FieldId.create("PlaceOrderCommand", "CustomerId"),
                            "CustomerId",
                            int.class,
                            null,
                            (byte)
                                    (SemanticFieldCapabilities.DEFAULT
                                            | SemanticFieldCapabilities.WRITABLE));
                    e.field(
                            FieldId.create("PlaceOrderCommand", "ProductId"),
                            "ProductId",
                            int.class,
                            null,
                            (byte)
                                    (SemanticFieldCapabilities.DEFAULT
                                            | SemanticFieldCapabilities.WRITABLE));
                    e.field(
                            FieldId.create("PlaceOrderCommand", "Quantity"),
                            "Quantity",
                            int.class,
                            null,
                            (byte)
                                    (SemanticFieldCapabilities.DEFAULT
                                            | SemanticFieldCapabilities.WRITABLE));
                    e.field(
                            FieldId.create("PlaceOrderCommand", "IdempotencyKey"),
                            "IdempotencyKey",
                            String.class,
                            null,
                            (byte)
        ...
        builder.entity(
                ...
                e -> {
                    ...
                    e.field(
                            FieldId.create("CancelOrderCommand", "Actor"),
                            "Actor",
                            String.class,
                            null,
                            (byte)
                    ...
                    e.field(
                            ...
                            (byte)
                                    (SemanticFieldCapabilities.DEFAULT
                                            | SemanticFieldCapabilities.WRITABLE));
                    e.field(
                            FieldId.create("CancelOrderCommand", "IdempotencyKey"),
                            "IdempotencyKey",
                            String.class,
                            null,
                            (byte)
    ...
    private static MutationSchema schema(SemanticModel model) {
        return new MutationSchema() {
            ...
            public MutationEntitySchema getEntity(EntityId id) {
                ...
                for (var f : e.fields()) {
                    var c = ColumnId.create(e.name(), f.name());
                    fields.put(f.id(), c);
                    columns.add(c);
                }
            ...
            public MutationRelationshipSchema getRelationship(RelationshipId id) {
                throw new UnsupportedOperationException(
                        "Command mutations do not use relationship filters.");
            }
        };
    }

    private static final class DomainMutationProvider
            implements IMutationBatchExecutionProvider, IMutationSecurityConformanceEvaluator {
        private final SupplyChainData data;
        private final Authorization.Context auth;

        DomainMutationProvider(SupplyChainData data, Authorization.Context auth) {
            this.data = data;
            this.auth = auth;
        }
            ...
            for (var op : ir.operations()) {
                if (op.kind() != com.foundgine.core.semantic.planning.mutation.MutationKind.CREATE)
                    throw new IllegalArgumentException(
                            "Advanced command provider only accepts CREATE command operations.");
                ...
                if ("PlaceOrderCommand".equals(name)) {
                    ...
                    var r =
                            service.placeOrder(
                                    ...
                                    List.of(
                                            new PlaceOrderService.OrderLine(
                                                    number(values, "ProductId"),
                                                    number(values, "Quantity"))),
                                    (String) values.get("IdempotencyKey"));
                    results.add(
                            new MutationResult(
                                    1, Map.of(field(PLACE_ORDER, "OrderId"), r.orderId())));
                    ...
                    results.add(
                            new MutationResult(
                                    1, Map.of(field(CANCEL_ORDER, "OrderId"), r.orderId())));
                } else
                    throw new IllegalArgumentException(
                            "Unknown Advanced mutation command: " + name);
            }
            return new MutationBatchResult(results);
        }

        private static Map<String, Object> values(
                com.foundgine.core.semantic.planning.mutation.MutationOperation op) {
            var out = new LinkedHashMap<String, Object>();
            for (var f : op.fields()) {
                if (f.source() != null)
                    throw new UnsupportedOperationException(
                            "Command provider does not support field correlation yet.");
        ...
        private static String knownFieldName(String entity, FieldId id) {
            for (String name :
                    List.of(
                            ...
                            "Quantity",
                            "IdempotencyKey",
                            "OrderId")) if (FieldId.create(entity, name).equals(id)) return name;
