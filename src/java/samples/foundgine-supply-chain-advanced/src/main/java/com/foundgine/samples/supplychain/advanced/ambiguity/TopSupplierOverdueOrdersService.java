...
/**
 * Java parity port of the C# advanced sample's ambiguity-resolution demo capability, {@code
 * find_top_supplier_overdue_orders} in {@code MCP.Foundgine/Program.cs} (backed by the schema in
 * {@code Database/Program.cs}).
 *
 * <p>"top supplier in &lt;state&gt;" is not a database key, so it is resolved through ranked
 * candidates plus evidence, not silently guessed: an unambiguous top-by-value supplier resolves and
 * executes; a tie at the top, or a supplier name that doesn't match anything, comes back as {@code
 * clarification_needed}/{@code not_found} with candidates and evidence instead of a guess. When the
 * caller (agent) has already been told candidates are tied and comes back with a specific name,
 * that closes the loop into a resolved result instead of asking again.
 *
 * <p><b>Known simplification vs. the C# original:</b> the C# version stamps every resolved response
 * with a real {@code SemanticPlan}/execution-IR fingerprint from the full planner + authorizer +
 * {@code ExecutionIRCompiler} chain. Wiring that whole chain for this one demo capability was out
 * of scope for this port; {@link #planFingerprint()} instead returns a stable hash of the (fixed)
 * Supplier read shape this capability always executes. The externally-visible shape of the response
 * ({@code plan} as an opaque short string) is preserved even though its provenance is weaker.
 */
public final class TopSupplierOverdueOrdersService {
    ...
    public Map<String, Object> findTopSupplierOverdueOrders(
            String actor, String state, String supplierName) throws SQLException {
        ...
        if (supplierName != null && !supplierName.isBlank()) {
            ...
            if (named.isEmpty()) {
                ...
                if (approx != null && !approx.matches().isEmpty()) {
                    ...
                    response.put(
                            ...
                            "'"
                                    + supplierName
                                    + "' does not exactly match any supplier in state '"
                                    + state
                                    + "', but "
                                    + approx.matches().size()
                                    + " looked similar via "
                    ...
                    response.put(
                            ...
                            List.of(
                                    "Re-call with the exact supplierName from the candidates"
                                            + " above.",
                ...
                response.put(
                        ...
                        "'"
                                + supplierName
                                + "' does not match any supplier in state '"
                                + state
                                + "'.");
                response.put(
                        "strategiesTried",
            ...
            var tiedAtTop =
                    candidates.stream()
                            .filter(
                                    x ->
                                            topValue.compareTo(
                                                            (BigDecimal) x.get("total_order_value"))
                                                    == 0)
                            .toList();
            ...
            if (tiedAtTop.size() > 1) {
                ...
                response.put(
                        "reason",
                        tiedAtTop.size()
                                + " suppliers are tied for 'top' by total order value in "
                                + "state '"
                                + state
                                + "'; the request cannot be resolved to one supplier without more "
                                + "specific intent.");
                response.put(
                        "candidates",
                        tiedAtTop.stream()
                ...
                response.put(
                        ...
                        List.of(
                                ...
                                "Give a tiebreak criterion (for example: most recent purchase"
                                        + " order, or lowest lead time).",
                                "Narrow to a more specific region than the state."));
                return response;
            }
        ...
        var overduePurchaseOrders =
                overdueRows.stream()
                        .map(
                                r -> {
                                    LocalDate expected =
                                            ((java.sql.Date) r.get("expected_date")).toLocalDate();
                                    Map<String, Object> row = new LinkedHashMap<>();
    ...
    // of them do.
    private ApproximateMatch tryApproximateSupplierMatch(String state, String supplierName)
            throws SQLException {
        var fuzzy = tryFuzzy(state, supplierName);
        if (!fuzzy.isEmpty()) return new ApproximateMatch("fuzzy", fuzzy);

        var fullText = tryFullText(state, supplierName);
        if (!fullText.isEmpty()) return new ApproximateMatch("fulltext", fullText);

        if (pgSearchEnabled()) {
            var search = trySearch(state, supplierName);
            if (!search.isEmpty()) return new ApproximateMatch("search", search);
        }
    ...
    private static List<Map<String, Object>> readRows(PreparedStatement stmt) throws SQLException {
        try (var rs = stmt.executeQuery()) {
            ...
            while (rs.next()) {
                ...
                for (int i = 1; i <= columns; i++) row.put(meta.getColumnLabel(i), rs.getObject(i));
                rows.add(row);
            }
            return rows;
        }
    ...
    private static String planFingerprint() {
        return hash(
                "{\"entity\":\"Supplier\",\"fields\":[\"Id\",\"Name\",\"State\",\"TotalOrderValue\","
                    + "\"NegotiatedCost\"]}");
    }
