...
public final class CancelOrderService {
    ...
    public CancelOrderService(SupplyChainData data) {
    ...
    public Result cancelOrder(String actor, Authorization.Context auth, int orderId, String key) {
        ...
        synchronized (lockFor(key)) {
            var prior =
                    data.cancellationIdempotency.stream()
            ...
            var customer =
                    data.customers.stream()
                            .filter(x -> x.id() == order.customerId())
                            .findFirst()
                            .orElseThrow(
                                    () -> new IllegalStateException("Order customer not found."));
            ...
            if (!"Pending".equalsIgnoreCase(order.status()))
                throw new IllegalStateException(
                        "Only pending orders can be cancelled by this boundary.");
            ...
            try {
                ...
                for (var allocation :
                    ...
                    var candidate =
                            data.inventory.stream()
                                    .filter(x -> x.id() == allocation.lotId())
                                    .filter(x -> auth.allowedWarehouses().contains(x.warehouseId()))
                                    .findFirst()
                                    .orElseThrow(
                                            () ->
                                                    new IllegalStateException(
                                                            "Allocated inventory lot cannot be"
                                                                    + " resolved."));
                    data.inventory.set(
                            data.inventory.indexOf(candidate),
                            new InventoryLot(
                                    candidate.id(),
                                    ...
                                    candidate
                                            .onHand()
                                            .add(BigDecimal.valueOf(allocation.quantity())),
                                    candidate.reserved(),
                ...
                data.orders.set(
                        ...
                        new Order(
                                ...
                                order.totalAmount(),
                                order.placedOn()));
                data.cancellationIdempotency.add(
                        new CancellationIdempotencyRecord(
                                key,
                                actor,
                                orderId,
                                requestFingerprint,
                ...
                return new Result(
                        ...
                        restored,
                        planFingerprint(),
                        evidence(actor, orderId, key, restored));
            } catch (RuntimeException ex) {
                data.orders.clear();
                data.orders.addAll(orders);
                data.inventory.clear();
                data.inventory.addAll(inventory);
                data.cancellationIdempotency.clear();
                data.cancellationIdempotency.addAll(idem);
                throw ex;
            }
        }
    }
    ...
    private String planFingerprint() {
        return sha256(
                        "cancel_order|"
                                ...
                                + "|Order|OrderItem|OrderAllocation|InventoryLot")
                .substring(0, 24);
    }

    private static String evidence(String actor, int order, String key, BigDecimal restored) {
        return sha256(
                "cancel_order|" + actor + "|" + order + "|" + key + "|" + restored.toPlainString());
    }
