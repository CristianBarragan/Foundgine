 ...
 *
 * <p>
 * This is the Java counterpart of the C# {@code Semantic/McpClient} console
 * client: it speaks plain JSON-RPC {@code tools/call} over HTTP to
 * {@link AdvancedMcpServer}, with no SDK dependency, exactly like the C#
 * sample's raw-HTTP client. Run {@link AdvancedMcpServer} first, then run
 * this class (optionally passing the MCP endpoint URL as the first
 * argument).
 * </p>
 */
public final class AdvancedMcpClientDemo {
    ...
    public static void main(String[] args) throws Exception {
        URI endpoint = URI.create(args.length > 0 ? args[0]
                : "http://" + AdvancedMcpServer.DEFAULT_HOST + ":" + AdvancedMcpServer.DEFAULT_PORT
        ...
        var replay = demo.callMutation(
                "{\"tool\":\"place_order\",\"actor\":\"alice\",\"customerId\":1,\"productId\":4,"
                        + "\"quantity\":2,\"idempotencyKey\":\"mcp-client-demo-place-1\"}");
        demo.report("place_order (idempotent replay)", replay);

        var cancelled = demo.callMutation(
                "{\"tool\":\"cancel_order\",\"actor\":\"alice\",\"orderId\":" + orderId
    ...
    private void report(String label, JsonNode rpcResponse) {
        ...
        String text = result.path("content").isArray() && result.path("content").size() > 0
    ...
    private static int extractFirstReturnedValue(JsonNode rpcResponse) {
        try {
            ...
            throw new IllegalStateException("Unable to extract order id from place_order response", e);
        }
    }
}
