 ...
 *
 * <p>MCP remains an external actor boundary: JSON is decoded into semantic commands and then handed
 * to the Runtime mutation pipeline. No MCP-specific concepts leak into the domain mutation
 * services.
 */
public final class AdvancedMcpFacade {
    ...
    public AdvancedMcpFacade(SupplyChainData data, Authorization.Context auth) {
        this.pipeline =
                new AdvancedMutationPipeline(
                        Objects.requireNonNull(data), Objects.requireNonNull(auth));
            ...
            if ("capabilities".equals(tool)) {
                return CompletableFuture.completedFuture(
                        Map.of(
                                "tools",
                                        new String[] {
                                            "place_order", "cancel_order", "capabilities"
                                        },
