...
public final class OpenIntent {
    ...
    public record Selection(String path, List<String> fields) {
        public Selection {
            fields = List.copyOf(fields);
        }
    }

    public record GroundedIntent(
            String text,
    ...
    public static GroundedIntent validate(GroundedIntent intent) {
        ...
        if (intent.selections().isEmpty())
            throw new IllegalArgumentException("Open intent must contain at least one selection.");
        if (intent.limit() != null && (intent.limit() < 1 || intent.limit() > 1000))
