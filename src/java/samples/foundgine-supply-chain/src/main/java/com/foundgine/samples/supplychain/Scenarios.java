...
public final class Scenarios {
    public record SupplierExposure(
            int productId, int supplierId, int depth, boolean cycleDetected) {}
    ...
    public static List<SupplierExposure> recursiveSupplierRisk(
            Map<Integer, List<Integer>> bom, int root, int maxDepth) {
    ...
    private static void walk(
            Map<Integer, List<Integer>> bom,
            int p,
            int depth,
            int max,
            Set<Integer> path,
            Set<Integer> visited,
            List<SupplierExposure> out) {
        if (depth > max) return;
        if (!path.add(p)) {
            out.add(new SupplierExposure(p, 0, depth, true));
            return;
        }
        if (!visited.add(p)) {
            path.remove(p);
        ...
        for (int child : bom.getOrDefault(p, List.of()))
            walk(bom, child, depth + 1, max, path, visited, out);
        path.remove(p);
    }
