...
public final class PostgresBatchedMutationExecutionProvider
	 ...
	 * {@code ownsTransaction} is false, the provider participates in the caller's
	 * current JDBC transaction and never commits or rolls it back.
	 */
	public PostgresBatchedMutationExecutionProvider(Connection connection, IMetadataProvider metadata,
	...
	public MutationSecurityConformanceResult evaluate(ExecutionMutationIR ir) {
		...
		if (compiler.tryCompile(ir) == null)
			fallbackCompiler.compile(ir);
		return new MutationSecurityConformanceResult(getClass().getName(),
				List.of(SecurityInvariantIds.PARAMETERIZED_VALUES, SecurityInvariantIds.ATOMIC_MUTATION), List.of());
	}
	...
	public MutationBatchResult executeBatch(MutationBatchPlan plan, ExecutionContext context) {
		...
		if (batched != null)
			return executeBatchedPlan(batched, CancellationToken.NONE);
	...
	public MutationBatchResult executeBatch(ProviderMutationBatchPlan plan, ExecutionContext context,
		...
		throw new IllegalArgumentException(
				"The PostgreSQL batched mutation provider requires a SqlBatchedMutationPlan or SqlMutationBatchPlan.");
	}
					...
					try (ResultSet reader = statement.getResultSet()) {
						while (reader.next()) {
							...
							try {
								...
								throw new IllegalStateException("Invalid PostgreSQL batched mutation JSON result.", ex);
							}
							...
							if (group.ordinalAddressable()) {
								...
								if (ordinalNode == null || !ordinalNode.canConvertToInt())
									throw new IllegalStateException("Batched mutation group '" + groupId
											+ "' did not return a valid __fg_corr correlation value.");
								int ordinal = ordinalNode.intValue();
								if (ordinal < 1 || ordinal > group.operationIndexes().size())
									throw new IllegalStateException("Batched mutation group '" + groupId
											+ "' returned invalid ordinal '" + ordinal + "'.");
								operationIndex = group.operationIndexes().get(ordinal - 1);
								if (!rowKeys.containsKey(groupId + ":" + ordinal))
									throw new IllegalStateException(
											"Batched mutation returned an unmapped row ordinal '" + ordinal
													+ "' for group '" + groupId + "'.");
							} else {
							...
							while (fields.hasNext()) {
								...
								if (name.equals("__fg_corr") || name.equals("__affected") || name.startsWith("r__k_"))
									continue;
	...
	private void bind(PreparedStatement statement,
			List<com.foundgine.providers.storage.sql.query.SqlParameterBinding> parameters) throws SQLException {
	...
	private Array createSqlArray(Object array) throws SQLException {
		...
		for (int i = 0; i < length; i++)
			values[i] = java.lang.reflect.Array.get(array, i);
	...
	private Object convertJsonValue(JsonNode node, Class<?> targetType) {
		...
		try {
			...
			if (target == UUID.class && node.isTextual())
			...
			if (target == Instant.class && node.isTextual())
				return Instant.parse(node.textValue());
			if (target == java.math.BigDecimal.class && node.isNumber())
