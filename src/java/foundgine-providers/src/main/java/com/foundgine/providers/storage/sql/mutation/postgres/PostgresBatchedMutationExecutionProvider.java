...
/**
 * Executes a PostgreSQL mutation batch as one physical statement when the batched compiler can
 * represent it safely. Otherwise the provider falls back to the ordinary sequential SQL mutation
 * provider.
 */
public final class PostgresBatchedMutationExecutionProvider
        implements IMutationBatchExecutionProvider, IMutationSecurityConformanceEvaluator {
    ...
    public PostgresBatchedMutationExecutionProvider(
            Connection connection, IMetadataProvider metadata) {
        this(connection, metadata, new ObjectMapper(), true);
    }

    /**
     * Constructs the provider with explicit transaction ownership. When {@code ownsTransaction} is
     * false, the provider participates in the caller's current JDBC transaction and never commits
     * or rolls it back.
     */
    public PostgresBatchedMutationExecutionProvider(
            Connection connection, IMetadataProvider metadata, boolean ownsTransaction) {
        this(connection, metadata, new ObjectMapper(), ownsTransaction);
    }

    public PostgresBatchedMutationExecutionProvider(
            Connection connection, IMetadataProvider metadata, ObjectMapper objectMapper) {
    ...
    public PostgresBatchedMutationExecutionProvider(
        ...
        this.fallbackProvider =
                new SqlMutationExecutionProvider(connection, metadata, ownsTransaction);
    ...
    public MutationBatchResult executeBatch(
            ExecutionMutationIR ir, ExecutionContext context, CancellationToken token) {
        Objects.requireNonNull(ir, "ir");
    ...
    public MutationBatchResult executeBatch(MutationBatchPlan plan, ExecutionContext context) {
        ...
        return fallbackProvider.executeBatch(
                fallbackCompiler.compile(plan), context, CancellationToken.NONE);
    }

    public MutationBatchResult executeBatch(
            ProviderMutationBatchPlan plan, ExecutionContext context) {
        return executeBatch(plan, context, CancellationToken.NONE);
    }

    public MutationBatchResult executeBatch(
            ProviderMutationBatchPlan plan, ExecutionContext context, CancellationToken token) {
        Objects.requireNonNull(plan, "plan");
        Objects.requireNonNull(context, "context");
        CancellationToken cancellation = token == null ? CancellationToken.NONE : token;
        cancellation.throwIfCancellationRequested();
        if (plan instanceof SqlBatchedMutationPlan batched)
            return executeBatchedPlan(batched, cancellation);
        if (plan instanceof SqlMutationBatchPlan sequential)
            return fallbackProvider.executeBatch(sequential, context, cancellation);
        throw new IllegalArgumentException(
                "The PostgreSQL batched mutation provider requires a SqlBatchedMutationPlan or"
                        + " SqlMutationBatchPlan.");
    }

    private MutationBatchResult executeBatchedPlan(
            SqlBatchedMutationPlan plan, CancellationToken cancellation) {
        boolean originalAutoCommit = true;
        boolean startedTransaction = false;
        try {
            if (connection.isClosed())
                throw new IllegalStateException("PostgreSQL connection is closed.");
            ...
            try (PreparedStatement statement =
                    connection.prepareStatement(toJdbcSql(plan.commandText()))) {
                ...
                try {
                    ...
                    if (!hasResultSet) {
                        throw new IllegalStateException(
                                "PostgreSQL batched mutation did not produce its result set.");
                    }
                    ...
                    for (SqlBatchedMutationPlan.RowKey key : plan.rowKeys())
                        rowKeys.put(key.groupId() + ":" + key.ordinal(), key);

                    MutationResult[] results = new MutationResult[plan.operationCount()];
                    boolean[] seen = new boolean[plan.operationCount()];
                    @SuppressWarnings("unchecked")
                    Map<FieldId, Object>[] returned = new Map[plan.operationCount()];
                    ...
                    try (ResultSet reader = statement.getResultSet()) {
                        while (reader.next()) {
                            ...
                            if (group == null)
                                throw new IllegalStateException(
                                        "PostgreSQL batched mutation returned unknown group '"
                                                + groupId
                                                + "'.");
                            ...
                            try {
                                ...
                                throw new IllegalStateException(
                                        "Invalid PostgreSQL batched mutation JSON result.", ex);
                            }
                            ...
                            if (group.ordinalAddressable()) {
                                ...
                                if (ordinalNode == null || !ordinalNode.canConvertToInt())
                                    throw new IllegalStateException(
                                            "Batched mutation group '"
                                                    + groupId
                                                    + "' did not return a valid __fg_corr"
                                                    + " correlation value.");
                            ...
                            while (fields.hasNext()) {
                                ...
                                if (name.equals("__fg_corr")
                                        || name.equals("__affected")
                                        || name.startsWith("r__k_")) continue;
                                if (!name.startsWith("r_")) continue;
                                long id;
                                try {
                                    id = Long.parseUnsignedLong(name.substring(2));
                    ...
                    for (SqlBatchedMutationPlan.GroupMeta group : plan.groups()) {
                        ...
                        for (int operationIndex : group.operationIndexes()) {
                            if (!seen[operationIndex])
                                throw new IllegalStateException(
                                        "PostgreSQL batched mutation produced no result for"
                                                + " operation "
                                                + operationIndex
                                                + ". This usually means two operations collapsed"
                                                + " onto the same conflict/correlation key.");
                        }
                    }
    ...
    private Object convertJsonValue(JsonNode node, Class<?> targetType) {
        ...
        try {
            ...
            if (target == LocalDate.class && node.isTextual())
                return LocalDate.parse(node.textValue());
            if (target == LocalDateTime.class && node.isTextual())
                return LocalDateTime.parse(node.textValue());
            if (target == OffsetDateTime.class && node.isTextual())
