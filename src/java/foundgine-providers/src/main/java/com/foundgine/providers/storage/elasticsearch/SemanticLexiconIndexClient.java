...
public final class SemanticLexiconIndexClient {
    ...
    public SemanticLexiconIndexClient(HttpClient c, URI b, String i) {
        ...
        if (i == null || i.isBlank()) throw new IllegalArgumentException("index");
        index = i;
    }
    ...
    private void send(String method, URI u, String body) {
        try {
            HttpRequest.Builder b =
                    HttpRequest.newBuilder(u).header("Content-Type", "application/json");
            ...
            if (x.statusCode() / 100 != 2)
                throw new IllegalStateException(
                        "Elasticsearch returned " + x.statusCode() + ": " + x.body());
        } catch (Exception e) {
