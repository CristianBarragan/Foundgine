 ...
 *
 * <p>This intentionally mirrors the C# provider: metadata is used only for relationship key
 * resolution and authorization resource-member resolution; backing-store fields never become
 * visible unless the execution node selected them.
 */
public final class InMemoryCompiler
                ...
                .forEach(
                        x ->
                                violations.add(
                                        "Invariant '"
                                                + x
                                                + "' is not certified by the in-memory execution"
                                                + " plan."));

        if (memoryPlan.ir() != ir) {
            violations.add(
                    "The certified InMemoryPlan does not reference the supplied ExecutionIR.");
        }
    ...
    public ExecutionResult executeAsyncResult(
        ...
        if (memoryPlan.authorizationBinding() == null) {
            throw new IllegalStateException(
                    "The in-memory provider refuses to execute a provider plan without"
                            + " authorization provenance.");
        }
        cancellationToken.throwIfCancellationRequested();
        if (metadata == null || data == null) {
            throw new IllegalStateException(
                    "InMemoryCompiler execution requires metadata and data. Construct it with"
                            + " InMemoryCompiler(metadata, data).");
        }
        var rows =
                executeNode(memoryPlan.ir().root(), context, cancellationToken, null, true)
                        .toList();
    ...
    private Stream<ExecutionRow> executeNode(
        ...
        if (node.authorization() != null) {
            candidates =
                    candidates.filter(
                            row -> evaluateAuthorization(node.authorization(), row, context));
        }
        ...
        for (var row : rows) {
            ...
            for (var child : node.children()) {
                childGroups.add(
                        executeNode(child, context, cancellationToken, row, false).toList());
            }
    ...
    private List<InMemoryRow> traverse(InMemoryRow parent, ExecutionIRNode child) {
        ...
        FieldId sourceField =
                fieldForColumn(relationship.source(), relationship.sourceKey().columnId());
        FieldId targetField =
                fieldForColumn(relationship.target(), relationship.targetKey().columnId());
        Object parentValue = parent.values().get(sourceField);

        return data.get(child.entityId()).stream()
                .filter(
                        row ->
                                row.values().containsKey(targetField)
                                        && Objects.equals(
                                                parentValue, row.values().get(targetField)))
                .toList();
    }

    private Stream<InMemoryRow> applyQueryOptions(
            Stream<InMemoryRow> rows, SemanticQueryOptions options) {
        ...
        for (var term : options.effectiveOrder().stream().filter(x -> x.isRootField()).toList()) {
            Comparator<InMemoryRow> termComparator =
            ...
            comparator =
                    comparator == null ? termComparator : comparator.thenComparing(termComparator);
        }
    ...
    private boolean evaluateFilter(SemanticFilterExpression filter, InMemoryRow row) {
        if (filter instanceof SemanticFieldFilter fieldFilter) {
            return compareValue(
                    row.values().get(fieldFilter.field()),
        ...
        return switch (predicate.kind()) {
            ...
            case NOT_EQUAL ->
                    !Objects.equals(
                            ...
                            evaluateValue(predicate.right(), row, context));
            case AND ->
                    evaluateAuthorization(predicate.left(), row, context)
    ...
    private Object readMember(
        ...
        throw new UnsupportedOperationException(
                "Only context and resource member authorization is supported by this minimal"
                        + " provider.");
    }
    ...
    private FieldId fieldForColumn(
            EntityId entityId, com.foundgine.core.abstractions.ColumnId columnId) {
