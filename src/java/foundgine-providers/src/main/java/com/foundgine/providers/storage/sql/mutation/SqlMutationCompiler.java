...
public final class SqlMutationCompiler {
    ...
    public SqlMutationBatchPlan compile(MutationBatchPlan plan) {
        ...
        for (var op : plan.operations()) ops.add(compile(new MutationPlan(List.of(op))));
    ...
    public SqlMutationPlan compile(MutationPlan plan) {
        ...
        if (plan.operations().size() != 1)
            throw new UnsupportedOperationException(
                    "A mutation plan must contain exactly one operation.");
    ...
    private SqlMutationPlan compileUpsert(MutationOperation op) {
        ...
        List<ColumnId> conflicts =
                op.conflictColumns() != null
                        ? op.conflictColumns()
                        : (e.primaryKey() != null ? List.of(e.primaryKey().columnId()) : List.of());
        if (conflicts.isEmpty())
            throw new IllegalArgumentException(
                    "Upsert '" + e.name() + "' has no conflict identity.");
        var fields = op.fields();
        StringBuilder sb =
                new StringBuilder("INSERT INTO ")
                        .append(table(e.effectiveStorageName()))
                        .append(" (");
        List<String> cols = new ArrayList<>();
        for (var f : fields) cols.add(q(resolveColumn(e, f.column())));
        ...
        for (var c : conflicts) cc.add(q(resolveColumn(e, c)));
        sb.append(String.join(", ", cc)).append(") ");
        var updates = fields.stream().filter(f -> !conflicts.contains(f.column())).toList();
        String fallback = null;
        if (updates.isEmpty()) sb.append("DO NOTHING");
        else {
            sb.append("DO UPDATE SET ");
            for (int i = 0; i < updates.size(); i++) {
                if (i > 0) sb.append(", ");
                var f = updates.get(i);
                sb.append(q(resolveColumn(e, f.column())))
                        .append(" = @p")
                        .append(fields.indexOf(f));
            }
            sb.append(" WHERE ");
            for (int i = 0; i < updates.size(); i++) {
                if (i > 0) sb.append(" OR ");
                String c = resolveColumn(e, updates.get(i).column());
                sb.append(table(e.effectiveStorageName()))
                        .append('.')
                        .append(q(c))
            ...
            boolean canFallback =
                    op.returnFields() != null
                            && !op.returnFields().isEmpty()
                            && conflicts.stream()
                                    .allMatch(
                                            c ->
                                                    fields.stream()
                                                            .anyMatch(f -> f.column().equals(c)));
            if (canFallback) {
                ...
                for (int i = 0; i < op.returnFields().size(); i++) {
                    ...
                    fb.append(table(e.effectiveStorageName()))
                            .append('.')
                            .append(q(c))
                            .append(" AS ")
                            .append(q(rn));
    ...
    private SqlMutationPlan compileCreate(MutationOperation op) {
        ...
        for (var f : fs) cs.add(q(resolveColumn(e, f.column())));
        ...
        sb.append(')');
        return new SqlMutationPlan(
                sb.toString() + returningSql(e, op.returnFields()),
                ps,
                returnBindings(e, op.returnFields()));
    }

    private SqlMutationPlan compileUpdate(MutationOperation op) {
        if (op.filter() == null) throw new IllegalArgumentException("Update requires a filter.");
        var e = metadata.getEntity(op.entity().id());
        StringBuilder sb =
    ...
    private SqlMutationPlan compileDelete(MutationOperation op) {
        if (op.filter() == null) throw new IllegalArgumentException("Delete requires a filter.");
        var e = metadata.getEntity(op.entity().id());
        List<SqlParameterBinding> ps = new ArrayList<>();
        String where =
                SemanticQuerySqlWriter.writeWhere(
                                ...
                                ps,
                                metadata,
                                com.foundgine.core.semantic.planning.AggregateExecutionStrategy
                                        .DEFAULT)
                        .replace("\"t0\".", table(e.effectiveStorageName()) + ".");
        return new SqlMutationPlan(
                "DELETE FROM " + table(e.effectiveStorageName()) + " WHERE " + where,
                ps,
                List.of());
    }
    ...
    private SqlParameterBinding binding(EntityMetadata e, MutationFieldValue f, String n) {
        var field =
                e.effectiveFields().stream()
                        .filter(x -> x.column() != null && x.column().columnId().equals(f.column()))
                        .findFirst()
                        .orElseThrow();
        return new SqlParameterBinding(n, f.value(), f.source(), null, field.clrType());
    }

    private List<MutationReturnBinding> appendReturning(
            StringBuilder sb, EntityMetadata e, List<FieldId> fs) {
    ...
    private String returningSql(EntityMetadata e, List<FieldId> fs) {
        ...
        for (int i = 0; i < r.size(); i++) {
            ...
            s.append(q(resolveColumn(e, f.column().columnId())))
                    .append(" AS ")
                    .append(q(r.get(i).resultName()));
        }
        return s.toString();
    }

    private List<MutationReturnBinding> returnBindings(EntityMetadata e, List<FieldId> fs) {
        List<FieldId> requested =
                fs != null && !fs.isEmpty()
                        ? fs
                        : e.effectiveFields().stream()
        ...
        for (var id : requested) r.add(new MutationReturnBinding(id, "r_" + id.value()));
        return r;
    }

    private FieldMetadata findField(EntityMetadata e, FieldId id) {
        return e.effectiveFields().stream()
                .filter(f -> f.id().equals(id))
                .findFirst()
                .orElseThrow(
                        () -> new IllegalArgumentException("Unknown return field '" + id + "'."));
    }

    private int firstFieldIndex(List<MutationFieldValue> fs, ColumnId c) {
        for (int i = 0; i < fs.size(); i++) if (fs.get(i).column().equals(c)) return i;
        return -1;
    }

    private String resolveColumn(EntityMetadata e, ColumnId id) {
        return e.columns().stream()
    ...
    private String resolveColumn(EntityMetadata e, FieldMetadata f) {
        if (f.column() == null)
            throw new IllegalArgumentException(
                    "Field '" + f.name() + "' has no storage column mapping.");
