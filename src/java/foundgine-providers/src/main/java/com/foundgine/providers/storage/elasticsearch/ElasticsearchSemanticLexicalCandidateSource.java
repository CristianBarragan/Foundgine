...
/** Elasticsearch lexical retrieval using the REST API; context is a retrieval hint only. */
public final class ElasticsearchSemanticLexicalCandidateSource
        ...
        try {
            Map<String, Object> mm =
                    Map.of(
                            ...
                            r.token(),
                            "fields",
                            List.of("canonicalName^4", "aliases^3", "searchText^2", "description"),
                            "fuzziness",
                            "AUTO");
                            ...
                            Map.of(
                                    ...
                                    List.of(
                                            Map.of(
                                                    ...
                                                    Map.of(
                                                            "kind",
                                                            r.effectiveKinds().stream()
                                                                    .map(Enum::toString)
                                                                    .toList()))))));
            HttpRequest req =
                    HttpRequest.newBuilder(
                                    base.resolve(
                                            "/" + URLEncoder.encode(index, "UTF-8") + "/_search"))
            ...
            for (JsonNode h : hits) {
                ...
                if (s.path("canonicalName").isMissingNode()) continue;
                SemanticLexicalCandidateKind k =
                        SemanticLexicalCandidateKind.valueOf(
                                s.path("kind").asText().toUpperCase(Locale.ROOT));
                double score = h.path("_score").asDouble();
                out.add(
                        new SemanticLexicalCandidate(
                                r.token(),
                                k,
                                s.path("canonicalName").asText(),
                                score,
                                uid(s, "entityId"),
                                rid(s, "relationshipId"),
                                fid(s, "fieldId"),
                                uid(s, "sourceEntityId"),
                                uid(s, "targetEntityId"),
                                s.path("value").isMissingNode() ? null : s.path("value").asText(),
                                List.of(
                                        new ResolutionEvidence(
                                                "Elasticsearch lexical match",
                                                CandidateEvidenceKind.BM25,
                                                score))));
            }
            return out;
        } catch (Exception e) {
            if (e instanceof RuntimeException x) throw x;
