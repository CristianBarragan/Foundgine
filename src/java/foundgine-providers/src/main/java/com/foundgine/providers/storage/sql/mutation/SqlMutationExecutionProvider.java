 ...
 *
 * <p>
 * The provider owns only the physical SQL execution boundary. Compilation
...
public class SqlMutationExecutionProvider implements IMutationExecutionProvider {
	 ...
	 * through the metadata constructor; otherwise callers should compile the IR
	 * explicitly before invoking the provider.
	 */
	public MutationBatchResult executeBatch(ExecutionMutationIR ir, ExecutionContext context) {
		...
		if (compiler == null)
			throw new IllegalStateException(
					"A metadata-aware SqlMutationCompiler is required to execute ExecutionMutationIR.");
	...
	public MutationBatchResult executeBatch(ProviderMutationBatchPlan plan, ExecutionContext context,
		...
		try {
			...
			if (ownsTransaction && oldAutoCommit)
			...
			try {
				if (ownsTransaction && !connection.getAutoCommit())
					connection.rollback();
