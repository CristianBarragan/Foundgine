...
public final class FoundgineMetadataGenerator {
    ...
    public static List<GeneratedSemanticField> fields(Class<?>... types) {
        ...
        for (Class<?> type : types) {
            ...
            long entityId =
                    entity.id() != 0
                            ? GeneratorSemanticIdentity.validateExplicitId(entity.id(), "entity")
                            : GeneratorSemanticIdentity.hash(
                                    GeneratorSemanticIdentity.entityKey(entityName));
                ...
                long fieldId =
                        annotation.id() != 0
                                ? GeneratorSemanticIdentity.validateExplicitId(
                                        annotation.id(), "field")
                                : GeneratorSemanticIdentity.hash(
                                        GeneratorSemanticIdentity.fieldKey(entityName, fieldName));
