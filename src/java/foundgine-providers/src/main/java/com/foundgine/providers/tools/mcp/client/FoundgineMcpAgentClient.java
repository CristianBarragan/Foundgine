 ...
 *
 * <p>The client is deliberately transport-only. It discovers the host-visible capability contract,
 * lets the caller construct a dynamic {@link ReadIntent}, and sends that intent to the canonical
 * Foundgine MCP query tool. Security context is never copied into the serialized intent by this
 * client.
 */
public final class FoundgineMcpAgentClient {
    ...
    public CompletionStage<JsonNode> executeQuery(ReadIntent intent) {
        ...
        try {
            ...
            return CompletableFuture.failedFuture(
                    new IllegalArgumentException("Unable to serialize MCP read intent", e));
        }
    }
                ...
                .thenCompose(
                        contract -> {
                            ...
                            if (intent == null) {
                                return CompletableFuture.failedFuture(
                                        new IllegalStateException(
                                                "The intent factory returned null."));
                            }
    ...
    private CompletionStage<JsonNode> callTool(String toolName, ObjectNode arguments) {
        try {
                            ...
                            response -> {
                                ...
                                try {
                                    return CompletableFuture.completedFuture(
                                            extractJsonPayload(response.body()));
            ...
            return CompletableFuture.failedFuture(
                    new IllegalStateException("Unable to create MCP request", e));
        }
    }
