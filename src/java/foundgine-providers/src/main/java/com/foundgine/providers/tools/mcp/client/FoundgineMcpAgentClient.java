 ...
 *
 * <p>
 * The client is deliberately transport-only. It discovers the host-visible
 ...
 * context is never copied into the serialized intent by this client.
 * </p>
 */
public final class FoundgineMcpAgentClient {
	...
	public CompletionStage<String> call(String intentJson) {
		...
		HttpRequest request = HttpRequest.newBuilder(endpoint).header("Content-Type", "application/json")
				...
				.POST(HttpRequest.BodyPublishers.ofString(intentJson)).build();
	...
	public CompletionStage<JsonNode> executeQuery(ReadIntent intent) {
		...
		try {
			...
			return callTool("foundgine_query", arguments).thenApply(payload -> readToolResult(payload, JsonNode.class));
		} catch (Exception e) {
			return CompletableFuture
					.failedFuture(new IllegalArgumentException("Unable to serialize MCP read intent", e));
		}
	}

	/**
	 * Discover capabilities, construct an intent from that contract, then execute
	 * it.
	 */
	public CompletionStage<JsonNode> discoverAndExecute(
		...
		return discoverCapabilities().thenCompose(contract -> {
			...
			if (intent == null) {
				return CompletableFuture.failedFuture(new IllegalStateException("The intent factory returned null."));
			}
