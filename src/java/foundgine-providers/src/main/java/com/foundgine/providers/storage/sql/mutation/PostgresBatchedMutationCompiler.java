 ...
 *
 * <p>
 * Create/Upsert operations at the same dependency level are grouped by shape
 * and expanded with PostgreSQL {@code unnest(array parameters)}.
 * Reference-valued fields use the source group's 1-based ordinal and an ord-map
 * CTE, so generated values flow between levels without client-side round trips.
 *
 * <p>
 * Update/Delete operations remain one CTE each, but are folded into the same
...
public final class PostgresBatchedMutationCompiler {
	...
	public SqlBatchedMutationPlan compile(MutationBatchPlan plan) {
		...
		for (MutationDependency dependency : plan.dependencies()) {
			if (dependency.sourceOperationIndex() < 0 || dependency.sourceOperationIndex() >= ops.size()
				...
				throw new IllegalStateException("Invalid mutation dependency " + dependency.sourceOperationIndex()
		...
		for (int i = 0; i < groups.size(); i++) {
			...
			if (group.kind == MutationKind.CREATE)
				...
				meta = writeUpdateOrDeleteGroup(sql, parameters, group, ops.get(group.operationIndexes.get(0)), forced,
						false);
		...
		for (int i = 0; i < output.size(); i++) {
			...
			sql.append(" AS __row FROM ").append(
					meta.ordinalAddressable && meta.ordMapCteName != null ? meta.ordMapCteName : meta.resultCteName)
					.append(" f");
		}
		sql.append(
		...
		for (int opIndex = 0; opIndex < ops.size(); opIndex++) {
			...
			rowKeys.add(
					new SqlBatchedMutationPlan.RowKey(group.groupId, groupOrdinal(group.operationIndexes, opIndex)));
		}

		List<SqlBatchedMutationPlan.GroupMeta> groupMetas = new ArrayList<>();
		for (GroupOutputMeta meta : output) {
			groupMetas.add(new SqlBatchedMutationPlan.GroupMeta(meta.groupId, meta.ordinalAddressable,
					meta.operationIndexes, meta.returnedFieldTypes));
		}
	 ...
	 * physical (JDBC-rewritten) SQL must contain exactly one {@code ?} per declared
	 * parameter. A mismatch here would otherwise surface much later, and much less
	 * clearly, as a driver-level
	 * {@code PSQLException: No value specified for parameter N} - or worse,
	 * silently bind the wrong value to the wrong position. Failing fast here, with
	 * the offending SQL in hand, is far cheaper to diagnose.
	 */
	private static void validatePlaceholderParity(String commandText, List<SqlParameterBinding> parameters) {
		...
		for (SqlParameterBinding parameter : parameters)
	...
	private static final java.util.regex.Pattern NAMED_PLACEHOLDER_PATTERN = java.util.regex.Pattern
			.compile("@(p[0-9]+)");
	...
	private static boolean requiresSequentialEntityMutation(List<MutationOperation> operations) {
		...
		for (MutationOperation operation : operations)
			kinds.computeIfAbsent(operation.entity().id(), ignored -> EnumSet.noneOf(MutationKind.class))
					.add(operation.kind());
	...
	private void validateCorrelation(List<MutationOperation> operations, List<MutationDependency> dependencies,
		...
		for (MutationDependency d : dependencies)
			dependencySet.add(dependencyKey(d.sourceOperationIndex(), d.targetOperationIndex(), d.sourceField(),
					d.targetColumn()));
		...
		for (int targetIndex = 0; targetIndex < operations.size(); targetIndex++) {
			...
			for (MutationFieldValue field : target.fields()) {
				...
				if (sourceIndex < 0 || sourceIndex >= operations.size())
					throw new IllegalStateException("Mutation operation " + targetIndex
							+ " references missing source operation " + sourceIndex + ".");
				...
				if (levels[sourceIndex] >= levels[targetIndex])
					throw new IllegalStateException("Mutation reference " + sourceIndex + " -> " + targetIndex
				...
				if (sourceOperation.kind() == MutationKind.DELETE)
					throw new UnsupportedOperationException(
							"Mutation operation " + targetIndex + " references a deleted source operation "
									+ sourceIndex + "; generated-key correlation cannot use DELETE.");

				FieldMetadata sourceField = findField(metadata.getEntity(sourceOperation.entity().id()),
						source.sourceField());
				if (sourceField.column() == null)
					throw new IllegalStateException("Source field " + source.sourceField().value()
							+ " has no physical column and cannot participate in provider correlation.");

				String dep = dependencyKey(sourceIndex, targetIndex, source.sourceField(), field.column());
				if (!dependencySet.contains(dep))
					throw new IllegalStateException("Mutation value reference " + sourceIndex + ":"
							+ source.sourceField().value() + " for target operation " + targetIndex + ":"
							+ field.column().value() + " has no matching dependency edge.");

				OpGroup sourceGroup = groups.get(opToGroup[sourceIndex]);
				if (!sourceGroup.operationIndexes.contains(sourceIndex))
					throw new IllegalStateException(
							"Source operation " + sourceIndex + " is not present in its physical group.");
				if (sourceGroup.kind == MutationKind.DELETE)
					throw new UnsupportedOperationException("Source operation " + sourceIndex
							+ " is a DELETE group and cannot provide an ordinal correlation map.");
			}
		}
	}

	private GroupOutputMeta writeCreateOrUpsertGroup(StringBuilder sql, List<SqlParameterBinding> parameters,
			OpGroup group, List<MutationOperation> operations, List<GroupOutputMeta> priorGroups,
			Set<FieldId> forcedReturns, boolean isUpsert) {
		EntityMetadata entity = metadata.getEntity(group.entity);
		List<MutationOperation> rows = new ArrayList<>();
		for (int index : group.operationIndexes)
			rows.add(operations.get(index));
		List<ColumnId> columns = new ArrayList<>();
		for (MutationFieldValue f : group.template.fields())
			columns.add(f.column());
		if (columns.isEmpty())
			throw new IllegalStateException("Mutation group " + group.groupId + " has no fields.");

		List<ColumnId> conflicts;
		if (isUpsert) {
			conflicts = group.template.conflictColumns() != null ? new ArrayList<>(group.template.conflictColumns())
			...
			for (ColumnId c : columns) {
				...
				for (MutationOperation r : rows) {
					if (fieldForColumn(r, c).source() != null) {
						allLiteral = false;
						break;
					}
				}
		...
		if (isUpsert && conflicts.isEmpty())
			throw new IllegalStateException("Upsert '" + entity.name() + "' has no conflict identity.");
		if (isUpsert) {
			for (ColumnId c : conflicts)
				if (!columns.contains(c))
					throw new UnsupportedOperationException("Upsert '" + entity.name()
							+ "' does not supply every conflict column; falling back to sequential execution.");
			for (MutationOperation r : rows)
				for (ColumnId c : conflicts) {
					MutationFieldValue f = fieldForColumn(r, c);
					if (f.source() == null && f.value() == null)
						throw new UnsupportedOperationException("Upsert '" + entity.name()
								+ "' contains a NULL literal conflict value; falling back to sequential execution.");
				}
			if (rows.size() > 1
					&& conflicts.stream()
							.allMatch(c -> rows.stream().allMatch(r -> fieldForColumn(r, c).source() == null))
					&& hasDuplicateLiteralKey(rows, conflicts))
				throw new UnsupportedOperationException("Upsert '" + entity.name()
						+ "' contains duplicate literal conflict keys; falling back to sequential execution.");
		}
		...
		for (ColumnId column : columns) {
			...
			if (templateField.source() == null) {
				...
				for (int i = 0; i < rows.size(); i++)
					values[i] = fieldForColumn(rows.get(i), column).value();
				...
				for (int i = 0; i < rows.size(); i++) {
					...
					if (sourceGroup == null || !sourceGroup.ordinalAddressable)
						throw new UnsupportedOperationException("Mutation operation " + source.sourceOperationIndex()
								+ " cannot be used as a batched reference source.");
					ordinals[i] = groupOrdinal(sourceGroup.operationIndexes, source.sourceOperationIndex());
				}
				String parameterName = bindTypedArrayParameter(parameters, "g" + group.groupId + "_" + name + "_ord",
						ordinals);
				sourceExpressions.add("@" + parameterName);
		...
		sql.append(src).append(" AS (\n  SELECT * FROM unnest(").append(String.join(", ", sourceExpressions))
		...
		for (ColumnId column : columns) {
			...
			if (field.source() == null) {
				...
				if (sourceGroup == null)
					throw new UnsupportedOperationException("Source operation " + field.source().sourceOperationIndex()
							+ " is not available to resolve references.");
				String sourceField = sourceGroup.returnedFieldNames.get(field.source().sourceField());
				if (sourceField == null)
					throw new UnsupportedOperationException("Source operation " + field.source().sourceOperationIndex()
							+ " does not return field " + field.source().sourceField().value() + ".");
		...
		for (ColumnId c : columns)
			if (fieldForColumn(rows.get(0), c).source() != null)
				referenceColumns.add(c);
		for (ColumnId column : referenceColumns) {
			MutationFieldValue field = fieldForColumn(rows.get(0), column);
			GroupOutputMeta source = Objects
					.requireNonNull(findPriorGroup(priorGroups, field.source().sourceOperationIndex()));
			String alias = "p" + column.value();
			sql.append("\n  JOIN ").append(source.ordMapCteName).append(" ").append(alias).append(" ON ").append(alias)
					.append(".__fg_corr = s.").append(q(resolveColumn(entity, column) + "__fg_corr"));
		}
		sql.append("\n)");
		...
		for (FieldId id : requestedFields)
			returnNames.put(id, "r_" + id.value());

		sql.append(",\n").append(ins).append(" AS (\n");
		if (!isUpsert) {
			sql.append("  MERGE INTO ").append(table(entity.effectiveStorageName())).append(" t\n").append("  USING ")
					.append(input).append(" r ON FALSE\n").append("  WHEN NOT MATCHED THEN INSERT (")
					.append(String.join(", ", columnNames.stream().map(PostgresBatchedMutationCompiler::q).toList()))
					.append(") VALUES (").append(String.join(", ", columnNames.stream().map(c -> "r." + q(c)).toList()))
					.append(")\n").append("  RETURNING r.__fg_corr, ").append(String.join(", ",
							returnColumns.stream().map(x -> "t." + q(x.columnName) + " AS " + q(x.alias)).toList()));
		} else {
			sql.append("  INSERT INTO ").append(table(entity.effectiveStorageName())).append(" (")
					.append(String.join(", ", columnNames.stream().map(PostgresBatchedMutationCompiler::q).toList()))
					.append(")\n  SELECT ")
					.append(String.join(", ", columnNames.stream().map(c -> "r." + q(c)).toList())).append("\n  FROM ")
					.append(input).append(" r\n  ON CONFLICT (")
					.append(String.join(", ", conflictNames.stream().map(PostgresBatchedMutationCompiler::q).toList()))
					.append(") ");
			List<String> updateColumns = new ArrayList<>();
			for (int i = 0; i < columnNames.size(); i++)
				if (!conflicts.contains(columns.get(i)))
					updateColumns.add(columnNames.get(i));
			if (updateColumns.isEmpty())
				sql.append("DO NOTHING");
			else
				sql.append("DO UPDATE SET ")
						.append(String.join(", ",
								updateColumns.stream().map(c -> q(c) + " = EXCLUDED." + q(c)).toList()))
						.append("\n  WHERE ")
						.append(String.join(" OR ", updateColumns.stream().map(c -> table(entity.effectiveStorageName())
								+ "." + q(c) + " IS DISTINCT FROM EXCLUDED." + q(c)).toList()));
		}
		if (isUpsert) {
			sql.append("\n  RETURNING ")
					.append(String.join(", ",
							returnColumns.stream().map(x -> q(x.columnName) + " AS " + q(x.alias)).toList()))
					.append("\n)");
		} else {
			sql.append("\n)");
		}
		...
		if (isUpsert) {
			sql.append(",\n").append(fin).append(" AS (\n  SELECT * FROM ").append(ins)
					...
					.append(String.join(" AND ",
							conflictNames.stream().map(c -> "t." + q(c) + " IS NOT DISTINCT FROM k." + q(c)).toList()))
					.append("\n  WHERE NOT EXISTS (SELECT 1 FROM ").append(ins).append(" i WHERE ")
					.append(String.join(" AND ", conflictNames.stream().map(
							c -> "i." + q(returnAliasForColumn(returnColumns, c)) + " IS NOT DISTINCT FROM k." + q(c))
							.toList()))
					.append(")\n)");
			sql.append(",\n").append(ordmap).append(" AS (\n  SELECT k.__fg_corr, f.*\n  FROM ").append(keys)
					...
					.append(String
							.join(" AND ",
									conflictNames.stream()
											.map(c -> "k." + q(c) + " IS NOT DISTINCT FROM f."
													+ q(returnAliasForColumn(returnColumns, c)))
											.toList()))
					.append("\n)");
		} else {
			sql.append(",\n").append(fin).append(" AS (SELECT * FROM ").append(ins).append(")").append(",\n")
					.append(ordmap).append(" AS (SELECT * FROM ").append(fin).append(")");
		}

		return new GroupOutputMeta(group.groupId, fin, ordmap, group.operationIndexes, true, false, returnNames,
				returnedTypes);
	}

	private GroupOutputMeta writeUpdateOrDeleteGroup(StringBuilder sql, List<SqlParameterBinding> parameters,
			OpGroup group, MutationOperation operation, Set<FieldId> forcedReturns, boolean isDelete) {
		EntityMetadata entity = metadata.getEntity(group.entity);
		List<FieldId> returnFields = new ArrayList<>();
		if (operation.returnFields() != null && !operation.returnFields().isEmpty())
			returnFields.addAll(operation.returnFields());
		else if (!isDelete)
			for (FieldMetadata f : entity.effectiveFields())
				if (f.column() != null)
					returnFields.add(f.id());
		for (FieldId f : forcedReturns)
			if (!returnFields.contains(f))
				returnFields.add(f);
		if (isDelete && !forcedReturns.isEmpty())
			throw new UnsupportedOperationException(
					"A Delete cannot currently be a source of a batched reference; falling back to sequential execution.");

		MutationOperation effective = new MutationOperation(operation.entity(), operation.kind(), operation.fields(),
				operation.filter(), operation.conflictColumns(),
		...
		if (isDelete && !rewritten.toLowerCase(Locale.ROOT).contains(" returning "))
		...
		for (SqlParameterBinding p : single.parameters()) {
			...
			parameters.add(
					new SqlParameterBinding("p" + (offset + old), p.value(), p.source(), p.contextPath(), p.clrType()));
		}
		...
		if (!isDelete) {
			String ordmap = "g" + group.groupId + "_ordmap";
			sql.append(",\n").append(ordmap).append(" AS (SELECT 1 AS __fg_corr, f.* FROM ").append(resultCte)
					.append(" f LIMIT 1)");
			return new GroupOutputMeta(group.groupId, ordmap, ordmap, group.operationIndexes, true, true, resultNames,
					resultTypes);
		}
		return new GroupOutputMeta(group.groupId, resultCte, null, group.operationIndexes, false, true, resultNames,
				resultTypes);
	}

	private static List<FieldId> requestedFields(MutationOperation template, EntityMetadata entity,
			Set<FieldId> forced) {
		LinkedHashSet<FieldId> ids = new LinkedHashSet<>();
		if (template.returnFields() != null && !template.returnFields().isEmpty())
			ids.addAll(template.returnFields());
		else
			for (FieldMetadata f : entity.effectiveFields())
				if (f.column() != null)
					ids.add(f.id());
	...
	private static List<ReturnColumn> buildReturnColumns(EntityMetadata entity, List<FieldId> requestedFields,
			List<ColumnId> conflicts) {
		List<ReturnColumn> result = new ArrayList<>();
		for (FieldId id : requestedFields) {
			FieldMetadata field = findField(entity, id);
			if (field.column() == null)
				throw new IllegalStateException("Return field '" + field.name() + "' has no storage column.");
		...
		for (ColumnId conflict : conflicts) {
			...
			if (!exists)
				result.add(new ReturnColumn(null, conflict, resolveColumn(entity, conflict),
	...
	private static Map<FieldId, Class<?>> buildReturnedTypes(EntityMetadata entity, List<FieldId> fields) {
		...
		for (FieldId id : fields) {
			FieldMetadata field = entity.effectiveFields().stream().filter(f -> f.id().equals(id)).findFirst()
					.orElse(null);
			if (field != null)
				result.put(id, field.clrType());
		}
		return result;
	}

	private static String returnAliasForColumn(List<ReturnColumn> columns, String columnName) {
		return columns.stream().filter(x -> x.columnName.equals(columnName)).findFirst().orElseThrow(
				() -> new IllegalStateException("Return column '" + columnName + "' was not projected.")).alias;
	}
	...
	private static boolean hasDuplicateLiteralKey(List<MutationOperation> rows, List<ColumnId> keyColumns) {
		...
		for (MutationOperation row : rows) {
			...
			for (ColumnId c : keyColumns)
				key.append('\u001f').append(stableValue(fieldForColumn(row, c).value()));
	...
	private static String bindTypedArrayParameter(List<SqlParameterBinding> parameters, String hint, Object[] values) {
		...
		if (first == null)
			throw new UnsupportedOperationException("Column '" + hint
					+ "' is NULL for every row in its batch group; its PostgreSQL array type cannot be inferred.");
		Class<?> elementType = first.getClass();
		for (Object value : values)
			if (value != null && !elementType.isInstance(value))
				throw new UnsupportedOperationException("Column '" + hint
						+ "' contains heterogeneous CLR value types; falling back to sequential execution.");
	...
	private static String shapeKey(MutationOperation operation, int[] opToGroup) {
		...
		for (MutationFieldValue f : fields) {
			if (f.source() == null)
				encoded.add(f.column().value() + ":lit");
			else
				encoded.add(f.column().value() + ":ref:" + opToGroup[f.source().sourceOperationIndex()] + ":"
						+ f.source().sourceField().value());
		}
		String conflicts = operation.conflictColumns() == null ? ""
				: operation.conflictColumns().stream().sorted(Comparator.comparingLong(ColumnId::value))
						.map(c -> Long.toString(c.value())).reduce((a, b) -> a + "," + b).orElse("");
		String returns = operation.returnFields() == null ? ""
				: operation.returnFields().stream().sorted(Comparator.comparingLong(FieldId::value))
						.map(f -> Long.toString(f.value())).reduce((a, b) -> a + "," + b).orElse("");
		return operation.entity().id().value() + "|" + operation.kind() + "|" + String.join(",", encoded) + "|"
				+ conflicts + "|" + returns;
	}
	...
	private static int[] computeLevels(int count, List<MutationDependency> dependencies) {
		...
		for (MutationDependency d : dependencies)
			incoming.computeIfAbsent(d.targetOperationIndex(), ignored -> new ArrayList<>()).add(d);
		int[] level = new int[count];
		boolean[] visiting = new boolean[count];
		boolean[] visited = new boolean[count];
		for (int i = 0; i < count; i++)
			visit(i, incoming, level, visiting, visited);
		return level;
	}

	private static int visit(int index, Map<Integer, List<MutationDependency>> incoming, int[] level,
			boolean[] visiting, boolean[] visited) {
		if (visited[index])
			return level[index];
		if (visiting[index])
	...
	private static String resolveColumn(EntityMetadata entity, ColumnId id) {
		return entity.columns().stream().filter(c -> c.id().equals(id)).map(ColumnMetadata::effectiveStorageName)
	...
	private static String table(String storageName) {
		return Arrays.stream(storageName.split("\\.")).map(String::trim).filter(s -> !s.isEmpty())
				.map(PostgresBatchedMutationCompiler::q).reduce((a, b) -> a + "." + b).orElseThrow();
	}

	private static int groupOrdinal(List<Integer> operationIndexes, int operationIndex) {
		int i = operationIndexes.indexOf(operationIndex);
		if (i < 0)
			throw new IllegalStateException("Operation " + operationIndex + " is not present in its mutation group.");
		return i + 1;
	}

	private static MutationFieldValue fieldForColumn(MutationOperation operation, ColumnId column) {
		return operation.fields().stream().filter(f -> f.column().equals(column)).findFirst().orElseThrow(
				() -> new IllegalStateException("Mutation operation does not define column '" + column.value() + "'."));
	}

	private static FieldMetadata findField(EntityMetadata entity, FieldId id) {
		return entity.effectiveFields().stream().filter(f -> f.id().equals(id)).findFirst().orElseThrow(
				() -> new IllegalArgumentException("Unknown field '" + id.value() + "' on '" + entity.name() + "'."));
	}

	private static GroupOutputMeta findPriorGroup(List<GroupOutputMeta> groups, int sourceIndex) {
		for (GroupOutputMeta g : groups)
			if (g.operationIndexes.contains(sourceIndex))
				return g;
		return null;
	}

	private static String dependencyKey(int source, int target, FieldId field, ColumnId column) {
		return source + ":" + target + ":" + field.value() + ":" + column.value();
	}

	private static List<SqlMutationPlan> dummyOperations(int count) {
		List<SqlMutationPlan> result = new ArrayList<>(count);
		for (int i = 0; i < count; i++)
			result.add(new SqlMutationPlan("", List.of(), List.of()));
		return result;
	}
	...
	private static final class OpGroup {
		...
		final MutationOperation template;
		final List<Integer> operationIndexes = new ArrayList<>();

		OpGroup(int groupId, EntityId entity, MutationKind kind, MutationOperation template) {
	...
	private static final class GroupOutputMeta {
		final int groupId;
		final String resultCteName;
		final String ordMapCteName;
		final List<Integer> operationIndexes;
		final boolean ordinalAddressable;
		final boolean singleResult;
		final Map<FieldId, String> returnedFieldNames;
		final Map<FieldId, Class<?>> returnedFieldTypes;

		GroupOutputMeta(int groupId, String resultCteName, String ordMapCteName, List<Integer> operationIndexes,
				boolean ordinalAddressable, boolean singleResult, Map<FieldId, String> returnedFieldNames,
				Map<FieldId, Class<?>> returnedFieldTypes) {
			this.groupId = groupId;
			this.resultCteName = resultCteName;
			this.ordMapCteName = ordMapCteName;
			this.operationIndexes = List.copyOf(operationIndexes);
			this.ordinalAddressable = ordinalAddressable;
			this.singleResult = singleResult;
			this.returnedFieldNames = Map.copyOf(returnedFieldNames);
			this.returnedFieldTypes = Map.copyOf(returnedFieldTypes);
		}
	}
}
