 ...
 *
 * <p>Create/Upsert operations at the same dependency level are grouped by shape and expanded with
 * PostgreSQL {@code unnest(array parameters)}. Reference-valued fields use the source group's
 * 1-based ordinal and an ord-map CTE, so generated values flow between levels without client-side
 * round trips.
 *
 * <p>Update/Delete operations remain one CTE each, but are folded into the same physical statement.
 * Unsupported shapes are rejected by {@link #tryCompile}; callers can then use the sequential SQL
 * mutation compiler.
 */
public final class PostgresBatchedMutationCompiler {
    ...
    public SqlBatchedMutationPlan compile(MutationBatchPlan plan) {
        ...
        if (plan.operations().isEmpty())
            throw new IllegalStateException(
                    "A mutation batch must contain at least one operation.");
        ...
        for (int level : distinctLevels) {
            ...
            for (int opIndex : indexes) {
                for (MutationFieldValue field : ops.get(opIndex).fields()) {
                    ...
                    forcedReturns
                            .computeIfAbsent(sourceGroup, ignored -> new LinkedHashSet<>())
        ...
        for (MutationDependency dependency : plan.dependencies()) {
            if (dependency.sourceOperationIndex() < 0
                    || dependency.sourceOperationIndex() >= ops.size()
                    || dependency.targetOperationIndex() < 0
                    || dependency.targetOperationIndex() >= ops.size()
                    || levels[dependency.targetOperationIndex()]
                            <= levels[dependency.sourceOperationIndex()]) {
                throw new IllegalStateException(
        ...
        for (int i = 0; i < groups.size(); i++) {
            ...
            if (group.kind == MutationKind.CREATE)
                ...
                meta =
                        writeUpdateOrDeleteGroup(
                                sql,
                                parameters,
                                group,
                                ops.get(group.operationIndexes.get(0)),
                                forced,
                                false);
                ...
                throw new UnsupportedOperationException(
                        "Unsupported mutation kind '" + group.kind + "'.");
            output.add(meta);
        }
        ...
        for (int i = 0; i < output.size(); i++) {
            ...
            if (meta.ordinalAddressable) {
                sql.append(
                        "jsonb_build_object('__fg_corr', f.__fg_corr) || (to_jsonb(f) -"
                                + " '__fg_corr')");
            } else {
            ...
            sql.append(" AS __row FROM ")
                    .append(
                            meta.ordinalAddressable && meta.ordMapCteName != null
                                    ? meta.ordMapCteName
                                    : meta.resultCteName)
                    .append(" f");
        }
        sql.append(
                "\n"
                        + ") __all ORDER BY __grp, CASE WHEN jsonb_exists(__row, '__fg_corr') THEN"
                        + " ((__row ->> '__fg_corr')::bigint) END");
        ...
        for (int opIndex = 0; opIndex < ops.size(); opIndex++) {
            ...
            rowKeys.add(
                    new SqlBatchedMutationPlan.RowKey(
                            group.groupId, groupOrdinal(group.operationIndexes, opIndex)));
        }

        List<SqlBatchedMutationPlan.GroupMeta> groupMetas = new ArrayList<>();
        for (GroupOutputMeta meta : output) {
            groupMetas.add(
                    new SqlBatchedMutationPlan.GroupMeta(
                            meta.groupId,
                            meta.ordinalAddressable,
                            meta.operationIndexes,
                            meta.returnedFieldTypes));
        }
        ...
        return new SqlBatchedMutationPlan(
                ...
                rowKeys,
                ops.size(),
                dummyOperations(ops.size()),
                plan.dependencies());
    }

    /**
     * Guards against a batched statement whose declared parameters and physical JDBC placeholders
     * have fallen out of sync. Every declared parameter must appear in the SQL exactly once as a
     * named {@code @pN} placeholder, and the physical (JDBC-rewritten) SQL must contain exactly one
     * {@code ?} per declared parameter. A mismatch here would otherwise surface much later, and
     * much less clearly, as a driver-level {@code PSQLException: No value specified for parameter
     * N} - or worse, silently bind the wrong value to the wrong position. Failing fast here, with
     * the offending SQL in hand, is far cheaper to diagnose.
     */
    private static void validatePlaceholderParity(
            String commandText, List<SqlParameterBinding> parameters) {
        ...
        for (Map.Entry<String, Integer> entry : occurrences.entrySet())
            if (entry.getValue() > 1) duplicated.add(entry.getKey());
        ...
        if (!missing.isEmpty() || !duplicated.isEmpty() || !undeclared.isEmpty()) {
            throw new IllegalStateException(
                    "Batched mutation compiler produced a SQL/parameter mismatch. "
                            + "Declared parameters not referenced in SQL: "
                            + missing
                            + "; parameters referenced more than once: "
                            + duplicated
                            + "; SQL placeholders with no declared parameter: "
                            + undeclared
                            + ".");
        }

        String jdbcSql =
                com.foundgine.providers.storage.sql.JdbcSqlPlaceholderRewriter.rewrite(commandText);
        long jdbcPlaceholderCount = jdbcSql.chars().filter(c -> c == '?').count();
        if (jdbcPlaceholderCount != parameters.size()) {
            throw new IllegalStateException(
                    "Batched mutation compiler declared "
                            + parameters.size()
                            + " parameter(s) but the JDBC-rewritten SQL contains "
                            + jdbcPlaceholderCount
                            + " '?' placeholder(s). This usually means a raw '?' character (for"
                            + " example a jsonb '?' / '?|' / '?&' operator) slipped into the"
                            + " generated SQL outside of a named @pN placeholder, which would"
                            + " otherwise fail at the driver with 'No value specified for parameter"
                            + " "
                            + (jdbcPlaceholderCount)
                            + "'.");
        }
    ...
    private static final java.util.regex.Pattern NAMED_PLACEHOLDER_PATTERN =
            java.util.regex.Pattern.compile("@(p[0-9]+)");
    ...
    private static boolean requiresSequentialEntityMutation(List<MutationOperation> operations) {
        ...
        for (MutationOperation operation : operations)
            kinds.computeIfAbsent(
                            operation.entity().id(), ignored -> EnumSet.noneOf(MutationKind.class))
                    .add(operation.kind());
        for (EnumSet<MutationKind> set : kinds.values()) {
            boolean insertLike =
                    set.contains(MutationKind.CREATE) || set.contains(MutationKind.UPSERT);
            boolean updateLike =
                    set.contains(MutationKind.UPDATE) || set.contains(MutationKind.DELETE);
    ...
    private void validateCorrelation(
        ...
        for (MutationDependency d : dependencies)
            dependencySet.add(
                    dependencyKey(
                            d.sourceOperationIndex(),
                            d.targetOperationIndex(),
                            d.sourceField(),
                            d.targetColumn()));
            ...
            for (MutationFieldValue field : target.fields()) {
                ...
                if (sourceIndex < 0 || sourceIndex >= operations.size())
                    throw new IllegalStateException(
                            "Mutation operation "
                                    + targetIndex
                                    + " references missing source operation "
                                    + sourceIndex
                                    + ".");
                ...
                if (sourceOperation.kind() == MutationKind.DELETE)
                    throw new UnsupportedOperationException(
                            "Mutation operation "
                                    + targetIndex
                                    + " references a deleted source operation "
                                    + sourceIndex
                                    + "; generated-key correlation cannot use DELETE.");

                FieldMetadata sourceField =
                        findField(
                                metadata.getEntity(sourceOperation.entity().id()),
                                source.sourceField());
                if (sourceField.column() == null)
                    throw new IllegalStateException(
                            "Source field "
                                    + source.sourceField().value()
                                    + " has no physical column and cannot participate in provider"
                                    + " correlation.");

                String dep =
                        dependencyKey(
                                sourceIndex, targetIndex, source.sourceField(), field.column());
                if (!dependencySet.contains(dep))
                    throw new IllegalStateException(
                            "Mutation value reference "
                                    ...
                                    + ":"
                                    + field.column().value()
                                    + " has no matching dependency edge.");
                ...
                if (!sourceGroup.operationIndexes.contains(sourceIndex))
                    throw new IllegalStateException(
                            "Source operation "
                                    + sourceIndex
                                    + " is not present in its physical group.");
                if (sourceGroup.kind == MutationKind.DELETE)
                    throw new UnsupportedOperationException(
                            "Source operation "
                                    + sourceIndex
                                    + " is a DELETE group and cannot provide an ordinal correlation"
                                    + " map.");
            }
        }
    }
    ...
    private GroupOutputMeta writeCreateOrUpsertGroup(
            ...
            OpGroup group,
            List<MutationOperation> operations,
            List<GroupOutputMeta> priorGroups,
            Set<FieldId> forcedReturns,
            boolean isUpsert) {
        ...
        if (isUpsert) {
            conflicts =
                    group.template.conflictColumns() != null
                            ? new ArrayList<>(group.template.conflictColumns())
                            : entity.primaryKey() != null
                                    ? List.of(entity.primaryKey().columnId())
                                    : List.of();
        } else {
        ...
        if (isUpsert && conflicts.isEmpty())
            throw new IllegalStateException(
                    "Upsert '" + entity.name() + "' has no conflict identity.");
        if (isUpsert) {
            for (ColumnId c : conflicts)
                if (!columns.contains(c))
                    throw new UnsupportedOperationException(
                            "Upsert '"
                                    + entity.name()
                                    + "' does not supply every conflict column; falling back to"
                ...
                for (ColumnId c : conflicts) {
                    ...
                    if (f.source() == null && f.value() == null)
                        throw new UnsupportedOperationException(
                                "Upsert '"
                                        + entity.name()
                                        + "' contains a NULL literal conflict value; falling back"
                            ...
                            .allMatch(
                                    c ->
                                            rows.stream()
                                                    .allMatch(
                                                            r ->
                                                                    fieldForColumn(r, c).source()
                                                                            == null))
                    && hasDuplicateLiteralKey(rows, conflicts))
                throw new UnsupportedOperationException(
                        "Upsert '"
                                + entity.name()
                                + "' contains duplicate literal conflict keys; falling back to"
                                + " sequential execution.");
        }
        ...
        for (ColumnId column : columns) {
            ...
            if (templateField.source() == null) {
                ...
                String parameterName =
                        bindTypedArrayParameter(
                                parameters, "g" + group.groupId + "_" + name, values);
                ...
                for (int i = 0; i < rows.size(); i++) {
                    ...
                    GroupOutputMeta sourceGroup =
                            findPriorGroup(priorGroups, source.sourceOperationIndex());
                    ...
                    ordinals[i] =
                            groupOrdinal(
                                    sourceGroup.operationIndexes, source.sourceOperationIndex());
                }
                String parameterName =
                        bindTypedArrayParameter(
                                parameters, "g" + group.groupId + "_" + name + "_ord", ordinals);
                sourceExpressions.add("@" + parameterName);
        ...
        sql.append(src)
                ...
                .append(
                        String.join(
                                ", ",
                                sourceAliases.stream()
                                        .map(PostgresBatchedMutationCompiler::q)
                                        .toList()))
                .append(", __fg_corr)\n)");
        ...
        for (ColumnId column : columns) {
            ...
            if (field.source() == null) {
                ...
                GroupOutputMeta sourceGroup =
                        findPriorGroup(priorGroups, field.source().sourceOperationIndex());
                if (sourceGroup == null)
                    throw new UnsupportedOperationException(
                            "Source operation "
                                    + field.source().sourceOperationIndex()
                                    + " is not available to resolve references.");
                String sourceField =
                        sourceGroup.returnedFieldNames.get(field.source().sourceField());
                if (sourceField == null)
                    throw new UnsupportedOperationException(
                            "Source operation "
                                    + field.source().sourceOperationIndex()
                                    + " does not return field "
        ...
        for (ColumnId c : columns)
            if (fieldForColumn(rows.get(0), c).source() != null) referenceColumns.add(c);
        for (ColumnId column : referenceColumns) {
            MutationFieldValue field = fieldForColumn(rows.get(0), column);
            GroupOutputMeta source =
                    Objects.requireNonNull(
                            findPriorGroup(priorGroups, field.source().sourceOperationIndex()));
            String alias = "p" + column.value();
            sql.append("\n  JOIN ")
                    .append(source.ordMapCteName)
                    .append(" ")
                    .append(alias)
                    .append(" ON ")
                    .append(alias)
        ...
        if (!isUpsert) {
            sql.append("  MERGE INTO ")
                    ...
                    .append(
                            String.join(
                                    ", ",
                                    returnColumns.stream()
                                            .map(x -> "t." + q(x.columnName) + " AS " + q(x.alias))
                                            .toList()));
        } else {
            ...
            for (int i = 0; i < columnNames.size(); i++)
                if (!conflicts.contains(columns.get(i))) updateColumns.add(columnNames.get(i));
                        ...
                        .append(
                                String.join(
                                        ...
                                        updateColumns.stream()
                                                .map(
                                                        c ->
                                                                table(entity.effectiveStorageName())
                                                                        + "."
                                                                        + q(c)
                                                                        + " IS DISTINCT FROM"
                                                                        + " EXCLUDED."
            ...
            sql.append("\n  RETURNING ")
                    .append(
                            String.join(
                                    ", ",
                                    returnColumns.stream()
                                            .map(x -> q(x.columnName) + " AS " + q(x.alias))
                                            .toList()))
                    .append("\n)");
        } else {
            sql.append("\n)");
    ...
    private GroupOutputMeta writeUpdateOrDeleteGroup(
        ...
        if (operation.returnFields() != null && !operation.returnFields().isEmpty())
            ...
            for (FieldMetadata f : entity.effectiveFields())
                if (f.column() != null) returnFields.add(f.id());
        for (FieldId f : forcedReturns) if (!returnFields.contains(f)) returnFields.add(f);
        if (isDelete && !forcedReturns.isEmpty())
            throw new UnsupportedOperationException(
                    "A Delete cannot currently be a source of a batched reference; falling back to"
                            + " sequential execution.");

        MutationOperation effective =
                new MutationOperation(
                        operation.entity(),
                        operation.kind(),
                        operation.fields(),
                        operation.filter(),
                        operation.conflictColumns(),
                        returnFields.isEmpty() && !isDelete ? null : returnFields);
        SqlMutationPlan single =
                new SqlMutationCompiler(metadata).compile(new MutationPlan(List.of(effective)));
        int offset = parameters.size();
        String rewritten =
                java.util.regex.Pattern.compile("@p([0-9]+)")
                        .matcher(single.commandText())
                        .replaceAll(
                                m ->
                                        Matcher.quoteReplacement(
                                                "@p" + (offset + Integer.parseInt(m.group(1)))));
    ...
    private static List<FieldId> requestedFields(
            MutationOperation template, EntityMetadata entity, Set<FieldId> forced) {
        ...
        if (template.returnFields() != null && !template.returnFields().isEmpty())
            ...
            for (FieldMetadata f : entity.effectiveFields())
                if (f.column() != null) ids.add(f.id());
        ids.addAll(forced);
        return new ArrayList<>(ids);
    }

    private static List<ReturnColumn> buildReturnColumns(
            EntityMetadata entity, List<FieldId> requestedFields, List<ColumnId> conflicts) {
        List<ReturnColumn> result = new ArrayList<>();
        for (FieldId id : requestedFields) {
            FieldMetadata field = findField(entity, id);
            if (field.column() == null)
                throw new IllegalStateException(
                        "Return field '" + field.name() + "' has no storage column.");
            ColumnId columnId = field.column().columnId();
            result.add(
                    new ReturnColumn(
                            id, columnId, resolveColumn(entity, columnId), "r_" + id.value()));
        }
    ...
    private static Map<FieldId, Class<?>> buildReturnedTypes(
            EntityMetadata entity, List<FieldId> fields) {
        Map<FieldId, Class<?>> result = new LinkedHashMap<>();
        for (FieldId id : fields) {
            FieldMetadata field =
                    entity.effectiveFields().stream()
    ...
    private static String returnAliasForColumn(List<ReturnColumn> columns, String columnName) {
        return columns.stream()
                .filter(x -> x.columnName.equals(columnName))
                .findFirst()
                .orElseThrow(
                        () ->
                                new IllegalStateException(
                                        "Return column '" + columnName + "' was not projected."))
                .alias;
    }

    private static boolean hasDuplicateLiteralKey(
            List<MutationOperation> rows, List<ColumnId> keyColumns) {
    ...
    private static String bindTypedArrayParameter(
            List<SqlParameterBinding> parameters, String hint, Object[] values) {
        Object first = Arrays.stream(values).filter(Objects::nonNull).findFirst().orElse(null);
        if (first == null)
            throw new UnsupportedOperationException(
                    "Column '"
                            + hint
                            + "' is NULL for every row in its batch group; its PostgreSQL array"
        ...
        for (Object value : values)
            if (value != null && !elementType.isInstance(value))
                throw new UnsupportedOperationException(
                        "Column '"
                                + hint
                                + "' contains heterogeneous CLR value types; falling back to"
                                + " sequential execution.");
        ...
        for (MutationFieldValue f : fields) {
            if (f.source() == null) encoded.add(f.column().value() + ":lit");
                ...
                encoded.add(
                        f.column().value()
                                + ":ref:"
                                + opToGroup[f.source().sourceOperationIndex()]
                                + ":"
                                + f.source().sourceField().value());
        }
        String conflicts =
                operation.conflictColumns() == null
                        ? ""
                        : operation.conflictColumns().stream()
                                .sorted(Comparator.comparingLong(ColumnId::value))
                                .map(c -> Long.toString(c.value()))
                                .reduce((a, b) -> a + "," + b)
                                .orElse("");
        String returns =
                operation.returnFields() == null
                        ? ""
                        : operation.returnFields().stream()
                                .sorted(Comparator.comparingLong(FieldId::value))
                                .map(f -> Long.toString(f.value()))
                                .reduce((a, b) -> a + "," + b)
                                .orElse("");
        return operation.entity().id().value()
                + "|"
                + operation.kind()
                + "|"
                + String.join(",", encoded)
    ...
    private static int visit(
            int index,
            Map<Integer, List<MutationDependency>> incoming,
            int[] level,
            boolean[] visiting,
            boolean[] visited) {
        ...
        for (MutationDependency d : incoming.getOrDefault(index, List.of())) {
            ...
            max =
                    Math.max(
                            max,
                            visit(d.sourceOperationIndex(), incoming, level, visiting, visited)
                                    + 1);
        }
        level[index] = max;
    ...
    private static String resolveColumn(EntityMetadata entity, ColumnId id) {
        return entity.columns().stream()
                ...
                .map(ColumnMetadata::effectiveStorageName)
                .findFirst()
                .orElseThrow(
                        () ->
                                new IllegalStateException(
    ...
    private static String table(String storageName) {
        return Arrays.stream(storageName.split("\\."))
                .map(String::trim)
                .filter(s -> !s.isEmpty())
                .map(PostgresBatchedMutationCompiler::q)
                .reduce((a, b) -> a + "." + b)
    ...
    private static MutationFieldValue fieldForColumn(MutationOperation operation, ColumnId column) {
        return operation.fields().stream()
                .filter(f -> f.column().equals(column))
                .findFirst()
                .orElseThrow(
                        () ->
                                new IllegalStateException(
                                        "Mutation operation does not define column '"
                                                + column.value()
                                                + "'."));
    }

    private static FieldMetadata findField(EntityMetadata entity, FieldId id) {
        return entity.effectiveFields().stream()
                .filter(f -> f.id().equals(id))
                .findFirst()
                .orElseThrow(
                        () ->
    ...
    private record ReturnColumn(
            FieldId fieldId, ColumnId columnId, String columnName, String alias) {}
    ...
    private static final class GroupOutputMeta {
        ...
        GroupOutputMeta(
                ...
                List<Integer> operationIndexes,
                boolean ordinalAddressable,
                boolean singleResult,
                Map<FieldId, String> returnedFieldNames,
