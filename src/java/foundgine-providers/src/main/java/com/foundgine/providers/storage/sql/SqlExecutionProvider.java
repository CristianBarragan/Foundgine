 ...
 *
 * <p>
 * The provider deliberately owns execution only: compilation remains in
 * {@link SqlCompiler}. Connection opening, caller transaction participation,
 * runtime authorization-context binding, cancellation and result
 * materialization happen here at the physical provider boundary.
 * </p>
 */
public final class SqlExecutionProvider implements IExecutionProvider {
	 ...
	 * before execution. The provider does not infer transaction ownership from this
	 * callback.
	 */
	...
	/**
	 * Explicit transaction-ownership constructor. When ownership is true and the
	 * connection is currently in auto-commit mode, the provider starts and owns a
	 * transaction for the execution. Existing caller transactions are never
	 * committed or rolled back by this provider.
	 */
	...
	private CompletionStage<ExecutionResult> executeStatement(SqlPlan sqlPlan, ExecutionContext context,
			CancellationToken token, long start, boolean startedTransaction, boolean originalAutoCommit)
			throws Exception {
		...
		try (PreparedStatement statement = connection.prepareStatement(jdbcSql)) {
			...
			try (cancellationRegistration) {
				...
				try (ResultSet rs = statement.executeQuery()) {
					...
					List<Integer> nodes = sqlPlan.authorization().stream().map(SqlAuthorizationPredicate::nodeId)
							.toList();
					String fingerprint = ExecutionEvidenceFactory.hash(buildPlanFingerprint(sqlPlan));
					ExecutionEvidence evidence = ExecutionEvidenceFactory.create("sql", fingerprint, nodes, rows.size(),
							elapsed, sqlPlan.commandText());
	...
	private static void bindParameters(PreparedStatement statement, SqlPlan plan, ExecutionContext context,
			CancellationToken token) throws SQLException {
		int index = 1;
			...
			if (binding.contextPath() != null) {
				if (!context.effectiveValues().containsKey(binding.contextPath())) {
					throw new IllegalStateException(
							"Execution context does not contain authorization value '" + binding.contextPath() + "'.");
				}
				value = context.effectiveValues().get(binding.contextPath());
				if (ExecutionContextKeys.PAGINATION_LIMIT.equals(binding.contextPath()) && plan.pagination() != null
	...
	private static List<ExecutionRow> readRows(ResultSet rs, SqlPlan plan, CancellationToken token)
			throws SQLException {
		List<ExecutionRow> rows = new ArrayList<>();
		Map<String, SqlColumnBinding> bindings = new HashMap<>();
		for (SqlColumnBinding b : plan.columns())
			bindings.put(b.resultName(), b);
	...
	private static ExecutionPageInfo buildPageInfo(List<ExecutionRow> rows, SqlPlan plan, ExecutionContext context) {
		...
		if (rows.isEmpty())
			return new ExecutionPageInfo(null, null, hasNext, hasCursor);
	...
	private static String buildPlanFingerprint(SqlPlan p) {
		String cols = p
				.columns().stream().map(x -> x.resultName() + "|" + x.entityId().value() + "|" + x.fieldId().value()
						+ "|" + x.columnName() + "|" + x.nodeId() + "|" + x.isCursor())
				.sorted().reduce("", (a, b) -> a + ";" + b);
		String auth = p.authorization().stream().map(x -> x.nodeId() + "|" + x.predicate()).sorted().reduce("",
				(a, b) -> a + ";" + b);
