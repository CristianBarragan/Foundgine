 ...
 *
 * <p>The provider deliberately owns execution only: compilation remains in {@link SqlCompiler}.
 * Connection opening, caller transaction participation, runtime authorization-context binding,
 * cancellation and result materialization happen here at the physical provider boundary.
 */
public final class SqlExecutionProvider implements IExecutionProvider {
    ...
    /**
     * Compatibility constructor for callers that need to configure a statement before execution.
     * The provider does not infer transaction ownership from this callback.
     */
    ...
    /**
     * Explicit transaction-ownership constructor. When ownership is true and the connection is
     * currently in auto-commit mode, the provider starts and owns a transaction for the execution.
     * Existing caller transactions are never committed or rolled back by this provider.
     */
    public SqlExecutionProvider(
            Connection connection, Transaction transaction, boolean ownsTransaction) {
    ...
    public CompletionStage<ExecutionResult> executeAsync(
            ProviderPlan plan, ExecutionContext context, CancellationToken token) {
        Objects.requireNonNull(plan, "plan");
        Objects.requireNonNull(context, "context");
        Objects.requireNonNull(token, "token");
        if (!(plan instanceof SqlPlan sqlPlan)) {
            return CompletableFuture.failedFuture(
                    new IllegalArgumentException("The SQL provider requires a SqlPlan."));
        }
        if (sqlPlan.authorizationBinding() == null) {
            return CompletableFuture.failedFuture(
                    new IllegalStateException(
                            "The SQL provider refuses to execute a provider plan without"
        ...
        try {
            ...
            if (connection.isClosed()) {
                return CompletableFuture.failedFuture(
                        new SQLException("The JDBC connection is closed."));
            }
            ...
            try {
                return executeStatement(
                        sqlPlan, context, token, start, startedTransaction, originalAutoCommit);
                    ...
                    token.register(
                            () -> {
                                try {
                                    ...
                                    // Best effort: cancellation remains observable through the
                                    // token.
                                }
                            });
            ...
            try (cancellationRegistration) {
                ...
                try (ResultSet rs = statement.executeQuery()) {
                    ...
                    return CompletableFuture.completedFuture(
                            new ExecutionResult(rows, pageInfo, evidence, null));
                }
            } finally {
                if (startedTransaction) restoreAutoCommit(originalAutoCommit);
            }
            ...
            if (binding.contextPath() != null) {
                if (!context.effectiveValues().containsKey(binding.contextPath())) {
                    throw new IllegalStateException(
                            "Execution context does not contain authorization value '"
                                    + binding.contextPath()
                                    + "'.");
                }
    ...
    private static ExecutionPageInfo buildPageInfo(
            List<ExecutionRow> rows, SqlPlan plan, ExecutionContext context) {
    ...
    private static String encodeCursor(ExecutionRow row, List<SqlCursorBinding> bindings) {
        ...
        for (var b : bindings) {
            ...
            if (value == null) {
                throw new IllegalStateException(
                        "Cursor ordering fields cannot contain null values.");
            }
            values.add(value);
        }
