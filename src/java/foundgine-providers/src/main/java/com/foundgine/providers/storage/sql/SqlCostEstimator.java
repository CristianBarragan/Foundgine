...
public final class SqlCostEstimator implements IProviderCostEstimator {
    ...
    public ProviderCostEstimate estimate(
        ...
        CostEstimateProvenance provenance =
                options.statisticsObservedAtUtc() != null && options.statisticsVersion() != null
                        ? CostEstimateProvenance.fromStatistics(
                                options.statisticsSource(),
                                options.statisticsVersion(),
                                options.statisticsObservedAtUtc(),
                                Instant.now(),
                                options.statisticsStaleAfter())
                        : CostEstimateProvenance.heuristic(
                                options.statisticsSource(), Instant.now());
        double confidence =
        ...
        return ProviderCostEstimate.from(
                provider(), Math.max(0d, cost), rows, confidence, provenance);
    }
    ...
    private double estimateNode(SemanticPlanNode node) {
        ...
        if (node.queryOptions() != null) {
            ...
            if (q.filter() != null) cost += estimateFilter(q.filter());
            cost += q.effectiveOrder().size() * options.orderTermCost();
            if (q.limit() != null && q.limit() > 0)
                cost = Math.max(1d, cost - options.limitAdjustment());
            if (q.offset() != null && q.offset() > 0) cost += options.offsetCost();
            if (q.hasCursor()) cost = Math.max(0d, cost + options.cursorAdjustment());
        }
        entity.name();
        for (var child : node.children()) cost += estimateNode(child);
        return cost;
    }
    ...
    private double estimateFilter(SemanticFilterExpression e) {
        ...
        if (e instanceof SemanticAggregateFilter a)
            return options.aggregateFilterCost()
                    + (a.predicate() == null ? 0d : estimateFilter(a.predicate()));
    ...
    private static double estimateRows(SemanticPlanNode n) {
        double local =
                n.queryOptions() != null
                                && n.queryOptions().limit() != null
                                && n.queryOptions().limit() > 0
                        ? n.queryOptions().limit()
                        : 1000d;
        return Math.max(
                1d,
                local + n.children().stream().mapToDouble(SqlCostEstimator::estimateRows).sum());
    }
}
