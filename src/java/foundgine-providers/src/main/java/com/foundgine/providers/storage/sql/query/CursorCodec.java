...
public final class CursorCodec {
    ...
    public static String encode(List<?> values) {
        ...
        if (values.isEmpty())
            throw new IllegalArgumentException(
                    "A compound cursor must contain at least one value.");
        try {
    ...
    public static List<JsonNode> decode(String cursor) {
        ...
        try {
            ...
            if (payload == null
                    || payload.path("Version").asInt(-1) != VERSION
                    || !payload.has("Values")
                    || !payload.get("Values").isArray()
                    || payload.get("Values").isEmpty())
                throw new IllegalArgumentException(
                        "The pagination cursor has an unsupported format.");
    ...
    public static Object convertValue(JsonNode value, Class<?> targetType) {
        ...
        if (value.isNull()) {
            ...
            throw new IllegalArgumentException(
                    "A null cursor value cannot be converted to non-nullable '"
                            + targetType.getName()
                            + "'.");
        }
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    "Cursor value could not be converted to '" + targetType.getName() + "'.", e);
        }
    }
