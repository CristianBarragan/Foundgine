...
public final class CursorCodec {
	...
	public static String encode(List<?> values) {
		...
		if (values.isEmpty())
			throw new IllegalArgumentException("A compound cursor must contain at least one value.");
		try {
	...
	public static List<JsonNode> decode(String cursor) {
		if (cursor == null || cursor.isBlank())
			throw new IllegalArgumentException("The pagination cursor is invalid.");
		try {
			byte[] bytes = Base64.getDecoder().decode(cursor);
			JsonNode payload = MAPPER.readTree(new String(bytes, StandardCharsets.UTF_8));
			if (payload == null || payload.path("Version").asInt(-1) != VERSION || !payload.has("Values")
					|| !payload.get("Values").isArray() || payload.get("Values").isEmpty())
			...
			throw new IllegalArgumentException("The pagination cursor is invalid.", e);
		} catch (Exception e) {
			throw new IllegalArgumentException("The pagination cursor is invalid.", e);
		}
	}
	...
	public static Object convertValue(JsonNode value, Class<?> targetType) {
		...
		if (value.isNull()) {
			...
			throw new IllegalArgumentException(
					"A null cursor value cannot be converted to non-nullable '" + targetType.getName() + "'.");
		}
		...
		try {
			...
			throw new IllegalArgumentException("Cursor value could not be converted to '" + targetType.getName() + "'.",
					e);
		}
	}
