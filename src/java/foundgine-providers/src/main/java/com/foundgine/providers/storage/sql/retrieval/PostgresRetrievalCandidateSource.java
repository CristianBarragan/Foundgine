 ...
 *
 * <p>Retrieval is advisory evidence only. Semantic resolution and authorization remain outside the
 * physical retrieval provider.
 */
public final class PostgresRetrievalCandidateSource implements IApproximateCandidateSource {
    ...
    public PostgresRetrievalCandidateSource(
            DataSource dataSource, IMetadataCatalog metadata, PostgresRetrievalOptions options) {
        ...
        return switch (request.strategy()) {
            ...
            default ->
                    throw new IllegalArgumentException(
                            "Unknown retrieval strategy: " + request.strategy());
        };
    }

    private List<RetrievalCandidate> executeText(
        ...
        try (Connection connection = dataSource.getConnection();
            ...
            if (request.strategy() == RetrievalStrategy.FUZZY
                    || request.strategy() == RetrievalStrategy.FULL_TEXT)
                statement.setString(i++, request.query());
            ...
            throw new IllegalStateException(
                    "PostgreSQL " + request.strategy() + " retrieval failed.", ex);
        }
    }

    private List<RetrievalCandidate> executeGraphSimilarity(
            EntityMetadata entity, SemanticRetrievalRequest request) {
        ...
        String sql =
                "SELECT * FROM cypher("
                        + quoteLiteral(options.ageGraphName())
                        + ", ?) "
            ...
            try (PreparedStatement command = connection.prepareStatement(sql)) {
                ...
                try (ResultSet result = command.executeQuery()) {
                    return readCandidates(
                            ...
                            entity.entityId(),
                            null,
                            CandidateEvidenceKind.GRAPH_SIMILARITY);
                }
            }
        } catch (SQLException ex) {
    ...
    private static List<RetrievalCandidate> readCandidates(
            ResultSet result, EntityId entityType, FieldId field, CandidateEvidenceKind kind)
            throws SQLException {
                    ...
                    new RetrievalCandidate(
                            ...
                            List.of(
                                    new ResolutionEvidence(
                                            kind
                                                    + " retrieval matched semantic candidate "
                                                    + recordId
                                                    + " with score "
                                                    + formatScore(score)
                                                    + ".",
                ...
                .orElseThrow(
                        () ->
                                new IllegalStateException(
                                        "Entity "
                                                + entity.name()
                                                + " has no string field available for approximate"
                                                + " retrieval."));
    }
    ...
    private static String quoteStorage(String value) {
        ...
        return Arrays.stream(parts)
                .filter(x -> !x.isBlank())
                .map(PostgresRetrievalCandidateSource::quoteIdentifier)
                .reduce((a, b) -> a + "." + b)
                .orElseThrow();
    }
