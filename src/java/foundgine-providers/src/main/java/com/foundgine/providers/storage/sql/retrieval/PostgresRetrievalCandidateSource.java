 ...
 *
 * <p>Retrieval is advisory evidence only. Semantic resolution and
 * authorization remain outside the physical retrieval provider.</p>
 */
public final class PostgresRetrievalCandidateSource implements IApproximateCandidateSource {
    ...
    public List<RetrievalCandidate> retrieve(SemanticRetrievalRequest request) {
        ...
        return switch (request.strategy()) {
            case FUZZY -> {
                ...
                yield executeText(entity, resolveField(entity, request.field()), request,
                        CandidateEvidenceKind.TRIGRAM, fuzzySql(entity, resolveField(entity, request.field())));
            }
            case FULL_TEXT -> {
                requireEnabled(options.enableFullText(), RetrievalStrategy.FULL_TEXT);
                yield executeText(entity, resolveField(entity, request.field()), request,
                        CandidateEvidenceKind.FULL_TEXT, fullTextSql(entity, resolveField(entity, request.field())));
            }
            case SEARCH -> {
                requireEnabled(options.enablePgSearch(), RetrievalStrategy.SEARCH);
                yield executeText(entity, resolveField(entity, request.field()), request,
                        CandidateEvidenceKind.BM25, searchSql(entity, resolveField(entity, request.field())));
            }
    ...
    private List<RetrievalCandidate> executeText(EntityMetadata entity, FieldMetadata field,
                                                  ...
                                                  CandidateEvidenceKind evidenceKind,
                                                  String sql) {
    ...
    private List<RetrievalCandidate> executeGraphSimilarity(EntityMetadata entity,
                                                              SemanticRetrievalRequest request) {
        ...
        String cypher = "MATCH (reference:" + sourceLabel + ")-[:" + edgeLabel + "]->(neighbor) " +
                "<-[:" + edgeLabel + "]-(candidate:" + sourceLabel + ") " +
                "WHERE reference.id = \"" + reference + "\" AND candidate.id <> \"" + reference + "\" " +
                "RETURN candidate.id, count(*) AS score " +
    ...
    private static List<RetrievalCandidate> readCandidates(ResultSet result, EntityId entityType,
                                                           FieldId field, CandidateEvidenceKind kind) throws SQLException {
        ...
        while (result.next()) {
            ...
            candidates.add(new RetrievalCandidate(
                    entityType, recordId, score, field, recordId,
                    List.of(new ResolutionEvidence(
                            kind + " retrieval matched semantic candidate " + recordId +
                                    " with score " + formatScore(score) + ".", kind, score)), kind));
        }
    ...
    private FieldMetadata resolveField(EntityMetadata entity, FieldId fieldId) {
        ...
        return entity.effectiveFields().stream()
                .filter(x -> x.clrType() == String.class && x.column() != null)
                .findFirst()
    ...
    private String fuzzySql(EntityMetadata entity, FieldMetadata field) {
        ...
        return "SELECT " + identity + ", similarity(" + column + ", ?) AS score " +
                "FROM " + table + " WHERE " + column + " % ? ORDER BY score DESC LIMIT ?";
    }
    ...
    private String fullTextSql(EntityMetadata entity, FieldMetadata field) {
        ...
        return "SELECT " + identity + ", ts_rank_cd(to_tsvector(" + config + ", COALESCE(" + column + "::text, '')), " +
                "websearch_to_tsquery(" + config + ", ?)) AS score FROM " + table +
                " WHERE to_tsvector(" + config + ", COALESCE(" + column + "::text, '')) @@ " +
                "websearch_to_tsquery(" + config + ", ?) ORDER BY score DESC LIMIT ?";
    }
    ...
    private String searchSql(EntityMetadata entity, FieldMetadata field) {
        ...
        String column = quoteIdentifier(getColumnName(entity, field));
        String identity = quoteIdentifier(getPrimaryKeyName(entity));
        return "SELECT " + identity + ", pdb.score(" + identity + ") AS score FROM " + table +
    ...
    private String getColumnName(EntityMetadata entity, FieldMetadata field) {
        ...
        return entity.columns().stream()
                ...
                .findFirst()
                .orElseThrow(() -> new IllegalStateException(
                        "Column " + field.column().columnId() + " for field " + field.name() +
    ...
    private String getPrimaryKeyName(EntityMetadata entity) {
        ...
        return entity.columns().stream()
                .filter(x -> x.id().equals(entity.primaryKey().columnId()))
                .findFirst()
                .orElseThrow(() -> new IllegalStateException(
                        "Primary key column " + entity.primaryKey().columnId() +
                                " is not registered on entity " + entity.name() + "."))
                .effectiveStorageName();
    }
    ...
    private static void requireEnabled(boolean enabled, RetrievalStrategy strategy) {
        if (!enabled) throw new UnsupportedOperationException(
                "PostgreSQL retrieval strategy " + strategy + " is disabled.");
    }

    private static String quoteStorage(String value) {
        String[] parts = value.split("\\.");
        if (parts.length == 0) throw new IllegalArgumentException("Storage name cannot be empty.");
