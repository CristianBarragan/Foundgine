...
public final class SemanticQuerySqlWriter {
    ...
    public static String writeWhere(
        ...
        return writeFilter(
                filter, entity, alias, parameters, metadata, aggregateStrategy, true, counters);
    }

    public static String writeOrder(
            List<SemanticOrderTerm> terms, EntityMetadata entity, String alias) {
        ...
        for (SemanticOrderTerm term : terms) {
            ...
            parts.add(
                    SqlCompiler.quoteIdentifier(alias)
                            + "."
    ...
    private static String writeFilter(
            ...
            IMetadataProvider metadata,
            AggregateExecutionStrategy aggregateStrategy,
            boolean allowAggregateStrategy,
            int[] counters) {
        ...
        if (expression instanceof SemanticRelationshipFilter rf)
            return writeRelationshipFilter(
                    rf, entity, alias, parameters, metadata, aggregateStrategy, counters);
    ...
    private static String writeRelationshipFilter(
        ...
        String exists =
                "EXISTS (SELECT 1 FROM "
                        ...
                        + " "
                        + SqlCompiler.quoteIdentifier(targetAlias)
                        + " WHERE "
                        + join
                        + " AND "
                        + predicate
    ...
    private static String writeAggregateFilter(
        ...
        if (allowStrategy && AggregateExecutionStrategyResolver.isEligibleFor(filter, strategy)) {
            String exists =
                    "EXISTS (SELECT 1 FROM "
                            ...
                            + join
                            + ")";
            return strategy == AggregateExecutionStrategy.COUNT_EXISTS_SHORT_CIRCUIT
                    ? exists
                    : "NOT " + exists;
        }
        String predicate =
        ...
        switch (filter.aggregate()) {
            ...
            case MIN, MAX -> {
                ...
                FieldMetadata field =
                        target.effectiveFields().stream()
                                .filter(x -> x.id().equals(filter.field()))
                                .findFirst()
                                .orElseThrow(
                                        () ->
                                                new IllegalArgumentException(
                                                        "Unknown aggregate filter field '"
                                                                + filter.field()
                                                                + "' on '"
                                                                + target.name()
                                                                + "'."));
        ...
        String name = "p" + counters[0]++;
        Object value =
                filter.aggregate() == SemanticFilterAggregate.COUNT
    ...
    private static long convertCountComparisonValue(Object value) {
        if (value == null)
            throw new IllegalArgumentException(
                    "A Count aggregate filter requires a comparison value.");
        if (value instanceof Number n) return n.longValue();
        try {
            return Long.parseLong(value.toString());
        } catch (RuntimeException ex) {
            throw new IllegalArgumentException(
                    "Count aggregate filter value '" + value + "' is not an integer.", ex);
        }
    }
    ...
    private static String renderJoinCondition(
            ColumnReference sourceRef,
            ColumnReference targetRef,
            EntityMetadata source,
            String sourceAlias,
    ...
    private static String renderReference(
        ...
        if (ref.entityId().equals(source.entityId())) {
            ...
            throw new IllegalArgumentException(
                    "Relationship join references an entity outside its endpoints.");
        ColumnMetadata column =
                entity.columns().stream()
                        .filter(x -> x.id().equals(ref.columnId()))
                        .findFirst()
                        .orElseThrow(
                                () ->
                                        new IllegalArgumentException(
    ...
    private static List<Object> normalizeList(Object value) {
        ...
        if (value.getClass().isArray()) {
            ...
            for (int i = 0; i < n; i++) r.add(Array.get(value, i));
            return r;
        }
