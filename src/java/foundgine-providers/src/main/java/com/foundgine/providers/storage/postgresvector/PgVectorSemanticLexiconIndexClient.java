...
public final class PgVectorSemanticLexiconIndexClient {
    ...
    public PgVectorSemanticLexiconIndexClient(
            DataSource d, PgVectorSemanticLexicalCandidateSource.EmbeddingGenerator e) {
        this(d, e, null);
    }

    public PgVectorSemanticLexiconIndexClient(
            DataSource d,
            PgVectorSemanticLexicalCandidateSource.EmbeddingGenerator e,
            PgVectorOptions o) {
        ...
        try (Connection c = ds.getConnection();
            ...
            s.execute(
                    "CREATE TABLE IF NOT EXISTS "
                            + table
                            + " (id bigserial primary key,canonical_name text not null,kind text"
                            + " not null,search_text text not null,aliases text[] not null default"
                            + " ARRAY[]::text[],description text,entity_id bigint,relationship_id"
                            + " bigint,field_id bigint,source_entity_id bigint,target_entity_id"
                            + " bigint,value text,embedding vector("
                            + options.dimensions()
                            + "))");
            s.execute(
                    "CREATE INDEX IF NOT EXISTS "
                            + identifier(options.tableName() + "_kind_idx")
                            + " ON "
                            + table
                            + " (kind)");
            s.execute(
                    "CREATE INDEX IF NOT EXISTS "
                            + identifier(options.tableName() + "_embedding_hnsw_idx")
                            + " ON "
                            + table
                            + " USING hnsw (embedding "
                            + VectorOpsClass(options.distance())
                            + ")");
    ...
    public void indexContract(SemanticContractSnapshot contract) {
        ...
        List<float[]> vectors =
                embeddings.embedMany(
                        entries.stream().map(SemanticLexiconEntry::searchText).toList());
        if (vectors.size() != entries.size())
            throw new IllegalStateException(
                    "Embedding generator returned a different number of vectors than lexicon"
                            + " entries.");
        try (Connection c = ds.getConnection()) {
            c.setAutoCommit(false);
            try {
                try (Statement s = c.createStatement()) {
                    s.executeUpdate("TRUNCATE TABLE " + options.qualifiedTableName());
                }
    ...
    public void indexEntry(SemanticLexiconEntry entry) {
        ...
        try (Connection c = ds.getConnection()) {
            insert(c, entry, vector);
        } catch (SQLException e) {
            throw new IllegalStateException("Unable to index pgvector lexicon entry", e);
        }
    }
    ...
    private void insert(Connection c, SemanticLexiconEntry e, float[] vector) throws SQLException {
        String sql =
                "INSERT INTO "
                        + options.qualifiedTableName()
                        + "(canonical_name,kind,search_text,aliases,description,entity_id,relationship_id,field_id,source_entity_id,target_entity_id,value,embedding)"
                        + " VALUES(?,?,?,?,?,?,?,?,?,?,?,?::vector)";
        try (PreparedStatement p = c.prepareStatement(sql)) {
            ...
            p.setString(11, e.value());
            p.setString(12, vectorLiteral(vector));
            p.executeUpdate();
        }
    }
