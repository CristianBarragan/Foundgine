	...
	public void ensureSchema() {
		...
		try (Connection c = ds.getConnection(); Statement s = c.createStatement()) {
			...
			s.execute("CREATE TABLE IF NOT EXISTS " + table + " ("
					...
					+ "field_id bigint,source_entity_id bigint,target_entity_id bigint,value text,embedding vector("
					+ options.dimensions() + "))");
			s.execute("CREATE INDEX IF NOT EXISTS " + identifier(options.tableName() + "_kind_idx") + " ON " + table
					+ " (kind)");
			s.execute("CREATE INDEX IF NOT EXISTS " + identifier(options.tableName() + "_embedding_hnsw_idx") + " ON "
					+ table + " USING hnsw (embedding " + VectorOpsClass(options.distance()) + ")");
	...
	public void indexContract(SemanticContractSnapshot contract) {
		...
		if (vectors.size() != entries.size())
			throw new IllegalStateException(
					"Embedding generator returned a different number of vectors than lexicon entries.");
		try (Connection c = ds.getConnection()) {
			...
			try {
				...
				for (int i = 0; i < entries.size(); i++)
					insert(c, entries.get(i), vectors.get(i));
				c.commit();
