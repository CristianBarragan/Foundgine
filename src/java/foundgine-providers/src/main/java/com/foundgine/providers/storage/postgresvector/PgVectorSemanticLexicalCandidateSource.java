...
/**
 * pgvector implementation of Foundgine's provider-neutral lexical candidate source. pgvector
 * proposes ranked hypotheses; the semantic contract remains authoritative for meaning, topology,
 * and authorization.
 */
public final class PgVectorSemanticLexicalCandidateSource
    ...
    public PgVectorSemanticLexicalCandidateSource(
            DataSource ds, EmbeddingGenerator eg, PgVectorOptions o) {
    ...
    public List<SemanticLexicalCandidate> retrieve(SemanticLexicalRequest request) {
        ...
        StringBuilder sql =
                new StringBuilder(
                                "SELECT"
                                    + " canonical_name,kind,entity_id,relationship_id,field_id,source_entity_id,target_entity_id,value,embedding"
                                    + " ")
                        .append(distanceOperator)
                        .append(" ?::vector AS distance FROM ")
                        .append(options.qualifiedTableName())
        ...
        try (Connection c = dataSource.getConnection();
            ...
            p.setArray(
                    i++,
                    c.createArrayOf(
                            "text", kinds.stream().map(Enum::toString).toArray(String[]::new)));
            if (request.contextEntity() != null) {
            ...
            try (ResultSet rs = p.executeQuery()) {
                ...
                while (rs.next()) {
                    SemanticLexicalCandidate candidate =
                            readCandidate(request.token(), rs, options.distance());
    ...
    private static SemanticLexicalCandidate readCandidate(
            String token, ResultSet rs, PgVectorDistance metric) throws SQLException {
        ...
        return new SemanticLexicalCandidate(
                ...
                List.of(
                        new ResolutionEvidence(
                                "pgvector nearest-neighbor match for '"
                                        + token
                                        + "' ("
                                        + metric
                                        + " distance "
                                        + distance
                                        + ").",
                                CandidateEvidenceKind.VECTOR_SIMILARITY,
                                score)));
    }
