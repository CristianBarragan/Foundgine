...
public final class PgVectorSemanticLexicalCandidateSource implements ISemanticLexicalCandidateSource {
    public interface EmbeddingGenerator {
        ...
        default List<float[]> embedMany(List<String> texts) { return texts.stream().map(this::embed).toList(); }
    }
    ...
    public List<SemanticLexicalCandidate> retrieve(SemanticLexicalRequest request) {
        ...
        StringBuilder sql = new StringBuilder("SELECT canonical_name,kind,entity_id,relationship_id,field_id,source_entity_id,target_entity_id,value,embedding ")
            .append(distanceOperator).append(" ?::vector AS distance FROM ").append(options.qualifiedTableName())
            .append(" WHERE kind = ANY(?)");
        if (request.contextEntity() != null) sql.append(" AND (entity_id = ? OR source_entity_id = ? OR target_entity_id = ?)");
        ...
        try (Connection c = dataSource.getConnection(); PreparedStatement p = c.prepareStatement(sql.toString())) {
            ...
            if (request.contextEntity() != null) {
                ...
                p.setLong(i++, id); p.setLong(i++, id); p.setLong(i++, id);
            }
            p.setString(i++, vectorParameter);
    ...
    private static String vectorLiteral(float[] vector) {
        ...
        for (int i = 0; i < vector.length; i++) { if (i > 0) b.append(','); b.append(Float.toString(vector[i])); }
        return b.append(']').toString();
    }
    private static SemanticLexicalCandidate readCandidate(String token, ResultSet rs, PgVectorDistance metric) throws SQLException {
        SemanticLexicalCandidateKind kind;
        try { kind = SemanticLexicalCandidateKind.valueOf(rs.getString(2).toUpperCase(Locale.ROOT)); }
        catch (IllegalArgumentException ex) { return null; }
        double distance = rs.getDouble(9);
        double score = ToScore(distance, metric);
        return new SemanticLexicalCandidate(token, kind, rs.getString(1), score,
            entityId(rs,3), relationshipId(rs,4), fieldId(rs,5), entityId(rs,6), entityId(rs,7), rs.getString(8),
            List.of(new ResolutionEvidence("pgvector nearest-neighbor match for '" + token + "' (" + metric + " distance " + distance + ").", CandidateEvidenceKind.VECTOR_SIMILARITY, score)));
    }
    private static EntityId entityId(ResultSet r, int i) throws SQLException { return r.getObject(i) == null ? null : new EntityId(r.getLong(i)); }
    private static RelationshipId relationshipId(ResultSet r, int i) throws SQLException { return r.getObject(i) == null ? null : new RelationshipId(r.getLong(i)); }
    private static FieldId fieldId(ResultSet r, int i) throws SQLException { return r.getObject(i) == null ? null : new FieldId(r.getLong(i)); }
}
