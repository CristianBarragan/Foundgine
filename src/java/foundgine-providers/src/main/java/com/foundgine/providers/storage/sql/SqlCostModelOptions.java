...
public record SqlCostModelOptions(
        ...
        double traverseCost,
        double filterCost,
        double relationshipFilterCost,
        double aggregateFilterCost,
        double orderTermCost,
        double limitAdjustment,
        double offsetCost,
        double cursorAdjustment,
        double traversalOrderDiscount,
        String statisticsSource,
        String statisticsVersion,
        Instant statisticsObservedAtUtc,
        Duration statisticsStaleAfter) {
    ...
    private static void validateNonNegative(double v, String n) {
        if (Double.isNaN(v) || Double.isInfinite(v) || v < 0) throw new IllegalArgumentException(n);
    }
}
