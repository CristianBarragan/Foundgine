...
class McpMutationBoundaryParityTest {
    ...
    void mutationToolIsTransportOnly() throws Exception {
        var tools =
                new FoundgineMcpMutationTools(
                        json -> CompletableFuture.completedFuture("accepted:" + json));
        assertEquals(
                "accepted:{\"mutation\":\"cancelOrder\"}",
                tools.foundgineMutation("{\"mutation\":\"cancelOrder\"}")
                        .toCompletableFuture()
                        .get());
    }
