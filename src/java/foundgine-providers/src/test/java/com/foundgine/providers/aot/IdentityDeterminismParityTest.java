 ...
 *
 * <p>The C# suite drives a real Roslyn {@code CSharpGeneratorDriver} against hand-written source
 * text and inspects the emitted {@code GeneratedMetadata} class. Java has no compile-time
 * source-generator equivalent here — {@link FoundgineMetadataGenerator} is a documented "runtime
 * reflection fallback" (see its class javadoc) that computes the same identities from annotated
 * {@code Class<?>} objects at runtime instead of from parsed source text. The properties under test
 * — that identity depends on declared semantic names, not on declaration order or on what else is
 * in the module — are still meaningful and portable, so this file exercises them against the
 * reflection path.
 *
 * <p>Two C# cases are intentionally NOT ported here, because they don't have a behavior to assert
 * on the Java side — see the notes at the bottom of this file rather than silently dropping them.
 */
class IdentityDeterminismParityTest {
    ...
    void generatedEntityAndFieldIdsAreIndependentOfDeclarationOrder() {
        var inOrder =
                byFieldName(FoundgineMetadataGenerator.fields(CustomerFieldsInDeclaredOrder.class));
        var reordered =
                byFieldName(FoundgineMetadataGenerator.fields(CustomerFieldsReordered.class));
    ...
    void existingGeneratedIdsSurviveAddingAnUnrelatedType() {
        ...
        var composed =
                byFieldName(
                        FoundgineMetadataGenerator.fields(CustomerAlone.class, UnrelatedOrder.class)
                                .stream()
    ...
    private static Map<String, GeneratedSemanticField> byFieldName(
            List<GeneratedSemanticField> fields) {
