...
/**
 * Provider-level SQL rendering contracts mirrored from the C# E2E SQL pipeline suite.
 * These tests intentionally inspect the provider-neutral plan -> physical SQL boundary
...
class SqlCompilerParityTest {
    ...
    void aggregateCountGreaterThanZeroUsesCountByDefault() {
        var filter = new SemanticAggregateFilter(
                CUSTOMER_ACCOUNTS, SemanticFilterAggregate.COUNT, null,
    ...
    private static MetadataRegistry metadata(String customerStorage, String accountStorage) {
        ...
        registry.register(new EntityMetadata(CUSTOMER, "Customer",
                ...
                customerStorage,
                List.of(
                        new FieldMetadata(CUSTOMER_ID, "Id", Long.class, new ColumnReference(CUSTOMER, customerId)),
                        new FieldMetadata(CUSTOMER_NAME, "Name", String.class, new ColumnReference(CUSTOMER, customerName))),
        ...
        registry.register(new EntityMetadata(ACCOUNT, "Account",
                List.of(new ColumnMetadata(accountId, "Id"), new ColumnMetadata(accountCustomerId, "CustomerId"),
                        new ColumnMetadata(accountBalance, "Balance")),
                accountStorage,
                List.of(
                        new FieldMetadata(ACCOUNT_ID, "Id", Long.class, new ColumnReference(ACCOUNT, accountId)),
                        new FieldMetadata(ACCOUNT_CUSTOMER_ID, "CustomerId", Long.class, new ColumnReference(ACCOUNT, accountCustomerId)),
                        new FieldMetadata(ACCOUNT_BALANCE, "Balance", Long.class, new ColumnReference(ACCOUNT, accountBalance))),
                new ColumnReference(ACCOUNT, accountId), null, false, null, null));

        registry.register(new RelationshipMetadata(
                CUSTOMER_ACCOUNTS, CUSTOMER, ACCOUNT, "Accounts",
                new ColumnReference(CUSTOMER, customerId),
                new ColumnReference(ACCOUNT, accountCustomerId), true, null));
        return registry;
    }
}
