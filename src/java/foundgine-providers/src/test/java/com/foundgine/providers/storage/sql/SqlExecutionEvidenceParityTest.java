...
/**
 * Port of C# {@code
 * Foundgine.E2E.Tests.EvidenceTests.Sql_execution_returns_provider_neutral_evidence} and {@code
 * Equivalent_sql_plans_produce_the_same_plan_fingerprint}.
 *
 * <p>The C# original resolves its plan through {@code
 * Foundgine.Generated.GeneratedMetadata.Registry} (a source-generator artifact this port has no
 * equivalent for) and executes against an in-memory SQLite database. This port builds an equivalent
 * hand-written {@link MetadataRegistry} for a tenant-scoped "contracts" entity and runs against the
 * same opt-in real-PostgreSQL pattern already used by {@code SqlExecutionPostgresE2ETest} / {@code
 * UpsertParityTest} (set {@code FOUNDGINE_POSTGRES_CONNECTION_STRING} to run these). The behavior
 * under test &mdash; that SQL execution returns provider-neutral, non-raw-text evidence, and that
 * structurally equivalent plans fingerprint identically &mdash; is preserved exactly; only the
 * metadata source and database backend differ.
 */
class SqlExecutionEvidenceParityTest {
    ...
    void sqlExecutionReturnsProviderNeutralEvidence() throws Exception {
        ...
        Assumptions.assumeTrue(
                url != null, "Set FOUNDGINE_POSTGRES_CONNECTION_STRING to run this test.");
        ...
        try (Connection connection = DriverManager.getConnection(url)) {
            ...
            assertFalse(
                    evidence.providerOperationFingerprint() == null
    ...
    void equivalentSqlPlansProduceTheSamePlanFingerprint() throws Exception {
        ...
        try (Connection connection = DriverManager.getConnection(url)) {
            ...
            ExecutionResult first =
                    new SqlExecutionProvider(connection)
                            .executeAsync(sql1, context, CancellationToken.NONE)
                            .toCompletableFuture()
                            .join();
            ExecutionResult second =
                    new SqlExecutionProvider(connection)
                            .executeAsync(sql2, context, CancellationToken.NONE)
                            .toCompletableFuture()
                            .join();
    ...
    private static SemanticPlan planFor(int rootId) {
        SemanticPlanNode root =
                new SemanticPlanNode(
                        ...
                        CONTRACT,
                        List.of(ID_FIELD, CONTRACT_TYPE_FIELD),
                        null,
                        null,
                        List.of(),
    ...
    private static MetadataRegistry metadata(String schema) {
        ...
        registry.register(
                new EntityMetadata(
                        ...
                        "Contract",
                        List.of(
                                new ColumnMetadata(ID_COLUMN, "id"),
                                new ColumnMetadata(CONTRACT_TYPE_COLUMN, "contract_type"),
                        ...
                        List.of(
                                new FieldMetadata(
                                        ...
                                        "Id",
                                        Long.class,
                                        new ColumnReference(CONTRACT, ID_COLUMN)),
                                new FieldMetadata(
                                        CONTRACT_TYPE_FIELD,
                                        "ContractType",
                                        Long.class,
    ...
    private static void createSchema(Connection c, String schema) throws SQLException {
        try (Statement s = c.createStatement()) {
            ...
            s.execute(
                    "CREATE TABLE \""
                            + schema
                            + "\".\"contracts\" (\"id\" BIGSERIAL PRIMARY KEY, \"contract_type\""
    ...
    private static void seed(Connection c, String schema) throws SQLException {
        try (Statement s = c.createStatement()) {
            s.execute(
                    "INSERT INTO \""
                            + schema
                            + "\".\"contracts\" (\"contract_type\",\"tenant_id\") VALUES (0, 7),"
                            + " (1, 9)");
        }
    }
