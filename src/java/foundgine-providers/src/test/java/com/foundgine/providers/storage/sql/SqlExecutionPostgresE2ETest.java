...
/**
 * Opt-in real PostgreSQL read-path parity coverage for the provider-neutral Execution IR ->
 * SqlCompiler -> JDBC execution boundary.
 *
 * <p>Set FOUNDGINE_POSTGRES_CONNECTION_STRING to a JDBC URL, for example
 * jdbc:postgresql://localhost:55432/foundgine_e2e?user=foundgine&password=foundgine. The test
 * creates an isolated schema and removes it in finally, so it does not depend on the repository's
 * seeded C# database.
 */
class SqlExecutionPostgresE2ETest {
    ...
    @Test
    void realPostgres_executesAuthorizedFilteredRelationshipQuery_andMaterializesCells()
            throws Exception {
        String url = jdbcUrl();
        Assumptions.assumeTrue(
                url != null,
                "Set FOUNDGINE_POSTGRES_CONNECTION_STRING to run PostgreSQL E2E tests.");
        ...
        try (Connection connection = DriverManager.getConnection(url)) {
            ...
            SemanticRelationshipFilter relationshipFilter =
                    new SemanticRelationshipFilter(
                            ACCOUNTS,
                            SemanticRelationshipQuantifier.SOME,
                            new SemanticFieldFilter(
                                    ACCOUNT_BALANCE, SemanticFilterOperator.GTE, 100L));

            SemanticQueryOptions options =
                    new SemanticQueryOptions(relationshipFilter, List.of(), 10, null, null);
            ...
            SemanticPlan plan =
                    new SemanticPlan(
                            ...
                            new SemanticPlanAuthorizationBinding(
                                    "postgres-e2e-contract", "postgres-e2e-auth"));

            SqlCompiler compiler = new SqlCompiler(metadata);
            SqlPlan sqlPlan = compiler.compile(plan);

            assertTrue(
                    sqlPlan.commandText().contains("EXISTS (SELECT 1 FROM"), sqlPlan.commandText());
            ...
            ExecutionResult result =
                    new SqlExecutionProvider(connection)
                            .executeAsync(sqlPlan, context, CancellationToken.NONE)
                            .toCompletableFuture()
                            .join();
    ...
    @Test
    void realPostgres_executesNestedRelationshipProjectionWithAuthorizationAndQuantifiers()
            throws Exception {
        String url = jdbcUrl();
        Assumptions.assumeTrue(
                url != null,
        ...
        try (Connection connection = DriverManager.getConnection(url)) {
                            ...
                            new SemanticAndFilter(
                                    List.of(
                                            ...
                                            new SemanticRelationshipFilter(
                                                    ...
                                                    new SemanticFieldFilter(
                                                            ACCOUNT_BALANCE,
                                                            SemanticFilterOperator.GT,
                                                            400L)))),
                            List.of(
                                    new SemanticOrderTerm(
                                            CUSTOMER_NAME, SemanticSortDirection.ASC)),
                            10,
                            null,
                            null);

            SemanticPlanNode child =
                    new SemanticPlanNode(
                            2,
                            ExecutionOperation.SCAN,
                            ACCOUNT,
                            List.of(ACCOUNT_ID, ACCOUNT_BALANCE),
                            ACCOUNTS,
                            null,
                            List.of(),
            ...
            SqlPlan sqlPlan =
                    new SqlCompiler(metadata)
                            .compile(
                                    new SemanticPlan(
                                            root,
                                            List.of(),
                                            new SemanticPlanAuthorizationBinding(
                                                    "postgres-nested-contract",
                                                    "postgres-nested-auth")));
            ...
            assertEquals(
                    150L,
                    ((Number) result.rows().get(0).values().get("__fg_2_Balance")).longValue());
            assertEquals(
                    250L,
                    ((Number) result.rows().get(1).values().get("__fg_2_Balance")).longValue());
            assertEquals(
                    300L,
                    ((Number) result.rows().get(2).values().get("__fg_2_Balance")).longValue());
    ...
    @Test
    void realPostgres_relationshipAllAndAggregateCountRemainFailClosedAndParameterized()
            throws Exception {
        String url = jdbcUrl();
        Assumptions.assumeTrue(
                url != null,
        ...
        try (Connection connection = DriverManager.getConnection(url)) {
            ...
            SemanticFilterExpression allBalancesAtLeast100 =
                    new SemanticRelationshipFilter(
                            ACCOUNTS,
            ...
            ExecutionResult allResult =
                    new SqlExecutionProvider(connection)
                            .executeAsync(
                                    allSql,
                                    new ExecutionContext(
                                            Map.of(ExecutionContextKeys.PAGINATION_LIMIT, 10)),
                                    CancellationToken.NONE)
                            .toCompletableFuture()
                            .join();
            assertEquals(3, allResult.rows().size());

            SemanticAggregateFilter countAtLeastOne =
    ...
    private static SemanticPlan planWithFilter(SemanticFilterExpression filter, int limit) {
        SemanticQueryOptions options =
                new SemanticQueryOptions(
                        filter,
                        List.of(new SemanticOrderTerm(CUSTOMER_ID, SemanticSortDirection.ASC)),
                        limit,
                        null,
                        null);
        SemanticPlanNode root =
        ...
        return new SemanticPlan(
                ...
                List.of(),
                new SemanticPlanAuthorizationBinding(
                        "postgres-quantifier-contract", "postgres-quantifier-auth"));
    }

    @Test
    void realPostgres_cursorPagination_roundTripsEndCursorAndUsesStablePrimaryKeyTieBreak()
            throws Exception {
        String url = jdbcUrl();
        Assumptions.assumeTrue(
                url != null,
        ...
        try (Connection connection = DriverManager.getConnection(url)) {
            ...
            ExecutionResult first =
                    new SqlExecutionProvider(connection)
                            .executeAsync(firstPlan, context, CancellationToken.NONE)
                            .toCompletableFuture()
                            .join();
            ...
            ExecutionResult second =
                    new SqlExecutionProvider(connection)
                            .executeAsync(secondPlan, context, CancellationToken.NONE)
                            .toCompletableFuture()
                            .join();
    ...
    @Test
    void realPostgres_aggregateCountOrdering_roundTripsCursorWithStableRootKeyTieBreak()
            throws Exception {
        String url = jdbcUrl();
        Assumptions.assumeTrue(
                url != null,
        ...
        try (Connection connection = DriverManager.getConnection(url)) {
            ...
            // ordering by a COUNT over a to-many relationship must stay a correlated subquery,
            // never a JOIN
            ...
            ExecutionResult first =
                    new SqlExecutionProvider(connection)
                            .executeAsync(firstSql, context, CancellationToken.NONE)
                            .toCompletableFuture()
                            .join();
            ...
            ExecutionResult second =
                    new SqlExecutionProvider(connection)
                            .executeAsync(secondSql, context, CancellationToken.NONE)
                            .toCompletableFuture()
                            .join();
    ...
    private static SemanticPlan cursorPlan(String after) {
                ...
                new SemanticPlanNode(
                        ...
                        AuthorizationPredicate.equal(
                                AuthorizationPredicate.member(
                                        ...
                                        "TenantId"),
                                AuthorizationPredicate.member(
                                        AuthorizationPredicate.contextParameter("tenant"), "id")),
                        null,
                        RelationshipTraversalMode.DEFAULT,
                        -1,
                        AggregateExecutionStrategy.DEFAULT);
        return new SemanticPlan(
                ...
                List.of(),
                new SemanticPlanAuthorizationBinding(
                        "postgres-cursor-contract", "postgres-cursor-auth"));
    }
    ...
    private static MetadataRegistry metadata(String schema) {
        ...
        registry.register(
                new EntityMetadata(
                        ...
                        "Customer",
                        List.of(
                                new ColumnMetadata(CUSTOMER_ID_COLUMN, "Id"),
        ...
        registry.register(
                new EntityMetadata(
                        ...
                        "Account",
                        List.of(
                                new ColumnMetadata(ACCOUNT_ID_COLUMN, "Id"),
    ...
    private static void createSchema(Connection c, String schema) throws SQLException {
        try (Statement s = c.createStatement()) {
            ...
            s.execute(
                    "CREATE TABLE \""
                            + schema
                            + "\".\"Customer\" (\"Id\" BIGSERIAL PRIMARY KEY, \"Name\" TEXT NOT"
                            + " NULL, \"TenantId\" TEXT NOT NULL)");
            s.execute(
                    "CREATE TABLE \""
                            + schema
                            + "\".\"Account\" (\"Id\" BIGSERIAL PRIMARY KEY, \"CustomerId\" BIGINT"
                            + " NOT NULL REFERENCES \""
                            + schema
                            + "\".\"Customer\"(\"Id\"), \"Balance\" BIGINT NOT NULL)");
        }
    }

    private static void seed(Connection c, String schema) throws SQLException {
        try (PreparedStatement customer =
                        c.prepareStatement(
                                "INSERT INTO \""
                                        + schema
                                        + "\".\"Customer\" (\"Name\",\"TenantId\") VALUES (?,?)",
                                Statement.RETURN_GENERATED_KEYS);
                PreparedStatement account =
                        c.prepareStatement(
                                "INSERT INTO \""
                                        + schema
                                        + "\".\"Account\" (\"CustomerId\",\"Balance\") VALUES"
                                        + " (?,?)")) {
    ...
    private static long insertCustomer(PreparedStatement ps, String name, String tenant)
            throws SQLException {
        ps.setString(1, name);
    ...
    private static void insertAccount(PreparedStatement ps, long customerId, long balance)
            throws SQLException {
    ...
    private static String convertAdoStyle(String value) {
        ...
        String database =
                parts.getOrDefault("database", parts.getOrDefault("database name", "postgres"));
