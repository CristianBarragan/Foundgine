 ...
 *
 * <p>
 * Set FOUNDGINE_POSTGRES_CONNECTION_STRING to a JDBC URL, for example
 * jdbc:postgresql://localhost:55432/foundgine_e2e?user=foundgine&password=foundgine.
 * The test creates an isolated schema and removes it in finally, so it does not
 * depend on the repository's seeded C# database.
 * </p>
 */
class SqlExecutionPostgresE2ETest {
	...
	void realPostgres_executesAuthorizedFilteredRelationshipQuery_andMaterializesCells() throws Exception {
		...
		try (Connection connection = DriverManager.getConnection(url)) {
			...
			SemanticRelationshipFilter relationshipFilter = new SemanticRelationshipFilter(ACCOUNTS,
					SemanticRelationshipQuantifier.SOME,
					new SemanticFieldFilter(ACCOUNT_BALANCE, SemanticFilterOperator.GTE, 100L));

			SemanticQueryOptions options = new SemanticQueryOptions(relationshipFilter, List.of(), 10, null, null);
			SemanticPlanNode root = new SemanticPlanNode(1, ExecutionOperation.SCAN, CUSTOMER,
					List.of(CUSTOMER_ID, CUSTOMER_NAME, CUSTOMER_TENANT), null, null, List.of(), options, authorization,
					null, RelationshipTraversalMode.DEFAULT, -1, AggregateExecutionStrategy.DEFAULT);
			SemanticPlan plan = new SemanticPlan(root, List.of(),
			...
			ExecutionContext context = new ExecutionContext(
					Map.of("tenant.id", "tenant-a", ExecutionContextKeys.PAGINATION_LIMIT, 10));
		...
		try (Connection connection = DriverManager.getConnection(url)) {
			...
			SemanticQueryOptions options = new SemanticQueryOptions(
					new SemanticAndFilter(List.of(
							new SemanticRelationshipFilter(ACCOUNTS, SemanticRelationshipQuantifier.SOME,
									new SemanticFieldFilter(ACCOUNT_BALANCE, SemanticFilterOperator.GTE, 100L)),
							new SemanticRelationshipFilter(ACCOUNTS, SemanticRelationshipQuantifier.NONE,
									new SemanticFieldFilter(ACCOUNT_BALANCE, SemanticFilterOperator.GT, 400L)))),
					List.of(new SemanticOrderTerm(CUSTOMER_NAME, SemanticSortDirection.ASC)), 10, null, null);

			SemanticPlanNode child = new SemanticPlanNode(2, ExecutionOperation.SCAN, ACCOUNT,
					List.of(ACCOUNT_ID, ACCOUNT_BALANCE), ACCOUNTS, null, List.of(),
					new SemanticQueryOptions(null, List.of(), null, null, null), null, null,
					RelationshipTraversalMode.DEFAULT, 1, AggregateExecutionStrategy.DEFAULT);
			SemanticPlanNode root = new SemanticPlanNode(1, ExecutionOperation.SCAN, CUSTOMER,
					List.of(CUSTOMER_ID, CUSTOMER_NAME), null, null, List.of(child), options, tenantAuthorization(),
					null, RelationshipTraversalMode.DEFAULT, -1, AggregateExecutionStrategy.DEFAULT);

			SqlPlan sqlPlan = new SqlCompiler(metadata).compile(new SemanticPlan(root, List.of(),
			...
			ExecutionResult result = new SqlExecutionProvider(connection).executeAsync(sqlPlan,
					new ExecutionContext(Map.of("tenant.id", "tenant-a", ExecutionContextKeys.PAGINATION_LIMIT, 10)),
	...
	void realPostgres_relationshipAllAndAggregateCountRemainFailClosedAndParameterized() throws Exception {
		...
		try (Connection connection = DriverManager.getConnection(url)) {
			...
			SemanticFilterExpression allBalancesAtLeast100 = new SemanticRelationshipFilter(ACCOUNTS,
			...
			ExecutionResult allResult = new SqlExecutionProvider(connection).executeAsync(allSql,
					new ExecutionContext(Map.of(ExecutionContextKeys.PAGINATION_LIMIT, 10)), CancellationToken.NONE)
					.toCompletableFuture().join();
			assertEquals(3, allResult.rows().size());

			SemanticAggregateFilter countAtLeastOne = new SemanticAggregateFilter(ACCOUNTS,
					SemanticFilterAggregate.COUNT, null, SemanticAggregateFilterOperator.GTE, 1L, null);
			...
			ExecutionResult countResult = new SqlExecutionProvider(connection).executeAsync(countSql,
					new ExecutionContext(Map.of(ExecutionContextKeys.PAGINATION_LIMIT, 10)), CancellationToken.NONE)
					.toCompletableFuture().join();
	...
	private static SemanticPlan planWithFilter(SemanticFilterExpression filter, int limit) {
		SemanticQueryOptions options = new SemanticQueryOptions(filter,
				List.of(new SemanticOrderTerm(CUSTOMER_ID, SemanticSortDirection.ASC)), limit, null, null);
		SemanticPlanNode root = new SemanticPlanNode(1, ExecutionOperation.SCAN, CUSTOMER,
				List.of(CUSTOMER_ID, CUSTOMER_NAME), null, null, List.of(), options, null, null,
				RelationshipTraversalMode.DEFAULT, -1, AggregateExecutionStrategy.DEFAULT);
	...
	void realPostgres_cursorPagination_roundTripsEndCursorAndUsesStablePrimaryKeyTieBreak() throws Exception {
		...
		try (Connection connection = DriverManager.getConnection(url)) {
			...
			ExecutionContext context = new ExecutionContext(
					Map.of("tenant.id", "tenant-a", ExecutionContextKeys.PAGINATION_LIMIT, 2));
			ExecutionResult first = new SqlExecutionProvider(connection)
					.executeAsync(firstPlan, context, CancellationToken.NONE).toCompletableFuture().join();
			...
			ExecutionResult second = new SqlExecutionProvider(connection)
					.executeAsync(secondPlan, context, CancellationToken.NONE).toCompletableFuture().join();
	...
	void realPostgres_aggregateCountOrdering_roundTripsCursorWithStableRootKeyTieBreak() throws Exception {
		...
		try (Connection connection = DriverManager.getConnection(url)) {
			...
			assertFalse(firstSql.commandText().contains("LEFT JOIN"), firstSql.commandText());

			ExecutionContext context = new ExecutionContext(
					Map.of("tenant.id", "tenant-a", ExecutionContextKeys.PAGINATION_LIMIT, 2));
			ExecutionResult first = new SqlExecutionProvider(connection)
					.executeAsync(firstSql, context, CancellationToken.NONE).toCompletableFuture().join();
			...
			ExecutionResult second = new SqlExecutionProvider(connection)
					.executeAsync(secondSql, context, CancellationToken.NONE).toCompletableFuture().join();
	...
	private static SemanticPlan aggregateCursorPlan(String after) {
		SemanticQueryOptions options = new SemanticQueryOptions(null, List.of(new SemanticOrderTerm(ACCOUNT_ID,
				SemanticSortDirection.DESC, List.of(ACCOUNTS), SemanticOrderAggregate.COUNT)), 2, null, after);
		SemanticPlanNode root = new SemanticPlanNode(1, ExecutionOperation.SCAN, CUSTOMER,
				List.of(CUSTOMER_ID, CUSTOMER_NAME), null, null, List.of(), options, tenantAuthorization(), null,
				RelationshipTraversalMode.DEFAULT, -1, AggregateExecutionStrategy.DEFAULT);
		return new SemanticPlan(root, List.of(), new SemanticPlanAuthorizationBinding(
				"postgres-aggregate-cursor-contract", "postgres-aggregate-cursor-auth"));
	}

	private static SemanticPlan cursorPlan(String after) {
		SemanticQueryOptions options = new SemanticQueryOptions(null,
				List.of(new SemanticOrderTerm(CUSTOMER_NAME, SemanticSortDirection.ASC)), 2, null, after);
		SemanticPlanNode root = new SemanticPlanNode(1, ExecutionOperation.SCAN, CUSTOMER,
	...
	private static MetadataRegistry metadata(String schema) {
		...
		registry.register(new EntityMetadata(
				CUSTOMER, "Customer", List
						.of(new ColumnMetadata(CUSTOMER_ID_COLUMN, "Id"),
								new ColumnMetadata(CUSTOMER_NAME_COLUMN,
										"Name"),
		...
		registry.register(new EntityMetadata(ACCOUNT, "Account",
				List.of(new ColumnMetadata(ACCOUNT_ID_COLUMN, "Id"),
						new ColumnMetadata(ACCOUNT_CUSTOMER_COLUMN,
								"CustomerId"),
		...
		registry.register(new RelationshipMetadata(ACCOUNTS, CUSTOMER, ACCOUNT, "Accounts",
				new ColumnReference(CUSTOMER, CUSTOMER_ID_COLUMN),
	...
	private static void createSchema(Connection c, String schema) throws SQLException {
		try (Statement s = c.createStatement()) {
			...
			s.execute("CREATE TABLE \"" + schema
					+ "\".\"Customer\" (\"Id\" BIGSERIAL PRIMARY KEY, \"Name\" TEXT NOT NULL, \"TenantId\" TEXT NOT NULL)");
			s.execute("CREATE TABLE \"" + schema
					+ "\".\"Account\" (\"Id\" BIGSERIAL PRIMARY KEY, \"CustomerId\" BIGINT NOT NULL REFERENCES \""
					+ schema + "\".\"Customer\"(\"Id\"), \"Balance\" BIGINT NOT NULL)");
		}
	}
	...
	private static void seedCursorData(Connection c, String schema) throws SQLException {
		try (PreparedStatement customer = c
				.prepareStatement("INSERT INTO \"" + schema + "\".\"Customer\" (\"Name\",\"TenantId\") VALUES (?,?)")) {
			for (int i = 1; i <= 4; i++) {
