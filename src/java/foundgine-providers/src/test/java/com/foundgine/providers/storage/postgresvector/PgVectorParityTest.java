...
final class PgVectorParityTest {
    @Test void defaults_match_documented_lexicon_shape() {
    ...
    @Test void qualified_table_name_quotes_custom_schema_and_table() {
    ...
    @Test void qualified_table_name_escapes_embedded_quotes() {
    ...
    @ParameterizedTest
    @CsvSource({"COSINE, 0.0, 1.0", "COSINE, 1.0, 0.0", "COSINE, 0.25, 0.75",
                "L2, 0.0, 1.0", "L2, 1.0, 0.5", "L2, 3.0, 0.25",
                "INNER_PRODUCT, -0.9, 0.9", "INNER_PRODUCT, 0.4, -0.4"})
    ...
    void vector_ops_class_matches_distance(PgVectorDistance distance) {
        String expected = switch (distance) { case COSINE -> "vector_cosine_ops"; case L2 -> "vector_l2_ops"; case INNER_PRODUCT -> "vector_ip_ops"; };
        assertEquals(expected, PgVectorSemanticLexiconIndexClient.VectorOpsClass(distance));
    }

    @Test void constructors_reject_missing_data_source_or_embedding_generator() {
        ...
        assertThrows(NullPointerException.class, () -> new PgVectorSemanticLexicalCandidateSource(new DummyDataSource(), null));
        assertThrows(NullPointerException.class, () -> new PgVectorSemanticLexiconIndexClient(null, generator));
        assertThrows(NullPointerException.class, () -> new PgVectorSemanticLexiconIndexClient(new DummyDataSource(), null));
    }

    @Test void null_options_use_documented_defaults() {
        var generator = (PgVectorSemanticLexicalCandidateSource.EmbeddingGenerator) token -> new float[]{1f};
        ...
        assertNotNull(source); assertNotNull(client);
    }
    ...
    private static final class DummyDataSource implements DataSource {
        ...
        public <T> T unwrap(Class<T> c) { throw new UnsupportedOperationException(); }
