 ...
 *
 * <p>The C# original proves that the {@code AggregateExecutionStrategy} hint set by {@code
 * AggregateCardinalityOptimizationRule} is actually consumed downstream: it used to be dropped
 * silently at the plan-lowering boundary and never reached the SQL compiler. {@code
 * SqlCompilerParityTest} already covers the compiler's rendering given a manually-constructed
 * strategy, and {@code AggregateExecutionStrategyResolverParityTest} already covers the resolver's
 * eligibility rules in isolation — but neither exercises the rule and the compiler
 * <em>together</em>, which is the specific regression this file guards against. This test drives
 * {@link AggregateCardinalityOptimizationRule#apply} and feeds its actual output straight into
 * {@link SqlCompiler}, using the same Customer/Account fixture {@code SqlCompilerParityTest} uses
 * (the C# suite uses its own {@code Banking} sample fixture, which has no Java equivalent, so the
 * fixture here is the fixture that's actually available).
 */
class AggregateExistenceSqlRenderingParityTest {
    ...
    void constantCountComparisonsAreNotMiscompiledAsExistenceTests(
        ...
        var plan =
                applyRule(
                        new SemanticAggregateFilter(
                                CUSTOMER_ACCOUNTS, SemanticFilterAggregate.COUNT, null, op, value));
    ...
    private static MetadataRegistry metadata() {
        ...
        registry.register(
                new EntityMetadata(
                        CUSTOMER,
                        "Customer",
                        List.of(new ColumnMetadata(customerId, "Id")),
                        "Customer",
                        List.of(
                                new FieldMetadata(
                                        CUSTOMER_ID,
        ...
        registry.register(
                new EntityMetadata(
                        ...
                        "Account",
                        List.of(
                                new ColumnMetadata(accountId, "Id"),
                                new ColumnMetadata(accountCustomerId, "CustomerId")),
                        "Account",
                        List.of(
                                new FieldMetadata(
                                        new FieldId(2),
                                        "Id",
                                        Long.class,
                                        new ColumnReference(ACCOUNT, accountId)),
                                new FieldMetadata(
                                        new FieldId(3),
                                        "CustomerId",
                                        Long.class,
