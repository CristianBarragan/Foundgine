...
class McpAgentClientParityTest {
	...
	void discoveryThenDynamicIntentThenExecutionUsesDiscoveredContract() throws Exception {
		...
		try (TestServer server = new TestServer(exchange -> {
			...
			if (body.contains("foundgine_capabilities")) {
				reply(exchange, rpc(
						"{\"version\":1,\"capabilities\":[{\"id\":\"Customer.read\",\"name\":\"Read Customer\",\"targetEntityId\":1,\"access\":{\"access\":\"ALLOWED\",\"predicate\":null},\"inputs\":[],\"constraints\":[],\"effects\":[],\"fields\":[\"Id\",\"Name\"],\"relationships\":[\"Transactions\"],\"operation\":\"read\",\"hasSideEffects\":false,\"isIdempotent\":false,\"version\":1,\"requiredSecurityInvariants\":[]}]}"));
			} else {
			...
			var result = client.discoverAndExecute(contract -> {
				...
				return new ReadIntent("Customer", java.util.List.of(new ReadSelection("Id"),
	...
	void sseToolResultIsUnwrapped() throws Exception {
		try (TestServer server = new TestServer(
				exchange -> reply(exchange, "event: message\ndata: " + rpc("{\"ok\":true}") + "\n\n"))) {
			var client = new FoundgineMcpAgentClient(HttpClient.newHttpClient(), server.uri());
	...
	void mcpErrorIsNotTreatedAsSuccess() throws Exception {
		try (TestServer server = new TestServer(
				exchange -> reply(exchange, "{\"jsonrpc\":\"2.0\",\"id\":1,\"error\":{\"message\":\"denied\"}}"))) {
			var client = new FoundgineMcpAgentClient(HttpClient.newHttpClient(), server.uri());
			var ex = assertThrows(Exception.class,
					() -> client.executeQuery(new ReadIntent("Customer", java.util.List.of(new ReadSelection("Id"))))
							.toCompletableFuture().get(5, TimeUnit.SECONDS));
	...
	private static String rpc(String nested) {
		return "{\"jsonrpc\":\"2.0\",\"id\":1,\"result\":{\"content\":[{\"type\":\"text\",\"text\":" + quote(nested)
				+ "}]}}";
	}
