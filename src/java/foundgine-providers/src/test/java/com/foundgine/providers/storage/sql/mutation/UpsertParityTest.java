 ...
 *
 * <p>The C# original executes against an in-memory SQLite database ({@code Microsoft.Data.Sqlite}).
 * The Java port has no embedded-database test dependency, so the compile-only assertions (which
 * don't need a live connection) are ported directly, and the execution-based assertions are ported
 * against the same opt-in real PostgreSQL pattern already used by {@code
 * SqlExecutionPostgresE2ETest} / the benchmark-agent E2E suite (set {@code
 * FOUNDGINE_POSTGRES_CONNECTION_STRING} to run them).
 */
class UpsertParityTest {
    ...
    void customConflictIdentityIsUsed() {
        ...
        var intent =
                new UpsertIntent(
                        CUSTOMER,
                        List.of(new MutationFieldValue(NAME_COLUMN, "Alice")),
                        List.of(NAME_COLUMN),
                        List.of(CUSTOMER_ID, CUSTOMER_NAME));

        var sql =
                new SqlMutationCompiler(metadata)
                        .compile(new MutationPlanner(schema()).plan(intent));
    ...
    void unchangedUpsertUsesDistinctGuard() {
        ...
        var intent =
                new UpsertIntent(
                        CUSTOMER,
                        List.of(
                                new MutationFieldValue(ID_COLUMN, 1),
                                new MutationFieldValue(NAME_COLUMN, "Alice")),
                        null,
                        List.of(CUSTOMER_ID, CUSTOMER_NAME));
        ...
        assertTrue(
                plan.fallbackCommandText().contains("IS NOT DISTINCT FROM"),
        ...
        try (Connection connection = DriverManager.getConnection(url)) {
            ...
            var result =
                    new SqlMutationExecutionProvider(connection)
                            .execute(plan, ExecutionContext.EMPTY);
        ...
        try (Connection connection = DriverManager.getConnection(url)) {
            ...
            var intent =
                    new UpsertIntent(
                            ...
                            List.of(
                                    new MutationFieldValue(ID_COLUMN, 1),
                                    new MutationFieldValue(NAME_COLUMN, "Bob")),
                            null,
                            List.of(CUSTOMER_ID, CUSTOMER_NAME));
            var plan =
                    new SqlMutationCompiler(metadata)
    ...
    private static void seedRow(Connection connection, String table, String name)
            throws java.sql.SQLException {
        try (var st = connection.createStatement()) {
    ...
    private static MutationSchema schema() {
        var entity =
                new MutationEntitySchema(
                        CUSTOMER,
                        "Customer",
                        Set.of(ID_COLUMN, NAME_COLUMN),
                        java.util.Map.of(CUSTOMER_ID, ID_COLUMN, CUSTOMER_NAME, NAME_COLUMN),
                        ID_COLUMN);
        ...
        registry.register(
                new EntityMetadata(
                        ...
                        List.of(
                                ...
                                new ColumnMetadata(NAME_COLUMN, "Name")),
                        storageName,
                        List.of(
                                new FieldMetadata(
                                        CUSTOMER_ID,
                                        "Id",
                                        Long.class,
                                        new ColumnReference(CUSTOMER, ID_COLUMN)),
                                new FieldMetadata(
                                        CUSTOMER_NAME,
                                        "Name",
                                        String.class,
