...
class InMemoryProviderParityTest {
    ...
    void execution_projects_only_selected_fields_and_does_not_expose_backing_values() {
        ...
        var data = new InMemoryDataSet().add(new InMemoryRow(CUSTOMER, Map.of(
                CUSTOMER_ID, 1,
                CUSTOMER_NAME, "Alice",
                internalRisk, new BigDecimal("987.65"))));
    ...
    void query_filter_order_offset_and_limit_are_applied_to_root_rows() {
        ...
        var data = new InMemoryDataSet()
                .add(customer(1, "Charlie"))
                .add(customer(2, "Alice"))
                .add(customer(3, "Bob"))
                .add(customer(4, "Dora"));
    ...
    void relationship_traversal_uses_metadata_join_keys_without_leaking_the_join_key() {
        ...
        var data = new InMemoryDataSet()
                .add(new InMemoryRow(CUSTOMER, Map.of(CUSTOMER_ID, 1, CUSTOMER_NAME, "Alice")))
                .add(new InMemoryRow(ACCOUNT, Map.of(
                        ACCOUNT_ID, 100,
        ...
        var child = new ExecutionIRNode(
                1, ExecutionOperation.TRAVERSE, ACCOUNT, List.of(BALANCE), ACCOUNTS, null,
                List.of(), null, null, null);
        var root = new ExecutionIRNode(
                0, ExecutionOperation.SCAN, CUSTOMER, List.of(CUSTOMER_NAME), null, null,
        ...
        assertFalse(row.effectiveCells().keySet().stream()
                .anyMatch(x -> x.fieldId().equals(ACCOUNT_CUSTOMER_ID)));
    }
    ...
    void authorization_filters_rows_using_resource_and_context_values() {
        ...
        var row = single(execute(metadata, data, ir(root),
                new ExecutionContext(Map.of("tenant.id", 2))));
    ...
    void provider_security_conformance_certifies_required_invariants() {
        var ir = new ExecutionIR(
                scan(CUSTOMER, List.of(CUSTOMER_ID), null, null),
    ...
    void execution_requires_authorization_provenance() {
        ...
        assertThrows(IllegalStateException.class,
                () -> compiler.executeAsyncResult(new InMemoryPlan(ir), new ExecutionContext(Map.of()), CancellationToken.NONE));
    }

    private static ExecutionIRNode scan(EntityId entity, List<FieldId> fields,
                                        SemanticQueryOptions options,
                                        AuthorizationPredicate authorization) {
        return new ExecutionIRNode(0, ExecutionOperation.SCAN, entity, fields,
    ...
    private static ExecutionResult execute(MetadataRegistry metadata, InMemoryDataSet data,
                                           ExecutionIR ir, ExecutionContext context) {
    ...
    private static Object valueFor(com.foundgine.core.execution.ExecutionRow row, FieldId field) {
        return row.effectiveCells().entrySet().stream()
                .filter(x -> x.getKey().fieldId().equals(field))
    ...
    private static MetadataRegistry customerMetadata() {
        ...
        metadata.register(new EntityMetadata(
                CUSTOMER, "Customer",
                List.of(new ColumnMetadata(col(11), "Id"), new ColumnMetadata(col(12), "Name")),
                null,
                List.of(
                        new FieldMetadata(CUSTOMER_ID, "Id", Integer.class, ref(CUSTOMER, 11)),
    ...
    private static MetadataRegistry customerAndAccountMetadata() {
        ...
        metadata.register(new EntityMetadata(
                ACCOUNT, "Account",
                List.of(new ColumnMetadata(col(21), "Id"), new ColumnMetadata(col(22), "Balance"),
                        new ColumnMetadata(col(23), "CustomerId")),
                null,
                List.of(
                        new FieldMetadata(ACCOUNT_ID, "Id", Integer.class, ref(ACCOUNT, 21)),
        ...
        metadata.register(new RelationshipMetadata(
                ACCOUNTS, CUSTOMER, ACCOUNT, "Accounts", ref(CUSTOMER, 11), ref(ACCOUNT, 23)));
        return metadata;
    }

    private static ColumnId col(int value) { return new ColumnId(value); }
    private static ColumnReference ref(EntityId entity, int column) { return new ColumnReference(entity, col(column)); }
}
