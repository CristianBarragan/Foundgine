 ...
 *
 * <p>Set {@code FOUNDGINE_POSTGRES_CONNECTION_STRING} to the JDBC URL, for
 * example {@code jdbc:postgresql://localhost:55432/foundgine_e2e?user=foundgine&password=foundgine}.
...
class PostgresBatchedMutationExecutionPostgresE2ETest {
    ...
    void realPostgres_executesGeneratedIdentityAcrossDependencyLevels_andRollsBackCallerTransaction() throws Exception {
        ...
        try (Connection connection = DriverManager.getConnection(url)) {
            ...
            long accountCustomerId = ((Number) result.results().get(2).returnedFields().get(ACCOUNT_CUSTOMER_ID_FIELD)).longValue();
    ...
    void realPostgres_providerOwnedTransaction_commitsExactlyOnce() throws Exception {
        ...
        try (Connection connection = DriverManager.getConnection(url)) {
            ...
            connection.setAutoCommit(false);
            try (PreparedStatement ps = connection.prepareStatement("DELETE FROM \"fg_correlation\".\"Account\" WHERE \"Name\" LIKE 'Java E2E %'")) {
                ps.executeUpdate();
            }
            try (PreparedStatement ps = connection.prepareStatement("DELETE FROM \"fg_correlation\".\"Customer\" WHERE \"Name\" LIKE 'Java E2E %'")) {
                ps.executeUpdate();
            }
            connection.commit();
            connection.setAutoCommit(true);
    ...
    private static MutationBatchPlan dependencyBatch() {
        MutationEntitySchema customer = new MutationEntitySchema(
                CUSTOMER, "Customer", Set.of(CUSTOMER_ID, CUSTOMER_NAME),
        ...
        MutationEntitySchema account = new MutationEntitySchema(
                ACCOUNT, "Account", Set.of(ACCOUNT_ID, ACCOUNT_CUSTOMER_ID, ACCOUNT_NAME),
                Map.of(ACCOUNT_ID_FIELD, ACCOUNT_ID, ACCOUNT_CUSTOMER_ID_FIELD, ACCOUNT_CUSTOMER_ID, ACCOUNT_NAME_FIELD, ACCOUNT_NAME), ACCOUNT_ID);
        ...
        MutationOperation accountForBob = new MutationOperation(account, MutationKind.CREATE,
                List.of(MutationFieldValue.fromPrevious(ACCOUNT_CUSTOMER_ID, 1, CUSTOMER_ID_FIELD),
                        new MutationFieldValue(ACCOUNT_NAME, unique("Java E2E Account"))), null, null,
    ...
    private static IMetadataProvider metadata() {
        ...
        registry.register(new EntityMetadata(CUSTOMER, "Customer",
                ...
                List.of(new FieldMetadata(CUSTOMER_ID_FIELD, "Id", Long.class, new ColumnReference(CUSTOMER, CUSTOMER_ID)),
                        new FieldMetadata(CUSTOMER_NAME_FIELD, "Name", String.class, new ColumnReference(CUSTOMER, CUSTOMER_NAME))),
                new ColumnReference(CUSTOMER, CUSTOMER_ID), null, false, null, null));
        registry.register(new EntityMetadata(ACCOUNT, "Account",
                List.of(new ColumnMetadata(ACCOUNT_ID, "Id"), new ColumnMetadata(ACCOUNT_CUSTOMER_ID, "CustomerId"), new ColumnMetadata(ACCOUNT_NAME, "Name")),
                "fg_correlation.Account",
                List.of(new FieldMetadata(ACCOUNT_ID_FIELD, "Id", Long.class, new ColumnReference(ACCOUNT, ACCOUNT_ID)),
                        new FieldMetadata(ACCOUNT_CUSTOMER_ID_FIELD, "CustomerId", Long.class, new ColumnReference(ACCOUNT, ACCOUNT_CUSTOMER_ID)),
    ...
    private static long count(Connection connection, String schema, String table) throws SQLException {
        try (PreparedStatement ps = connection.prepareStatement("SELECT COUNT(*) FROM \"" + schema + "\".\"" + table + "\"")) {
    ...
    private static String jdbcUrl() {
        ...
        for (String item : configured.split(";")) {
            int eq = item.indexOf('=');
            if (eq > 0) parts.put(item.substring(0, eq).trim().toLowerCase(Locale.ROOT), item.substring(eq + 1).trim());
        }
        ...
        return "jdbc:postgresql://" + host + ":" + port + "/" + db + "?user=" + encode(user) + "&password=" + encode(password);
    }
