...
/**
 * Real PostgreSQL execution parity tests. These are intentionally opt-in so a normal Java build
 * does not require PostgreSQL to be running.
 *
 * <p>Set {@code FOUNDGINE_POSTGRES_CONNECTION_STRING} to the JDBC URL, for example {@code
 * jdbc:postgresql://localhost:55432/foundgine_e2e?user=foundgine&password=foundgine}. An
 * ADO.NET-style Foundgine connection string is also accepted and converted by {@link #jdbcUrl()}
 * for parity with the repository's C# E2E scripts.
 */
class PostgresBatchedMutationExecutionPostgresE2ETest {
    ...
    @Test
    void
            realPostgres_executesGeneratedIdentityAcrossDependencyLevels_andRollsBackCallerTransaction()
                    throws Exception {
        String url = jdbcUrl();
        ...
        try (Connection connection = DriverManager.getConnection(url)) {
            ...
            PostgresBatchedMutationExecutionProvider provider =
                    new PostgresBatchedMutationExecutionProvider(connection, metadata(), false);
            ...
            long aliceId =
                    ((Number) result.results().get(0).returnedFields().get(CUSTOMER_ID_FIELD))
                            .longValue();
            long bobId =
                    ((Number) result.results().get(1).returnedFields().get(CUSTOMER_ID_FIELD))
                            .longValue();
        ...
        try (Connection connection = DriverManager.getConnection(url)) {
            ...
            try (PreparedStatement ps =
                    connection.prepareStatement(
                            "DELETE FROM \"fg_correlation\".\"Account\" WHERE \"Name\" LIKE 'Java"
                                    + " E2E %'")) {
                ps.executeUpdate();
            }
            try (PreparedStatement ps =
                    connection.prepareStatement(
                            "DELETE FROM \"fg_correlation\".\"Customer\" WHERE \"Name\" LIKE 'Java"
                                    + " E2E %'")) {
                ps.executeUpdate();
            }
            connection.commit();
        ...
        try (Connection connection = DriverManager.getConnection(url)) {
            ...
            try (CancellationTokenSource source = new CancellationTokenSource()) {
                ...
                assertThrows(
                        ...
                        () ->
                                new PostgresBatchedMutationExecutionProvider(
                                                connection, metadata(), true)
                                        .executeBatch(
                                                ExecutionMutationIR.from(dependencyBatch()),
    ...
    private static MutationBatchPlan dependencyBatch() {
        MutationEntitySchema customer =
                new MutationEntitySchema(
                        CUSTOMER,
                        "Customer",
                        Set.of(CUSTOMER_ID, CUSTOMER_NAME),
                        Map.of(CUSTOMER_ID_FIELD, CUSTOMER_ID, CUSTOMER_NAME_FIELD, CUSTOMER_NAME),
                        CUSTOMER_ID);
        MutationEntitySchema account =
                new MutationEntitySchema(
                        ACCOUNT,
                        "Account",
                        Set.of(ACCOUNT_ID, ACCOUNT_CUSTOMER_ID, ACCOUNT_NAME),
                        Map.of(
                                ACCOUNT_ID_FIELD,
                                ACCOUNT_ID,
                                ACCOUNT_CUSTOMER_ID_FIELD,
                                ACCOUNT_CUSTOMER_ID,
                                ACCOUNT_NAME_FIELD,
                                ACCOUNT_NAME),
                        ACCOUNT_ID);
        ...
        MutationOperation alice =
                new MutationOperation(
                        customer,
                        MutationKind.CREATE,
                        List.of(new MutationFieldValue(CUSTOMER_NAME, unique("Java E2E Alice"))),
                        null,
                        null,
        ...
        MutationOperation bob =
                new MutationOperation(
                        customer,
                        MutationKind.CREATE,
                        List.of(new MutationFieldValue(CUSTOMER_NAME, unique("Java E2E Bob"))),
                        null,
                        null,
                        List.of(CUSTOMER_ID_FIELD, CUSTOMER_NAME_FIELD));
                ...
                new MutationOperation(
                        ...
                        List.of(
                                MutationFieldValue.fromPrevious(
                                        ACCOUNT_CUSTOMER_ID, 1, CUSTOMER_ID_FIELD),
                                new MutationFieldValue(ACCOUNT_NAME, unique("Java E2E Account"))),
                        null,
                        null,
                        List.of(ACCOUNT_ID_FIELD, ACCOUNT_CUSTOMER_ID_FIELD, ACCOUNT_NAME_FIELD));

        return new MutationBatchPlan(
                List.of(alice, bob, accountForBob),
    ...
    private static IMetadataProvider metadata() {
        ...
        registry.register(
                new EntityMetadata(
                        ...
                        "Customer",
                        List.of(
                                new ColumnMetadata(CUSTOMER_ID, "Id"),
                                new ColumnMetadata(CUSTOMER_NAME, "Name")),
        ...
        registry.register(
                new EntityMetadata(
                        ...
                        "Account",
                        List.of(
                                new ColumnMetadata(ACCOUNT_ID, "Id"),
                                new ColumnMetadata(ACCOUNT_CUSTOMER_ID, "CustomerId"),
                                new ColumnMetadata(ACCOUNT_NAME, "Name")),
                        ...
                        List.of(
                                new FieldMetadata(
                                        ...
                                        "Id",
                                        Long.class,
                                        new ColumnReference(ACCOUNT, ACCOUNT_ID)),
                                new FieldMetadata(
                                        ACCOUNT_CUSTOMER_ID_FIELD,
                                        "CustomerId",
                                        Long.class,
    ...
    private static void ensureCorrelationSchema(Connection connection) throws SQLException {
        try (Statement statement = connection.createStatement()) {
            ...
            statement.execute(
                    "CREATE TABLE IF NOT EXISTS \"fg_correlation\".\"Customer\" (\"Id\" BIGSERIAL"
                            + " PRIMARY KEY, \"Name\" TEXT NOT NULL)");
            statement.execute(
                    "CREATE TABLE IF NOT EXISTS \"fg_correlation\".\"Account\" (\"Id\" BIGSERIAL"
                            + " PRIMARY KEY, \"CustomerId\" BIGINT NOT NULL REFERENCES"
                            + " \"fg_correlation\".\"Customer\"(\"Id\"), \"Name\" TEXT NOT NULL)");
        }
    }
    ...
    private static String jdbcUrl() {
        ...
        if (configured.startsWith("jdbc:")) return configured;
        ...
        for (String item : configured.split(";")) {
            ...
            if (eq > 0)
                parts.put(
                        item.substring(0, eq).trim().toLowerCase(Locale.ROOT),
