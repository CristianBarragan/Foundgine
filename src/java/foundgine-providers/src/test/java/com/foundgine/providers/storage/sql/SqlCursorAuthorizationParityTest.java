 ...
 * get subtly wrong: compound keyset cursors and authorization predicates.
 * These mirror the C# provider's physical SQL contracts without requiring a
...
class SqlCursorAuthorizationParityTest {
    ...
    void cursorOrderingAddsPrimaryKeyTieBreaker() {
        var options = new SemanticQueryOptions(
                null,
                List.of(new SemanticOrderTerm(NAME, SemanticSortDirection.ASC)),
                10,
                null,
                CursorCodec.encode(List.of("Bob", 7L)));

        var node = new ExecutionIRNode(1, ExecutionOperation.SCAN, CUSTOMER,
                List.of(ID, NAME), null, null, List.of(), options, null, AggregateExecutionStrategy.DEFAULT);
    ...
    void descendingCursorUsesLessThanForSeek() {
        var options = new SemanticQueryOptions(
                null,
                List.of(new SemanticOrderTerm(NAME, SemanticSortDirection.DESC)),
                10,
                null,
                CursorCodec.encode(List.of("Bob", 7L)));
    ...
    void cursorWithWrongArityFailsClosed() {
        var options = new SemanticQueryOptions(
                ...
                10,
                null,
                CursorCodec.encode(List.of("Bob")));
    ...
    void authorizationResourceAndContextMembersRemainParameterized() {
        var resource = AuthorizationPredicate.member(
                AuthorizationPredicate.resourceParameter("resource"), "Id");
        var tenant = AuthorizationPredicate.member(
                AuthorizationPredicate.contextParameter("tenant"), "Id");
        var predicate = AuthorizationPredicate.equal(resource, tenant);

        var node = new ExecutionIRNode(1, ExecutionOperation.SCAN, CUSTOMER,
                List.of(ID), null, null, List.of(), null, predicate, AggregateExecutionStrategy.DEFAULT);

        var plan = new SqlCompiler(metadata()).compile(
                new ExecutionIR(node, List.of(SecurityInvariantIds.AUTHORIZATION_REQUIRED),
        ...
        assertEquals("tenant.Id", plan.parameters().stream()
                .filter(p -> p.name().equals("auth0"))
                .findFirst().orElseThrow().contextPath());
    ...
    void authorizationBooleanCompositionPreservesParentheses() {
        ...
        assertTrue(plan.commandText().contains("((\"t1\".\"Id\" = @auth0) AND NOT ((\"t1\".\"Id\" <> @auth1)))"), plan.commandText());
    }
    ...
    void cursorCodecRoundTripsTemporalAndDecimalValues() {
        ...
        assertEquals("2026-09-12T10:15:30Z", CursorCodec.convertValue(values.get(0), java.time.Instant.class).toString());
    ...
    private static MetadataRegistry metadata() {
        ...
        registry.register(new EntityMetadata(
                CUSTOMER,
                "Customer",
                List.of(new ColumnMetadata(idColumn, "Id"), new ColumnMetadata(nameColumn, "Name")),
                "Customer",
                List.of(
                        new FieldMetadata(ID, "Id", Long.class, new ColumnReference(CUSTOMER, idColumn)),
