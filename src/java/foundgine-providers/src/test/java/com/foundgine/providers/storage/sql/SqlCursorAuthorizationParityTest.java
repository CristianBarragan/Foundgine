...
/**
 * SQL provider parity tests for the two stateful boundaries that are easy to get subtly wrong:
 * compound keyset cursors and authorization predicates. These mirror the C# provider's physical SQL
 * contracts without requiring a live PostgreSQL server.
 */
class SqlCursorAuthorizationParityTest {
    ...
    void cursorOrderingAddsPrimaryKeyTieBreaker() {
        ...
        assertTrue(
                plan.commandText().contains("ORDER BY \"t1\".\"Name\" ASC, \"t1\".\"Id\" ASC"),
                plan.commandText());
        assertTrue(plan.commandText().contains("AND \"t1\".\"Id\" > @"), plan.commandText());
        assertEquals(
                1, plan.parameters().stream().filter(p -> p.name().equals("__fg_limit")).count());
    }
    ...
    void descendingCursorUsesLessThanForSeek() {
        ...
        assertTrue(
                plan.commandText().contains("ORDER BY \"t1\".\"Name\" DESC, \"t1\".\"Id\" ASC"),
                plan.commandText());
    }
    ...
    void authorizationBooleanCompositionPreservesParentheses() {
        ...
        var right =
                AuthorizationPredicate.notEqual(
                        AuthorizationPredicate.member(
                                AuthorizationPredicate.resourceParameter("resource"), "Id"),
        ...
        assertTrue(
                plan.commandText()
                        .contains("((\"t1\".\"Id\" = @auth0) AND NOT ((\"t1\".\"Id\" <> @auth1)))"),
                plan.commandText());
    }
    ...
    void cursorCodecRoundTripsTemporalAndDecimalValues() {
        ...
        assertEquals(
                "2026-09-12T10:15:30Z",
                CursorCodec.convertValue(values.get(0), java.time.Instant.class).toString());
        assertEquals(
                "12.50",
                CursorCodec.convertValue(values.get(1), java.math.BigDecimal.class).toString());
                ...
                new EntityMetadata(
                        ...
                        List.of(
                                new FieldMetadata(
                                        ...
                                        "Id",
                                        Long.class,
                                        new ColumnReference(CUSTOMER, idColumn)),
                                new FieldMetadata(
                                        NAME,
                                        "Name",
                                        String.class,
                                        new ColumnReference(CUSTOMER, nameColumn))),
                        new ColumnReference(CUSTOMER, idColumn),
                        null,
                        false,
                        null,
