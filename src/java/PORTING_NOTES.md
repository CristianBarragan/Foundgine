...
samples/Foundgine.SupplyChain*→ foundgine-samples-*           not started
benchmarks/*                  → foundgine-benchmarks-*        not started
security/Foundgine.RedTeam    → foundgine-redteam             not started
```
...
## Advanced SupplyChain sample — Java port milestone

The Java port now includes a dedicated `samples/foundgine-supply-chain-advanced`
Maven module. It mirrors the high-value application pieces of
`Foundgine.SupplyChain.Advanced` without pretending Java AOT discovery is complete:

- provider-neutral SupplyChain domain records and identifiers;
- deterministic fixture data, including the tenant-b warehouse, expired
  certification, delayed/partial shipments, and deliberate BOM cycle;
- application-owned authorization context with tenant/warehouse boundaries;
- recursive supplier-risk traversal with the same bounded cycle detection;
- fulfillment planning with authorized inventory and inbound filtering;
- manual semantic contract with aliases and the two application traversals;
- adversarial JUnit coverage for tenant isolation, recursive safety, temporal
  fixture validity, and semantic-contract construction;
- executable Java `Main` for the advanced sample.

Java AOT/source discovery remains a later step. The advanced sample deliberately
uses a manual semantic model now so its behavior can progress independently of the
unfinished Java AOT generator. The next advanced-sample passes should port the
claims/validation layer, MCP API/client surface, PostgreSQL execution wiring,
open-intent mutation/high-assurance path, grounding/retrieval tests, and benchmark
harness, then connect those pieces to the Java runtime/providers.
