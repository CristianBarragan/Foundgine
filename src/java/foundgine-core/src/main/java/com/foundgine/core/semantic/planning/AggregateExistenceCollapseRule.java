...
/**
 * Collapses a bare predicate-bearing COUNT comparison into SOME/NONE when the provider declares
 * quantifier support.
 */
public final class AggregateExistenceCollapseRule implements IPlanRewriteRule {
    ...
    public boolean canApply(SemanticPlan plan) {
        ...
        return providerCapability.supportsRelationshipQuantifiers()
                && containsEligible(plan.root());
    }
    ...
    public SemanticPlan apply(SemanticPlan plan) {
        ...
        AggregateExistenceCollapseProof.create(
                plan, candidate, providerCapability, predicateShapeMatches(plan, candidate));
        return candidate;
    }
    ...
    private static SemanticPlanNode rewriteNode(SemanticPlanNode node, boolean[] changed) {
        ...
        return changed[0] && (options != node.queryOptions() || childChanged)
    ...
    private static SemanticFilterExpression rewriteFilter(
            SemanticFilterExpression filter, boolean[] changed) {
    ...
    private static boolean containsEligible(SemanticPlanNode node) {
        return (node.queryOptions() != null
                ...
                || node.children().stream()
                        .anyMatch(AggregateExistenceCollapseRule::containsEligible);
    }

    private static boolean containsEligible(SemanticFilterExpression filter) {
        if (filter instanceof SemanticAggregateFilter a) return isEligible(a);
        if (filter instanceof SemanticAndFilter a)
            return a.expressions().stream()
                    .anyMatch(AggregateExistenceCollapseRule::containsEligible);
        if (filter instanceof SemanticOrFilter o)
            return o.expressions().stream()
                    .anyMatch(AggregateExistenceCollapseRule::containsEligible);
    ...
    private static boolean isEligible(SemanticAggregateFilter a) {
        if (a.aggregate() != SemanticFilterAggregate.COUNT
                || a.field() != null
                || a.predicate() == null) return false;
    ...
    private static boolean isEmptyStrategy(SemanticAggregateFilter a) {
        return AggregateExecutionStrategyResolver.resolve(a.operator(), a.value())
                == AggregateExecutionStrategy.COUNT_EMPTY_SHORT_CIRCUIT;
    }
    ...
    private static void collect(SemanticFilterExpression f, List<String> out) {
        ...
        else if (f instanceof SemanticAndFilter a) a.expressions().forEach(x -> collect(x, out));
        else if (f instanceof SemanticOrFilter o) o.expressions().forEach(x -> collect(x, out));
    }
}
