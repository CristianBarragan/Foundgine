...
public final class SemanticMutationPlanner {
    public SemanticMutationPlan plan(SemanticMutationOperationGraph graph) {
        ...
        for (int i = 0; i < graph.operations().size(); i++) {
            ...
            ops.add(
                    new SemanticMutationPlan.SemanticMutationOperationPlan(
                            Integer.toString(i),
                            op.entity(),
                            op.kind(),
                            op.fields(),
        ...
        var dependencies =
                new LinkedHashMap<
                        DependencyKey, SemanticMutationPlan.SemanticMutationDependencyPlan>();
        for (int target = 0; target < graph.operations().size(); target++) {
            ...
            for (var field : op.fields())
                if (field.source() != null) {
                    ...
                    validateSourceIndex(
                            source.sourceOperationIndex(), target, graph.operations().size());
                    validateReturned(
                            graph.operations().get(source.sourceOperationIndex()),
                            source.sourceField(),
                            source.sourceOperationIndex(),
                            target);
                    var d =
                            new SemanticMutationPlan.SemanticMutationDependencyPlan(
            ...
            for (var d : op.dependencies()) {
                ...
                validateReturned(
                        graph.operations().get(d.sourceOperationIndex()),
                        d.sourceField(),
                        d.sourceOperationIndex(),
                        target);
                var planned =
                        new SemanticMutationPlan.SemanticMutationDependencyPlan(
                                id(d.sourceOperationIndex()),
                                id(target),
                                d.sourceField(),
                                d.targetField(),
                                d.relationship());
                ...
                if (existing != null
                        ...
                        && planned.relationship() != null) dependencies.put(key, planned);
                else dependencies.putIfAbsent(key, planned);
            }
        }
        var result = new ArrayList<>(dependencies.values());
        result.sort(
                Comparator.comparingInt(
                                (SemanticMutationPlan.SemanticMutationDependencyPlan d) ->
                                        Integer.parseInt(d.fromOperationId()))
                        .thenComparingInt(d -> Integer.parseInt(d.toOperationId()))
                        .thenComparingLong(d -> d.sourceField().value())
                        .thenComparingLong(d -> d.targetField().value()));
    ...
    private static void validateReturned(
            SemanticMutationOperation source, FieldId field, int sourceIndex, int targetIndex) {
