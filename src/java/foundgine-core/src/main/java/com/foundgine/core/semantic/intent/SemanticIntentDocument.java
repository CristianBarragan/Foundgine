...
public record SemanticIntentDocument(String contractFingerprint, ReadIntent intent, int version) {
    ...
    public SemanticIntentDocument(String contractFingerprint, ReadIntent intent) { this(contractFingerprint, intent, CURRENT_VERSION); }
    public SemanticIntentDocument validate() {
        if (version != CURRENT_VERSION) throw new IllegalArgumentException("Unsupported semantic intent document version '" + version + "'.");
        if (contractFingerprint.isBlank()) throw new IllegalArgumentException("Semantic intent document requires a contract fingerprint.");
        return this;
    }
}
