...
final class SemanticAuthorizationContractValidator {
    ...
    private static void validateNode(
        ...
        if (root && (n.viaRelationship() != null || n.viaConnection() != null))
            throw new IllegalArgumentException(
                    "Root semantic node " + n.id() + " cannot specify a parent edge.");
        if (!root && n.viaRelationship() == null && n.viaConnection() == null)
            throw new IllegalArgumentException(
        ...
        if (n.viaRelationship() != null) {
            ...
            if (parent != null) {
                var rel =
                        parent.relationships().stream()
                                .filter(x -> x.id().equals(n.viaRelationship()))
                                .findFirst()
                                .orElseThrow(
                                        () ->
        ...
        for (var f : new LinkedHashSet<>(fields))
            if (!f.equals(entity.identity().fieldId())
                    && entity.fields().stream().noneMatch(x -> x.id().equals(f)))
                throw new IllegalArgumentException(
                        "Semantic operation node "
                                + n.id()
