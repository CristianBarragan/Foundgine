...
public final class SemanticContractAttestation {
    ...
    public static void ensureMatches(SemanticModel model, String expectedFingerprint) {
        ...
        throw new IllegalStateException(
                "Semantic contract attestation failed. Expected fingerprint '"
                        + expected
                        + "', but the runtime semantic model has fingerprint '"
    ...
    private static String normalize(String s) {
        var x = s.trim();
        return x.regionMatches(true, 0, "sha256:", 0, 7)
                ? x.substring(7).toLowerCase(Locale.ROOT)
