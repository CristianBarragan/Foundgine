...
/**
 * Produces a deterministic key for an execution plan. The complete authorized plan is represented,
 * including authorization predicates and request values. This intentionally keys exact plans rather
 * than pretending that arbitrary filter values can safely share a compiled provider plan.
 */
public final class SemanticPlanFingerprint {
    ...
    /**
     * Creates a cache key for the static query shape. Pagination values are deliberately excluded
     * because the SQL provider binds LIMIT/OFFSET at execution time. Filters, ordering and
     * authorization remain part of the key so unrelated query shapes do not share a provider plan.
     */
    ...
    private static void appendNode(
            StringBuilder builder, SemanticPlanNode node, boolean includePaginationValues) {
        builder.append("node(")
                .append(node.id())
                .append('|')
                ...
                .append('|')
                .append(
                        node.relationshipCardinality() != null
                                ? node.relationshipCardinality().toString()
                                : "-")
                .append('|')
                .append((byte) node.traversalMode().ordinal())
                .append('|')
                .append(node.traversalOrder())
                .append('|')
                .append((byte) node.aggregateExecutionStrategy().ordinal())
                .append(')');
    ...
    private static void appendQueryOptions(
            StringBuilder builder, SemanticQueryOptions options, boolean includePaginationValues) {
        if (options == null) {
        ...
        if (includePaginationValues) {
            builder.append(options.limit() != null ? options.limit().toString() : "-")
                    .append('|')
                    .append(options.offset() != null ? options.offset().toString() : "-")
                    .append('|');
        ...
        switch (filter) {
            ...
            case SemanticFieldFilter field -> {
                builder.append("field(")
                        .append(field.field().value())
                        .append('|')
                        .append((byte) field.operator().ordinal())
                        .append('|');
            ...
            case SemanticRelationshipFilter relationship -> {
                builder.append("relationship(")
                        .append(relationship.relationship().value())
                        .append('|')
                        .append((byte) relationship.quantifier().ordinal())
                        .append('|');
            ...
            case SemanticAggregateFilter aggregate -> {
                builder.append("aggregate(")
                        .append(aggregate.relationship().value())
                        .append('|')
                        .append((byte) aggregate.aggregate().ordinal())
                        .append('|')
                        .append(
                                aggregate.field() != null
                                        ? String.valueOf(aggregate.field().value())
                                        : "-")
                        .append('|')
                        .append((byte) aggregate.operator().ordinal())
                        .append('|');
