 ...
 * exact plans rather than pretending that arbitrary filter values can safely share a compiled
 * provider plan.
 */
public final class SemanticPlanFingerprint {
    ...
    /**
     * Creates a cache key for the static query shape. Pagination values are deliberately
     * excluded because the SQL provider binds LIMIT/OFFSET at execution time. Filters, ordering
     * and authorization remain part of the key so unrelated query shapes do not share a provider
     * plan.
     */
    ...
    private static void appendNode(StringBuilder builder, SemanticPlanNode node, boolean includePaginationValues) {
        builder.append("node(")
                ...
                .append((byte) node.operation().ordinal()).append('|')
                ...
                .append('|')
                .append((byte) node.traversalMode().ordinal()).append('|')
                .append(node.traversalOrder()).append('|')
                .append((byte) node.aggregateExecutionStrategy().ordinal()).append(')');
    ...
    private static void appendQueryOptions(
        ...
        for (SemanticOrderTerm term : options.effectiveOrder()) {
            builder.append(term.field().value()).append(':')
                    .append((byte) term.direction().ordinal()).append(':')
        ...
        switch (filter) {
            ...
            case SemanticFieldFilter field -> {
                builder.append("field(").append(field.field().value()).append('|')
                        .append((byte) field.operator().ordinal()).append('|');
            ...
            case SemanticAggregateFilter aggregate -> {
                builder.append("aggregate(").append(aggregate.relationship().value()).append('|')
                        ...
                        .append(aggregate.field() != null ? String.valueOf(aggregate.field().value()) : "-")
                        .append('|')
