...
public final class SemanticModel {
    ...
    public SemanticModel(
            Map<EntityId, SemanticEntity> entities,
            List<SemanticTraversal> traversals,
            boolean frozen) {
    ...
    public SemanticModel(
            Map<EntityId, SemanticEntity> entities, List<SemanticTraversal> traversals) {
    ...
    public void ensureFrozen() {
        if (!frozen)
            throw new IllegalStateException(
                    "The semantic model must be frozen before it can be used as a trusted semantic"
                            + " contract.");
    }
    ...
    public SemanticTraversal getTraversal(EntityId source, String name) {
        return traversals.stream()
                .filter(t -> t.source().equals(source) && t.name().equalsIgnoreCase(name))
                .findFirst()
                .orElseThrow(
                        () ->
                                new NoSuchElementException(
