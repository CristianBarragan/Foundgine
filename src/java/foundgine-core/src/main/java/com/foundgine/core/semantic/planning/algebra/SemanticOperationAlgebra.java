...
public final class SemanticOperationAlgebra {
    ...
    public static SemanticOperationGraph where(
            SemanticOperationGraph graph, SemanticFilterExpression predicate) {
        ...
        var combined =
                existing == null ? predicate : new SemanticAndFilter(List.of(existing, predicate));
        var options =
                root.queryOptions() == null ? new SemanticQueryOptions() : root.queryOptions();
        var updatedOptions =
                new SemanticQueryOptions(
                        ...
                        options.offset(),
                        options.after());
        return SemanticOperationGraph.create(
                new SemanticOperation(
                        new SemanticReadNode(
                                root.id(),
                                root.entityId(),
                                root.fields(),
                                root.viaRelationship(),
                                root.viaConnection(),
                                root.children(),
                                updatedOptions,
                                root.authorization(),
    ...
    private static SemanticReadNode normalizeNode(SemanticReadNode node) {
        var children =
                node.children().stream().map(SemanticOperationAlgebra::normalizeNode).toList();
    ...
    private static void visit(
            SemanticOperationGraph graph, int id, Set<Integer> visited, Integer parent) {
        if (!visited.add(id))
            throw new IllegalStateException(
                    "Semantic operation graph contains a cycle or duplicate reference at node '"
                            + id
                            + "'.");
        var node = graph.getNode(id);
        if (!Objects.equals(node.parentId(), parent))
            throw new IllegalStateException(
                    "Semantic operation graph parent mismatch for node '" + id + "'.");
