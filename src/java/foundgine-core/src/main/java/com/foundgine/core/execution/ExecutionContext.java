 ...
 *
 * <p>Runtime values supplied to an already-planned execution. Semantic
 * planning remains independent of these values. A deadline is an
 * execution-time security boundary and is never part of semantic plan shape.
 *
 * <p>The two-argument canonical constructor mirrors the C# record's
 * {@code Values} constructor parameter plus its {@code DeadlineUtc}
 * {@code init}-only property; the one-argument constructor mirrors
 * constructing the C# record without setting {@code DeadlineUtc}.
 */
public record ExecutionContext(Map<String, Object> values, Instant deadlineUtc) {
    ...
    /**
     * Port of {@code TryGetValue}. Returns an empty {@link Optional} both when
     * the key is absent and when it is present with a {@code null} value —
     * unlike the C# {@code out bool} overload, a Java {@code Optional} cannot
     * distinguish the two. No ported call site needs that distinction today.
     */
    ...
    /** Returns a copy of this context with {@code deadlineUtc} set, mirroring the C# {@code with { DeadlineUtc = ... }} pattern. */
