...
public final class SecurityWarrantCanonicalizer {
    ...
    public static String unsignedJson(SecurityWarrant w) {
        ...
        grants.sort(
                Comparator.comparing(CapabilityGrant::capability)
                        .thenComparing(CapabilityGrant::operation)
        ...
        if (w.parentId() == null) s.append("null");
        else str(s, w.parentId());
        key(s, "parentDigest", false);
        if (w.parentDigest() == null) s.append("null");
        else str(s, w.parentDigest());
    ...
    private static void str(StringBuilder s, String v) {
        s.append('"');
        if (v == null) throw new NullPointerException("canonical string");
