...
public final class SemanticModelBuilder {
    ...
    public SemanticModelBuilder entity(
            EntityId id, String name, Consumer<SemanticEntityBuilder> configure) {
        ...
        for (var e : model.entities()) {
            if (entities.containsKey(e.id()))
                throw new IllegalStateException(
                        "Semantic entity '" + e.id() + "' is already registered.");
            entities.put(e.id(), e);
        }
    ...
    public SemanticModelBuilder overlay(SemanticModel overlay) {
        for (var oe : overlay.entities()) {
            var existing =
                    entities.values().stream()
                            .filter(e -> e.name().equalsIgnoreCase(oe.name()))
                            .findFirst()
                            .orElse(null);
            if (existing == null) {
            ...
            if (existing.modelType() != null
                    && oe.modelType() != null
                    && !existing.modelType().equals(oe.modelType()))
                throw new IllegalStateException(
                        "Incompatible CLR model types for '" + existing.name() + "'.");
            ...
            for (var of : oe.fields()) {
                ...
                if (!f.clrType().equals(of.clrType()))
                    throw new IllegalStateException(
                            "Incompatible CLR types for field '"
                                    + existing.name()
                                    + "."
                                    + f.name()
                                    + "'.");
                fs.set(
                        ...
                        new SemanticField(
                                ...
                                f.name(),
                                f.clrType(),
                                of.semanticType() != null ? of.semanticType() : f.semanticType(),
                                of.capabilities(),
                                merge(f.effectiveAliases(), of.effectiveAliases()),
                                mergeConstraints(
                                        f.effectiveConstraints(), of.effectiveConstraints()),
                                of.nullableOverride() != null
                                        ? of.nullableOverride()
                                        : f.nullableOverride()));
            }
            ...
            for (var or : oe.relationships()) {
                ...
                if (!r.target().equals(or.target()) || r.cardinality() != or.cardinality())
                    throw new IllegalStateException(
                            "Conflicting semantic relationship '"
                                    + existing.name()
                                    + "."
                                    + r.name()
                                    + "'.");
            ...
            entities.put(
                    ...
                    new SemanticEntity(
                            ...
                            fs,
                            rs,
                            merge(existing.effectiveAliases(), oe.effectiveAliases()),
                            existing.modelType()));
        }
        for (var t : overlay.traversals())
            if (traversals.stream()
                    .noneMatch(
                            x ->
                                    x.source().equals(t.source())
                                            && x.name().equalsIgnoreCase(t.name())))
                traversals.add(t);
        return this;
    }

    public SemanticModelBuilder traversal(
            String sourceName, String name, String... relationshipNames) {
        var source =
                entities.values().stream()
                        .filter(e -> e.name().equalsIgnoreCase(sourceName))
                        .findFirst()
                        .orElseThrow(
                                () ->
        ...
        for (var rn : relationshipNames) {
            var owner = current;
            var r =
                    owner.relationships().stream()
                            .filter(x -> x.name().equalsIgnoreCase(rn))
                            .findFirst()
                            .orElseThrow(
                                    () ->
                                            new IllegalStateException(
    ...
    public SemanticModelBuilder traversal(EntityId source, String name, RelationshipId... path) {
        if (path.length == 0)
            throw new IllegalArgumentException(
                    "A semantic traversal must contain at least one relationship.");
        if (!entities.containsKey(source))
            throw new IllegalStateException(
                    "Source semantic entity '"
                            + source
                            + "' must be registered before declaring traversal '"
                            + name
                            + "'.");
        if (entities.get(source).relationships().stream()
                        .anyMatch(r -> r.name().equalsIgnoreCase(name))
                || traversals.stream()
                        .anyMatch(
                                t -> t.source().equals(source) && t.name().equalsIgnoreCase(name)))
            throw new IllegalStateException(
                    "Semantic traversal '"
                            + name
                            + "' conflicts with an existing relationship or traversal on entity '"
        ...
        for (var rid : path) {
            ...
            var r =
                    e.relationships().stream()
                            .filter(x -> x.id().equals(rid))
                            .findFirst()
                            .orElseThrow(
                                    () ->
                                            new IllegalStateException(
    ...
    private void validateUnique(SemanticEntity e) {
        if (e.fields().stream().map(SemanticField::id).distinct().count() != e.fields().size())
            throw new IllegalStateException(
                    "Semantic entity '" + e.name() + "' contains duplicate field identities.");
        if (e.fields().stream().map(x -> x.name().toLowerCase(Locale.ROOT)).distinct().count()
                != e.fields().size())
            throw new IllegalStateException(
                    "Semantic entity '" + e.name() + "' contains duplicate field names.");
        if (e.relationships().stream().map(SemanticRelationship::id).distinct().count()
                != e.relationships().size())
            throw new IllegalStateException(
                    "Semantic entity '"
                            + e.name()
                            + "' contains duplicate relationship identities.");
        if (e.relationships().stream()
    ...
    private void validateConstraints(SemanticEntity e) {
        for (var f : e.fields())
            for (var c : f.effectiveConstraints()) {
                if (c.kind() == SemanticConstraintKind.RANGE
                        && c.minimum() == null
                        && c.maximum() == null)
                    throw new IllegalStateException(
                            "Field '"
                                    ...
                                    + "."
                                    + f.name()
                                    + "' declares a Range constraint without a minimum or"
                                    + " maximum.");
                if (c.kind() != SemanticConstraintKind.RANGE
    ...
    private void validateGlobalRelationshipIdentities() {
        ...
        for (var e : entities.values())
            for (var r : e.relationships()) {
                ...
                if (prior != null
                        && (!prior.name().equals(r.name())
                                || !prior.target().equals(r.target())
                                || prior.cardinality() != r.cardinality()))
                    throw new IllegalStateException(
                            "Relationship identity conflict: '" + r.id() + "'.");
            }
    }
    ...
    private static List<SemanticAlias> merge(List<SemanticAlias> a, List<SemanticAlias> b) {
        ...
        for (var v : b)
            if (x.stream().noneMatch(y -> y.name().equalsIgnoreCase(v.name()))) x.add(v);
        return List.copyOf(x);
    }

    private static List<SemanticConstraint> mergeConstraints(
            List<SemanticConstraint> a, List<SemanticConstraint> b) {
        var x = new ArrayList<>(a);
        for (var v : b) if (!x.contains(v)) x.add(v);
        return List.copyOf(x);
    }
}
