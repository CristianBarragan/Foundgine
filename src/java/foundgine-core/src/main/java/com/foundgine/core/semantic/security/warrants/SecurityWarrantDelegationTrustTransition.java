...
/**
 * Port of {@code
 * Foundgine.Core.Semantic.Security.Warrants.SecurityWarrantDelegationTrustTransition}.
 *
 * <p>Pairs delegation-time trust validation ({@link #validateAndCapture}) with an execution-time
 * re-check ({@link #assertUnchanged}) so a delegation is authorized against a trust snapshot, and
 * that exact snapshot — not a fresh lookup — is what execution later confirms is still current.
 */
public final class SecurityWarrantDelegationTrustTransition {
    ...
    public static DelegationIssuerTrustSnapshot validateAndCapture(
        ...
        if (snapshot.keyState() != DelegationIssuerKeyState.ACTIVE) {
            throw new IllegalStateException(
                    "Only an active issuer key may authorize a new delegation.");
        }
        return snapshot;
    }
    ...
    /** Final execution gate: trust and key lifecycle must be unchanged since validation. */
