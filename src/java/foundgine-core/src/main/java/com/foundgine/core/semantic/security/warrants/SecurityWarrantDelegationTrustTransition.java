...
/**
 * Port of {@code Foundgine.Core.Semantic.Security.Warrants.SecurityWarrantDelegationTrustTransition}.
 *
 * <p>Pairs delegation-time trust validation ({@link #validateAndCapture})
 * with an execution-time re-check ({@link #assertUnchanged}) so a
 * delegation is authorized against a trust snapshot, and that exact
 * snapshot — not a fresh lookup — is what execution later confirms is
 * still current.
 */
