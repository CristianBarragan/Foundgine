...
public final class SemanticPredicateAlgebra {
    ...
    public static SemanticFilterExpression and(SemanticFilterExpression... predicates) {
        return combine(true, predicates);
    }

    public static SemanticFilterExpression or(SemanticFilterExpression... predicates) {
        return combine(false, predicates);
    }

    private static SemanticFilterExpression combine(
            boolean and, SemanticFilterExpression... predicates) {
        Objects.requireNonNull(predicates);
        var terms = Arrays.stream(predicates).filter(Objects::nonNull).toList();
        if (terms.isEmpty())
