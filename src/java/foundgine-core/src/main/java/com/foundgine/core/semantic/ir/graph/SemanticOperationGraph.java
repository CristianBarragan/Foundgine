...
public final class SemanticOperationGraph {
    ...
    public SemanticOperationGraphNode getNode(int id) {
        ...
        if (n == null)
            throw new NoSuchElementException(
                    "Semantic operation graph does not contain node '" + id + "'.");
        return n;
    }
    ...
    private static void build(
            SemanticReadNode n,
            Map<Integer, SemanticOperationGraphNode> map,
            Integer parent,
            boolean root) {
        if (map.putIfAbsent(
                        n.id(),
                        new SemanticOperationGraphNode(
                                ...
                                n.viaRelationship(),
                                n.viaConnection(),
                                n.children().stream().map(SemanticReadNode::id).toList(),
                                parent,
                                root ? n.queryOptions() : null,
                                n.authorization()))
                != null)
