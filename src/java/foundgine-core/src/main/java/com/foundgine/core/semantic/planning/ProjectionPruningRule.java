...
/**
 * Removes redundant duplicate projection fields while preserving first
 * occurrence order.
 */
public final class ProjectionPruningRule implements IPlanRewriteRule {
	...
	public List<String> preconditions() {
		return List.of("plan contains a projection", "projection contains redundant fields");
	}

	@Override
	public List<String> securityObligations() {
		return List.of("visibility.field", "visibility.relationship", "authorization.required");
	}
	...
	private static SemanticPlanNode rewriteNode(SemanticPlanNode node, boolean[] changed) {
		...
		if (fields != node.fields() || childChanged)
			return new SemanticPlanNode(node.id(), node.operation(), node.entityId(), fields, node.viaRelationship(),
					node.viaConnection(), children, node.queryOptions(), node.authorization(),
	...
	private static List<FieldId> dedupe(List<FieldId> fields, boolean[] changed) {
		...
		List<FieldId> result = new ArrayList<>(fields.size());
	...
	private static boolean containsRedundantProjection(SemanticPlanNode node) {
		return hasDuplicates(node.fields())
				|| node.children().stream().anyMatch(ProjectionPruningRule::containsRedundantProjection);
	}

	private static boolean hasDuplicates(List<FieldId> fields) {
		Set<FieldId> seen = new HashSet<>();
		for (var f : fields)
			if (!seen.add(f))
				return true;
