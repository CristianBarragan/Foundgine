...
public final class ResultMaterializer {
	...
	public SemanticResult materialize(SemanticPlan plan, ExecutionResult result) {
		...
		var page = result.pageInfo() == null ? null
				: new SemanticResultPageInfo(result.pageInfo().startCursor(), result.pageInfo().endCursor(),
						result.pageInfo().hasNextPage(), result.pageInfo().hasPreviousPage());
		var e = result.evidence();
		var evidence = e == null ? null
				: new SemanticResultEvidence(e.provider(), e.planFingerprint(), e.authorizedNodeIds(), e.rowsReturned(),
						e.elapsedMilliseconds(), e.providerOperationFingerprint(), e.intentFingerprint(),
						e.authorizationFingerprint());
		return new SemanticResult(roots, page, evidence);
	}

	private void addNode(SemanticPlanNode planNode, ExecutionRow row, List<SemanticResultNode> siblings,
		...
		if (node == null) {
			...
			for (var field : planNode.fields())
				values.put(field, getValue(row, planNode, field));
		...
		for (var child : planNode.children()) {
			...
			SemanticRelationship relationship = entity.relationships().stream()
					.filter(x -> x.id().equals(relationshipId)).findFirst().orElseThrow(() -> new IllegalStateException(
