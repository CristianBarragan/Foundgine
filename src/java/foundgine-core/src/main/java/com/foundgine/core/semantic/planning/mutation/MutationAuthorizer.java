...
public final class MutationAuthorizer {
            ...
            for (var field : op.fields()) {
                ...
                require(
                        policy.getFieldAccess(
                                entity.id(), field.field(), AuthorizationOperation.WRITE),
                ...
                if (!entity.fields().containsKey(field))
                    throw new IllegalStateException(
                            "Conflict field '"
                                    ...
                                    + "'.");
                require(
                        policy.getFieldAccess(entity.id(), field, AuthorizationOperation.WRITE),
                ...
                if (!entity.fields().containsKey(field))
                    throw new IllegalStateException(
                            "Return field '"
                                    ...
                                    + "'.");
                require(
                        policy.getFieldAccess(entity.id(), field, AuthorizationOperation.READ),
        ...
        for (var field : op.fields()) {
            var fieldId =
                    entity.fields().entrySet().stream()
                            .filter(e -> Objects.equals(e.getValue(), field.column()))
                            .map(Map.Entry::getKey)
                            .findFirst()
                            .orElseThrow(
                                    () ->
                                            new IllegalStateException(
                                                    "Mutation field column '"
                                                            + field.column().value()
                                                            + "' has no semantic field mapping on '"
                                                            + entity.name()
                                                            + "'."));
            require(
                    policy.getFieldAccess(entity.id(), fieldId, AuthorizationOperation.WRITE),
    ...
    private void validateFilter(SemanticFilterExpression filter, MutationEntitySchema entity) {
        ...
        if (filter instanceof SemanticFieldFilter f) {
            require(
                    policy.getFieldAccess(entity.id(), f.field(), AuthorizationOperation.READ),
        ...
        if (filter instanceof SemanticRelationshipFilter r) {
            require(
                    policy.getRelationshipAccess(
                            entity.id(), r.relationship(), AuthorizationOperation.READ),
        ...
        if (filter instanceof SemanticAggregateFilter a) {
            require(
                    policy.getRelationshipAccess(
                            entity.id(), a.relationship(), AuthorizationOperation.READ),
                    "aggregate filter through relationship '" + a.relationship().value() + "'");
            if (a.field() != null) {
                var rel = schema.getRelationship(a.relationship());
                require(
                        policy.getFieldAccess(rel.target(), a.field(), AuthorizationOperation.READ),
        ...
        throw new UnsupportedOperationException(
                "Mutation authorization does not support filter '"
