...
public record RewriteRuleCompositionOptions(
        int maxRuleApplications,
        int maxPlanVisits,
        RuleSelectionPolicy selectionPolicy,
        IProviderCostEstimator providerCostEstimator,
    ...
    public RewriteRuleCompositionOptions validate() {
        ...
        (providerCostSelectionPolicy == null
                        ? new ProviderCostSelectionPolicy()
                        : providerCostSelectionPolicy)
                .validate();
        if (providerCostEstimator != null
                && (providerCostEstimator.provider() == null
                        || providerCostEstimator.provider().isBlank()))
            throw new IllegalArgumentException(
                    "Provider cost estimator must identify its provider.");
        return this;
    }
}
