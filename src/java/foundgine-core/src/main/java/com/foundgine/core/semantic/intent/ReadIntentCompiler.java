...
public final class ReadIntentCompiler {
    ...
    public SemanticRequest compile(ReadIntent intent) {
        ...
        var selections =
                intent.selections().stream().map(s -> compileSelection(c, root, s)).toList();
        ...
        return new SemanticRequest(
                ...
                new SemanticQueryOptions(
                        filter, order, intent.limit(), intent.offset(), intent.after()),
                intent.security());
    }
    ...
    /**
     * Compiles and validates a dynamic intent against provider-neutral operation graph resource
     * limits before the graph can reach planning or execution. This mirrors the C# overload and
     * keeps security bounds inside Core rather than relying on a particular JSON/MCP adapter.
     */
    public SemanticOperationGraph compileOperationGraph(
            ReadIntent intent, SecurityResourceLimits limits) {
        Objects.requireNonNull(intent, "intent");
        Objects.requireNonNull(limits, "limits");
        var request = compile(intent);
        var graph =
                SemanticOperationGraph.create(
                        SemanticOperationCompiler.compile(
                                new SemanticRequestResolver(snapshot()).resolve(request)));
    ...
    public SemanticIntentResolution resolveDocument(SemanticIntentDocument document) {
        ...
        if (!document.contractFingerprint().equals(contractFingerprint()))
            throw new IllegalArgumentException(
                    "Semantic intent document is bound to contract '"
                            + document.contractFingerprint()
                            + "', but the resolver uses '"
                            + contractFingerprint()
                            + "'.");
        return new SemanticIntentResolution(
                document, compile(document.intent()), contractFingerprint());
    }

    public SemanticOperationGraph resolveDocumentGraph(SemanticIntentDocument document) {
        return compileOperationGraph(resolveDocument(document).document().intent());
    }

    private SemanticSelection compileSelection(
            SemanticContractSnapshot c, SemanticEntity entity, ReadSelection selection) {
        ...
        if (selection.field() != null) {
            ...
            if (!selection.effectiveChildren().isEmpty())
                throw invalid(
                        "Field '" + entity.name() + "." + f.name() + "' cannot have children.");
        ...
        var children =
                selection.effectiveChildren().stream()
                        .map(x -> compileSelection(c, c.get(path.get(path.size() - 1).target()), x))
                        .toList();
        for (int i = path.size() - 1; i >= 0; i--)
            children = List.of(new SemanticSelection(null, path.get(i).id(), children));
        return children.get(0);
    }

    private SemanticFilterExpression compileFilter(
            SemanticContractSnapshot c, SemanticEntity entity, ReadFilter filter) {
        if (filter instanceof ReadFieldFilter f)
            return new SemanticFieldFilter(
                    findField(entity, f.field()).id(), f.operator(), f.value());
        if (filter instanceof ReadRelationshipFilter r) {
            var path = resolveRelationshipPath(c, entity, r.relationship());
            if (path.size() > 1 && r.quantifier() != SemanticRelationshipQuantifier.SOME)
                throw invalid(
                        "Logical traversal filter currently supports only SOME quantification"
                                + " across multi-hop paths.");
            SemanticFilterExpression p =
                    compileFilter(c, c.get(path.get(path.size() - 1).target()), r.predicate());
            for (int i = path.size() - 1; i >= 0; i--)
                p =
        ...
        if (filter instanceof ReadAndFilter a) {
            if (a.expressions().isEmpty()) throw invalid("AND filter cannot be empty.");
            return new SemanticAndFilter(
                    a.expressions().stream().map(x -> compileFilter(c, entity, x)).toList());
        }
        if (filter instanceof ReadOrFilter o) {
            if (o.expressions().isEmpty()) throw invalid("OR filter cannot be empty.");
            return new SemanticOrFilter(
                    o.expressions().stream().map(x -> compileFilter(c, entity, x)).toList());
        }
        throw invalid("Unsupported read filter '" + filter.getClass().getSimpleName() + "'.");
    }

    private SemanticOrderTerm compileOrder(
            SemanticContractSnapshot c, SemanticEntity root, ReadOrder order) {
        var entity = root;
        var path = new ArrayList<RelationshipId>();
        for (var name : order.effectivePath())
            for (var r : resolveRelationshipPath(c, entity, name)) {
                path.add(r.id());
                entity = c.get(r.target());
            }
        var field =
                order.aggregate() == SemanticOrderAggregate.COUNT
    ...
    private SemanticEntity findEntity(SemanticContractSnapshot c, String name) {
        try {
            return c.resolveEntity(name);
        } catch (Exception e) {
            throw invalid("Unknown entity '" + name + "'.");
        }
    }

    private static SemanticField findField(SemanticEntity e, String name) {
        return e.fields().stream()
                ...
                .orElseGet(
                        () -> {
                            if (e.identity().name().equalsIgnoreCase(name))
                                return new SemanticField(
                                        e.identity().fieldId(), e.identity().name(), Object.class);
                            throw invalid("Unknown field '" + e.name() + "." + name + "'.");
                        });
    }

    private List<SemanticRelationship> resolveRelationshipPath(
            SemanticContractSnapshot c, SemanticEntity entity, String name) {
        var direct =
        ...
        var t =
                c.traversals().stream()
                        .filter(
                                x ->
                                        x.source().equals(entity.id())
                                                && x.name().equalsIgnoreCase(name))
                        .findFirst()
                        .orElseThrow(
                                () ->
                                        invalid(
                                                "Unknown relationship or semantic traversal '"
                                                        + entity.name()
                                                        + "."
                                                        + name
                                                        + "'."));
        var current = entity;
        var result = new ArrayList<SemanticRelationship>();
        for (var id : t.path()) {
            final var currentEntity = current;
            var r =
                    current.relationships().stream()
                                    ...
                                    () ->
                                            invalid(
                                                    "Traversal '"
                                                            ...
                                                            + "'."));
            result.add(r);
            current = c.get(r.target());
        }
        return result;
    }

    private static IllegalArgumentException invalid(String message) {
        return new IllegalArgumentException("Invalid read intent: " + message);
    }
}
