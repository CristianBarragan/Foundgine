...
public record AggregateRewriteProof(
    ...
    public boolean isSatisfied() {
        return semanticEquivalence.isSatisfied()
    ...
    public static AggregateRewriteProof create(SemanticPlan before, SemanticPlan after,
                                               SemanticAggregateSemantics from, SemanticAggregateSemantics to,
                                               AggregateCardinalityProof cardinalityProof,
                                               AggregateProviderCapability providerCapability,
                                               ProviderCostEstimate costEstimate) {
        Objects.requireNonNull(before); Objects.requireNonNull(after); Objects.requireNonNull(from);
        Objects.requireNonNull(to); Objects.requireNonNull(cardinalityProof); Objects.requireNonNull(providerCapability);
        var semantic = SemanticEquivalenceProof.create(before, after);
        if (!providerCapability.supports(to.aggregate()))
            throw new IllegalStateException("Provider '"+providerCapability.providerName()+"' does not declare support for aggregate '"+to.aggregate()+"'.");
        ...
        if (!combined.isLegal()) throw new IllegalStateException("Aggregate rewrite rejected because it does not preserve aggregate semantics: "+String.join(" ",combined.violations()));
        var auth = AuthorizationPreservationProof.create(before,after);
        if (!auth.isSatisfied()) throw new IllegalStateException("Aggregate rewrite rejected because it does not preserve the source plan's security contract: "+String.join(" ",auth.violations()));
        return new AggregateRewriteProof(semantic,empty,nul,dup,card,to.aggregate(),providerCapability,costEstimate,auth);
    }
}
