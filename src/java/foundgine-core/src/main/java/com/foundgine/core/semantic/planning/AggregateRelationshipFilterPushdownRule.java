...
/**
 * Pushes an existential relationship predicate into a COUNT aggregate. COUNT(R) > 0 AND SOME(R, P)
...
public final class AggregateRelationshipFilterPushdownRule implements IPlanRewriteRule {
        ...
        if (filter instanceof SemanticAndFilter and) {
            ...
            for (int i = 0; i < expressions.size(); i++) {
                ...
                for (int j = 0; j < expressions.size(); j++) {
                    if (i == j
                            || !(expressions.get(j) instanceof SemanticRelationshipFilter r)
                            || r.quantifier() != SemanticRelationshipQuantifier.SOME
                            || !r.relationship().equals(a.relationship())) continue;
                    var rewritten =
                    ...
                    for (int k = 0; k < expressions.size(); k++) {
                        if (k == i) remaining.add(rewritten);
                        else if (k != j) remaining.add(expressions.get(k));
                    }
                    changed[0] = true;
    ...
    private static boolean containsEligible(SemanticPlanNode node) {
        return (node.queryOptions() != null
                ...
                || node.children().stream()
                        .anyMatch(AggregateRelationshipFilterPushdownRule::containsEligible);
    }
    ...
    private static boolean containsEligible(SemanticFilterExpression filter) {
        if (filter instanceof SemanticAndFilter and)
            return and.expressions().stream()
                            .anyMatch(
                                    x ->
                                            x instanceof SemanticAggregateFilter a
                                                    && isEligibleCountExists(a))
                    && and.expressions().stream()
                            .anyMatch(
                                    x ->
                                            x instanceof SemanticRelationshipFilter r
                                                    && r.quantifier()
    ...
    private static boolean isEligibleCountExists(SemanticAggregateFilter a) {
        return a.aggregate() == SemanticFilterAggregate.COUNT
                && a.field() == null
                && a.predicate() == null
                && AggregateExecutionStrategyResolver.resolve(a.operator(), a.value())
