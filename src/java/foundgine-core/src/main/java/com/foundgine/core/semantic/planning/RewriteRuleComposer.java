...
public final class RewriteRuleComposer {
    ...
    public RewriteRuleComposer(
            Iterable<IPlanRewriteRule> rules, RewriteRuleCompositionOptions options) {
        ...
        this.selector =
                new RewriteRuleSelector(
                        this.options.selectionPolicy(),
                        this.options.providerCostEstimator(),
    ...
    public RewriteRuleCompositionResult compose(SemanticPlan plan) {
        ...
        for (int pass = 0; ; pass++) {
            ...
            for (var rule : orderedRules) {
                if (rule.isIdempotent() && appliedIdempotent.contains(rule.name())) continue;
                if (rule.mustRunAfter().stream().anyMatch(x -> !appliedRules.contains(x))) continue;
                ...
                if (blocked) continue;
                if (rule.conflictsWith().stream().anyMatch(appliedRules::contains)) continue;
                eligible.add(rule);
            }
            String selectedName;
            ...
            if (candidate == current) {
                ...
                if (rule.isIdempotent()) appliedIdempotent.add(rule.name());
                continue;
            }
            ...
            if (!result.isSatisfied())
                throw new IllegalStateException(
                        "Rewrite rule '"
                                + rule.name()
                                + "' did not satisfy its proof obligations.");
            ...
            if (seen.size() > options.maxPlanVisits())
                throw new IllegalStateException(
                        "Rewrite rule composition exceeded the maximum plan-visit budget.");
    ...
    private static RewriteRuleCompositionResult result(
            SemanticPlan p,
            List<PlanRewriteRuleResult> a,
            List<RewriteRuleCandidate> h,
            List<ProviderAwareRewriteRuleCandidate> ph) {
        return new RewriteRuleCompositionResult(
                p, a, a.stream().mapToDouble(PlanRewriteRuleResult::costImpact).sum(), true, h, ph);
    }

    private static List<IPlanRewriteRule> toList(Iterable<IPlanRewriteRule> rules) {
        var r = new ArrayList<IPlanRewriteRule>();
        for (var x : rules) r.add(x);
        return r;
    }
    ...
    private static List<IPlanRewriteRule> orderRules(List<IPlanRewriteRule> rules) {
        ...
        for (var r : rules) {
            ...
            for (var conflict : r.conflictsWith())
                if (byName.containsKey(conflict)
                        && byName.get(conflict).conflictsWith().contains(r.name()))
                    throw new IllegalStateException(
                            "Rewrite rules '"
                                    + r.name()
                                    + "' and '"
                                    + conflict
                                    + "' conflict and cannot be composed.");
        }
        ...
        for (var r : rules) {
            for (var after : r.mustRunAfter()) addEdge(edges, indegree, after, r.name());
            for (var before : r.mustRunBefore()) addEdge(edges, indegree, r.name(), before);
        }
        var queue =
                new PriorityQueue<IPlanRewriteRule>(
                        Comparator.comparingInt(IPlanRewriteRule::priority)
                                .thenComparing(IPlanRewriteRule::name));
        for (var r : rules) if (indegree.get(r.name()) == 0) queue.add(r);
        var ordered = new ArrayList<IPlanRewriteRule>();
        while (!queue.isEmpty()) {
            var r = queue.remove();
            ordered.add(r);
            for (var next : edges.get(r.name()))
                if (indegree.merge(next, -1, Integer::sum) == 0) queue.add(byName.get(next));
        }
    ...
    private static void addEdge(
            Map<String, Set<String>> edges, Map<String, Integer> indegree, String from, String to) {
        if (edges.get(from).add(to)) indegree.merge(to, 1, Integer::sum);
    }

    public static PlanRewriteRuleResult applyRule(
            IPlanRewriteRule rule, SemanticPlan before, SemanticPlan after) {
