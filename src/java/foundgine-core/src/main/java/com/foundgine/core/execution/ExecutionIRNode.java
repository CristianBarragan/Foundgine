...
public record ExecutionIRNode(
    ...
    public ExecutionIRNode {
        ...
        aggregateExecutionStrategy = aggregateExecutionStrategy == null
    ...
    static ExecutionIRNode from(SemanticPlanNode node) {
        return new ExecutionIRNode(
                node.id(), node.operation(), node.entityId(), node.fields(),
                node.viaRelationship(), node.viaConnection(),
                node.children().stream().map(ExecutionIRNode::from).toList(),
                node.queryOptions(), node.authorization(),
