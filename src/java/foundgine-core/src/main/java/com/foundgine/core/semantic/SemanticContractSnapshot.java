...
public final class SemanticContractSnapshot {
    ...
    private Map<EntityId, Map<String, Integer>> buildEntity() {
        ...
        for (var e : entities.values()) m.put(e.id(), aliasMap(e.effectiveAliases()));
    ...
    private Map<EntityId, Map<FieldId, Map<String, Integer>>> buildField() {
        ...
        for (var e : entities.values()) {
            ...
            for (var f : e.fields()) x.put(f.id(), aliasMap(f.effectiveAliases()));
    ...
    private Map<RelationshipId, Map<String, Integer>> buildRelationship() {
        ...
        for (var e : entities.values())
            for (var r : e.relationships()) m.put(r.id(), aliasMap(r.effectiveAliases()));
        return Collections.unmodifiableMap(m);
    }
    ...
    private static Map<String, Integer> aliasMap(List<SemanticAlias> aliases) {
        ...
        for (var a : aliases) if (a.weight() != null) m.merge(a.name(), a.weight(), Math::max);
        return Collections.unmodifiableMap(m);
    }
                ...
                .orElseThrow(
                        () ->
                                new NoSuchElementException(
                                        "Semantic entity '" + name + "' is not defined."));
    }
    ...
    public boolean tryGetAlias(
            EntityId entityId, FieldId fieldId, String alias, java.util.function.IntConsumer sink) {
    ...
    public boolean tryGetAlias(
            RelationshipId relationshipId, String alias, java.util.function.IntConsumer sink) {
    ...
    private static OptionalInt weight(Map<String, Integer> m, String a) {
        if (m == null) return OptionalInt.empty();
        var v = m.get(a);
