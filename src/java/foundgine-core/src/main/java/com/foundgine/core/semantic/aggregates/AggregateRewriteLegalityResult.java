...
public record AggregateRewriteLegalityResult(boolean isLegal, List<String> violations) {
    public AggregateRewriteLegalityResult {
        violations = violations == null ? List.of() : List.copyOf(violations);
    }

    public static final AggregateRewriteLegalityResult LEGAL =
            new AggregateRewriteLegalityResult(true, List.of());

    public static AggregateRewriteLegalityResult illegal(String... violations) {
        if (violations == null || violations.length == 0)
            throw new IllegalArgumentException(
                    "An illegal result must carry at least one violation.");
        return new AggregateRewriteLegalityResult(false, List.of(violations));
    }

    public static AggregateRewriteLegalityResult combine(
            AggregateRewriteLegalityResult... results) {
        var violations =
