...
/**
 * Resolves a protocol-neutral SemanticRequest against the static semantic model and produces the
 * request SemanticGraph. Provider- and protocol-independent.
 */
public final class SemanticRequestResolver {
    ...
    public SemanticGraph resolve(SemanticRequest request) {
        ...
        SemanticFilterValidator.validate(
                request.options() == null ? null : request.options().filter(), root, contract);
        var normalized = normalizeQueryOptions(request.options(), root);
        SemanticQueryOptionsValidator.validate(normalized, root);
        validateOrdering(
                normalized == null ? List.of() : normalized.effectiveOrder(),
                root,
                request.selections());
    ...
    private SemanticQueryOptions normalizeQueryOptions(
            SemanticQueryOptions options, SemanticEntity root) {
                            ...
                            .anyMatch(
                                    x ->
                                            x.path().isEmpty()
                                                    ...
                                                    && x.field().equals(root.identity().fieldId()));
            if (!hasIdentity)
                order.add(
                        new SemanticOrderTerm(
                                root.identity().fieldId(), SemanticSortDirection.ASC));
        }
        return new SemanticQueryOptions(
                options.filter(), order, options.limit(), options.offset(), options.after());
    }
    ...
    private SemanticOrderTerm canonicalizeOrderTerm(SemanticOrderTerm term, SemanticEntity root) {
        ...
        return new SemanticOrderTerm(
                entity.identity().fieldId(), term.direction(), term.path(), term.aggregate());
    }
    ...
    private void resolveSelections(
            ...
            SemanticGraph graph,
            SemanticGraph.SemanticGraphNode parent,
        ...
        for (var selection : selections) {
            if (selection.field() != null && selection.relationship() != null)
                throw invalidSelection(
                        "A selection cannot contain both a field and a relationship.");
            if (selection.field() == null && selection.relationship() == null)
                throw invalidSelection("A selection must contain a field or a relationship.");
            if (selection.field() != null) {
                if (!selection.children().isEmpty())
                    throw invalidSelection(
                            "Field '" + selection.field() + "' cannot have child selections.");
                ...
                if (!relationshipIds.add(relationshipId))
                    throw invalidSelection(
                            "Relationship '"
                                    ...
                                    + entity.name()
                                    + "'. Merge repeated selections in the adapter before"
                                    + " resolution.");
                var relationship =
                        entity.relationships().stream()
        ...
        for (var selected : relationships) {
            ...
            resolveSelections(
                    target, selected.children(), graph, node, selected.relationship().id(), false);
        }
    }

    private void validateOrdering(
            List<SemanticOrderTerm> terms,
            SemanticEntity root,
            List<SemanticSelection> selections) {
        for (var term : terms) {
            var entity = root;
            ...
            for (var relationshipId : term.path()) {
                ...
                if (relationship.cardinality() == RelationshipCardinality.MANY) {
                    if (!relationshipId.equals(term.path().get(term.path().size() - 1))
                            || !term.isAggregate())
                        throw new UnsupportedOperationException(
                                "Ordering through collection relationship '"
                                        + entity.name()
                                        + "."
                                        + relationship.name()
                                        + "' requires an explicit aggregate semantics (COUNT, MIN,"
                                        + " or MAX).");
                ...
                var aggregateCollectionHop =
                        relationship.cardinality() == RelationshipCardinality.MANY
                                && term.isAggregate()
                                && relationshipId.equals(term.path().get(term.path().size() - 1));
                if (!aggregateCollectionHop
                        && !isRelationshipSelected(currentSelections, relationshipId))
                    throw new IllegalStateException(
                            "Order path '"
                                    + String.join(
                                            ".",
                                            term.path().stream().map(Object::toString).toList())
                                    + "' requires relationship '"
                                    + entity.name()
                                    + "."
                                    + relationship.name()
                                    + "' to be selected. The provider will not introduce an"
                                    + " implicit join solely for ordering.");
                entity = contract.get(relationship.target());
            ...
            if (term.isAggregate()) {
                ...
                if (finalRelationship == null)
                    throw invalidSelection(
                            "Aggregate order relationship '"
                                    + term.path().get(term.path().size() - 1)
                                    + "' is not defined.");
                if (finalRelationship.cardinality() != RelationshipCardinality.MANY)
                    throw invalidSelection(
                            "Aggregate ordering is only valid on collection relationships.");
                if ((term.aggregate() == SemanticOrderAggregate.MIN
                                || term.aggregate() == SemanticOrderAggregate.MAX)
                        && !isFieldSortable(entity, term.field()))
                    throw invalidSelection(
                            "Aggregate order field '" + term.field() + "' is not sortable.");
                ...
                throw invalidSelection(
                        "Field '" + entity.name() + "." + term.field() + "' is not sortable.");
        }
    }
    ...
    private static boolean isFieldSelectable(SemanticEntity entity, FieldId fieldId) {
        ...
        return entity.fields().stream()
                .anyMatch(
                        x ->
                                x.id().equals(fieldId)
                                        && (x.capabilities() & SemanticFieldCapabilities.SELECTABLE)
                                                != 0);
    }
    ...
    private static boolean isFieldSortable(SemanticEntity entity, FieldId fieldId) {
        ...
        return entity.fields().stream()
                .anyMatch(
                        x ->
                                x.id().equals(fieldId)
                                        && (x.capabilities() & SemanticFieldCapabilities.SORTABLE)
                                                != 0);
    }
    ...
    private static boolean isRelationshipSelected(
            List<SemanticSelection> selections, RelationshipId id) {
        return selections.stream().anyMatch(s -> id.equals(s.relationship()));
    }

    private static List<SemanticSelection> findRelationshipSelections(
            List<SemanticSelection> selections, RelationshipId id) {
        return selections.stream()
                .filter(s -> id.equals(s.relationship()))
                .findFirst()
                .map(SemanticSelection::children)
                .orElse(List.of());
    }
    ...
    private record RelationshipSelection(
            SemanticRelationship relationship, List<SemanticSelection> children) {}
}
