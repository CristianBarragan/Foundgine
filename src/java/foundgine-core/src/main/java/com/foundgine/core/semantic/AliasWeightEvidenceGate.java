...
public final class AliasWeightEvidenceGate {
    ...
    public record AliasWeightEvidenceResult(
        ...
        public AliasWeightEvidenceResult {
            ...
            relationshipWeights =
                    relationshipWeights == null ? Map.of() : Map.copyOf(relationshipWeights);
            violatingEntities =
                    violatingEntities == null ? List.of() : List.copyOf(violatingEntities);
            violatingFields = violatingFields == null ? List.of() : List.copyOf(violatingFields);
            violatingRelationships =
                    violatingRelationships == null
                            ? List.of()
                            : List.copyOf(violatingRelationships);
        }
    ...
    public static AliasWeightEvidenceResult evaluate(
        ...
        model.ensureFrozen();
        return evaluate(
                model.createSnapshot(), minimumWeight, lexicalResolution, modelKnownWithCertainty);
    }
    ...
    public static AliasWeightEvidenceResult evaluate(
        ...
        var modelEvidence =
                modelKnownWithCertainty
                        ? ModelResolutionEvidence.KNOWN_WITH_CERTAINTY
        ...
        if (lexicalResolution == null || lexicalResolution.steps().isEmpty())
            return new AliasWeightEvidenceResult(
                    AliasEvidenceStatus.NOT_APPLICABLE,
                    modelEvidence,
                    Map.of(),
                    Map.of(),
                    Map.of(),
        ...
        for (var step : lexicalResolution.steps()) {
            ...
            switch (c.kind()) {
                case ENTITY, NODE -> {
                    if (c.entityId() != null)
                        model.aliasWeight(c.entityId(), c.token())
                                .ifPresent(v -> ew.merge(c.entityId(), v, Math::max));
                }
                case FIELD -> {
        ...
        var ve =
                ew.entrySet().stream()
                        .filter(x -> x.getValue() < minimumWeight)
        ...
        var status =
                !applicable
                        ? AliasEvidenceStatus.NOT_APPLICABLE
                        : (ve.isEmpty() && vf.isEmpty() && vr.isEmpty()
                                ? AliasEvidenceStatus.SUFFICIENT
                                : AliasEvidenceStatus.INSUFFICIENT);
        return new AliasWeightEvidenceResult(
                status, modelEvidence, ew, fw, rw, ve, vf, vr, model.contractFingerprint());
    }

    public static AliasWeightEvidenceResult evaluate(
            SemanticContractSnapshot model, int minimumWeight) {
        return evaluate(model, minimumWeight, null, false);
    }

    public record AliasInterpretationEvidence(
            AliasEvidenceStatus status,
            Map<EntityId, Integer> entityWeights,
            Map<FieldId, Integer> fieldWeights,
            Map<RelationshipId, Integer> relationshipWeights,
        ...
        public static AliasInterpretationEvidence from(AliasWeightEvidenceResult result) {
            return new AliasInterpretationEvidence(
                    result.status(),
                    result.entityWeights(),
                    result.fieldWeights(),
