...
public record GroundingDecision(String expression, GroundingOutcome outcome, GroundingInterpretation committed,
		List<GroundingInterpretation> competingInterpretations, String reason,
		List<SemanticLexicalCandidate> rootCandidates, GroundingBudgetLimit budgetLimit,
		List<GroundingInterpretation> partialInterpretationsAtCutoff, AliasInterpretationEvidence aliasEvidence,
	...
	public GroundingDecision {
		...
		partialInterpretationsAtCutoff = partialInterpretationsAtCutoff == null ? List.of()
				: List.copyOf(partialInterpretationsAtCutoff);
	...
	public GroundingDecision(String expression, GroundingOutcome outcome, GroundingInterpretation committed,
			List<GroundingInterpretation> competing, String reason, List<SemanticLexicalCandidate> roots) {
	...
	public boolean hadCompetingMeanings() {
		return !competingInterpretations.isEmpty();
	}

	public List<GroundingInterpretation> effectivePartialInterpretationsAtCutoff() {
		return partialInterpretationsAtCutoff;
	}

	public List<CandidateTruncation> effectiveTruncationRisks() {
		return truncationRisks;
	}
}
