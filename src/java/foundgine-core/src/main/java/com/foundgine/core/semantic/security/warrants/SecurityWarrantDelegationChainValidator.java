...
public final class SecurityWarrantDelegationChainValidator {
    ...
    public static void validate(
            List<SecurityWarrant> chain, Instant now, String expectedRootDigest) {
        if (chain == null) throw new NullPointerException("chain");
        if (chain.isEmpty())
            throw new IllegalStateException(
                    "A delegation chain must contain at least one warrant.");
        if (chain.size() > MAX_DELEGATION_DEPTH + 1)
            throw new IllegalStateException(
                    "Delegation chain exceeds the maximum supported depth.");
        SecurityWarrant root = chain.get(0);
        if (root.parentId() != null
                || root.parentDigest() != null
                || !root.delegationPath().isEmpty())
        ...
        if (expectedRootDigest != null && !root.digest().equals(expectedRootDigest))
            throw new IllegalStateException(
                    "Delegation chain root does not match the expected root digest.");
        ...
        for (int i = 1; i < chain.size(); i++) {
            ...
            if (!parent.isTimeValid(now))
                throw new IllegalStateException(
                        "A delegation ancestor is no longer currently valid.");
            ...
            if (!java.util.Objects.equals(child.parentId(), parent.id()))
                throw new IllegalStateException(
                        "Delegation chain contains a parent-id substitution or reorder.");
            if (!java.util.Objects.equals(child.parentDigest(), parent.digest()))
                throw new IllegalStateException(
                        "Delegation chain contains a parent-digest substitution.");
            if (!java.util.Objects.equals(child.issuer(), parent.subject()))
                throw new IllegalStateException(
                        "Delegation chain issuer does not match the parent subject.");
