...
/**
 * Port of {@code Foundgine.Core.Semantic.Security.SecurityInvariantContractValidator}.
 *
 * <p>Validates the machine-readable security contract before a capability is allowed to cross into
 * provider planning. This is a structural proof gate, not an authorization decision.
 */
public final class SecurityInvariantContractValidator {
    ...
    public static List<String> validate(SemanticCapability capability) {
        ...
        for (String id : capability.requiredSecurityInvariants()) {
            if (!SecurityInvariantRegistry.contains(id)) {
                errors.add(
                        "Capability '"
                                + capability.id()
                                + "' references unknown security invariant '"
                                + id
                                + "'.");
            }
        }
        ...
        if (capability.hasSideEffects()
                && !invariants.contains(SecurityInvariantIds.RUNTIME_AUTHORIZATION)) {
            errors.add(
                    "Mutating capability '"
                            + capability.id()
                            + "' must require security invariant '"
                            + SecurityInvariantIds.RUNTIME_AUTHORIZATION
                            + "'.");
        }

        if (capability.hasSideEffects()
                && !invariants.contains(SecurityInvariantIds.AUTHORIZATION_REQUIRED)) {
            errors.add(
                    "Mutating capability '"
                            + capability.id()
                            + "' must require security invariant '"
        ...
        if (!capability.fields().isEmpty()
                && !invariants.contains(SecurityInvariantIds.FIELD_VISIBILITY)) {
            errors.add(
                    "Capability '"
                            + capability.id()
                            + "' exposes fields without security invariant '"
        ...
        if (!capability.relationships().isEmpty()
            ...
            errors.add(
                    "Capability '"
                            + capability.id()
                            + "' exposes relationships without a relationship-visibility"
                            + " invariant.");
        }
