 ...
 * ConnectionId, ModelId, RelationshipId, AuthorizationId). The C# types wrap
 * a {@code ulong}; Java has no unsigned 64-bit primitive, so the identity is
 * stored as a signed {@code long} using its full 64-bit bit pattern, and
 * this class is responsible for translating that bit pattern to/from the
 * unsigned decimal representation used on the wire (matching
 * {@code System.Text.Json}'s handling of {@code ulong}).
 */
public final class UnsignedLongJson {
    ...
    /** Writes {@code value} (interpreted as unsigned) as a JSON number, e.g. {@code 18446744073709551615}. */
     ...
     * {@code {"Value": <number>}} object shape emitted by older Foundgine
     * versions. Mirrors {@code *IdJsonConverter.Read} in the C# source.
     */
    ...
    /** Parses an unsigned decimal string (e.g. a JSON property name used as a map key) into its bit pattern. */
