...
public final class SecurityInvariantPlanRequirements {
    ...
    public static SemanticPlan attach(SemanticPlan plan, Iterable<String> capabilityInvariants) {
        ...
        return new SemanticPlan(
                plan.root(), ids.stream().sorted().toList(), plan.authorizationBinding());
    }
    ...
    private static void collect(SemanticPlanNode node, Set<String> ids) {
        ...
        if (!node.fields().isEmpty()) ids.add(SecurityInvariantIds.FIELD_VISIBILITY);
        if (!node.children().isEmpty()) ids.add(SecurityInvariantIds.RELATIONSHIP_VISIBILITY);
        if (node.authorization() != null) ids.add(SecurityInvariantIds.RUNTIME_AUTHORIZATION);
