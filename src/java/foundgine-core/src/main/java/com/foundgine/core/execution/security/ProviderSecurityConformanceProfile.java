...
public record ProviderSecurityConformanceProfile(
        String provider, Collection<String> preservedSecurityInvariants, Collection<String> notes) {
    public ProviderSecurityConformanceProfile {
        Objects.requireNonNull(provider, "provider");
        preservedSecurityInvariants =
                List.copyOf(
                        preservedSecurityInvariants == null
                                ? List.of()
                                : preservedSecurityInvariants);
        ...
        for (var invariant : requiredInvariants) {
            if (!SecurityInvariantRegistry.contains(invariant))
                throw new IllegalStateException(
                        "Unknown required security invariant '" + invariant + "'.");
        }
        return SecurityInvariantAttestation.create(
                provider, requiredInvariants, preservedSecurityInvariants);
    }
}
