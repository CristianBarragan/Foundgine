...
public final class AuthorizationCanonicalizationRule implements IPlanRewriteRule {
	...
	public List<String> preconditions() {
		return List.of("plan contains an authorization predicate", "predicate uses supported boolean structure");
	}
	...
	private static SemanticPlanNode rewriteNode(SemanticPlanNode node, boolean[] changed) {
		...
		if (nodeChanged)
			changed[0] = true;
		return nodeChanged
				? new SemanticPlanNode(node.id(), node.operation(), node.entityId(), node.fields(),
						node.viaRelationship(), node.viaConnection(), children, node.queryOptions(), normalized,
	...
	private static boolean containsAuthorization(SemanticPlanNode node) {
		return node.authorization() != null
				|| node.children().stream().anyMatch(AuthorizationCanonicalizationRule::containsAuthorization);
	}
	...
	private static AuthorizationPredicate normalize(AuthorizationPredicate predicate, boolean[] changed) {
		...
		var current = left == predicate.left() && right == predicate.right() ? predicate
				: new AuthorizationPredicate(predicate.kind(), predicate.name(), predicate.value(), left, right);
	...
	private static void flatten(AuthorizationPredicateKind kind, AuthorizationPredicate node,
		...
		if (node.kind() == kind) {
			if (node.left() != null)
				flatten(kind, node.left(), out);
			if (node.right() != null)
				flatten(kind, node.right(), out);
		} else
			out.add(node);
	}

	private static AuthorizationPredicate rebuildBalanced(AuthorizationPredicateKind kind,
			List<AuthorizationPredicate> operands) {
		AuthorizationPredicate result = operands.get(0);
		for (int i = 1; i < operands.size(); i++)
			result = kind == AuthorizationPredicateKind.AND ? AuthorizationPredicate.and(result, operands.get(i))
