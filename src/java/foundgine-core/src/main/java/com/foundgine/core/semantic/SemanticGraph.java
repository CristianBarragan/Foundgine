...
public final class SemanticGraph {
    ...
    public SemanticGraph withAuthorization(int nodeId, AuthorizationPredicate authorization) {
        ...
        return new SemanticGraph(
                nodes.stream()
                        .map(n -> n.id() == nodeId ? n.withAuthorization(authorization) : n)
                        .toList(),
                options);
    }

    public SemanticGraphNode addRoot(
            EntityId entityId, Collection<FieldId> fields, AuthorizationPredicate authorization) {
    ...
    public SemanticGraphNode addConnection(
            EntityId entityId, ConnectionId connectionId, SemanticGraphNode parent) {
    ...
    private SemanticGraphNode add(
        ...
        if (relationshipId != null && connectionId != null)
            throw new IllegalArgumentException(
                    "A semantic node cannot be reached through both a relationship and a"
                            + " connection.");
        if (parent == null && connectionId != null)
            throw new IllegalArgumentException(
                    "A root semantic node cannot be reached through a connection.");
        var node =
                new SemanticGraphNode(
                        nodes.size(),
                        entityId,
                        relationshipId,
                        connectionId,
    ...
    public record SemanticGraphNode(
            ...
            AuthorizationPredicate authorization,
            List<FieldId> fields,
            Map<String, String> semanticAnnotations,
            SemanticTraversalOrigin traversalOrigin,
            SemanticIntentOrigin intentOrigin,
            SemanticExpectedCardinality expectedCardinality,
            boolean nullablePath,
        ...
        public SemanticGraphNode {
            ...
            expectedCardinality =
                    expectedCardinality == null
                            ? SemanticExpectedCardinality.UNKNOWN
                            : expectedCardinality;
            semanticConstraints =
                    semanticConstraints == null ? List.of() : List.copyOf(semanticConstraints);
        }
