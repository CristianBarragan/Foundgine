...
/**
 * Produces a canonical semantic identity used to determine whether two plans have the same
 * provider-neutral meaning. Unlike the execution fingerprint, this representation normalizes only
 * transformations defined as semantically equivalent.
 */
public final class SemanticEquivalenceFingerprint {
    ...
    private static void appendNode(StringBuilder b, SemanticPlanNode node) {
        b.append("node(")
                .append(node.id())
                .append('|')
                .append((byte) node.operation().ordinal())
                .append('|')
                .append(node.entityId().value())
                .append('|')
                .append(node.viaRelationship() != null ? node.viaRelationship().value() : "-")
                .append('|')
                .append(node.viaConnection() != null ? node.viaConnection().value() : "-")
                .append(')');
        b.append("fields[");
        Set<Long> seen = new HashSet<>();
        for (var f : node.fields()) if (seen.add(f.value())) b.append(f.value()).append(',');
        b.append("]|");
    ...
    private static void appendQueryOptions(StringBuilder b, SemanticQueryOptions options) {
        ...
        b.append("query[")
                .append(options.limit() != null ? options.limit() : "-")
                .append('|')
                .append(options.offset() != null ? options.offset() : "-")
                .append('|');
        appendValue(b, options.after());
        b.append("|order[");
        for (var term : options.effectiveOrder()) {
            b.append(term.field().value())
                    .append(':')
                    .append((byte) term.direction().ordinal())
                    .append(':')
                    .append((byte) term.aggregate().ordinal())
                    .append(':');
            for (var r : term.effectivePath()) b.append(r.value()).append('.');
            b.append(',');
        }
        b.append("]|");
    ...
    private static void appendFilter(StringBuilder b, SemanticFilterExpression filter) {
        ...
        if (filter instanceof SemanticFieldFilter f) {
            b.append("field(")
                    .append(f.field().value())
                    .append('|')
                    .append((byte) f.operator().ordinal())
                    .append('|');
        ...
        if (filter instanceof SemanticRelationshipFilter r) {
            b.append("relationship(")
                    .append(r.relationship().value())
                    .append('|')
                    .append((byte) r.quantifier().ordinal())
                    .append('|');
        ...
        if (filter instanceof SemanticAggregateFilter a) {
            b.append("aggregate(")
                    .append(a.relationship().value())
                    .append('|')
                    .append((byte) a.aggregate().ordinal())
                    .append('|')
                    .append(a.field() != null ? a.field().value() : "-")
                    .append('|')
                    .append((byte) a.operator().ordinal())
                    .append('|');
        ...
        throw new UnsupportedOperationException(
                "Cannot establish semantic equivalence for filter '"
                        + filter.getClass().getSimpleName()
                        + "'.");
    }

    private static void appendCanonicalBooleanFilter(
            StringBuilder b, SemanticFilterExpression filter) {
        ...
        if (terms == null) {
            ...
            else throw new IllegalStateException("Expected boolean filter.");
            return;
        }
        b.append("dnf[");
        ...
        for (var term : terms) {
            var keys =
                    term.stream()
                            .map(SemanticEquivalenceFingerprint::createFilterKey)
                            .sorted()
                            .toList();
            normalized.add(keys);
        }
    ...
    private static SemanticFilterExpression canonicalizeAggregateRelationshipPushdown(
        ...
        if (filter instanceof SemanticAndFilter a) {
            ...
            for (int i = 0; i < raw.size(); i++) {
                if (!(raw.get(i) instanceof SemanticAggregateFilter aggregate)
                        ...
                        || !isCountExistenceComparison(aggregate)) continue;
                for (int j = 0; j < raw.size(); j++) {
                    if (i == j
                            || !(raw.get(j) instanceof SemanticRelationshipFilter r)
                            || r.quantifier() != SemanticRelationshipQuantifier.SOME
                            || !r.relationship().equals(aggregate.relationship())) continue;
                    var pushed =
                    ...
                    for (int k = 0; k < raw.size(); k++) {
                        ...
                        else if (k != j) remaining.add(raw.get(k));
                    }
                    SemanticFilterExpression merged =
                            remaining.size() == 1
                                    ? remaining.get(0)
            ...
            var expressions =
                    a.expressions().stream()
                            .map(
                                    SemanticEquivalenceFingerprint
                                            ::canonicalizeAggregateRelationshipPushdown)
                            .toList();
        ...
        if (filter instanceof SemanticAggregateFilter a
                && a.aggregate() == SemanticFilterAggregate.COUNT
                && a.field() == null
                && a.predicate() != null) {
            ...
            return new SemanticAggregateFilter(
                    a.relationship(),
                    a.aggregate(),
                    a.field(),
                    a.operator(),
        ...
        if (filter instanceof SemanticRelationshipFilter r) {
            ...
            return p == r.predicate()
                    ? r
                    : new SemanticRelationshipFilter(r.relationship(), r.quantifier(), p);
        }
        return filter;
    }
    ...
    private static List<List<SemanticFilterExpression>> toDnf(SemanticFilterExpression e, int max) {
        if (e instanceof SemanticAndFilter a) {
            ...
            for (var child : a.expressions()) {
                ...
                if (childTerms == null) return null;
        ...
        if (e instanceof SemanticOrFilter o) {
            ...
            for (var child : o.expressions()) {
                ...
                result.addAll(t);
                if (result.size() > max) return null;
            }
            return result;
        }
    ...
    private static void appendCommutativeFilter(
            StringBuilder b, String kind, List<SemanticFilterExpression> expressions) {
    ...
    private static void appendAuthorization(StringBuilder b, AuthorizationPredicate p) {
        if (p != null) appendAuthorizationNode(b, canonicalAuthorization(p));
    }
    ...
    private static AuthorizationPredicate canonicalAuthorization(AuthorizationPredicate p) {
        ...
        if (current.kind() == AuthorizationPredicateKind.AND
                || current.kind() == AuthorizationPredicateKind.OR) {
            ...
            for (int i = 1; i < ops.size(); i++)
                result =
                        current.kind() == AuthorizationPredicateKind.AND
    ...
    private static void flattenAuthorization(
            ...
            AuthorizationPredicate n,
            List<AuthorizationPredicate> out) {
        if (n.kind() == kind) {
            if (n.left() != null) flattenAuthorization(kind, n.left(), out);
            if (n.right() != null) flattenAuthorization(kind, n.right(), out);
        } else out.add(n);
    }
    ...
    private static void appendAuthorizationNode(StringBuilder b, AuthorizationPredicate p) {
        ...
        b.append('|');
        if (p.left() != null) appendAuthorizationNode(b, p.left());
        b.append('|');
        if (p.right() != null) appendAuthorizationNode(b, p.right());
        b.append(')');
    }
    ...
    private static void appendValue(StringBuilder b, Object value) {
        ...
        if (value instanceof String s) b.append(s.length()).append(':').append(s);
