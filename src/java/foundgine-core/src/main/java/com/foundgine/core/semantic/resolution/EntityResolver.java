...
/**
 * Provider-independent semantic reference resolver. Resolution never invents a
 * data identity: zero candidates is NotFound and multiple candidates is
 * Ambiguous.
 */
public final class EntityResolver {
	...
	private ICandidateSource requireCandidateSource() {
		if (!(candidates instanceof ICandidateSource source))
			throw new UnsupportedOperationException(
					"The configured candidate source does not support identity resolution.");
		return source;
	}
	...
	public ResolutionResult resolveByIdentity(EntityId entityType, String identityLiteral) {
		...
		var evidence = List.of(new ResolutionEvidence("Looked up " + entity.name() + "." + entity.identity().name()
		...
		if (matches.size() > 1)
			return ResolutionResult.ambiguous(
					matches.size() + " " + entity.name() + " records matched identity '" + identityLiteral + "'.",
					evidence);
	...
	public ResolutionResult resolveByRelationship(ResolvedReference source, String relationshipName) {
		...
		var evidence = List.of(new ResolutionEvidence("Traversed " + sourceEntity.name() + "." + relationship.name()
				+ ": " + matches.size() + " candidate(s)."));
		if (matches.isEmpty())
			return ResolutionResult.notFound(
					"No " + target.name() + " found via " + sourceEntity.name() + "." + relationship.name() + ".",
					evidence);
		if (matches.size() > 1)
			return ResolutionResult.ambiguous(matches.size() + " " + target.name() + " found via " + sourceEntity.name()
					+ "." + relationship.name() + ".", evidence);
		return ResolutionResult.success(new ResolvedReference(relationship.target(), matches.get(0).identityValue(),
				0.9, "Uniquely resolved via " + sourceEntity.name() + "." + relationship.name() + ".", evidence));
	}
	...
	public ResolutionResult resolveBySemanticIdentity(String entityName, String identityLiteral) {
		...
		if (entity == null)
			return ResolutionResult.notFound("No semantic entity named '" + entityName + "' is defined.", List
					.of(new ResolutionEvidence("Semantic entity '" + entityName + "' was not found in the model.")));
	...
	public ResolutionResult resolveByTraversal(ResolvedReference source, String traversalName) {
		...
		for (var relationshipId : traversal.path()) {
			...
			var relationship = entity.relationships().stream().filter(x -> x.id().equals(relationshipId)).findFirst()
					.orElse(null);
			if (relationship == null)
				return ResolutionResult.notFound("Traversal '" + traversalName
						+ "' contains an undeclared relationship '" + relationshipId + "'.", evidence);
		...
		return ResolutionResult.success(new ResolvedReference(current.entityType(), current.identityValue(),
				current.confidence(), current.reason(), evidence));
	}
	...
	public ResolutionResult resolveByTemporalIdentity(EntityId entityType, String identityLiteral,
			OffsetDateTime asOf) {
		if (!(candidates instanceof IAdvancedCandidateSource advanced))
			throw new UnsupportedOperationException(
	...
	private ResolutionResult fuzzyResult(String name, List<String> names) {
		var match = names.stream().map(n -> new NameDistance(n, levenshtein(name, n))).sorted(Comparator
				.comparingInt(NameDistance::distance).thenComparing(NameDistance::name, String.CASE_INSENSITIVE_ORDER))
				.findFirst().orElse(null);
		if (match == null || match.distance() > Math.max(2, name.length() / 3))
			return ResolutionResult.notFound("No close semantic match was found for '" + name + "'.", List.of(
					new ResolutionEvidence("Fuzzy semantic search considered " + names.size() + " declared name(s).")));
		return ResolutionResult.notFound(
				"Semantic name '" + name + "' matched '" + match.name()
						+ "', but fuzzy matching does not invent a record identity.",
	...
	public List<RetrievalCandidate> retrieve(SemanticRetrievalRequest request) {
		...
		return source.retrieve(request).stream().filter(x -> x.entityType().equals(request.entityType()))
				.filter(x -> request.referenceIdentity() == null
						|| !request.referenceIdentity().equals(x.identityValue()))
				.sorted(Comparator.comparingDouble(RetrievalCandidate::score).reversed()
						.thenComparing(RetrievalCandidate::recordId))
	...
	private static ResolutionResult resolveCandidates(EntityId type, String name, List<IdentityCandidate> matches,
		...
		if (matches.isEmpty())
			return ResolutionResult.notFound("No " + name + " matched the supplied " + kind + ".", evidence);
		if (matches.size() > 1)
			return ResolutionResult
					.ambiguous(matches.size() + " " + name + " records matched the supplied " + kind + ".", evidence);
		return ResolutionResult.success(new ResolvedReference(type, matches.get(0).identityValue(), 1.0,
