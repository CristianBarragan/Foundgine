...
/**
 * Provider-independent semantic reference resolver. Resolution never invents a data identity: zero
 * candidates is NotFound and multiple candidates is Ambiguous.
 */
public final class EntityResolver {
    ...
    public ResolutionResult resolveByIdentity(EntityId entityType, String identityLiteral) {
        ...
        return ResolutionResult.success(
                new ResolvedReference(
                        entityType,
                        matches.get(0).identityValue(),
                        1.0,
                        "Explicit identity matched "
                                + entity.name()
                                + "."
                                + entity.identity().name()
                                + ".",
    ...
    public ResolutionResult resolveByRelationship(
            ResolvedReference source, String relationshipName) {
        var sourceEntity = model.get(source.entityType());
        var relationship =
                sourceEntity.relationships().stream()
                        .filter(r -> r.name().equalsIgnoreCase(relationshipName))
                        .findFirst()
                        .orElse(null);
        ...
        var matches =
                requireCandidateSource()
                        .findByRelationship(relationship.id(), source.identityValue());
        ...
        for (var relationshipId : traversal.path()) {
            ...
            if (relationship == null)
                return ResolutionResult.notFound(
                        "Traversal '"
                                + traversalName
                                + "' contains an undeclared relationship '"
                                + relationshipId
                                + "'.",
                        evidence);
    ...
    public ResolutionResult resolveByCompositeKey(
            EntityId entityType, Map<String, String> identityValues) {
        if (!(candidates instanceof IAdvancedCandidateSource advanced))
            throw new UnsupportedOperationException(
                    "The configured candidate source does not support composite identity"
                            + " resolution.");
        var entity = model.get(entityType);
        ...
        var evidence =
                List.of(
                        new ResolutionEvidence(
                                "Looked up composite identity on "
                                        + entity.name()
                                        + ": "
                                        + matches.size()
                                        + " match(es)."));
        return resolveCandidates(
                entityType, entity.name(), matches, evidence, "composite identity");
    }

    public ResolutionResult resolveByTemporalIdentity(
            EntityId entityType, String identityLiteral, OffsetDateTime asOf) {
        if (!(candidates instanceof IAdvancedCandidateSource advanced))
            throw new UnsupportedOperationException(
                    "The configured candidate source does not support temporal identity"
                            + " resolution.");
        var entity = model.get(entityType);
    ...
    public ResolutionResult resolveByFuzzyMatch(String name, EntityId sourceEntity) {
        var names =
                model.get(sourceEntity).relationships().stream()
                        .map(SemanticRelationship::name)
                        .toList();
    ...
    private ResolutionResult fuzzyResult(String name, List<String> names) {
        var match =
                names.stream()
                        .map(n -> new NameDistance(n, levenshtein(name, n)))
                        .sorted(
                                Comparator.comparingInt(NameDistance::distance)
                                        .thenComparing(
                                                NameDistance::name, String.CASE_INSENSITIVE_ORDER))
                        .findFirst()
                        .orElse(null);
        if (match == null || match.distance() > Math.max(2, name.length() / 3))
            return ResolutionResult.notFound(
                    "No close semantic match was found for '" + name + "'.",
                    List.of(
                            new ResolutionEvidence(
                                    "Fuzzy semantic search considered "
                                            + names.size()
                                            + " declared name(s).")));
        return ResolutionResult.notFound(
                "Semantic name '"
                ...
                List.of(
                        new ResolutionEvidence(
                                "Closest semantic match: '"
                                        + match.name()
                                        + "'. The caller must explicitly select the resulting"
                                        + " semantic entity or relationship before data"
                                        + " resolution.")));
    }
    ...
    public List<RetrievalCandidate> retrieve(SemanticRetrievalRequest request) {
        ...
        if (!(candidates instanceof IApproximateCandidateSource source))
            throw new UnsupportedOperationException(
                    "The configured candidate source does not support "
                            + request.strategy()
                            + " retrieval.");
        return source.retrieve(request).stream()
                ...
                .filter(
                        x ->
                                request.referenceIdentity() == null
                                        || !request.referenceIdentity().equals(x.identityValue()))
                .sorted(
                        Comparator.comparingDouble(RetrievalCandidate::score)
                                .reversed()
    ...
    private static ResolutionResult resolveCandidates(
            EntityId type,
            String name,
            List<IdentityCandidate> matches,
            List<ResolutionEvidence> evidence,
            String kind) {
        if (matches.isEmpty())
            return ResolutionResult.notFound(
                    "No " + name + " matched the supplied " + kind + ".", evidence);
        if (matches.size() > 1)
            return ResolutionResult.ambiguous(
                    matches.size() + " " + name + " records matched the supplied " + kind + ".",
                    evidence);
        return ResolutionResult.success(
                new ResolvedReference(
                        type,
                        ...
                        Math.min(
                                ...
                                previous[j - 1]
                                        + (Character.toUpperCase(a.charAt(i - 1))
                                                        == Character.toUpperCase(b.charAt(j - 1))
                                                ? 0
                                                : 1));
            previous = current;
        }
