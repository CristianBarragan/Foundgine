...
/**
 * Port of {@code Foundgine.Core.Execution.Mutation.ExecutionMutationIR} and its companion {@code
 * ExecutionMutationIRCompiler}.
 *
 * <p>Canonical provider-neutral execution representation for mutation work.
 *
 * <p>Mutation semantics are resolved before this boundary. This representation contains the
 * concrete provider-neutral work required to execute a mutation batch, including dependency edges,
 * but contains no SQL or provider-specific plan types.
 *
 * <p>C#'s {@code RequiredSecurityInvariants} is an {@code init}-only property defaulting to an
 * empty array; ported here as a third canonical-constructor component (defaulted to empty by the
 * two-argument constructor and by {@link #from(MutationBatchPlan)}), mirroring every construction
 * path this codebase actually exercises.
 */
public record ExecutionMutationIR(
    ...
    public ExecutionMutationIR(
            List<MutationOperation> operations, List<MutationDependency> dependencies) {
    ...
    public static ExecutionMutationIR from(
            MutationBatchPlan plan, List<String> requiredSecurityInvariants) {
        ...
        List<String> normalized =
                requiredSecurityInvariants == null
                        ? List.of()
                        : new LinkedHashSet<>(requiredSecurityInvariants)
    ...
    /**
     * Materializes the canonical provider-neutral mutation batch consumed by existing mutation
     * compilers. No correlation-specific representation is introduced here.
     */
    ...
    /**
     * Derives dependency edges from field-level value references. This is a validation/consistency
     * operation; {@link #dependencies()} remains the canonical execution graph input.
     */
        ...
        for (int targetIndex = 0; targetIndex < operations.size(); targetIndex++) {
            for (var field : operations.get(targetIndex).fields()) {
                ...
                if (source.sourceOperationIndex() < 0
                        || source.sourceOperationIndex() >= operations.size()) {
                    throw new IllegalStateException(
    ...
    /**
     * Ensures canonical dependency metadata agrees with dependency edges derivable from field-level
     * value references.
     */
    public void validateDerivedDependencies() {
        Comparator<MutationDependency> order =
                Comparator.comparingInt(MutationDependency::sourceOperationIndex)
    ...
    private static void validateDependencies(
            int operationCount, List<MutationDependency> dependencies) {
    ...
    /**
     * Port of {@code Foundgine.Core.Execution.Mutation.ExecutionMutationIRCompiler}: explicit
     * lowering from the provider-neutral mutation planning artifact to the canonical execution
     * representation.
     */
