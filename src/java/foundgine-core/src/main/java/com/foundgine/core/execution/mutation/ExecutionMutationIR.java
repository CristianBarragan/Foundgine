 ...
 *
 * <p>
 * C#'s {@code RequiredSecurityInvariants} is an {@code init}-only property
 ...
 */
public record ExecutionMutationIR(List<MutationOperation> operations, List<MutationDependency> dependencies,
	...
	public static ExecutionMutationIR from(MutationBatchPlan plan, List<String> requiredSecurityInvariants) {
		...
		List<String> normalized = requiredSecurityInvariants == null ? List.of()
		...
		for (int targetIndex = 0; targetIndex < operations.size(); targetIndex++) {
			for (var field : operations.get(targetIndex).fields()) {
				...
				if (source.sourceOperationIndex() < 0 || source.sourceOperationIndex() >= operations.size()) {
					throw new IllegalStateException("Mutation value reference points to invalid source operation "
							+ source.sourceOperationIndex() + ".");
				}

				if (source.sourceOperationIndex() >= targetIndex) {
					throw new IllegalStateException("Mutation value reference must point from an earlier operation: "
							+ source.sourceOperationIndex() + " -> " + targetIndex + ".");
				}

				result.add(new MutationDependency(source.sourceOperationIndex(), targetIndex, source.sourceField(),
						field.column()));
			}
		}
	...
	/**
	 * Ensures canonical dependency metadata agrees with dependency edges derivable
	 * from field-level value references.
	 */
	public void validateDerivedDependencies() {
		Comparator<MutationDependency> order = Comparator.comparingInt(MutationDependency::sourceOperationIndex)
				.thenComparingInt(MutationDependency::targetOperationIndex)
				.thenComparingLong(d -> d.sourceField().value()).thenComparingLong(d -> d.targetColumn().value());
	...
	private static void validateDependencies(int operationCount, List<MutationDependency> dependencies) {
		for (MutationDependency dependency : dependencies) {
			if (dependency.sourceOperationIndex() < 0 || dependency.sourceOperationIndex() >= operationCount
				...
				throw new IllegalStateException("Mutation dependency indexes are outside the execution IR: "
			...
			if (dependency.sourceOperationIndex() >= dependency.targetOperationIndex()) {
				throw new IllegalStateException("Mutation dependency must point from an earlier operation: "
						+ dependency.sourceOperationIndex() + " -> " + dependency.targetOperationIndex() + ".");
			}
		}
	}
	...
	/**
	 * Port of
	 * {@code Foundgine.Core.Execution.Mutation.ExecutionMutationIRCompiler}:
	 * explicit lowering from the provider-neutral mutation planning artifact to the
