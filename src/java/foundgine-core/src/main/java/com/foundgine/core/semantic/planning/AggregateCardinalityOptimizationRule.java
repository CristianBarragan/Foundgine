...
/**
 * Adds a physical short-circuit hint for COUNT predicates whose truth value depends only on whether
 * a collection is empty. The semantic filter remains unchanged, so COUNT semantics, authorization,
 * relationship visibility and null/empty behavior stay defined by the semantic layer.
 */
public final class AggregateCardinalityOptimizationRule implements IPlanRewriteRule {
    ...
    private static SemanticPlanNode rewrite(SemanticPlanNode node, boolean[] changed) {
        AggregateExecutionStrategy candidate =
                getStrategy(node.queryOptions() == null ? null : node.queryOptions().filter());
        AggregateExecutionStrategy strategy =
                candidate == null ? AggregateExecutionStrategy.DEFAULT : candidate;
    ...
    private static AggregateExecutionStrategy getStrategy(SemanticFilterExpression filter) {
        ...
        for (SemanticAggregateFilter aggregate : aggregates) {
            if (aggregate.aggregate() != SemanticFilterAggregate.COUNT
                    ...
                    || aggregate.predicate() != null) return null;
        }
        AggregateExecutionStrategy result = null;
        for (SemanticAggregateFilter aggregate : aggregates) {
            AggregateExecutionStrategy strategy =
                    AggregateExecutionStrategyResolver.resolve(
    ...
    private static void collectAggregates(
            SemanticFilterExpression filter, Collection<SemanticAggregateFilter> result) {
