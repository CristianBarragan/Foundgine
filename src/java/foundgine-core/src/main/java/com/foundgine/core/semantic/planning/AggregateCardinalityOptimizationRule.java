...
public final class AggregateCardinalityOptimizationRule implements IPlanRewriteRule {
    @Override public String name() { return "aggregate.cardinality.short-circuit"; }
    ...
    @Override public List<String> securityObligations() { return List.of(
            "authorization.required", "authorization.runtime", "visibility.relationship",
            "planning.cache-context-isolation"); }
    @Override public double costImpact() { return 0.5d; }
    @Override public double benefitEstimate() { return 3.0d; }
    @Override public boolean isIdempotent() { return true; }
    @Override public int priority() { return 40; }

    @Override public boolean canApply(SemanticPlan plan) {
    ...
    private static SemanticPlanNode rewrite(SemanticPlanNode node, boolean[] changed) {
        AggregateExecutionStrategy candidate = getStrategy(node.queryOptions() == null ? null : node.queryOptions().filter());
        AggregateExecutionStrategy strategy = candidate == null ? AggregateExecutionStrategy.DEFAULT : candidate;
        if (strategy != node.aggregateExecutionStrategy()) changed[0] = true;
        ...
        if (childrenChanged) changed[0] = true;
        ...
        if (strategy != node.aggregateExecutionStrategy() || childrenChanged) {
            return new SemanticPlanNode(node.id(), node.operation(), node.entityId(), node.fields(),
                    node.viaRelationship(), node.viaConnection(), children,
                    node.queryOptions(), node.authorization(), node.relationshipCardinality(),
                    node.traversalMode(), node.traversalOrder(), strategy);
        }
        return node;
    }

    private static boolean requiresRewrite(SemanticPlanNode node) {
        AggregateExecutionStrategy desired = getStrategy(node.queryOptions() == null ? null : node.queryOptions().filter());
        if (desired == null) desired = AggregateExecutionStrategy.DEFAULT;
        return desired != node.aggregateExecutionStrategy() || node.children().stream().anyMatch(AggregateCardinalityOptimizationRule::requiresRewrite);
    }
    ...
    private static AggregateExecutionStrategy getStrategy(SemanticFilterExpression filter) {
        ...
        for (SemanticAggregateFilter aggregate : aggregates) {
            if (aggregate.aggregate() != SemanticFilterAggregate.COUNT || aggregate.field() != null || aggregate.predicate() != null)
                return null;
        }
        AggregateExecutionStrategy result = null;
        for (SemanticAggregateFilter aggregate : aggregates) {
            AggregateExecutionStrategy strategy = AggregateExecutionStrategyResolver.resolve(aggregate.operator(), aggregate.value());
            if (strategy == null) return null;
            if (result == null) result = strategy;
            else if (result != strategy) return null;
        }
        return result;
    }
