 ...
 *
 * <p>
 * This is provider-neutral logical pushdown. It does not move predicates across
 * relationship boundaries, authorization boundaries, pagination, or
...
public final class PredicatePushdownRule implements IPlanRewriteRule {
	...
	public List<String> preconditions() {
		return List.of("plan contains a query filter", "filter contains AND with OR operand",
	...
	private static SemanticPlanNode rewriteNode(SemanticPlanNode node, boolean[] changed) {
		...
		if (childChanged)
			changed[0] = true;
		return (options != node.queryOptions() || childChanged) ? new SemanticPlanNode(node.id(), node.operation(),
				node.entityId(), node.fields(), node.viaRelationship(), node.viaConnection(), children, options,
				node.authorization(), node.relationshipCardinality(), node.traversalMode(), node.traversalOrder(),
				node.aggregateExecutionStrategy()) : node;
	}
	...
	private static SemanticFilterExpression pushOnce(SemanticFilterExpression filter) {
		if (filter instanceof SemanticAndFilter and) {
			...
			if (or.expressions().isEmpty() || or.expressions().size() * Math.max(1, other.size()) > MAX_EXPANSION_TERMS)
				return filter;
