...
/**
 * Pushes a conjunctive predicate into an OR predicate using the distributive law: (A OR B) AND C
 * becomes (A AND C) OR (B AND C).
 *
 * <p>This is provider-neutral logical pushdown. It does not move predicates across relationship
 * boundaries, authorization boundaries, pagination, or cardinality-changing operations.
 */
public final class PredicatePushdownRule implements IPlanRewriteRule {
    ...
    @Override
    public String name() {
        return "predicate.pushdown.disjunction";
    }

    @Override
    public List<String> preconditions() {
        return List.of(
                "plan contains a query filter",
                "filter contains AND with OR operand",
                "rewrite expansion remains bounded");
    }

    @Override
    public List<String> securityObligations() {
        return List.of("authorization.required", "visibility.field", "visibility.relationship");
    }
    ...
    private static SemanticFilterExpression pushOnce(SemanticFilterExpression filter) {
        if (filter instanceof SemanticAndFilter and) {
            ...
            for (int i = 0; i < expressions.size(); i++)
                if (expressions.get(i) instanceof SemanticOrFilter) {
                    orIndex = i;
                    break;
                }
            if (orIndex < 0) return filter;
            var or = (SemanticOrFilter) expressions.get(orIndex);
            var other = new ArrayList<>(expressions);
            other.remove(orIndex);
            if (or.expressions().isEmpty()
                    || or.expressions().size() * Math.max(1, other.size()) > MAX_EXPANSION_TERMS)
                return filter;
            List<SemanticFilterExpression> distributed = new ArrayList<>(or.expressions().size());
            for (var branch : or.expressions()) {
                List<SemanticFilterExpression> terms = new ArrayList<>();
                terms.add(branch);
                terms.addAll(other);
        ...
        if (filter instanceof SemanticOrFilter or)
            return new SemanticOrFilter(
                    or.expressions().stream().map(PredicatePushdownRule::pushOnce).toList());
        if (filter instanceof SemanticRelationshipFilter relationship) {
            ...
            return predicate == relationship.predicate()
                    ...
                    : new SemanticRelationshipFilter(
                            relationship.relationship(), relationship.quantifier(), predicate);
        }
        return filter;
    }

    private static boolean containsPushableFilter(SemanticPlanNode node) {
        return (node.queryOptions() != null
                        && node.queryOptions().filter() != null
                        && containsPushable(node.queryOptions().filter()))
    ...
    private static boolean containsPushable(SemanticFilterExpression filter) {
        if (filter instanceof SemanticAndFilter and)
            return and.expressions().stream()
                    .anyMatch(x -> x instanceof SemanticOrFilter || containsPushable(x));
        if (filter instanceof SemanticOrFilter or)
            return or.expressions().stream().anyMatch(PredicatePushdownRule::containsPushable);
        if (filter instanceof SemanticRelationshipFilter relationship)
            return containsPushable(relationship.predicate());
        return false;
    }
}
