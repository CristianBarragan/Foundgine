...
public final class Planner implements IPlanner {
	...
	public SemanticPlan plan(SemanticOperation operation) {
		...
		return new SemanticPlan(build(operation.root(), new HashSet<>(), true));
	}
	...
	private SemanticPlanNode build(SemanticReadNode n, Set<Integer> visited, boolean root) {
		...
		var op = root ? ExecutionOperation.SCAN
				: (n.viaConnection() != null ? ExecutionOperation.TRAVERSE_CONNECTION : ExecutionOperation.TRAVERSE);
		var children = n.children().stream().map(c -> build(c, visited, false)).toList();
		return new SemanticPlanNode(n.id(), op, n.entityId(), n.fields(), n.viaRelationship(), n.viaConnection(),
				children, root ? n.queryOptions() : null, n.authorization(), null, RelationshipTraversalMode.DEFAULT,
				-1, AggregateExecutionStrategy.DEFAULT);
	}
}
