 ...
 *
 * <p>The C# type is a positional record with 9 canonical constructor
 * parameters plus 5 additional {@code init}-only properties that default
 * ({@code Operation = "read"}, {@code HasSideEffects = false},
 * {@code IsIdempotent = false}, {@code Version = SemanticVersionSet.CurrentCapabilityVersion},
 * {@code RequiredSecurityInvariants = []}) when not set via object-initializer
 * syntax at the call site. Java records have a single canonical constructor,
 * so all 14 values are record components here; {@link #ofDefaults} is the
...
public record SemanticCapability(
     ...
     * defaults for the rest ({@code Operation = "read"}, {@code HasSideEffects = false},
     * {@code IsIdempotent = false}, {@code Version = SemanticVersionSet.CurrentCapabilityVersion},
    ...
    public static SemanticCapability ofDefaults(
        ...
        return new SemanticCapability(
                ...
                "read", false, false, SemanticVersionSet.CURRENT_CAPABILITY_VERSION, List.of());
    }

    /**
     * Port of the C# computed property {@code EffectiveSecurityInvariants}.
     *
     * <p>Returns the canonical invariant set when callers did not explicitly
     * supply one.
     */
