...
public final class SemanticAuthorizationCapabilityDiscovery {
    ...
    public static SemanticAuthorizationCapabilities describe(
        ...
        return new SemanticAuthorizationCapabilities(
                model.entities().stream()
                        .sorted(
                                Comparator.comparing(
                                        SemanticEntity::name, String.CASE_INSENSITIVE_ORDER))
                        .map(e -> describeEntity(model, policy, e))
                        .toList());
    }

    private static SemanticAuthorizationCapability describeEntity(
            SemanticModel m, ISemanticAuthorizationPolicy p, SemanticEntity e) {
        var read =
                effective(
                        p.getEntityAccess(e.id(), AuthorizationOperation.READ),
                        predicateDecision(p.getPredicate(e.id(), AuthorizationOperation.READ)));
        var write =
                effective(
                        p.getEntityAccess(e.id(), AuthorizationOperation.WRITE),
                        predicateDecision(p.getPredicate(e.id(), AuthorizationOperation.WRITE)));
        var fields =
                e.fields().stream()
                        .sorted(
                                Comparator.comparing(
                                        SemanticField::name, String.CASE_INSENSITIVE_ORDER))
                        .map(
                                f ->
                                        new SemanticFieldAuthorizationCapability(
                                                f.id(),
                                                f.name(),
                                                describeDecision(
                                                        effective(
        ...
        var rels =
                e.relationships().stream()
                        .sorted(
                                Comparator.comparing(
                                        SemanticRelationship::name, String.CASE_INSENSITIVE_ORDER))
                        .map(
                                r -> {
        ...
        return new SemanticAuthorizationCapability(
                e.id(), e.name(), describeDecision(read), describeDecision(write), fields, rels);
    }
