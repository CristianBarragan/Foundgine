...
public record RuleSelectionPolicy(
        boolean preferHigherBenefit, boolean penalizeRewriteCost, double minimumScore) {
    public RuleSelectionPolicy() {
        this(true, true, Double.NEGATIVE_INFINITY);
    }

    public RuleSelectionPolicy validate() {
        if (Double.isNaN(minimumScore) || Double.isInfinite(minimumScore) && minimumScore > 0)
            throw new IllegalArgumentException("Minimum score is invalid.");
        return this;
    }
    ...
    public RewriteScore score(IPlanRewriteRule rule) {
        ...
        if (!penalizeRewriteCost)
            return new RewriteScore(
                    preferHigherBenefit ? benefit.estimatedBenefit() : -benefit.estimatedBenefit());
