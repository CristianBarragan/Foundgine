 ...
 *
 * <p>Certifies that a provider can execute a given {@link ExecutionMutationIR} while preserving
 * every security invariant it requires, and re-checks that certificate at the exact point of
 * execution.
 */
public final class MutationExecutionSecurityGate {

    private static final Set<String> PROVIDER_OWNED_INVARIANTS =
            Set.of(
    ...
    public static MutationExecutionSecurityCertificate certify(
        ...
        if (required.isEmpty())
            throw new IllegalStateException(
                    "Mutation execution requires an explicit non-empty security invariant"
                            + " contract.");

        for (String id : required)
            if (!SecurityInvariantRegistry.contains(id))
                throw new IllegalStateException(
                        "Unknown required security invariant '" + id + "'.");

        Set<String> preserved = new LinkedHashSet<>();
        upstreamPreserved.forEach(preserved::add);

        List<String> providerRequired =
                required.stream().filter(PROVIDER_OWNED_INVARIANTS::contains).toList();

        if (!providerRequired.isEmpty()
                && !(provider instanceof IMutationSecurityConformanceEvaluator)) {
            throw new IllegalStateException(
                    "Mutation provider '"
        ...
        if (provider instanceof IMutationSecurityConformanceEvaluator concrete) {
            ...
            if (!Objects.equals(result.provider(), providerName))
                throw new IllegalStateException(
                        "Mutation provider conformance identity '"
                                + result.provider()
                                + "' does not match '"
                                + providerName
                                + "'.");
        ...
        MutationExecutionSecurityCertificate certificate =
                MutationExecutionSecurityCertificate.create(
    ...
    public static void ensureExecutable(
        ...
        if (!certificate.isBoundTo(ir, provider))
            throw new IllegalStateException(
                    "Mutation execution certificate is not bound to the exact mutation IR and"
                            + " provider instance being executed.");
