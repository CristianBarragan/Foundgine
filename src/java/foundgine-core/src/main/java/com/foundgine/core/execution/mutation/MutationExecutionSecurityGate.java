...
/**
 * Port of
 * {@code Foundgine.Core.Execution.Mutation.MutationExecutionSecurityGate}.
 *
 * <p>
 * Certifies that a provider can execute a given {@link ExecutionMutationIR}
...
public final class MutationExecutionSecurityGate {
	...
	private static final Set<String> PROVIDER_OWNED_INVARIANTS = Set.of(SecurityInvariantIds.PARAMETERIZED_VALUES,
			SecurityInvariantIds.ATOMIC_MUTATION, SecurityInvariantIds.MUTATION_ROW_LOCKING);

	private MutationExecutionSecurityGate() {
	}

	public static MutationExecutionSecurityCertificate certify(ExecutionMutationIR ir, Object provider,
			String providerName, Iterable<String> upstreamPreserved) {
		...
		if (!providerRequired.isEmpty() && !(provider instanceof IMutationSecurityConformanceEvaluator)) {
			throw new IllegalStateException("Mutation provider '" + provider.getClass().getSimpleName()
					+ "' has no concrete security conformance evaluator for: " + String.join(", ", providerRequired)
					+ ".");
		}
		...
		if (provider instanceof IMutationSecurityConformanceEvaluator concrete) {
			...
			if (!Objects.equals(result.provider(), providerName))
				throw new IllegalStateException("Mutation provider conformance identity '" + result.provider()
						+ "' does not match '" + providerName + "'.");

			result.ensureSatisfied(providerRequired);
			preserved.addAll(result.satisfied());
		}

		MutationExecutionSecurityCertificate certificate = MutationExecutionSecurityCertificate.create(ir, provider,
				providerName, required, preserved);
		certificate.ensureSatisfied();
		return certificate;
	}

	public static void ensureExecutable(ExecutionMutationIR ir, Object provider,
