...
/**
 * Fail-closed proof for collapsing predicate-bearing COUNT comparisons into relationship
 * quantifiers.
 */
public record AggregateExistenceCollapseProof(
    ...
    public static AggregateExistenceCollapseProof create(
        ...
        if (!predicateShapePreserved)
            throw new IllegalStateException(
                    "Existence collapse rejected because the COUNT predicate shape was not"
                            + " preserved.");
        if (!providerCapability.supportsRelationshipQuantifiers())
            throw new IllegalStateException(
                    "Provider '"
                            + providerCapability.providerName()
        ...
        if (!authorization.isSatisfied())
            throw new IllegalStateException(
                    "Existence collapse rejected because the source plan's security contract was"
                            + " not preserved: "
