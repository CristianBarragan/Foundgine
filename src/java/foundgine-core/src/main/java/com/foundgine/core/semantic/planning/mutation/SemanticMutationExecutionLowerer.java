...
/**
 * Port of
 * {@code Foundgine.Core.Semantic.Planning.Mutation.SemanticMutationExecutionLowerer}.
 *
 * <p>
 * Lowers a semantic mutation plan into provider-neutral execution work. This is
 * the first boundary where semantic {@code FieldId} values are resolved to
 * physical {@code ColumnId} values. Provider-specific SQL remains outside this
 * type.
 *
 * <p>
 * C# declares this type in the {@code Execution/Mutation} source file but under
 * the {@code Foundgine.Core.Semantic.Planning.Mutation} namespace; this port
 * follows the namespace (matching this codebase's package-mirrors- namespace
 * convention), not the C# file's location on disk.
 */
public final class SemanticMutationExecutionLowerer {
	...
	public ExecutionMutationIR lower(SemanticMutationPlan plan) {
		...
		for (SemanticMutationOperationPlan operation : plan.operations()) {
			...
			for (var field : operation.fields()) {
				...
				MutationValueReference source = field.source() == null ? null
						: new MutationValueReference(field.source().sourceOperationIndex(),
			...
			if (operation.kind() == SemanticMutationKind.UPSERT) {
				...
				for (var field : operation.conflictFields()) {
					...
					if (column == null)
						throw new IllegalStateException("Semantic conflict field '" + field.value()
								+ "' is not mapped on '" + entity.name() + "'.");
					mapped.add(column);
				}
			...
			operations.add(new MutationOperation(entity, kind, fields, operation.filter(), conflicts,
		...
		for (SemanticMutationDependencyPlan dependency : plan.dependencies()) {
			...
			if (source < 0 || source >= operations.size() || target < 0 || target >= operations.size()
					|| source >= target)
				throw new IllegalStateException(
			...
			if (targetColumn == null)
				throw new IllegalStateException("Semantic dependency target field '" + dependency.targetField().value()
						+ "' is not writable on '" + targetEntity.name() + "'.");
	...
	private static void validate(SemanticMutationOperationPlan operation, MutationEntitySchema entity) {
		...
		if (operation.kind() != SemanticMutationKind.DELETE && operation.fields().isEmpty())
			throw new IllegalStateException(operation.kind() + " mutations must contain at least one field value.");
