 ...
 *
 * <p>Lowers a semantic mutation plan into provider-neutral execution work. This is the first
 * boundary where semantic {@code FieldId} values are resolved to physical {@code ColumnId} values.
 * Provider-specific SQL remains outside this type.
 *
 * <p>C# declares this type in the {@code Execution/Mutation} source file but under the {@code
 * Foundgine.Core.Semantic.Planning.Mutation} namespace; this port follows the namespace (matching
 * this codebase's package-mirrors- namespace convention), not the C# file's location on disk.
 */
    ...
    public ExecutionMutationIR lower(SemanticMutationPlan plan) {
            ...
            if (operation.kind() == SemanticMutationKind.UPSERT) {
                ...
                if (mapped.isEmpty())
                    throw new IllegalStateException(
                            "Upsert for '"
                                    + entity.name()
                                    + "' requires semantic conflict fields.");
        ...
        for (SemanticMutationDependencyPlan dependency : plan.dependencies()) {
            ...
            try {
                ...
                throw new IllegalStateException(
                        "Semantic mutation operation IDs must be stable numeric ordinals for"
                                + " execution lowering.");
            }
            ...
            if (targetColumn == null)
                throw new IllegalStateException(
                        "Semantic dependency target field '"
                                + dependency.targetField().value()
                                + "' is not writable on '"
                                + targetEntity.name()
                                + "'.");

            dependencies.add(
                    new MutationDependency(source, target, dependency.sourceField(), targetColumn));
        }

        return ExecutionMutationIR.from(
                new MutationBatchPlan(operations, dependencies), plan.requiredSecurityInvariants());
    }

    private static void validate(
            SemanticMutationOperationPlan operation, MutationEntitySchema entity) {
        if ((operation.kind() == SemanticMutationKind.UPDATE
                        || operation.kind() == SemanticMutationKind.DELETE)
