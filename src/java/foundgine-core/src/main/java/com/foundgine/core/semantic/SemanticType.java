...
public sealed interface SemanticType
        permits SemanticType.Scalar,
                SemanticType.EnumType,
                SemanticType.ObjectType,
                SemanticType.CollectionType {
    ...
    static SemanticType fromClass(Class<?> type) {
        ...
        if (type == byte.class
                ...
                || type == Short.class
                || type == int.class
                || type == Integer.class) return new Scalar(SemanticScalarKind.INT32);
        if (type == long.class || type == Long.class) return new Scalar(SemanticScalarKind.INT64);
        if (type == float.class
                || type == Float.class
                || type == double.class
                || type == Double.class
                || type == BigDecimal.class) return new Scalar(SemanticScalarKind.DECIMAL);
