...
public final class SemanticAuthorizer {
 ...
 public SemanticGraph authorize(SemanticGraph graph){
  ...
  for(var source:graph.nodes()){
   ...
   if(source.authorization()!=null)authorization=AuthorizationDecision.combine(authorization,predicateDecision(source.authorization()));
 ...
 private SemanticReadNode authorizeNode(SemanticReadNode n,boolean root){
  ...
  if(n.authorization()!=null)effective=AuthorizationDecision.combine(effective,predicateDecision(n.authorization()));if(!effective.isAllowed()){if(root)throw new SemanticAuthorizationException("Authorization constraints denied semantic node '"+n.id()+"'.");return null;}
