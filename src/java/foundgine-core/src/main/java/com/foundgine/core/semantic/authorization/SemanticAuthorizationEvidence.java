...
public record SemanticAuthorizationEvidence(
        String contractFingerprint,
        String authorizationFingerprint,
        Long authorizationVersion,
        String authorizationAuthorityFingerprint) {
    ...
    public void ensureMatches(SemanticContractSnapshot c) {
        ...
        if (!contractFingerprint.equals(c.contractFingerprint()))
            throw new SemanticAuthorizationException(
                    "Authorization evidence is bound to semantic contract '"
                            + contractFingerprint
                            + "', but the supplied contract is '"
                            + c.contractFingerprint()
                            + "'.");
    }
