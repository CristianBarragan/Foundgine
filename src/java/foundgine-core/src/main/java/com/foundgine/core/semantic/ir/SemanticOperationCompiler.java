...
public final class SemanticOperationCompiler {
    ...
    public static SemanticOperation compile(SemanticGraph graph) {
        ...
        var ids =
                graph.nodes().stream()
                        .map(SemanticGraph.SemanticGraphNode::id)
    ...
    private static SemanticReadNode build(
            SemanticGraph.SemanticGraphNode n,
            List<SemanticGraph.SemanticGraphNode> all,
            Set<Integer> ids,
            Set<Integer> visited,
        ...
        var children =
                all.stream()
                        .filter(x -> Objects.equals(x.parentId(), n.id()))
                        .map(x -> build(x, all, ids, visited, null, false))
                        .toList();
