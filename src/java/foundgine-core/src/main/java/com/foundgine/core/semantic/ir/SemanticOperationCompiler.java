...
public final class SemanticOperationCompiler {
    ...
    public static SemanticOperation compile(SemanticGraph graph) {
        ...
        if (graph.nodes().isEmpty()) throw new IllegalStateException("Cannot compile an empty semantic graph.");
        var roots = graph.nodes().stream().filter(n -> n.parentId() == null).toList();
        if (roots.size() != 1) throw new IllegalStateException("A semantic operation requires exactly one root semantic node.");
        ...
        if (visited.size() != graph.nodes().size()) throw new IllegalStateException("Semantic graph contains unreachable nodes.");
    ...
    private static SemanticReadNode build(SemanticGraph.SemanticGraphNode n, List<SemanticGraph.SemanticGraphNode> all,
        ...
        if (!visited.add(n.id())) throw new IllegalStateException("Semantic graph contains a cycle at node " + n.id());
        if (n.parentId() != null && !ids.contains(n.parentId())) throw new IllegalStateException("Semantic node " + n.id() + " references missing parent node " + n.parentId());
        if (!root && n.viaRelationship() == null && n.viaConnection() == null) throw new IllegalStateException("Non-root semantic node " + n.id() + " must specify a relationship or connection.");
        if (root && (n.viaRelationship() != null || n.viaConnection() != null)) throw new IllegalStateException("Root semantic node " + n.id() + " cannot specify a parent edge.");
