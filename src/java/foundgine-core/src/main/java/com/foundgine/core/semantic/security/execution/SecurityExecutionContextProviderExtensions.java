...
public final class SecurityExecutionContextProviderExtensions {
    ...
    /**
     * Returns the current trusted context or throws when the host has not established one. This
     * intentionally fails closed.
     */
    public static SecurityExecutionContext requireSecurityExecutionContext(
            ISecurityExecutionContextProvider provider,
        ...
        if (operationDescription == null || operationDescription.isBlank())
            throw new IllegalArgumentException(
                    "Value cannot be null or whitespace: operationDescription");
        ...
        if (context == null) {
            throw new SecurityException(
                    transportName
                            ...
                            + " requires a host-supplied SecurityExecutionContext. "
                            + "The "
                            + transportName
                            + " caller cannot supply identity, tenant, audience, or warrant"
                            + " context.");
        }
        return context;
    }
