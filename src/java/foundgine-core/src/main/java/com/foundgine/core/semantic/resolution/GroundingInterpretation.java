...
public record GroundingInterpretation(
    ...
    @Deprecated
    public double confidence() { return interpretationScore; }

    public List<ResolutionEvidence> lexicalEvidence() {
        return steps.stream()
                .flatMap(x -> x.candidate().effectiveEvidence().stream())
                .toList();
    }

    public AliasInterpretationEvidence effectiveAliasEvidence() {
        return aliasEvidence != null
                ? aliasEvidence
                : new AliasInterpretationEvidence(
