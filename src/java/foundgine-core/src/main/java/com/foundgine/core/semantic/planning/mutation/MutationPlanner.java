...
public final class MutationPlanner {
    ...
    public MutationBatchPlan plan(SemanticMutationOperationGraph graph) {
        ...
        for (var op : graph.operations()) operations.add(lower(op));
    ...
    private void validateSemanticOperation(
            SemanticMutationOperation op, MutationEntitySchema entity) {
        if ((op.kind() == SemanticMutationKind.UPDATE || op.kind() == SemanticMutationKind.DELETE)
                && op.filter() == null)
            throw new IllegalStateException(
                    "Unfiltered "
                            + op.kind()
                            + " mutations are not permitted for '"
                            + entity.name()
                            + "'.");
        ...
        if (op.kind() != SemanticMutationKind.DELETE && op.fields().isEmpty())
            throw new IllegalStateException(
                    op.kind() + " mutations must contain at least one field value.");
    ...
    private List<MutationDependency> buildSemanticDependencies(
            SemanticMutationOperationGraph graph, List<MutationOperation> operations) {
        ...
        for (int target = 0; target < operations.size(); target++) {
            ...
            for (var field : semantic.fields())
                if (field.source() != null) {
                    ...
                    if (!operations
                            .get(source.sourceOperationIndex())
                            .returnFields()
                            .contains(source.sourceField()))
                        throw new IllegalStateException(
                                "Semantic mutation operation "
                                        + target
                                        + " references field '"
            ...
            for (var d : semantic.dependencies()) {
                ...
                if (targetColumn == null)
                    throw new IllegalStateException(
                            "Semantic dependency target field '"
                                    + d.targetField().value()
                                    + "' is not writable on '"
                                    + targetOp.entity().name()
                                    + "'.");
                ...
                boolean duplicate =
                        result.stream()
                                .anyMatch(
                                        x ->
                                                x.sourceOperationIndex() == d.sourceOperationIndex()
                                                        && x.targetOperationIndex() == targetIndex
                                                        && x.sourceField().equals(d.sourceField())
    ...
    private static void validateSource(int source, int target, int count) {
        if (source < 0 || source >= count || source >= target)
            throw new IllegalStateException(
                    "Mutation operation "
                            + target
                            + " must reference an earlier operation; source "
                            + source
                            + " is invalid.");
    }
    ...
    public MutationPlan plan(UpsertIntent intent) {
        ...
        List<ColumnId> conflicts =
                intent.conflictColumns() != null
                        ? intent.conflictColumns()
                        : (entity.primaryKeyColumn() == null
                                ? List.<ColumnId>of()
                                : List.of(entity.primaryKeyColumn()));
        if (conflicts.isEmpty())
            throw new IllegalStateException(
                    "Upsert for '"
                            + entity.name()
                            + "' requires conflict columns or a primary key.");
        for (var c : conflicts)
    ...
    public MutationBatchPlan plan(MutationBatchIntent intent) {
        ...
        for (var i : intent.operations()) ops.add(planSingle(i));
    ...
    public MutationBatchPlan plan(List<NestedMutationIntent> batch) {
        ...
        for (var item : batch) {
            ...
            for (var d : p.dependencies())
                deps.add(
                        new MutationDependency(
                                d.sourceOperationIndex() + offset,
                                d.targetOperationIndex() + offset,
                                d.sourceField(),
                                d.targetColumn()));
        }
    ...
    public MutationBatchPlan plan(NestedMutationIntent intent) {
        ...
        var ops = new ArrayList<MutationOperation>();
        for (var i : intents) ops.add(planSingle(i));
        for (var b : bindings) {
            ...
            if (!r.source().equals(parent.entity().id()) || !r.target().equals(child.entity().id()))
                throw new IllegalStateException(
                        "Nested mutation relationship '"
                                + r.name()
                                + "' does not match parent/child entities.");
            ...
            if (!pk.equals(r.sourceColumn()))
                throw new IllegalStateException(
                        "Nested mutation relationship '"
                                + r.name()
                                + "' requires the parent join column to be the parent primary key;"
                                + " found column '"
                                + r.sourceColumn().value()
                                + "'.");
            if (child.fields().stream()
                    .anyMatch(f -> f.column().equals(r.targetColumn()) && f.source() == null))
                throw new IllegalStateException(
                        "Child mutation '"
                                + child.entity().name()
                                + "' explicitly supplies relationship column '"
                                + r.targetColumn().value()
                                + "'. Nested mutation propagation must own that value.");
            var pkField =
                    parent.entity().fields().entrySet().stream()
                            .filter(e -> pk.equals(e.getValue()))
                            ...
                            .orElseThrow(
                                    () ->
                                            new IllegalStateException(
                                                    "Primary key column '"
                                                            + pk.value()
                                                            + "' has no field mapping on '"
                                                            + parent.entity().name()
                                                            + "'."));
            var returns =
                    parent.returnFields() == null ? List.<FieldId>of() : parent.returnFields();
            if (!returns.contains(pkField))
                throw new IllegalStateException(
                        "Parent mutation '"
                                + parent.entity().name()
                                + "' must return its primary key field '"
                                + pkField.value()
                                + "' for nested propagation.");
    ...
    private void flatten(
            NestedMutationIntent node,
            Integer parent,
            MutationRelationshipSchema relationship,
            List<IMutationIntent> intents,
            List<Binding> bindings) {
        int index = intents.size();
        intents.add(node.mutation());
        if (parent != null) bindings.add(new Binding(parent, index, relationship));
        for (var child : node.children()) {
            ...
            if (!r.source().equals(node.mutation().entity())
                ...
                throw new IllegalStateException(
                        "Nested mutation relationship '"
                                + r.name()
                                + "' does not match nested mutation entities.");
            flatten(child.mutation(), index, r, intents, bindings);
        }
    }

    private MutationOperation planSingle(IMutationIntent intent) {
        if (intent instanceof MutationIntent m) return plan(m).operations().getFirst();
        if (intent instanceof UpsertIntent u) return plan(u).operations().getFirst();
        throw new UnsupportedOperationException(
                "Unsupported mutation intent '" + intent.getClass().getName() + "'.");
    }
        ...
        for (int target = 0; target < operations.size(); target++)
            for (var f : operations.get(target).fields())
                if (f.source() != null) {
                    ...
                    d.add(
                            new MutationDependency(
                                    source, target, f.source().sourceField(), f.column()));
                }
        return d;
    }
    ...
    private static void validateDirect(
        ...
        if (kind != MutationKind.DELETE && fields.isEmpty())
            throw new IllegalStateException(
                    kind + " mutations must contain at least one field value.");
