...
/**
 * Assigns a deterministic physical traversal order to eligible sibling
 * relationship nodes.
 */
public final class RelationshipJoinOrderingRule implements IPlanRewriteRule {
	@Override
	public String name() {
		return "relationship.join.order";
	}

	@Override
	public List<String> preconditions() {
		return List.of("plan contains two or more sibling relationship traversals",
	...
	public SemanticPlan apply(SemanticPlan plan) {
		...
		var root = rewrite(plan.root(), changed);
	...
	private static SemanticPlanNode rewrite(SemanticPlanNode node, boolean[] changed) {
		...
		for (int i = 0; i < children.size(); i++) {
			...
			if (child.viaRelationship() != null && child.relationshipCardinality() != null
		...
		if (candidates.size() >= 2) {
			candidates.sort(Comparator.comparingInt((Integer i) -> selectivityClass(children.get(i)))
					.thenComparingInt(
							i -> children.get(i).relationshipCardinality() == RelationshipCardinality.ONE ? 0 : 1)
					.thenComparingLong(i -> children.get(i).viaRelationship().value())
					.thenComparingInt(Integer::intValue));
			for (int rank = 0; rank < candidates.size(); rank++) {
				int index = candidates.get(rank);
				var child = children.get(index);
				if (child.traversalOrder() != rank) {
					children.set(index, copyWithTraversalOrder(child, rank));
					changed[0] = true;
				}
			}
		}
		...
		for (int i = 0; i < children.size(); i++) {
			var before = children.get(i);
			var after = rewrite(before, changed);
			if (after != before) {
	...
	private static int selectivityClass(SemanticPlanNode node) {
		...
		if (filter != null)
			score -= 3;
		if (filter instanceof SemanticRelationshipFilter || filter instanceof SemanticAggregateFilter)
			score -= 1;
		if (node.queryOptions() != null && node.queryOptions().limit() != null && node.queryOptions().limit() > 0)
			score -= 2;
	...
	private static boolean containsEligibleSiblingSet(SemanticPlanNode node) {
		long count = node.children().stream().filter(
				c -> c.viaRelationship() != null && c.relationshipCardinality() != null && c.traversalOrder() < 0)
				.count();
		return count >= 2
				|| node.children().stream().anyMatch(RelationshipJoinOrderingRule::containsEligibleSiblingSet);
	}

	private static SemanticPlanNode copyWithTraversalOrder(SemanticPlanNode n, int order) {
		return new SemanticPlanNode(n.id(), n.operation(), n.entityId(), n.fields(), n.viaRelationship(),
				n.viaConnection(), n.children(), n.queryOptions(), n.authorization(), n.relationshipCardinality(),
				n.traversalMode(), order, n.aggregateExecutionStrategy());
	}

	private static SemanticPlanNode copyWithChildren(SemanticPlanNode n, List<SemanticPlanNode> children) {
		return new SemanticPlanNode(n.id(), n.operation(), n.entityId(), n.fields(), n.viaRelationship(),
				n.viaConnection(), children, n.queryOptions(), n.authorization(), n.relationshipCardinality(),
				n.traversalMode(), n.traversalOrder(), n.aggregateExecutionStrategy());
	}
}
