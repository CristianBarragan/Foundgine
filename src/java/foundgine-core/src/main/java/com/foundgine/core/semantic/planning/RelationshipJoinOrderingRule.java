...
public final class RelationshipJoinOrderingRule implements IPlanRewriteRule {
    ...
    private static SemanticPlanNode rewrite(SemanticPlanNode node, boolean[] changed) {
        ...
        for (int i = 0; i < children.size(); i++) {
            ...
            if (child.viaRelationship() != null
                    && child.relationshipCardinality() != null
                    && child.traversalOrder() < 0) candidates.add(i);
        }
        if (candidates.size() >= 2) {
            candidates.sort(
                    Comparator.comparingInt((Integer i) -> selectivityClass(children.get(i)))
                            .thenComparingInt(
                                    i ->
                                            children.get(i).relationshipCardinality()
        ...
        return childChanged || !children.equals(node.children())
                ? copyWithChildren(node, children)
                : node;
    }
    ...
    private static int selectivityClass(SemanticPlanNode node) {
        ...
        if (filter instanceof SemanticRelationshipFilter
                || filter instanceof SemanticAggregateFilter) score -= 1;
        if (node.queryOptions() != null
                && node.queryOptions().limit() != null
                && node.queryOptions().limit() > 0) score -= 2;
    ...
    private static boolean containsEligibleSiblingSet(SemanticPlanNode node) {
        long count =
                node.children().stream()
                        .filter(
                                c ->
                                        c.viaRelationship() != null
                                                && c.relationshipCardinality() != null
                                                && c.traversalOrder() < 0)
                        .count();
        return count >= 2
                || node.children().stream()
                        .anyMatch(RelationshipJoinOrderingRule::containsEligibleSiblingSet);
    }
    ...
    private static SemanticPlanNode copyWithTraversalOrder(SemanticPlanNode n, int order) {
        return new SemanticPlanNode(
                ...
                n.queryOptions(),
                n.authorization(),
                n.relationshipCardinality(),
                n.traversalMode(),
                order,
                n.aggregateExecutionStrategy());
    }

    private static SemanticPlanNode copyWithChildren(
            SemanticPlanNode n, List<SemanticPlanNode> children) {
        return new SemanticPlanNode(
                n.id(),
                n.operation(),
                n.entityId(),
