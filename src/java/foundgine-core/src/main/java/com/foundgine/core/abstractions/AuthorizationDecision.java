 ...
 *
 * <p>Provider-independent authorization result. Conditional access carries a predicate that must
 * remain part of execution semantics and be evaluated at the provider boundary with the current
 * execution context.
 */
public record AuthorizationDecision(AuthorizationAccess access, AuthorizationPredicate predicate) {
    ...
    public static final AuthorizationDecision DENIED =
            new AuthorizationDecision(AuthorizationAccess.DENIED);
    public static final AuthorizationDecision ALLOWED =
    ...
    public static AuthorizationDecision combine(
            AuthorizationDecision left, AuthorizationDecision right) {
