...
public record PlanOptimizationProof(
        String ruleName,
        boolean semanticMeaningPreserved,
        boolean securityPreserved,
        boolean authorizationBindingPreserved,
        double estimatedBenefit,
        double estimatedRewriteCost) {
    public boolean isSatisfied() {
        return semanticMeaningPreserved
                && securityPreserved
    ...
    public static PlanOptimizationProof create(
        ...
        var proof =
                new PlanOptimizationProof(
                        rule.name(),
                        semantic.isSatisfied(),
                        security.isSatisfied(),
                        authorization.isSatisfied(),
                        Math.max(0, rule.benefitEstimate()),
                        Math.max(0, rule.costImpact()));
        if (!proof.isSatisfied())
            throw new IllegalStateException(
                    "Optimization '"
                            + rule.name()
                            + "' is not admissible because one or more preservation obligations"
                            + " failed.");
        return proof;
    }
}
