...
public final class SemanticExecutionAuthorizationRevalidator implements IExecutionAuthorizationRevalidator {
	@Override
	public void validate(SemanticContractSnapshot contract, SemanticAuthorizationEvidence evidence,
		...
		if (!currentAuthority.allowed())
			throw new SecurityException("The current authorization authority is revoked; execution fails closed.");
		...
		if (evidence.authorizationVersion() != currentAuthority.version())
			throw new IllegalStateException("Authorization evidence version " + evidence.authorizationVersion()
					+ " is no longer current; current authority version is " + currentAuthority.version()
					+ ". Execution fails closed.");
		if (evidence.authorizationAuthorityFingerprint() == null
				|| !evidence.authorizationAuthorityFingerprint().equals(currentAuthority.fingerprint()))
			throw new IllegalStateException(
