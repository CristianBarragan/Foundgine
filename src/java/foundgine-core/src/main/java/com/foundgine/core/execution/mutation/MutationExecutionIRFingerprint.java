...
/**
 * Deterministic fingerprint of an {@link ExecutionMutationIR}, used to bind a {@link
 * MutationExecutionSecurityCertificate} to the exact IR it was issued for.
 *
 * <p>C# builds an anonymous-object tree and hashes its {@code System.Text.Json} serialization
 * (explicitly avoiding serializing {@code MutationEntitySchema} directly, since its {@code Fields}
 * member is keyed by {@code FieldId} and {@code FieldId} is a value object rather than a JSON
 * property-name type). This port instead follows this codebase's established canonical-fingerprint
 * convention (see {@code SemanticModelFingerprint}): a length-prefixed delimited {@link
 * StringBuilder} representation, hashed with SHA-256. This achieves the same goal — a deterministic
 * fingerprint independent of any JSON library's map/dictionary-key behavior — without depending on
 * JSON at all. Operation and dependency order is preserved as given (both are
 * execution-order-significant), while inherently unordered pieces (schema columns, schema fields)
 * are sorted for determinism, matching the original.
 */
final class MutationExecutionIRFingerprint {
    ...
    static String create(ExecutionMutationIR ir) {
        ...
        for (MutationDependency dependency : ir.dependencies()) {
            append(
                    ...
                    dependency.sourceField().value(),
                    dependency.targetColumn().value());
        }

        ir.requiredSecurityInvariants().stream()
                .sorted()
    ...
    private static void appendEntity(StringBuilder canonical, MutationEntitySchema entity) {
        ...
        append(
                canonical,
                "operation-entity-pk",
                primaryKeyColumn == null ? "" : String.valueOf(primaryKeyColumn.value()));
    }
