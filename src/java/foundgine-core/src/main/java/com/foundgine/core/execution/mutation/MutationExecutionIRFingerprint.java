 ...
 *
 * <p>
 * C# builds an anonymous-object tree and hashes its {@code System.Text.Json}
 * serialization (explicitly avoiding serializing {@code MutationEntitySchema}
 * directly, since its {@code Fields} member is keyed by {@code FieldId} and
 * {@code FieldId} is a value object rather than a JSON property-name type).
 * This port instead follows this codebase's established canonical-fingerprint
 * convention (see {@code SemanticModelFingerprint}): a length-prefixed
 * delimited {@link StringBuilder} representation, hashed with SHA-256. This
 * achieves the same goal — a deterministic fingerprint independent of any JSON
...
final class MutationExecutionIRFingerprint {
	...
	static String create(ExecutionMutationIR ir) {
		...
		for (MutationOperation operation : ir.operations()) {
			...
			append(canonical, "operation-filter", operation.filter() == null ? "" : String.valueOf(operation.filter()));
		...
		for (MutationDependency dependency : ir.dependencies()) {
			append(canonical, "dependency", dependency.sourceOperationIndex(), dependency.targetOperationIndex(),
					dependency.sourceField().value(), dependency.targetColumn().value());
		}

		ir.requiredSecurityInvariants().stream().sorted().forEach(id -> append(canonical, "required-invariant", id));
	...
	private static void appendEntity(StringBuilder canonical, MutationEntitySchema entity) {
		...
		entity.fields().entrySet().stream().sorted(Comparator.comparingLong(e -> e.getKey().value())).forEach(e -> {
		...
		append(canonical, "operation-entity-pk",
