 ...
 *
 * <p>
 * The factory method and bind check are package-private ({@link #create} and
 * {@link #isBoundTo}) rather than public, since this certificate is only meant
 * to be constructed and checked from within this package.
 */
public final class MutationExecutionSecurityCertificate {
	...
	private MutationExecutionSecurityCertificate(ExecutionMutationIR boundIr, Object boundProvider, String provider,
			List<String> required, List<String> preserved, List<String> missing) {
	...
	static MutationExecutionSecurityCertificate create(ExecutionMutationIR ir, Object provider, String providerName,
			Iterable<String> required, Iterable<String> preserved) {
		...
		return new MutationExecutionSecurityCertificate(ir, provider, providerName, requiredSet, preservedSet, missing);
	}

	boolean isBoundTo(ExecutionMutationIR ir, Object provider) {
		return boundIr == ir && boundProvider == provider
				&& Objects.equals(irFingerprint, MutationExecutionIRFingerprint.create(ir));
	}

	public void ensureSatisfied() {
		if (!isSatisfied()) {
			throw new IllegalStateException("Mutation provider '" + provider
					+ "' cannot satisfy required security invariants: " + String.join(", ", missing) + ".");
		}
	}
