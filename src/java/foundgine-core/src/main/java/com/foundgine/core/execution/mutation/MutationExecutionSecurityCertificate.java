...
/**
 * In-process execution certificate bound to one exact mutation IR and one exact provider instance.
 * It is deliberately non-serializable/non-transferable.
 *
 * <p>The factory method and bind check are package-private ({@link #create} and {@link #isBoundTo})
 * rather than public, since this certificate is only meant to be constructed and checked from
 * within this package.
 */
public final class MutationExecutionSecurityCertificate {
    ...
    static MutationExecutionSecurityCertificate create(
        ...
        List<String> missing =
                requiredSet.stream().filter(id -> !preservedLookup.contains(id)).sorted().toList();
    ...
    private static List<String> distinctSorted(Iterable<String> values) {
        ...
        for (String value : values) distinct.add(value);
