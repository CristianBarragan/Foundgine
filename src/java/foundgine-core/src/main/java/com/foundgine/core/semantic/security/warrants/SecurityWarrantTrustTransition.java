...
public final class SecurityWarrantTrustTransition {
    private SecurityWarrantTrustTransition() {}
    ...
    public static DelegationIssuerTrustSnapshot validateAndCapture(
        ...
        var s = trust.capture(child.issuer(), child.keyId());
        if (s.keyState() != DelegationIssuerKeyState.ACTIVE)
    ...
    public static void assertUnchanged(
            ...
            ISecurityWarrantDelegationTrustStateResolver trust) {
        if (snapshot == null || trust == null) throw new NullPointerException();
