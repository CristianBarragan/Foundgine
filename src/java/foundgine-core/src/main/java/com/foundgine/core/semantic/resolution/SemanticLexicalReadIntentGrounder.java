...
public final class SemanticLexicalReadIntentGrounder {
    ...
    public SemanticLexicalReadIntentGrounder(
            SemanticContractSnapshot contract, SemanticLexicalResolver resolver) {
    ...
    public ReadIntent ground(
        ...
        if (decision.outcome() != GroundingOutcome.COMMITTED || decision.committed() == null)
            throw new IllegalStateException(
                    "Lexical intent could not be committed: "
                            + decision.outcome()
                            + ". "
                            + decision.reason());
    ...
    private ReadIntent buildIntent(GroundingInterpretation interpretation) {
        ...
        for (var step : interpretation.steps()) {
            ...
            if (c.kind() == SemanticLexicalCandidateKind.RELATIONSHIP
                ...
                if (c.relationshipId() == null || c.sourceEntityId() == null) continue;
            ...
            if (c.kind() == SemanticLexicalCandidateKind.FIELD
                    && c.fieldId() != null
                    && c.entityId() != null) {
            ...
            if (c.kind() == SemanticLexicalCandidateKind.VALUE) {
                if (c.fieldId() == null || c.entityId() == null)
                    throw new IllegalStateException(
                            "Lexical value '"
                                    + c.canonicalName()
                                    + "' has no semantic field binding; intent was not committed to"
                                    + " execution.");
                var route = findEntityPath(root.id(), c.entityId());
        ...
        if (selections.isEmpty()) selections.add(new ReadSelection(root.identity().name()));
        return new ReadIntent(
                root.name(),
                selections,
                filters.isEmpty()
                        ? null
                        : filters.size() == 1 ? filters.get(0) : new ReadAndFilter(filters),
                List.of(),
                null,
                null,
                null,
    ...
    private ReadFilter buildValueFilter(
            List<RelationshipId> route, EntityId owner, FieldId field, String value) {
        ...
        for (int i = route.size() - 1; i >= 0; i--)
            result =
                    new ReadRelationshipFilter(
                            findRelationshipById(route.get(i)).name(),
    ...
    private void addSelectionPath(
            ...
            EntityId rootId,
            List<RelationshipId> route,
            FieldId field) {
        if (route.isEmpty()) {
            if (field == null) return;
            String name = findField(contract.get(rootId), field).name();
            if (selections.stream()
                    .noneMatch(x -> x.field() != null && x.field().equalsIgnoreCase(name)))
        ...
        for (int i = 0; i < route.size(); i++) {
            ...
            if (i == route.size() - 1) {
                ...
                String fieldName =
                        field != null ? findField(target, field).name() : target.identity().name();
                if (children.stream()
                        .noneMatch(x -> x.field() != null && x.field().equalsIgnoreCase(fieldName)))
                    children.add(new ReadSelection(fieldName));
            }
            var replacement =
                    new ReadSelection(existing.field(), existing.relationship(), children);
    ...
    private SemanticRelationship findRelationshipById(RelationshipId id) {
        return contract.entities().stream()
                .flatMap(x -> x.relationships().stream())
                .filter(x -> x.id().equals(id))
                .findFirst()
                .orElseThrow(
                        () ->
                                new IllegalStateException(
                                        "Semantic relationship " + id + " is not defined."));
    }
    ...
    private List<RelationshipId> findEntityPath(EntityId source, EntityId target) {
        ...
        while (!q.isEmpty()) {
            ...
            for (var r : contract.get(n.id()).relationships()) {
                ...
                p.add(r.id());
                if (r.target().equals(target)) return p;
                if (visited.add(r.target())) q.add(new Node(r.target(), p));
            }
        }
        throw new IllegalStateException(
                "No semantic path connects entity " + source + " to entity " + target + ".");
    }
                        ...
                        () -> {
                            if (entity.identity().fieldId().equals(id))
                                return new SemanticField(
                                        ...
                                        Object.class);
                            throw new IllegalStateException(
                                    "Field " + id + " is not defined on '" + entity.name() + "'.");
                        });
    }
}
