...
public record DelegationIssuerTrustSnapshot(String issuer, long sequence, String trustFingerprint, String keyId,
		DelegationIssuerKeyState keyState) {
	public void assertCurrent(ISecurityWarrantDelegationTrustStateResolver resolver) {
		var c = resolver.capture(issuer, keyId);
		if (c.sequence() != sequence
				|| !java.security.MessageDigest.isEqual(java.util.HexFormat.of().parseHex(c.trustFingerprint()),
						java.util.HexFormat.of().parseHex(trustFingerprint))
				|| c.keyState() != keyState)
