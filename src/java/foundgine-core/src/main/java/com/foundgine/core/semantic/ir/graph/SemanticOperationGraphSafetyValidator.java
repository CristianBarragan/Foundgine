 ...
 *
 * <p>Enforces bounded graph shape after dynamic intent has been resolved into canonical semantic
 * operations. This is intentionally provider-neutral and protects callers that construct operation
 * graphs without going through the JSON/MCP adapters.
 *
 * <p><b>Porting decision:</b> the C# implementation uses a local function ({@code Walk}) that
 * closes over mutable local counters ({@code edges}, {@code fields}, {@code maxDepth}, {@code
 * visited}) by reference. Java lambdas/local classes cannot mutate captured local primitives, so
 * the walk is ported as a private recursive method on a small mutable {@link WalkState} holder that
 * plays the same role as the captured locals.
 */
public final class SemanticOperationGraphSafetyValidator {
    ...
    public static void validate(SemanticOperationGraph graph, SecurityResourceLimits limits) {
        ...
        if (nodes > limits.maxOperationGraphNodes()) {
            reject(
                    "Semantic operation graph exceeds the configured maximum of "
                            + limits.maxOperationGraphNodes()
                            + " nodes.");
        }
        ...
        if (state.edges > limits.maxOperationGraphEdges()) {
            reject(
                    "Semantic operation graph exceeds the configured maximum of "
                            + limits.maxOperationGraphEdges()
                            + " edges.");
        }
        if (state.fields > limits.maxOperationGraphFields()) {
            reject(
                    "Semantic operation graph exceeds the configured maximum of "
                            + limits.maxOperationGraphFields()
                            + " fields.");
        }
    ...
    private static void walk(
        ...
        if (!state.visited.add(node.id())) {
            reject(
                    "Semantic operation graph contains a repeated node '"
                            + node.id()
                            + "' or cycle.");
        }
        ...
        if (depth > limits.maxOperationGraphDepth()) {
            reject(
                    "Semantic operation graph depth exceeds the configured maximum of "
                            + limits.maxOperationGraphDepth()
                            + " levels.");
        }
