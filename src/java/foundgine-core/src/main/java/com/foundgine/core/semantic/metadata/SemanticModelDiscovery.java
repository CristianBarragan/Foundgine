...
public final class SemanticModelDiscovery {
    ...
    public static SemanticModel discover(IMetadataCatalog metadata) {
        ...
        var entities =
                new LinkedHashMap<com.foundgine.core.abstractions.EntityId, SemanticEntity>();
        for (var item : metadata.entities()) {
                ...
                fields.add(
                        new SemanticField(
                                ...
                                f.effectiveAliases().stream()
                                        .map(a -> new SemanticAlias(a.name(), a.weight()))
                                        .toList(),
                                List.of(),
                                null));
            ...
            if (item.primaryKey() != null)
                for (var f : item.effectiveFields())
                    if (f.column() != null
                            && f.column().columnId().equals(item.primaryKey().columnId())) {
                        primary =
                                fields.stream()
                                        .filter(x -> x.id().equals(f.id()))
                                        .findFirst()
                                        .orElse(null);
                        break;
                    }
            if (primary == null)
                throw new IllegalStateException(
                        "Metadata entity '"
                                + item.name()
                                + "' has no field corresponding to its primary key.");
            entities.put(
                    item.entityId(),
                    new SemanticEntity(
                            item.entityId(),
