...
public interface IPlanner {
    ...
    default SemanticPlan plan(SemanticContractSnapshot contract, SemanticOperation operation) {
        if (contract == null || operation == null) throw new NullPointerException();
        SemanticOperationContractValidator.validate(operation, contract);
        return plan(operation);
    }

    default SemanticPlan plan(
            SemanticContractSnapshot contract, SemanticAuthorizationResult authorization) {
        if (contract == null || authorization == null) throw new NullPointerException();
        authorization.ensureMatches(contract);
        var p = plan(contract, authorization.operation());
        return new SemanticPlan(
                p.root(),
                p.requiredSecurityInvariants(),
