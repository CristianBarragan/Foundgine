...
/**
 * Enforces resource and complexity bounds at the semantic boundary. Adapter-level limits are
 * defense-in-depth; this validator is the canonical engine-side guard and therefore also protects
 * non-JSON callers.
 */
public final class SecurityResourceLimitValidator {
    ...
    public static void validate(SemanticRequest request, SecurityResourceLimits limits) {
        ...
        if (selectionNodes > limits.maxSelectionNodes())
            reject(
                    "Selection complexity exceeds the configured maximum of "
                            + limits.maxSelectionNodes()
                            + " nodes.");
        ...
        if (options.limit() != null
                && options.limit() > 0
                && options.limit() > limits.maxPageSize())
            reject(
                    "Requested page size exceeds the configured maximum of "
                            + limits.maxPageSize()
                            + ".");
        if (options.offset() != null
                && options.offset() > 0
                && options.offset() > limits.maxOffset())
            reject(
                    "Requested offset exceeds the configured maximum of "
                            + limits.maxOffset()
                            + ".");
        if (options.after() != null && options.after().length() > limits.maxCursorLength())
            reject(
                    "Cursor length exceeds the configured maximum of "
                            + limits.maxCursorLength()
                            + ".");

        if (options.effectiveOrder().size() > limits.maxOrderTerms())
            reject(
                    "Order complexity exceeds the configured maximum of "
                            + limits.maxOrderTerms()
                            + " terms.");

        for (SemanticOrderTerm term : options.effectiveOrder()) {
            if (term.effectivePath().size() > limits.maxOrderPathDepth())
                reject(
                        "Order relationship path exceeds the configured maximum of "
                                + limits.maxOrderPathDepth()
                                + " levels.");
        }
        ...
        if (options.filter() != null) {
            ...
            if (filterNodes > limits.maxFilterNodes())
                reject(
                        "Filter complexity exceeds the configured maximum of "
    ...
    private static int countSelections(
            List<SemanticSelection> selections, int depth, SecurityResourceLimits limits) {
        ...
        for (SemanticSelection selection : selections) {
            count++;
            if (count > limits.maxSelectionNodes()) return count;
    ...
    public static void validateFilter(
            SemanticFilterExpression filter, SecurityResourceLimits limits) {
        countFilter(filter, 1, limits);
    }

    private static int countFilter(
            SemanticFilterExpression filter, int depth, SecurityResourceLimits limits) {
        ...
        } else if (filter instanceof SemanticAggregateFilter aggregate
