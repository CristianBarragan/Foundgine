 ...
 *
 * <p>Concrete provider evidence for mutation execution. Provider declarations are not sufficient to
 * cross the mutation execution boundary.
 */
public record MutationSecurityConformanceResult(
        String provider, List<String> satisfied, List<String> violations) {
    ...
    public void ensureSatisfied(Collection<String> required) {
        ...
        if (violations.isEmpty() && missing.isEmpty()) return;

        List<String> reasons = new ArrayList<>(violations);
        reasons.addAll(
                missing.stream()
                        .map(x -> "required invariant '" + x + "' was not satisfied")
                        .toList());

        throw new IllegalStateException(
                "Mutation provider '"
                        + provider
                        + "' security conformance failed: "
