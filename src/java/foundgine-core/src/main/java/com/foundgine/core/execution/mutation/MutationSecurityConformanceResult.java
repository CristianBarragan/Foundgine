...
/**
 * Port of
 * {@code Foundgine.Core.Execution.Mutation.MutationSecurityConformanceResult}.
 *
 * <p>
 ...
 */
public record MutationSecurityConformanceResult(String provider, List<String> satisfied, List<String> violations) {
	...
	public void ensureSatisfied(Collection<String> required) {
		...
		List<String> missing = required.stream().filter(id -> !satisfiedSet.contains(id)).distinct().sorted().toList();
		...
		throw new IllegalStateException("Mutation provider '" + provider + "' security conformance failed: "
