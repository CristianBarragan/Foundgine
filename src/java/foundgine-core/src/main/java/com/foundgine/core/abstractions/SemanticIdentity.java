 ...
 *
 * <p>Canonical semantic identity and stable hashing rules shared by runtime
 * identifiers and the AOT generator. The canonical key is intentionally
 * independent of JVM/CLR metadata, declaration order, and runtime state, so
 * the 64-bit hash produced here is byte-for-byte identical to the one
