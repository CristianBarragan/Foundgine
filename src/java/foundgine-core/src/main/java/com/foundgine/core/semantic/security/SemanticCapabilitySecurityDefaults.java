...
/**
 * Port of {@code Foundgine.Core.Semantic.Security.SemanticCapabilitySecurityDefaults}.
 *
 * <p>Derives the minimum security contract for generic semantic capabilities.
 *
 * <p>Named {@code forCapability} rather than the C# name {@code For} because {@code for} is a
...
public final class SemanticCapabilitySecurityDefaults {
    ...
    public static List<String> forCapability(SemanticCapability capability) {
        ...
        if (capability.hasSideEffects()
                || capability.access().access() == AuthorizationAccess.CONDITIONAL) {
