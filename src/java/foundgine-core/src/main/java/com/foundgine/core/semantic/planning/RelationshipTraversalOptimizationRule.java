...
public final class RelationshipTraversalOptimizationRule implements IPlanRewriteRule {
	@Override
	public String name() {
		return "relationship.traversal.strategy";
	}

	@Override
	public List<String> preconditions() {
		return List.of("plan contains relationship traversal nodes", "relationship cardinality metadata is available");
	}

	@Override
	public List<String> securityObligations() {
		return List.of("authorization.required", "visibility.relationship", "planning.cache-context-isolation");
	}
	...
	private static SemanticPlanNode rewrite(SemanticPlanNode node, boolean[] changed) {
		...
		if (node.viaRelationship() != null && node.relationshipCardinality() != null) {
			var target = node.relationshipCardinality() == RelationshipCardinality.ONE
		...
		if (mode != node.traversalMode() || childChanged)
			return new SemanticPlanNode(node.id(), node.operation(), node.entityId(), node.fields(),
					...
					node.relationshipCardinality(), mode, node.traversalOrder(), node.aggregateExecutionStrategy());
		return node;
	}

	private static boolean containsEligible(SemanticPlanNode node) {
		return (node.viaRelationship() != null && node.relationshipCardinality() != null)
				|| node.children().stream().anyMatch(RelationshipTraversalOptimizationRule::containsEligible);
	}
}
