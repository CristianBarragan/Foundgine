...
public final class SecurityWarrantAuthorization {
    ...
    public static boolean allows(
        ...
        if (!java.util.Objects.equals(warrant.subject(), subject)
                || !java.util.Objects.equals(warrant.audience(), audience)
                || !warrant.isTimeValid(Instant.now())) return false;
        boolean grant =
                warrant.grants().stream()
                        .anyMatch(
                                g ->
                                        java.util.Objects.equals(g.capability(), capability)
                                                && java.util.Objects.equals(
                                                        g.operation(), operation)
                                                && (!requireResourceScopeMatch
                                                        || g.resourceScopes().isEmpty()
                                                        || (resourceScope != null
                                                                && g.resourceScopes()
                                                                        .contains(resourceScope))));
        if (!grant) return false;
        var c = warrant.constraints();
        if (!c.allowedTenants().isEmpty()
                && (tenant == null || !c.allowedTenants().contains(tenant))) return false;
        if (!c.allowedOperations().isEmpty() && !c.allowedOperations().contains(operation))
            return false;
        if (requireResourceScopeMatch
                && !c.resourceScopes().isEmpty()
        ...
        if (requestedAmount != null
                && c.maxAmount() != null
                && requestedAmount.compareTo(c.maxAmount()) > 0) return false;
        return true;
    }

    public static boolean allows(
            SecurityWarrant w, String s, String a, String c, String o, String t, String r) {
