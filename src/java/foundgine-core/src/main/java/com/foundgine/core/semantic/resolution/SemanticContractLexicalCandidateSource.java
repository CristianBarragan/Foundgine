...
public final class SemanticContractLexicalCandidateSource implements ISemanticLexicalCandidateSource {
	...
	@Override
	public List<SemanticLexicalCandidate> retrieve(SemanticLexicalRequest request,
			ISemanticLexicalCandidateSource.CancellationToken cancellationToken) {
		Objects.requireNonNull(request);
		Objects.requireNonNull(cancellationToken);
		if (cancellationToken.isCancellationRequested())
			throw new java.util.concurrent.CancellationException();

		return entries.stream().filter(x -> request.effectiveKinds().contains(x.kind()))
				.map(x -> new ScoredEntry(x, score(request.token(), x))).filter(x -> x.score() > 0)
				...
				.limit(request.limit())
				.map(x -> new SemanticLexicalCandidate(request.token(), x.entry().kind(), x.entry().canonicalName(),
						x.score(), x.entry().entityId(), x.entry().relationshipId(), x.entry().fieldId(),
						x.entry().sourceEntityId(), x.entry().targetEntityId(), x.entry().value(),
						List.of(new ResolutionEvidence("Semantic contract lexical match for '" + request.token() + "'.",
	...
	private static double score(String token, SemanticLexiconEntry entry) {
		return StreamSupport
				.stream(java.util.stream.Stream
						.concat(java.util.stream.Stream.of(entry.canonicalName(), entry.searchText()),
