...
public final class SemanticContractLexicalCandidateSource
    ...
    @Override
    public List<SemanticLexicalCandidate> retrieve(
                                ...
                                new SemanticLexicalCandidate(
                                        ...
                                        List.of(
                                                new ResolutionEvidence(
                                                        "Semantic contract lexical match for '"
                                                                + request.token()
                                                                + "'.",
        ...
        return StreamSupport.stream(
                        java.util.stream.Stream.concat(
                                        java.util.stream.Stream.of(
                                                entry.canonicalName(), entry.searchText()),
                                        entry.effectiveAliases().stream())
                                .spliterator(),
                        false)
                .mapToDouble(x -> similarity(token, x))
                .max()
                .orElse(0);
    }
