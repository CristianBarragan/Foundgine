...
public final class SecurityInvariantProofGate {
    private static final Set<String> CONCRETE_EVALUATION_REQUIRED =
            Set.of(
    ...
    public static ProviderPlan attachAndValidate(
            ProviderPlan plan, ExecutionIR ir, IProviderPlanCompiler compiler) {
        ...
        if (required.isEmpty())
            throw new IllegalStateException(
                    "ExecutionIR contains no security obligations. An executable provider plan must"
                            + " carry a non-empty security certificate.");
        ...
        if (compiler instanceof IProviderSecurityConformanceEvaluator evaluator) {
            ...
            var missing =
                    required.stream()
                            .filter(x -> !conformance.satisfied().contains(x))
                            .sorted()
                            .toList();
            if (!missing.isEmpty())
                throw new IllegalStateException(
                        "Provider '"
                                + plan.provider()
                                + "' executable conformance did not satisfy required invariants: "
                                + String.join(", ", missing)
                                + ".");
            ...
            var missing =
                    required.stream().filter(x -> !declaredPreserved.contains(x)).sorted().toList();
            if (!missing.isEmpty())
                throw new IllegalStateException(
                        "Provider '"
                                + plan.provider()
                                + "' declared preservation does not satisfy required invariants: "
                                + String.join(", ", missing)
                                + ".");
        }
