...
public final class SecurityInvariantProofGate {
    private static final Set<String> CONCRETE_EVALUATION_REQUIRED = Set.of(
    ...
    public static ProviderPlan attachAndValidate(
        ...
        if (compiler instanceof IProviderSecurityConformanceEvaluator evaluator) {
            ...
            if (!missing.isEmpty()) throw new IllegalStateException(
                    "Provider '" + plan.provider() + "' executable conformance did not satisfy required invariants: "
                            + String.join(", ", missing) + ".");
            ...
            if (!missing.isEmpty()) throw new IllegalStateException(
                    "Provider '" + plan.provider() + "' declared preservation does not satisfy required invariants: "
                            + String.join(", ", missing) + ".");
        }
