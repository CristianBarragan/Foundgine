 ...
 * rewrite rule was evaluated.
 */
public record SecurityObligationProof(List<String> obligations, List<String> preserved, List<String> notRequired,
	...
	public static SecurityObligationProof create(IPlanRewriteRule rule, SemanticPlan before, SemanticPlan after) {
		...
		for (var obligation : obligations)
			if (!SecurityInvariantRegistry.contains(obligation))
				violations.add("Rewrite rule '" + rule.name() + "' declares unknown security obligation '" + obligation
						+ "'.");
		...
		for (var obligation : obligations) {
			...
			if (!afterSet.contains(obligation)) {
				violations.add("Rewrite rule '" + rule.name() + "' dropped declared security obligation '" + obligation
						+ "'.");
				continue;
			}
