 ...
 *
 * <p>
 * Computes execution levels directly from canonical mutation dependencies. No
 * correlation-shaped compatibility model is involved.
 */
	...
	public static List<List<Integer>> compute(int operationCount, Iterable<MutationDependency> dependencies) {
		...
		for (MutationDependency dependency : edges) {
			if (dependency.sourceOperationIndex() < 0 || dependency.sourceOperationIndex() >= operationCount
				...
				throw new IllegalStateException("Mutation dependency indexes are outside the execution graph: "
		...
		while (!remaining.isEmpty()) {
			List<Integer> level = remaining.stream().filter(i -> incoming.get(i) == 0).sorted().toList();
