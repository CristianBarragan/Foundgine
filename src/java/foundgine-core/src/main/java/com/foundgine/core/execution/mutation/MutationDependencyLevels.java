 ...
 *
 * <p>Computes execution levels directly from canonical mutation dependencies. No correlation-shaped
 * compatibility model is involved.
 */
public final class MutationDependencyLevels {

    private MutationDependencyLevels() {}

    public static List<List<Integer>> compute(
            int operationCount, Iterable<MutationDependency> dependencies) {
        if (operationCount < 0)
        ...
        for (MutationDependency dependency : edges) {
            ...
            if (dependency.sourceOperationIndex() == dependency.targetOperationIndex()) {
                throw new IllegalStateException(
                        "Mutation dependency cycle detected at operation "
                                + dependency.sourceOperationIndex()
                                + ".");
            }
        }

        if (operationCount == 0) return List.of();
        ...
        for (int i = 0; i < operationCount; i++) remaining.add(i);

        List<List<Integer>> levels = new ArrayList<>();

        while (!remaining.isEmpty()) {
            List<Integer> level =
                    remaining.stream().filter(i -> incoming.get(i) == 0).sorted().toList();
            ...
            for (int node : level) {
                ...
                for (int target : outgoing.get(node)) incoming.merge(target, -1, Integer::sum);
            }
        }
