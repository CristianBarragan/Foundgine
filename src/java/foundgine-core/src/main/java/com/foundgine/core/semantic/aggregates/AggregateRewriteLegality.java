...
public final class AggregateRewriteLegality {
    ...
    public static AggregateRewriteLegalityResult checkEmptySemantics(
        ...
        return from.emptyCollectionResult() == to.emptyCollectionResult()
                ...
                : AggregateRewriteLegalityResult.illegal(
                        "empty-collection semantics differ: '"
                                + from.aggregate()
                                + "' yields "
                                + describe(from.emptyCollectionResult())
                                + " for an empty collection, but '"
                                + to.aggregate()
                                + "' yields "
                                + describe(to.emptyCollectionResult())
                                + ".");
    }
    ...
    public static AggregateRewriteLegalityResult checkNullSemantics(
        ...
        return from.nullInputBehavior() == to.nullInputBehavior()
                ? AggregateRewriteLegalityResult.LEGAL
                : AggregateRewriteLegalityResult.illegal(
                        "NULL-input semantics differ: '"
                                + from.aggregate()
                                + "' is "
                                + describe(from.nullInputBehavior())
                                + ", but '"
                                + to.aggregate()
                                + "' is "
    ...
    public static AggregateRewriteLegalityResult checkDuplicateSensitivity(
        ...
        return from.isDuplicateSensitive() == to.isDuplicateSensitive()
                ? AggregateRewriteLegalityResult.LEGAL
                : AggregateRewriteLegalityResult.illegal(
                        "duplicate sensitivity differs: '"
                                + from.aggregate()
                                + "' is "
                                + (from.isDuplicateSensitive()
                                        ? "duplicate-sensitive"
                                        : "duplicate-insensitive")
                                + ", but '"
                                + to.aggregate()
                                + "' is "
                                + (to.isDuplicateSensitive()
                                        ? "duplicate-sensitive"
                                        : "duplicate-insensitive")
                                + ".");
    }
    ...
    public static AggregateRewriteLegalityResult checkCardinalityRequirement(
        ...
        return knowledge != SemanticCardinalityKnowledge.UNKNOWN
                ? AggregateRewriteLegalityResult.LEGAL
                : AggregateRewriteLegalityResult.illegal(
                        "cardinality proof is required to substitute '"
                                + to.aggregate()
                                + "' for '"
                                + from.aggregate()
                                + "', but the relationship cardinality is unknown at rewrite"
                                + " time.");
    }
    ...
    public static AggregateRewriteLegalityResult checkSubstitution(
        ...
        return checkSubstitution(
                SemanticAggregateSemanticsCatalog.forAggregate(from),
                SemanticAggregateSemanticsCatalog.forAggregate(to),
                knowledge);
    }

    public static AggregateRewriteLegalityResult checkSubstitution(
            SemanticFilterAggregate from, SemanticFilterAggregate to) {
