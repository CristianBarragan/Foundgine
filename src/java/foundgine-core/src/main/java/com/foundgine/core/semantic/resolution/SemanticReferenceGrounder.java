...
/**
 * Grounds a natural-language reference against the semantic model before data retrieval.
 * Entity/field aliases narrow the search space; provider-backed retrieval contributes ranked data
 * evidence. No provider-specific search technology leaks into this layer.
 */
public final class SemanticReferenceGrounder {
    ...
    public List<SemanticReferenceEvidence> ground(
            String query, RetrievalStrategy strategy, int entityLimit, int candidateLimit) {
        ...
        var matches =
                model.entities().stream()
                        ...
                        .filter(
                                x ->
                                        x.entity().fields().stream()
                                                .anyMatch(
                                                        field ->
                                                                field.clrType() == String.class
                                                                        && (field.capabilities()
                                                                                        & SemanticFieldCapabilities
                                                                                                .SENSITIVE)
                                                                                == 0))
                        .sorted(
                                Comparator.comparingDouble(EntityScore::score)
                                        .reversed()
                                        .thenComparing(
                                                x -> x.entity().name(),
        ...
        for (var match : matches) {
            ...
            results.add(
                    new SemanticReferenceEvidence(
                            query,
                            retrieval,
                            confidence,
                            "'"
                                    + query
                                    + "' grounded against semantic entity '"
                                    + match.entity().name()
                                    + "' and field '"
                                    + field.name()
                                    + "'."));
        ...
        return results.stream()
                .sorted(
                        Comparator.comparingDouble(SemanticReferenceEvidence::confidence)
                                .reversed())
                .toList();
    }
    ...
    private static double entityScore(SemanticEntity entity, String query) {
        ...
        var aliasScore =
                entity.effectiveAliases().stream()
                        .mapToDouble(a -> similarity(query, a.name()))
                        .max()
                        .orElse(0d);
        var fieldScore =
                        ...
                        .flatMap(
                                f ->
                                        java.util.stream.Stream.concat(
                                                java.util.stream.Stream.of(f.name()),
                                                f.effectiveAliases().stream()
                                                        .map(SemanticAlias::name)))
                        .mapToDouble(n -> similarity(query, n))
                        .max()
                        .orElse(0d);
        ...
        return entity.fields().stream()
                .filter(
                        field ->
                                field.clrType() == String.class
                                        && (field.capabilities()
                                                        & SemanticFieldCapabilities.SENSITIVE)
                                                == 0)
                .map(
                        field ->
                                new FieldScore(
                                                ...
                                                + (0.5d
                                                        * Math.max(
                                                                ...
                                                                field.effectiveAliases().stream()
                                                                        ...
                                                                        .orElse(0d)))))
                .sorted(
                        Comparator.comparingDouble(FieldScore::score)
                                .reversed()
                                .thenComparing(
                                        x -> x.field().name(), String.CASE_INSENSITIVE_ORDER))
