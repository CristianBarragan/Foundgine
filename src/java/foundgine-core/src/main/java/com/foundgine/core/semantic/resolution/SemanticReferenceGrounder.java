...
public final class SemanticReferenceGrounder {
    ...
    public List<SemanticReferenceEvidence> ground(String query, RetrievalStrategy strategy, int entityLimit, int candidateLimit) {
        if (query == null || query.isBlank()) throw new IllegalArgumentException("Reference query cannot be empty.");
        if (entityLimit < 1) throw new IllegalArgumentException("entityLimit must be positive.");
        if (candidateLimit < 1) throw new IllegalArgumentException("candidateLimit must be positive.");

        var matches = model.entities().stream()
            .map(entity -> new EntityScore(entity, entityScore(entity, query)))
            .filter(x -> x.entity().fields().stream().anyMatch(field ->
                field.clrType() == String.class && (field.capabilities() & SemanticFieldCapabilities.SENSITIVE) == 0))
            .sorted(Comparator.comparingDouble(EntityScore::score).reversed()
                .thenComparing(x -> x.entity().name(), String.CASE_INSENSITIVE_ORDER))
            .limit(entityLimit)
            .toList();
        ...
        for (var match : matches) {
            ...
            var retrieval = candidates.retrieve(new SemanticRetrievalRequest(
                match.entity().id(), field.id(), query, strategy, candidateLimit));
            if (retrieval == null || retrieval.isEmpty()) continue;
            ...
            results.add(new SemanticReferenceEvidence(query, retrieval, confidence,
                "'" + query + "' grounded against semantic entity '" + match.entity().name() +
                "' and field '" + field.name() + "'."));
        }
        return results.stream().sorted(Comparator.comparingDouble(SemanticReferenceEvidence::confidence).reversed()).toList();
    }

    private static double entityScore(SemanticEntity entity, String query) {
        var entityScore = similarity(query, entity.name());
        var aliasScore = entity.effectiveAliases().stream().mapToDouble(a -> similarity(query, a.name())).max().orElse(0d);
        var fieldScore = entity.fields().stream()
            .flatMap(f -> java.util.stream.Stream.concat(java.util.stream.Stream.of(f.name()), f.effectiveAliases().stream().map(SemanticAlias::name)))
    ...
    private static SemanticField selectSearchField(SemanticEntity entity, String query) {
        return entity.fields().stream()
            .filter(field -> field.clrType() == String.class && (field.capabilities() & SemanticFieldCapabilities.SENSITIVE) == 0)
            .map(field -> new FieldScore(field, 0.5d + (0.5d * Math.max(
                similarity(query, field.name()),
                field.effectiveAliases().stream().mapToDouble(a -> similarity(query, a.name())).max().orElse(0d)))))
            .sorted(Comparator.comparingDouble(FieldScore::score).reversed()
                .thenComparing(x -> x.field().name(), String.CASE_INSENSITIVE_ORDER))
            .map(FieldScore::field)
            .findFirst().orElse(null);
    }

    private static double similarity(String left, String right) {
        if (left.equalsIgnoreCase(right)) return 1d;
        if (containsIgnoreCase(left, right) || containsIgnoreCase(right, left)) return 0.8d;
