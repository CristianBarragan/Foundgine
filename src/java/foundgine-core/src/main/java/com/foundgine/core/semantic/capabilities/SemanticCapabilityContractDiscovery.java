 ...
 *
 * <p>Builds the canonical, machine-readable capability contract for a frozen semantic model under
 * an authorization policy. Capability discovery ({@link SemanticAuthorizationCapabilityDiscovery})
 * intentionally hides authorization predicates; this class rehydrates only the top-level entity
 * read/write decision with the exact policy predicate so planning/security consumers can carry it
 * forward, while the descriptive authorization capability surface underneath stays predicate-free.
 *
 * <p><b>Porting decisions:</b>
 *
 * <ul>
 *   <li>C# {@code yield return} iterator methods ({@code BuildCapabilities}, {@code
 *       BuildMutationActions}) are ported as methods returning a fully materialized {@link List},
 *       built in the same emission order.
 *   <li>The C# {@code entity with { Read = ..., Write = ... }} non-destructive mutation is ported
 *       as constructing a new {@link SemanticAuthorizationCapability} record with the same fields
 *       except the patched {@code read}/{@code write}.
 *   <li>{@code SemanticCapability} object-initializer syntax (setting {@code Operation}/{@code
 *       HasSideEffects}/{@code IsIdempotent} on top of the 9 positional constructor args) is ported
 *       via the private {@link #capability} helper, which fills in the same C# {@code init}
 *       defaults for {@code version} and {@code requiredSecurityInvariants} that every call site
 *       here relies on.
 *   <li>The C# extension method {@code ClrTypeName} is ported as the private static method {@link
 *       #clrTypeName}; {@code Type.FullName ?? Name} is ported as {@link Class#getTypeName()},
 *       which the JVM never returns null for.
 * </ul>
 */
public final class SemanticCapabilityContractDiscovery {
    ...
    public static SemanticCapabilityContract describe(
            SemanticModel model, ISemanticAuthorizationPolicy policy) {
        ...
        var capabilities =
                discovered.entities().stream()
                        .map(
                                entity ->
                                        new SemanticAuthorizationCapability(
                                                entity.entityId(),
                                                entity.name(),
                                                preservePredicate(
                                                        entity.read(),
    ...
    private static AuthorizationDecision preservePredicate(
            AuthorizationDecision decision, AuthorizationPredicate predicate) {
    ...
    private static List<SemanticCapability> buildCapabilities(
        ...
        var readRelationships =
                java.util.stream.Stream.concat(
                                ...
                                model.traversals().stream()
                                        .filter(
                                                x ->
                                                        x.source().equals(entity.entityId())
                                                                && traversalIsReadable(
                                                                        model, x, policy))
                        ...
                        entity.write().isAllowed()
                                ? List.of(
                                        new SemanticCapabilityEffect(
                                                ...
                                                "May modify "
                                                        + entity.name()
                                                        + " data when execution-time authorization"
                                                        + " permits it."))
                                : List.of(),
                        writeFields,
                        writeRelationships,
        ...
        for (var traversal : model.traversals()) {
            if (!traversal.source().equals(entity.entityId())
                    || !traversalIsReadable(model, traversal, policy)) {
                continue;
            }
            var pathDescription =
                    traversal.path().stream()
                    ...
                    capability(
                            ...
                            List.of(
                                    new SemanticCapabilityConstraint(
                                            "semantic-path",
                                            "Logical traversal expands through relationship path "
                                                    + pathDescription
                                                    + "; every hop remains subject to"
    ...
    private static boolean traversalIsReadable(
            SemanticModel model, SemanticTraversal traversal, ISemanticAuthorizationPolicy policy) {
        ...
        for (var relationshipId : traversal.path()) {
            ...
            if (relationship == null
                    || !policy.getRelationshipAccess(
                                    current.id(), relationship.id(), AuthorizationOperation.READ)
                            .isAllowed()) {
                return false;
            }
    ...
    private static List<SemanticCapabilityConstraint> buildWriteConstraints() {
        return List.of(
                ...
                new SemanticCapabilityConstraint(
                        "writable-fields",
                        "Every requested field must be writable under the effective authorization"
                                + " policy."));
    }

    private static List<SemanticCapability> buildMutationActions(
            SemanticModel model, SemanticAuthorizationCapability entity) {
        List<SemanticCapability> result = new ArrayList<>();
                    ...
                    switch (action) {
                        case "create" ->
                                List.of(
                                        new SemanticCapabilityConstraint(
                                                "writable-fields",
                                                "Every supplied field must be writable."));
                        case "update" ->
                                List.of(
                                        new SemanticCapabilityConstraint(
                                                "target-selection",
                                                "A target filter or equivalent identity selection"
                                                        + " is required."),
                                        new SemanticCapabilityConstraint(
                                                "writable-fields",
                                                "Every supplied field must be writable."));
                        ...
                        case "upsert" ->
                                List.of(
                                        new SemanticCapabilityConstraint(
                                                "conflict-key",
                                                "A conflict key or equivalent identity must"
                                                        + " determine the upsert target."),
                                        new SemanticCapabilityConstraint(
                                                "writable-fields",
                                                "Every supplied field must be writable."));
                        default -> List.of();
            ...
            effects.add(
                    new SemanticCapabilityEffect(
                            ...
                            "May "
                                    ...
                                    + " "
                                    + entity.name()
                                    + " data when execution-time authorization permits it."));
            if (action.equals("create") || action.equals("update") || action.equals("upsert")) {
                effects.add(
                        new SemanticCapabilityEffect(
                                "field.mutation", "May change writable field values."));
            }
            ...
            result.add(
                    capability(
                            ...
                            action.equals("update")
                                    ...
                                    || action.equals("upsert")));
        }

        return result;
    }

    private static List<SemanticCapabilityInput> buildWriteInputs(
            SemanticModel model, SemanticAuthorizationCapability entity) {
        return entity.fields().stream()
                .filter(x -> x.write().isAllowed())
                .map(
                        field ->
                                new SemanticCapabilityInput(
                                        field.name(),
                ...
                .sorted(
                        Comparator.comparing(
                                SemanticCapabilityInput::name, String.CASE_INSENSITIVE_ORDER))
                .toList();
    }

    private static String clrTypeName(
            SemanticFieldAuthorizationCapability field, SemanticModel model, EntityId entityId) {
        var semanticField =
                model.get(entityId).fields().stream()
