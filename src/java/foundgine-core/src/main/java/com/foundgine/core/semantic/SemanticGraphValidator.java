...
public final class SemanticGraphValidator {
    ...
    public static void validate(
            SemanticGraph graph, SemanticModel model, SemanticGraphValidationMode mode) {
        Objects.requireNonNull(graph);
                ...
                graph.nodes().stream()
                        .collect(
                                java.util.stream.Collectors.toMap(
                                        SemanticGraph.SemanticGraphNode::id, x -> x));
    ...
    private static void validateIdsAndReachability(
            SemanticGraph graph, SemanticGraphValidationMode mode) {
        ...
        if (mode != SemanticGraphValidationMode.STRICT && roots.isEmpty())
            throw new IllegalStateException(
                    "A semantic graph must contain at least one root node.");
            ...
            if (n.parentId() != null) {
                if (!idSet.contains(n.parentId()))
                    throw new IllegalStateException(
                            "Semantic node "
                                    + n.id()
                                    + " references missing parent node "
                                    + n.parentId()
                                    + ".");
    ...
    private static void validateNode(
            SemanticGraph.SemanticGraphNode node,
            Map<Integer, SemanticGraph.SemanticGraphNode> byId,
            SemanticModel model,
            SemanticGraphValidationMode mode) {
        var entity =
                model.entities().stream()
                        .filter(e -> e.id().equals(node.entityId()))
                        .findFirst()
                        .orElse(null);
            ...
            if (node.viaRelationship() == null && node.viaConnection() == null)
                throw new IllegalStateException(
                        "Non-root semantic node "
                                + node.id()
                                + " must specify a relationship or connection.");
            if (node.viaRelationship() != null && node.viaConnection() != null)
                throw new IllegalStateException(
                        "Semantic node "
                                + node.id()
                                + " cannot specify both relationship and connection edges.");
        ...
        if (entity == null) {
            if (mode == SemanticGraphValidationMode.FEDERATED
                    || mode == SemanticGraphValidationMode.EXPLORATORY) return;
            throw new IllegalStateException(
                    "Semantic node "
                            + node.id()
