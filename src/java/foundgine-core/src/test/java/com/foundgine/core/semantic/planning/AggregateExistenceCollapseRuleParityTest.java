 ...
 *
 * <ul>
 *   <li>The only existing Java coverage touching this class, {@code
 *       AggregateExistenceSqlRenderingParityTest}, tests the SQL-rendering effect of the rule
 *       end-to-end via {@code SqlCompiler}; nothing exercised {@code
 *       AggregateExistenceCollapseRule} directly at the planning-node level (rejecting providers
 *       without quantifier support, rejecting comparisons that aren't provably existence/emptiness
 *       tests, or the security-invariant preservation contract). This file closes that gap.
 *   <li>C#'s {@code Assert.Same(plan, rule.Apply(plan))} (reference equality) is ported as Java
 *       reference equality via {@code assertSame}, which the Java rule supports directly: {@code
 *       apply} returns the original {@code SemanticPlan} instance, unchanged, when {@code canApply}
 *       is false or no rewrite occurred.
 * </ul>
 */
class AggregateExistenceCollapseRuleParityTest {

    private static final RelationshipId RELATIONSHIP = new RelationshipId(10);
    private static final SemanticFilterExpression PREDICATE =
    ...
    static Stream<org.junit.jupiter.params.provider.Arguments> nonCollapsingCases() {
        return Stream.of(
                arguments(SemanticAggregateFilterOperator.GTE, 0L),
                arguments(SemanticAggregateFilterOperator.GT, 1L),
                arguments(SemanticAggregateFilterOperator.LT, 0L),
                arguments(SemanticAggregateFilterOperator.LTE, -1L));
    }

    @ParameterizedTest
    @MethodSource("collapsingCases")
    void barePredicateBearingCountCollapsesToMatchingQuantifier(
        ...
        var rule =
                new AggregateExistenceCollapseRule(AggregateProviderCapabilityRegistry.GENERIC_SQL);
        var optimized = rule.apply(plan);

        var result =
                assertInstanceOf(
                        SemanticRelationshipFilter.class, optimized.root().queryOptions().filter());
    ...
    void providerWithoutRelationshipQuantifiersIsRejected() {
        var capability =
                new AggregateProviderCapability(
                        "limited", List.of(SemanticFilterAggregate.COUNT), true, false);
        var plan =
                planWith(
                        new SemanticAggregateFilter(
                                RELATIONSHIP,
    ...
    void existingSecurityContractIsPreserved() {
        var root =
                new SemanticPlanNode(
                        ...
                        null);
        var plan =
                new SemanticPlan(root, List.of("tenant-isolation", "authorization.required"), null);
        ...
        assertEquals(
                plan.effectiveSecurityInvariants().stream().sorted().toList(),
