...
class SemanticLexicalResolverParityTest {
    ...
    private SemanticContractSnapshot contract() {
        ...
        var model = new SemanticModelBuilder()
            .entity(PRODUCT, "Product", e -> e.identity(id, "Id").field(id, "Id", String.class))
            .build().freeze();
    ...
    @Test void tooManyTokensFailBeforeRetrieval() {
    ...
    @Test void competingSemanticMeaningsRequireClarification() {
        var first = new SemanticLexicalCandidate("name", SemanticLexicalCandidateKind.FIELD, "Name", .91,
            PRODUCT, null, FieldId.create("Product", "Name"), PRODUCT, PRODUCT, null, List.of());
        ...
        var resolver = new SemanticLexicalResolver(contract(), source(List.of(first, second)), 20, 4, .03, 32, 5000, 1000, 100, null);
    ...
    @Test void cancelledRetrievalFailsClosed() {
        var resolver = new SemanticLexicalResolver(contract(), request -> List.of(
            new SemanticLexicalCandidate(request.token(), SemanticLexicalCandidateKind.ENTITY, "Product", .9, PRODUCT, null, null, PRODUCT, PRODUCT, null, List.of())));
