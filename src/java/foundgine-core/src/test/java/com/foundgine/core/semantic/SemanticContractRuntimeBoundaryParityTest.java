 ...
 *
 * <p>The runtime resolver is intentionally constructed from the immutable
 * {@link SemanticContractSnapshot}; provider metadata is not part of this
 * boundary. The Java API has no C# model-constructor compatibility overload,
 * so the snapshot-only contract is the canonical surface.
 */
class SemanticContractRuntimeBoundaryParityTest {
    ...
    void resolverAcceptsTheRuntimeContractSnapshot() {
        ...
        var model = new SemanticModelBuilder()
                .entity(entityId, "Customer",
                        entity -> entity
                                .field(fieldId, "Id", int.class)
                                .identity(fieldId, "Id"))
                .build()
                .freeze();
    ...
    void resolverIsProviderIndependentAtTheContractBoundary() {
        ...
        var contract = new SemanticContractSnapshot(model);
        var graph = new SemanticRequestResolver(contract).resolve(
                new SemanticRequest(entityId,
                        List.of(new SemanticSelection(fieldId, null, List.of())),
                        null,
                        null));
