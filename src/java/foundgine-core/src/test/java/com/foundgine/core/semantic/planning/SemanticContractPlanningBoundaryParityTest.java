...
class SemanticContractPlanningBoundaryParityTest {
    private static SemanticContractSnapshot contract() {
        var model = new SemanticModelBuilder()
                .entity(new EntityId(1), "Customer", e -> e
                        ...
                        .field(new FieldId(10), "Name", String.class)
                ...
                .entity(new EntityId(2), "Order", e -> e
                        ...
                        .field(new FieldId(20), "Number", String.class))
                .build();
        return model.freeze().createSnapshot();
    }

    @Test void plannerAcceptsOperationBelongingToFrozenContract() {
    ...
    @Test void plannerRejectsUnknownEntityIdentity() {
        var graph = new SemanticGraph(); graph.addRoot(new EntityId(999), List.of(new FieldId(1)));
        var operation = SemanticOperationCompiler.compile(graph);
        var ex = assertThrows(IllegalStateException.class, () -> new Planner().plan(contract(), operation));
        assertTrue(ex.getMessage().contains("999"));
    }

    @Test void plannerRejectsUnknownFieldIdentity() {
        var graph = new SemanticGraph(); graph.addRoot(new EntityId(1), List.of(new FieldId(999)));
        var operation = SemanticOperationCompiler.compile(graph);
        var ex = assertThrows(IllegalStateException.class, () -> new Planner().plan(contract(), operation));
