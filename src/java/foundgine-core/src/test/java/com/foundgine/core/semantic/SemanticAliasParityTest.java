...
class SemanticAliasParityTest {
	...
	void entityAliasResolvesWithoutChangingIdentity() {
		...
		var model = new SemanticModelBuilder()
				.entity(id, "Customer", e -> e.alias("Client").identity(FieldId.create("Customer", "Id"), "Id"))
				.build();
	...
	void fieldAndRelationshipAliasesResolveToCanonicalDeclarations() {
		...
		var model = new SemanticModelBuilder()
				.entity(customer, "Customer",
						e -> e.identity(FieldId.create("Customer", "Id"), "Id").field(field, "Name", String.class)
				...
				.entity(account, "Account", e -> e.identity(FieldId.create("Account", "Id"), "Id")).build();
	...
	void aliasCollisionBetweenEntitiesIsRejected() {
		...
		assertThrows(IllegalStateException.class, () -> new SemanticModelBuilder()
				.entity(first, "Customer", e -> e.alias("Party").identity(FieldId.create("Customer", "Id"), "Id"))
				.entity(second, "Account", e -> e.alias("Party").identity(FieldId.create("Account", "Id"), "Id"))
				.build());
	}
}
