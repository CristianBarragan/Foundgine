...
/**
 * Parity tests for concrete provider security certification and execution
 * binding.
 */
class ProviderExecutableConformanceParityTest {
	...
	void certificationResultRequiresEveryRequiredInvariant() {
		var result = new ProviderSecurityConformanceResult("test",
				List.of(SecurityInvariantIds.AUTHORIZATION_REQUIRED, SecurityInvariantIds.TENANT_ISOLATION),
				List.of(SecurityInvariantIds.AUTHORIZATION_REQUIRED), List.of());
	...
	void certificationResultFailsOnProviderViolation() {
		var result = new ProviderSecurityConformanceResult("test", List.of(SecurityInvariantIds.AUTHORIZATION_REQUIRED),
				List.of(SecurityInvariantIds.AUTHORIZATION_REQUIRED), List.of("authorization predicate was lost"));
	...
	private static ExecutionIR testIr(String invariant) {
		var node = new ExecutionIRNode(1, ExecutionOperation.SCAN, new com.foundgine.core.abstractions.EntityId(1),
				List.of(), null, null, List.of(), null, null, null);
		return new ExecutionIR(node, List.of(invariant),
				new com.foundgine.core.semantic.planning.SemanticPlanAuthorizationBinding("contract", "authorization"));
	}
	...
	private static final class HostileCertifiedCompiler implements IProviderPlanCompiler,
		...
		public ProviderPlan compile(ExecutionIR ir) {
			return new TestPlan("hostile-certified");
		}
	...
	private static final class ProfileOnlyCompiler
			implements IProviderPlanCompiler, ISecurityInvariantProviderCompiler {
		@Override
		public java.util.Collection<String> preservedSecurityInvariants() {
			return List.of(SecurityInvariantIds.AUTHORIZATION_REQUIRED);
		}
	...
	private static final class HonestCompiler implements IProviderPlanCompiler, ISecurityInvariantProviderCompiler,
