...
class ProviderAwareCostSelectionParityTest {
	...
	void providerCostCanChangeRuleSelection() {
		...
		var estimator = new FakeProviderCostEstimator((before, candidate, rule) -> rule.name().equals("test.expensive")
	...
	void providerCostIsAdvisoryAndDoesNotReplaceProofChecks() {
		...
		var estimator = new FakeProviderCostEstimator(
				(before, candidate, rule) -> ProviderCostEstimate.from("test-provider", 0d, 1d, 1d, null));
	...
	void providerSelectionHistoryContainsEstimateAndScore() {
		...
		var estimator = new FakeProviderCostEstimator(
				(before, candidate, selected) -> ProviderCostEstimate.from("test-provider", 3d, 12d, 0.75d, null));
	...
	private static SemanticPlan plan() {
		...
		var node = new SemanticPlanNode(1, ExecutionOperation.SCAN, new EntityId(1), List.of(new FieldId(1)), null,
				null, List.of(), null, auth, null, RelationshipTraversalMode.DEFAULT, -1,
	...
	private static final class FakeProviderCostEstimator implements IProviderCostEstimator {
		...
		private FakeProviderCostEstimator(EstimatorFunction function) {
	...
	private static final class SelectionRule implements IPlanRewriteRule {
		...
		private SelectionRule(String name, double benefit, double cost, boolean changesMeaning) {
			...
			this.cost = cost;
			this.changesMeaning = changesMeaning;
		}
		...
		public SemanticPlan apply(SemanticPlan plan) {
			...
			return new SemanticPlan(
					new SemanticPlanNode(root.id(), root.operation(), root.entityId(), List.of(new FieldId(99)),
							root.viaRelationship(), root.viaConnection(), root.children(), root.queryOptions(),
							root.authorization(), root.relationshipCardinality(), root.traversalMode(),
							root.traversalOrder(), root.aggregateExecutionStrategy()),
					plan.requiredSecurityInvariants(), plan.authorizationBinding());
		}
	}
}
