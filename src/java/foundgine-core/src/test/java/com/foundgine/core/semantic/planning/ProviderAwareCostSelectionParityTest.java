...
class ProviderAwareCostSelectionParityTest {
    ...
    void providerCostCanChangeRuleSelection() {
        ...
        var estimator =
                new FakeProviderCostEstimator(
                        (before, candidate, rule) ->
                                rule.name().equals("test.expensive")
                                        ? ProviderCostEstimate.from(
                                                "test-provider", 100d, 10d, 0.9d, null)
                                        : ProviderCostEstimate.from(
                                                "test-provider", 1d, 10d, 0.9d, null));

        var selector =
                new RewriteRuleSelector(null, estimator, new ProviderCostSelectionPolicy(true, 1d));
        ...
        public SemanticPlan apply(SemanticPlan plan) {
            ...
            return new SemanticPlan(
                    new SemanticPlanNode(
                            ...
                            root.operation(),
                            root.entityId(),
                            List.of(new FieldId(99)),
                            root.viaRelationship(),
                            root.viaConnection(),
                            root.children(),
                            root.queryOptions(),
                            root.authorization(),
                            root.relationshipCardinality(),
                            root.traversalMode(),
                            root.traversalOrder(),
