...
class PlanRewriteRuleContractParityTest {
    ...
    @Test void authorizationRuleExposesAuditableContract() {
    ...
    @Test void ruleApplicationProducesProofs() {
    ...
    @Test void optimizerUsesRuleContractAndRecordsRuleName() {
        var predicate = AuthorizationPredicate.not(AuthorizationPredicate.not(
                AuthorizationPredicate.equal(
                        AuthorizationPredicate.member(AuthorizationPredicate.resourceParameter("resource"), "TenantId"),
                        AuthorizationPredicate.member(AuthorizationPredicate.contextParameter("user"), "TenantId"))));
    ...
    @Test void ruleCannotApplyWithoutPrecondition() {
        var rule = new AuthorizationCanonicalizationRule();
    ...
    @Test void unknownSecurityObligationFailsClosed() {
        var rule = new IPlanRewriteRule() {
            public String name(){return "test.unknown-obligation";}
            public List<String> preconditions(){return List.of();}
            public List<String> securityObligations(){return List.of("security.this-does-not-exist");}
            public double costImpact(){return 0d;}
            public boolean canApply(SemanticPlan p){return true;}
            public SemanticPlan apply(SemanticPlan p){return p;}
        };
