...
/** Parity coverage for immutable delegation-chain ancestry and deterministic chain digests. */
class SecurityWarrantDelegationChainParityTest {
    private static SecurityWarrant root() {
        ...
        return SecurityWarrant.ofDefaults("root", "issuer", "subject", "audience", List.of(),
                new SecurityWarrantConstraints(), now.minusSeconds(10), now.plusSeconds(300),
                "root-nonce", "root-key", null, new byte[0]);
    }

    @Test void singleRootChainIsValid() {
    ...
    @Test void chainDigestIsDeterministic() {
        var chain = List.of(root());
