...
class ExecutionIRParityE2ETest {
	...
	void semanticPlanLowersWithoutChangingTopology() {
		var child = new SemanticPlanNode(2, ExecutionOperation.TRAVERSE, EntityId.create("OrderItem"), List.of(),
				RelationshipId.create("Order", "Items"), null, List.of());
		var root = new SemanticPlanNode(1, ExecutionOperation.SCAN, EntityId.create("Order"), List.of(), null, null,
				List.of(child));
	...
	void executionBoundaryRejectsPlansWithoutAuthorizationBinding() {
		var plan = new SemanticPlan(new SemanticPlanNode(1, ExecutionOperation.SCAN, EntityId.create("Customer"),
