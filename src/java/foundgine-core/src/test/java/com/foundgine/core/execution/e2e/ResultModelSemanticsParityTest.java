 ...
 * <ul>
 * <li>The C# original's first case
 * ({@code Materializer_returns_the_canonical_semantic_result_type}) asserts
 * {@code Assert.IsType<SemanticResult>(result)} - a runtime type check that
 * only has meaning in C# because {@code ResultMaterializer.Materialize}
 * could in principle be widened to return a base type or interface. The
 * Java port's {@code ResultMaterializer.materialize} signature already
 * declares a return type of {@code SemanticResult} (a Java {@code record},
 * not subclassable), so the type is a compile-time guarantee here and a
 * dedicated runtime-type-check test would be vacuous. That case is not
 * ported; the other two, which assert genuine behavioral contracts, are.</li>
 * <li>The existing {@code ResultMaterializationParityE2ETest} in this
 * package covers row-collapsing and null-root/child-identity handling, but
 * not the two contracts ported here: that identity fields are not required
 * to appear in the field projection to reconstruct distinct nodes, and that
 * page info / evidence survive materialization unchanged.</li>
 * </ul>
 */
class ResultModelSemanticsParityTest {
	...
	private static SemanticModel modelWithNameOnly() {
		return new SemanticModelBuilder()
				.entity(CUSTOMER, "Customer",
						e -> e.identity(ID, "Id").field(ID, "Id", Long.class).field(NAME, "Name", String.class))
				.build();
	}
	...
	void identityIsNotRequiredInTheProjectionToReconstructUniqueNodes() {
		...
		var plan = new SemanticPlan(
				new SemanticPlanNode(1, ExecutionOperation.SCAN, CUSTOMER, List.of(NAME), null, null, List.of()));

		ExecutionRow row = row(1L, "Ada");
		ExecutionRow row2 = row(2L, "Grace");
		ExecutionRow row3 = row(1L, "Ada");

		var result = new ResultMaterializer(model).materialize(plan, new ExecutionResult(List.of(row, row2, row3)));
	...
	void materializationPreservesExecutionPageInfoAndEvidence() {
		var model = modelWithNameOnly();
		var plan = new SemanticPlan(
				new SemanticPlanNode(1, ExecutionOperation.SCAN, CUSTOMER, List.of(ID), null, null, List.of()));
		...
		var materialized = new ResultMaterializer(model).materialize(plan,
				new ExecutionResult(List.of(row), pageInfo, evidence, null));

		assertEquals(new SemanticResultPageInfo("start", "end", true, false), materialized.pageInfo());
