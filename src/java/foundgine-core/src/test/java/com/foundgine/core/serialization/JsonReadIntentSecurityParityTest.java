...
class JsonReadIntentSecurityParityTest {
	...
	void agentCannotManufactureSecurityOrProviderAuthority() {
		...
		String[] forbidden = { "tenantId", "subject", "audience", "warrant", "authorization", "capabilityId",
				"provider", "connectionString", "sql" };

		for (String property : forbidden) {
			var json = "{\"rootEntity\":\"Customer\",\"selections\":[{\"field\":\"Id\"}],\"" + property
					+ "\":\"attacker-controlled\"}";
	...
	void maliciousPredicateStructureIsBoundedBeforePlanning() {
		var adapter = new JsonReadIntentAdapter(new JsonReadIntentAdapterOptions(32, 256, 4, 8, 16, true));
		String json = """
				{
				  "rootEntity":"Customer",
				  "selections":[{"field":"Id"}],
