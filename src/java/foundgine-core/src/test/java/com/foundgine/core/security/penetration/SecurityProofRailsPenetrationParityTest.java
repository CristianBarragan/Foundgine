...
class SecurityProofRailsPenetrationParityTest {
    private static ExecutionIR ir(String... inv) {
        return new ExecutionIR(
                new ExecutionIRNode(
                        ...
                        null),
                List.of(inv),
                new SemanticPlanAuthorizationBinding("c", "a"));
    }
    ...
    private static class Good
        ...
        public ProviderSecurityConformanceResult evaluate(ExecutionIR i, ProviderPlan p) {
            return new ProviderSecurityConformanceResult(
                    p.provider(),
                    i.requiredSecurityInvariants(),
                    i.requiredSecurityInvariants(),
                    List.of());
        }
    }

    private static class DeclarationOnly
    ...
    void proofCannotBeReusedAfterSecurityObligationChange() {
        ...
        assertThrows(
                ...
                () ->
                        SecurityInvariantExecutionGate.ensureExecutable(
                                p, ir(SecurityInvariantIds.TENANT_ISOLATION)));
    }
        ...
        assertThrows(
                ...
                () ->
                        SecurityInvariantProofGate.attachAndValidate(
                                new Plan("weak"),
                                ir(SecurityInvariantIds.RUNTIME_AUTHORIZATION),
                                new DeclarationOnly()));
    }
    ...
    void unknownInvariantCannotCrossCertificationBoundary() {
        assertThrows(
                ...
                () ->
                        SecurityInvariantProofGate.attachAndValidate(
                                new Plan("p1"), ir("security.attacker-controlled"), new Good()));
    }
