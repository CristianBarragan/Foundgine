    ...
    void requirementsAreDerivedFromAuthorizedShape() {
        var node =
                new SemanticPlanNode(
                        ...
                        new EntityId(1),
                        java.util.List.of(new FieldId(2)),
                        null,
                        null,
                        java.util.List.of());
        ...
        assertTrue(
                plan.requiredSecurityInvariants()
                        .contains(SecurityInvariantIds.PLAN_CACHE_CONTEXT_ISOLATION));
        assertTrue(
                plan.requiredSecurityInvariants().contains(SecurityInvariantIds.FIELD_VISIBILITY));
    }
    ...
    void authorizationPredicateRequiresRuntimeAuthorization() {
        var predicate =
                AuthorizationPredicate.equal(
                        AuthorizationPredicate.constant("1"), AuthorizationPredicate.constant("1"));
        var node =
                new SemanticPlanNode(
                        1,
                        ExecutionOperation.SCAN,
        ...
        assertThrows(
                ...
                () ->
                        SecurityInvariantPlanRequirements.attach(
                                new SemanticPlan(node), java.util.List.of("not-a-real-invariant")));
    }
}
