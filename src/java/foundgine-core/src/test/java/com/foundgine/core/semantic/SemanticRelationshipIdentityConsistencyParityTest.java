	...
	void sameRelationshipIdentityMustAgreeOnTarget() {
		...
		var builder = new SemanticModelBuilder()
				.entity(customer, "Customer",
						e -> e.identity(FieldId.create("Customer", "Id"), "Id").relationship(relationship, "Orders",
								order, RelationshipCardinality.MANY))
		...
		// A second declaration of the same relationship identity is expressed by an
		// overlay.
		var conflicting = new SemanticModelBuilder()
				.entity(customer, "Customer",
						e -> e.identity(FieldId.create("Customer", "Id"), "Id").relationship(relationship, "Orders",
								invoice, RelationshipCardinality.MANY))
				.entity(invoice, "Invoice", e -> e.identity(FieldId.create("Invoice", "Id"), "Id")).build();
	...
	void sameRelationshipIdentityMustAgreeOnCardinality() {
		...
		var first = new SemanticModelBuilder()
				.entity(customer, "Customer",
						e -> e.identity(FieldId.create("Customer", "Id"), "Id").relationship(relationship, "Orders",
								order, RelationshipCardinality.MANY))
				.entity(order, "Order", e -> e.identity(FieldId.create("Order", "Id"), "Id")).build();
		var second = new SemanticModelBuilder()
				.entity(customer, "Customer",
						e -> e.identity(FieldId.create("Customer", "Id"), "Id").relationship(relationship, "Orders",
								order, RelationshipCardinality.ONE))
				.entity(order, "Order", e -> e.identity(FieldId.create("Order", "Id"), "Id")).build();

		assertThrows(IllegalStateException.class,
				() -> new SemanticModelBuilder().importModel(first).overlay(second).build());
	}
}
