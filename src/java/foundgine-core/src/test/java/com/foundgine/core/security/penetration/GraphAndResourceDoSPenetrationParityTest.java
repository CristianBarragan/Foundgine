...
/**
 * Port of GraphAndResourceDoSPenetrationTests: hostile nested transport graphs are bounded before
 * execution.
 */
class GraphAndResourceDoSPenetrationParityTest {
    @Test
    void deepRelationshipSelectionIsRejectedBeforeSemanticExecution() {
        ...
        var adapter =
                new JsonReadIntentAdapter(
                        new JsonReadIntentAdapterOptions(32, 256, 32, 256, 16, true));
        assertThrows(IllegalStateException.class, () -> adapter.parse(json));
    }

    @Test
    void deepFilterExpressionIsRejectedBeforePlanning() {
        String value = "{\"kind\":\"field\",\"field\":\"Id\",\"operator\":\"EQ\",\"value\":1}";
        for (int i = 1; i < 80; i++) value = "{\"kind\":\"and\",\"expressions\":[" + value + "]}";
        String json =
                "{\"rootEntity\":\"Customer\",\"selections\":[{\"field\":\"Id\"}],\"filter\":"
                        + value
                        + "}";
        assertThrows(
                IllegalStateException.class,
                () -> new JsonReadIntentAdapter(new JsonReadIntentAdapterOptions()).parse(json));
    }
}
