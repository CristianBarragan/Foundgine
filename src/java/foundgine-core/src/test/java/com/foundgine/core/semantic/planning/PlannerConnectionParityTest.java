    ...
    void connectionChildLowersToConnectionTraversal() {
        ...
        var child =
                new SemanticReadNode(
                        2, new EntityId(2), List.of(), null, connection, List.of(), null, null);
        var root =
                new SemanticReadNode(
                        1, new EntityId(1), List.of(), null, null, List.of(child), null, null);
        var plan = new Planner().plan(new SemanticOperation(root));
        assertEquals(ExecutionOperation.SCAN, plan.root().operation());
        assertEquals(
                ExecutionOperation.TRAVERSE_CONNECTION, plan.root().children().get(0).operation());
