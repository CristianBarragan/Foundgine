...
class SemanticQueryPaginationParityTest {
	private static SemanticEntity root() {
		return new SemanticEntity(new EntityId(1), "Customer",
				new SemanticFieldIdentity(new com.foundgine.core.abstractions.FieldId(1), "id"),
				List.<SemanticField>of(), List.<SemanticRelationship>of());
	}

	@Test
	void cursorRequiresPositiveLimit() {
	...
	@Test
	void cursorCannotBeCombinedWithOffset() {
		var options = new SemanticQueryOptions(null, List.of(), 10, 5, "cursor-1");
		assertThrows(IllegalStateException.class, () -> SemanticQueryOptionsValidator.validate(options, root()));
	}

	@Test
	void negativePaginationValuesAreRejected() {
		assertThrows(IllegalStateException.class, () -> SemanticQueryOptionsValidator
				.validate(new SemanticQueryOptions(null, List.of(), -1, null, null), root()));
		assertThrows(IllegalStateException.class, () -> SemanticQueryOptionsValidator
				.validate(new SemanticQueryOptions(null, List.of(), 10, -1, null), root()));
	}
}
