 ...
 *
 * <ul>
 *   <li>The C# original resolves this against the archived {@code
 *       Foundgine.E2E.Tests.Banking.BankingSemanticModel} fixture (Customer -&gt; Account -&gt;
 *       Transaction with sequential integer ids like {@code new EntityId(1)}). No Java port of that
 *       fixture exists anywhere in this repository (the same gap already noted by {@code
 *       AggregateExistenceSqlRenderingParityTest}), and none of the existing Java fixtures (e.g.
 *       {@code SqlCompilerParityTest}'s Customer/Account pair) go three entities deep. Rather than
 *       invent a shared Banking fixture this test builds its own minimal Customer -&gt; Account
 *       -&gt; Transaction model inline, using the name-derived {@code EntityId.create}/{@code
 *       FieldId.create} identities the Java port actually uses instead of the C# original's literal
 *       sequential ids.
 *   <li>This is a genuine gap, not a duplicate: no existing Java test drives the full {@code
 *       SemanticRequest -> SemanticRequestResolver -> SemanticAuthorizer -> Planner} pipeline with
 *       two levels of relationship traversal and asserts the resulting nested plan tree. The
 *       closest existing coverage ({@code ExecutionAlgebraInvariantParityTest}) builds a {@code
 *       SemanticGraph} directly, bypassing resolution and authorization entirely, and only goes one
 *       level deep.
 * </ul>
 */
class FoundginePipelineParityTest {
    ...
    private static final RelationshipId CUSTOMER_ACCOUNTS =
            RelationshipId.create("Customer", "Accounts");
    private static final RelationshipId ACCOUNT_TRANSACTIONS =
            RelationshipId.create("Account", "Transactions");
    ...
    private static SemanticModel model() {
        return new SemanticModelBuilder()
                .entity(
                        ...
                        "Customer",
                        e ->
                                e.identity(CUSTOMER_ID, "Id")
                                        .field(CUSTOMER_ID, "Id", Long.class)
                                        .relationship(
                                                CUSTOMER_ACCOUNTS,
                                                "Accounts",
                                                ACCOUNT,
                ...
                .entity(
                        ...
                        "Account",
                        e ->
                                e.identity(ACCOUNT_ID, "Id")
                                        .field(ACCOUNT_ID, "Id", Long.class)
                                        .relationship(
                ...
                .entity(
                        ...
                        "Transaction",
                        e ->
                                e.identity(TRANSACTION_ID, "Id")
                                        .field(TRANSACTION_ID, "Id", Long.class)
                                        .field(
                                                TRANSACTION_AMOUNT,
                                                "Amount",
    ...
    void bankingShapedThesisReachesProviderIndependentExecutionPlan() {
        var request =
                new SemanticRequest(
                        CUSTOMER,
                        List.of(
                                new SemanticSelection(CUSTOMER_ID, null, List.of()),
                                new SemanticSelection(
                                        null,
                                        CUSTOMER_ACCOUNTS,
                                        List.of(
                                                new SemanticSelection(ACCOUNT_ID, null, List.of()),
                                                new SemanticSelection(
                                                        null,
                                                        ACCOUNT_TRANSACTIONS,
                                                        List.of(
