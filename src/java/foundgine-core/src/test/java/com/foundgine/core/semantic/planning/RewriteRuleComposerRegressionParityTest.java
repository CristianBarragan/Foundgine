...
/**
 * Port of {@code RewriteRuleComposerRegressionTests}
...
class RewriteRuleComposerRegressionParityTest {
	...
	/**
	 * Mirrors the C# nested {@code EquivalentRebuildRule}: rebuilds an identical
	 * plan each pass.
	 */
	private static final class EquivalentRebuildRule implements IPlanRewriteRule {
		@Override
		public String name() {
			return "test.equivalent-rebuild";
		}
		...
		public SemanticPlan apply(SemanticPlan plan) {
			var root = plan.root();
			var rebuilt = new SemanticPlanNode(root.id(), root.operation(), root.entityId(), List.copyOf(root.fields()),
					root.viaRelationship(), root.viaConnection(), root.children(), root.queryOptions(),
					root.authorization(), root.relationshipCardinality(), root.traversalMode(), root.traversalOrder(),
