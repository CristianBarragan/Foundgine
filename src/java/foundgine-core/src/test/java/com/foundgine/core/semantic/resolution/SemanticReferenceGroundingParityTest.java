...
class SemanticReferenceGroundingParityTest {
    ...
    @Test
    void relationshipGroundingNeverInventsTargetIdentity() {
        var source =
                new ICandidateSource() {
                    public List<IdentityCandidate> findByIdentity(EntityId type, String value) {
                        return List.of();
                    }

                    public List<IdentityCandidate> findByRelationship(
                            RelationshipId relationship, String source) {
    ...
    @Test
    void missingTraversalRelationshipFailsClosed() {
        var source =
                new ICandidateSource() {
                    public List<IdentityCandidate> findByIdentity(EntityId type, String value) {
                        return List.of();
        ...
        var customer = new ResolvedReference(CUSTOMER, "C-1", 1.0, "explicit", List.of());
        var result =
                new EntityResolver(model(), source)
                        .resolveByRelationship(customer, "does-not-exist");
        assertEquals(ResolutionOutcome.NOT_FOUND, result.outcome());
    }

    @Test
    void multipleRelationshipTargetsRemainAmbiguous() {
        var source =
                new ICandidateSource() {
                    public List<IdentityCandidate> findByIdentity(EntityId type, String value) {
                        return List.of();
                    ...
                    public List<IdentityCandidate> findByRelationship(
                            RelationshipId relationship, String source) {
                        return List.of(
                                new IdentityCandidate("O-1", "one"),
                                new IdentityCandidate("O-2", "two"));
                    }
                };
        var customer = new ResolvedReference(CUSTOMER, "C-1", 1.0, "explicit", List.of());
        assertEquals(
                ...
                new EntityResolver(model(), source)
                        .resolveByRelationship(customer, "orders")
                        .outcome());
    }
}
