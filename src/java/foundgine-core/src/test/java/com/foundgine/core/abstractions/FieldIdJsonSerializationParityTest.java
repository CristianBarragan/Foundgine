 ...
 *
 * <p>The C# original is regression coverage for a bug where mutation execution results ({@code
 * IReadOnlyDictionary<FieldId, object?> ReturnedValues}, see {@code MutationResult}) threw {@code
 * NotSupportedException} the moment they were serialized, because {@code System.Text.Json}'s
 * default converter for a struct can't double as a dictionary-key converter without explicitly
 * overriding the property-name read/write hooks.
 *
 * <p>The Java port targets the equivalent Jackson failure mode: a {@link
 * com.fasterxml.jackson.databind.annotation.JsonSerialize}/{@code JsonDeserialize}-annotated record
 * used as a plain value serializes fine by default, but used as a {@code Map} key needs {@code
 * keyUsing}/{@code keyUsing} deserializer wiring or it fails — see {@link FieldId.KeySerializer}
 * and {@link FieldId.KeyDeserializer}. {@link
 * com.foundgine.core.execution.mutation.MutationResult#returnedValues()} is exactly this shape
 * ({@code Map<FieldId, Object>}), so this is not a hypothetical: it's the same class of bug the C#
...
class FieldIdJsonSerializationParityTest {
    ...
    void dictionaryKeyedByFieldIdSerializesWithoutThrowing() {
        ...
        Map<FieldId, Object> values =
                Map.of(new FieldId(1), "TRK-1'; DROP TABLE shipments; --", new FieldId(2), 42);
    ...
    void dictionaryKeyedByFieldIdRoundTripsThroughSerialization() throws Exception {
        ...
        Map<FieldId, Object> result =
                mapper.readValue(
                        json,
                        mapper.getTypeFactory()
                                .constructMapType(Map.class, FieldId.class, Object.class));
