...
/** Hostile mutation-plan parity tests: unauthorized fields, filters, and batch operations fail closed. */
class MutationAuthorizationPenetrationParityTest {
    ...
    void unauthorizedWriteFieldIsRejected() {
        ...
        var plan = new MutationPlan(List.of(new MutationOperation(
                entity, MutationKind.UPDATE,
    ...
    void unauthorizedReturnFieldIsRejected() {
        ...
        var schema = new TestSchema(entity);
        var plan = new MutationPlan(List.of(new MutationOperation(
                entity, MutationKind.UPDATE, List.of(), null, null, List.of(new FieldId(2)))));
    ...
    void unauthorizedFilterFieldIsRejected() {
        ...
        var plan = new MutationPlan(List.of(new MutationOperation(
                entity, MutationKind.DELETE, List.of(), filter)));
    ...
    void batchWithUnauthorizedOperationLastRejectsEntireBatch() {
        ...
        var graph = new SemanticMutationOperationGraph(List.of(
                SemanticMutationBuilder.update(entity.id(), List.of(new SemanticMutationField(new FieldId(1), "ok")), null, null),
                SemanticMutationBuilder.update(entity.id(), List.of(new SemanticMutationField(new FieldId(2), "attacker")), null, null)));
    ...
    private static MutationEntitySchema entity(int id, String name, Map.Entry<Integer, Integer>... fields) {
        ...
        for (var field : fields) map.put(new FieldId(field.getKey()), new ColumnId(field.getValue()));
    ...
    private static class AllowOnlyFieldPolicy extends AllowAllSemanticAuthorizationPolicy {
        ...
        AllowOnlyFieldPolicy(FieldId allowed) { this.allowed = allowed; }
        @Override public boolean canWriteEntity(EntityId entityId) { return true; }
        @Override public boolean canWriteField(EntityId entityId, FieldId fieldId) { return fieldId.equals(allowed); }
        @Override public boolean canAccessField(EntityId entityId, FieldId fieldId) { return fieldId.equals(allowed); }
    }

    private record TestSchema(MutationEntitySchema entity) implements MutationSchema {
        @Override public MutationEntitySchema getEntity(EntityId entityId) {
            if (entity.id().equals(entityId)) return entity;
            throw new NoSuchElementException();
        }
        @Override public MutationRelationshipSchema getRelationship(RelationshipId relationshipId) {
            throw new NoSuchElementException();
        }
    }
}
