...
class AuthorizationPreservationParityTest {
    ...
    @Test void identicalSecurityContractIsPreserved() {
        var source = plan(List.of(SecurityInvariantIds.TENANT_ISOLATION, SecurityInvariantIds.AUTHORIZATION_REQUIRED));
        var rewritten = plan(List.of(SecurityInvariantIds.AUTHORIZATION_REQUIRED, SecurityInvariantIds.TENANT_ISOLATION));
    ...
    @Test void droppedSecurityInvariantIsDetected() {
        var source = plan(List.of(SecurityInvariantIds.TENANT_ISOLATION, SecurityInvariantIds.AUTHORIZATION_REQUIRED));
