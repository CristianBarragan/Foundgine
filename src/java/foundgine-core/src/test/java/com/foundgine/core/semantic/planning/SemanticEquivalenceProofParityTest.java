...
class SemanticEquivalenceProofParityTest {
    ...
    void optimizerProducesSatisfiedSemanticEquivalenceProof() {
        ...
        var plan =
                new SemanticPlan(
                        nodeWithAuthorization(predicate),
    ...
    void meaningfulFieldChangeIsNotSemanticallyEquivalent() {
        ...
        assertThrows(
                IllegalStateException.class, () -> SemanticEquivalenceProof.create(first, second));
    }
        ...
        var second =
                new SemanticPlan(
                        node(
                                List.of(new FieldId(1)),
                                null,
                                new SemanticQueryOptions(null, List.of(), 20, null, null)));
    ...
    void securityContractChangeIsNotSemanticallyEquivalent() {
        var first =
                new SemanticPlan(
                        node(List.of(new FieldId(1)), null),
                        List.of(SecurityInvariantIds.AUTHORIZATION_REQUIRED),
                        null);
        var second =
                new SemanticPlan(
                        node(List.of(new FieldId(1)), null),
                        List.of(SecurityInvariantIds.TENANT_ISOLATION),
                        null);
    ...
    private static SemanticPlanNode node(
            List<FieldId> fields, AuthorizationPredicate authorization) {
