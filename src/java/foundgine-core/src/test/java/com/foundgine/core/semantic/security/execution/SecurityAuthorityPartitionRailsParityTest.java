...
/** Mirrors the C# authority-partition rails: authority-bearing cache keys must never alias. */
class SecurityAuthorityPartitionRailsParityTest {
    @Test void subjectChangeChangesPartition() {
        var first = context("agent-a", "foundgine", "tenant-a", "customer/1", warrant("w1", "n1"));
        var second = new SecurityExecutionContext(first.warrant(), "agent-b", first.audience(), first.tenant(), first.resourceScope());
        assertNotEquals(first.authorityCachePartition(), second.authorityCachePartition());
    }

    @Test void audienceChangeChangesPartition() {
        var first = context("agent", "audience-a", "tenant-a", null, warrant("w1", "n1"));
        var second = new SecurityExecutionContext(first.warrant(), first.subject(), "audience-b", first.tenant(), first.resourceScope());
        assertNotEquals(first.authorityCachePartition(), second.authorityCachePartition());
    }

    @Test void tenantChangeChangesPartition() {
        var first = context("agent", "foundgine", "tenant-a", null, warrant("w1", "n1"));
        var second = new SecurityExecutionContext(first.warrant(), first.subject(), first.audience(), "tenant-b", first.resourceScope());
        assertNotEquals(first.authorityCachePartition(), second.authorityCachePartition());
    }

    @Test void resourceScopeChangeChangesPartition() {
        var first = context("agent", "foundgine", "tenant-a", "customer/1", warrant("w1", "n1"));
        var second = new SecurityExecutionContext(first.warrant(), first.subject(), first.audience(), first.tenant(), "customer/2");
        assertNotEquals(first.authorityCachePartition(), second.authorityCachePartition());
    }

    @Test void warrantDigestChangeChangesPartition() {
        var first = context("agent", "foundgine", "tenant-a", "customer/1", warrant("w1", "n1"));
    ...
    @Test void nullAndExplicitContextValuesDoNotAlias() {
        var w = warrant("w", "n");
    ...
    @Test void delimiterCharactersCannotAliasDifferentFields() {
        var w = warrant("w", "n");
    ...
    @Test void sameWarrantAndContextPartitionIdentically() {
    ...
    private static SecurityExecutionContext context(String subject, String audience, String tenant, String resource, SecurityWarrant w) {
    ...
    private static SecurityWarrant warrant(String id, String nonce) {
        return SecurityWarrant.ofDefaults(id, "issuer", "agent", "foundgine",
                ...
                new SecurityWarrantConstraints(), Instant.now().minusSeconds(60),
                Instant.now().plusSeconds(3600), nonce, "key-1", null, new byte[0]);
    }
}
