...
class SecurityExecutionContextTest {
	private static SecurityWarrant warrant() {
		return SecurityWarrant.ofDefaults("w1", "issuer", "subject", "audience", List.<CapabilityGrant>of(),
				new SecurityWarrantConstraints(), Instant.now().minusSeconds(1), Instant.now().plusSeconds(60), "nonce",
				"key", null, new byte[0]);
	}
	...
	void providerFailsClosedWhenContextMissing() {
		...
		assertThrows(SecurityException.class, () -> SecurityExecutionContextProviderExtensions
				.requireSecurityExecutionContext(provider, "MCP", "execution"));
	}
	...
	void providerReturnsHostContext() {
		...
		assertSame(expected, SecurityExecutionContextProviderExtensions.requireSecurityExecutionContext(provider, "MCP",
				"execution"));
	}
}
