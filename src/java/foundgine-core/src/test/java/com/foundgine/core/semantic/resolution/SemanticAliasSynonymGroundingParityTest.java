...
/**
 * Port of {@code Foundgine.Core.Semantic.Tests.SemanticAliasSynonymGroundingTests}.
 *
 * <p>Case study proving that a synonym only grounds to the same interpretation as the canonical
 * word if it survives both hops of the real architecture: declared as an alias on the semantic
 * contract (folded into the lexicon by {@link SemanticLexiconProjection}), and matched by whatever
 * sits behind the candidate source ({@link ISemanticLexicalCandidateSource}). This builds a minimal
 * but real contract (Supplier aliased "Seller", PurchaseOrder aliased "Buys"), projects it with the
 * production {@code SemanticLexiconProjection}, and backs {@link
 * SemanticLexicalResolver#ground(String)} with a source that matches a token against either an
 * entry's canonical name or its declared aliases — the same contract the canonical name is matched
 * against.
 */
class SemanticAliasSynonymGroundingParityTest {
    ...
    void buysGroundsToTheSameInterpretationAsPurchaseOrder() {
        ...
        assertEquals(
                "PurchaseOrder", canonical.committed().steps().get(0).candidate().canonicalName());
    ...
    void fullParaphraseGroundsEveryTokenTheSameWayAsTheCanonicalSentence() {
        ...
        for (var pair :
                List.of(
                        new Pair("purchase order", "buys", PURCHASE_ORDER),
        ...
        var model =
                new SemanticModelBuilder()
                        .entity(
                                SUPPLIER,
                                "Supplier",
                                e -> e.alias("Seller").identity(new FieldId(1), "Id"))
                        .entity(
                                PURCHASE_ORDER,
                                "PurchaseOrder",
                                e -> e.alias("Buys").identity(new FieldId(2), "Id"))
                        .build()
                        .freeze();
    ...
    /**
     * Stand-in for a real retrieval provider: matches a token against either an entry's canonical
     * name or any of its declared aliases, exactly the lookup an Elasticsearch/pgvector index built
     * from {@code SemanticLexiconProjection} output performs.
     */
    private record AliasAwareLexicalSource(List<SemanticLexiconEntry> lexicon)
            implements ISemanticLexicalCandidateSource {
        @Override
        public List<SemanticLexicalCandidate> retrieve(SemanticLexicalRequest request) {
            return lexicon.stream()
                    .filter(entry -> request.effectiveKinds().contains(entry.kind()))
                    .filter(
                            entry ->
                                    entry.canonicalName().equalsIgnoreCase(request.token())
                                            || entry.effectiveAliases().stream()
                                                    .anyMatch(
                                                            a ->
                                                                    a.equalsIgnoreCase(
                                                                            request.token())))
