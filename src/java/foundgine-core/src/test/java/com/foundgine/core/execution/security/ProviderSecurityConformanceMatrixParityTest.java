...
class ProviderSecurityConformanceMatrixParityTest {
    private static final List<String> QUERY_REQUIREMENTS = List.of(
    ...
    private static final List<String> TRANSFER_REQUIREMENTS = List.of(
    ...
    void lyingProviderCannotSatisfyUnclaimedRequirement() {
        ...
        var proof = matrix.evaluate("hostile-provider", List.of(
                SecurityInvariantIds.AUTHORIZATION_REQUIRED, SecurityInvariantIds.TENANT_ISOLATION));
        assertFalse(proof.isSatisfied());
    ...
    void providerClaimsCannotInventUnknownInvariants() {
        assertThrows(IllegalStateException.class, () -> new ProviderSecurityConformanceMatrix().register(
                new ProviderSecurityConformanceProfile("hostile-provider",
