...
/**
 * Penetration-style parity tests for the agent -> semantic model -> authorization -> planner
 * boundary. Inputs represent hostile model/MCP output and must fail closed before provider
 * execution.
 */
class SemanticBoundaryPenetrationParityTest {
    ...
    void deniedFieldIsRemovedBeforePlanning() {
                        ...
                        .plan(
                                new com.foundgine.core.semantic.ir.SemanticOperation(
                                        new SemanticReadNode(
                                                0,
                                                new EntityId(1),
                                                List.of(new FieldId(1), new FieldId(2)),
                                                null,
                                                null,
                                                List.of(),
                                                null,
        ...
        var authorizedOperation =
                new com.foundgine.core.semantic.ir.SemanticOperation(
                        new SemanticReadNode(
                                authorizedNode.id(),
                                authorizedNode.entityId(),
                                authorizedNode.fields(),
                                authorizedNode.viaRelationship(),
                                authorizedNode.viaConnection(),
                                List.of(),
                                authorized.options(),
                                authorizedNode.authorization()));
    ...
    void deniedRelationshipRemovesEntireDescendantSubtree() {
        var grandchild =
                new SemanticReadNode(
                        2,
                        new EntityId(3),
                        List.of(new FieldId(3)),
                        new RelationshipId(11),
                        null,
                        List.of(),
                        null,
                        null);
    ...
    void authorizationPredicateSurvivesSemanticPlanning() {
        ...
        var authorized =
                new SemanticAuthorizer(new PredicatePolicy(predicate)).authorize(operation);
        var plan = new Planner().plan(authorized);
    ...
    void authorizationPredicateChangesPlanFingerprint() {
        ...
        var planA =
                planner.plan(new SemanticAuthorizer(new PredicatePolicy(a)).authorize(operationA));
        var planB =
                planner.plan(new SemanticAuthorizer(new PredicatePolicy(b)).authorize(operationB));

        assertNotEquals(
                SemanticPlanFingerprint.create(planA), SemanticPlanFingerprint.create(planB));
        assertNotEquals(
                SemanticPlanFingerprint.createShapeKey(planA),
                SemanticPlanFingerprint.createShapeKey(planB));
    }
    ...
    private static class RootOnlyPolicy extends AllowAllSemanticAuthorizationPolicy {
        ...
        @Override
        public boolean canAccessRelationship(
                EntityId sourceEntityId, RelationshipId relationshipId) {
            return false;
        }
    }

    private record PredicatePolicy(AuthorizationPredicate predicate)
            implements ISemanticAuthorizationPolicy {
        ...
        @Override
        public AuthorizationDecision getFieldAccess(
                EntityId entityId, FieldId fieldId, AuthorizationOperation operation) {
            return AuthorizationDecision.ALLOWED;
        }
