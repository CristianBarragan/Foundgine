...
/**
 * Penetration-style parity tests for the agent -> semantic model -> authorization -> planner boundary.
 * Inputs represent hostile model/MCP output and must fail closed before provider execution.
 */
class SemanticBoundaryPenetrationParityTest {
    ...
    void deniedFieldIsRemovedBeforePlanning() {
        ...
        var plan = new Planner().plan(new com.foundgine.core.semantic.ir.SemanticOperation(
                new SemanticReadNode(
                        0, new EntityId(1), List.of(new FieldId(1), new FieldId(2)),
                        null, null, List.of(), null, null)));

        // Plan the authorized graph through the same canonical operation boundary used by the runtime.
    ...
    void deniedRelationshipRemovesEntireDescendantSubtree() {
        var grandchild = new SemanticReadNode(2, new EntityId(3), List.of(new FieldId(3)),
                new RelationshipId(11), null, List.of(), null, null);
        var child = new SemanticReadNode(1, new EntityId(2), List.of(new FieldId(2)),
                new RelationshipId(10), null, List.of(grandchild), null, null);
        var operation = new com.foundgine.core.semantic.ir.SemanticOperation(
                new SemanticReadNode(0, new EntityId(1), List.of(new FieldId(1)), null, null,
                        List.of(child), null, null));
    ...
    private static class DenyAllPolicy extends AllowAllSemanticAuthorizationPolicy {
        @Override public boolean canAccessEntity(EntityId entityId) { return false; }
    }

    private static class FieldAllowPolicy extends AllowAllSemanticAuthorizationPolicy {
        @Override public boolean canAccessField(EntityId entityId, FieldId fieldId) {
            return fieldId.equals(new FieldId(2));
        }
    }

    private static class RootOnlyPolicy extends AllowAllSemanticAuthorizationPolicy {
        @Override public boolean canAccessEntity(EntityId entityId) { return entityId.equals(new EntityId(1)); }
        @Override public boolean canAccessRelationship(EntityId sourceEntityId, RelationshipId relationshipId) { return false; }
    }

    private record PredicatePolicy(AuthorizationPredicate predicate) implements ISemanticAuthorizationPolicy {

        @Override public boolean canAccessEntity(EntityId entityId) { return true; }
        @Override public boolean canAccessField(EntityId entityId, FieldId fieldId) { return true; }
        @Override public boolean canAccessRelationship(EntityId entityId, RelationshipId relationshipId) { return true; }
        @Override public boolean canWriteEntity(EntityId entityId) { return true; }
        @Override public boolean canWriteField(EntityId entityId, FieldId fieldId) { return true; }
        @Override public boolean canWriteRelationship(EntityId entityId, RelationshipId relationshipId) { return true; }
        @Override public AuthorizationPredicate getPredicate(EntityId entityId, AuthorizationOperation operation) {
            return operation == AuthorizationOperation.READ ? predicate : null;
        }
        @Override public AuthorizationDecision getEntityAccess(EntityId entityId, AuthorizationOperation operation) {
            return AuthorizationDecision.ALLOWED;
        }
        @Override public AuthorizationDecision getEntityAccess(EntityId entityId, AuthorizationOperation operation, AuthorizationOperationName operationName) {
            return AuthorizationDecision.ALLOWED;
        }
        @Override public AuthorizationDecision getFieldAccess(EntityId entityId, FieldId fieldId, AuthorizationOperation operation) {
            return AuthorizationDecision.ALLOWED;
        }
        @Override public AuthorizationDecision getRelationshipAccess(EntityId entityId, RelationshipId relationshipId, AuthorizationOperation operation) {
            return AuthorizationDecision.ALLOWED;
        }
    }
}
