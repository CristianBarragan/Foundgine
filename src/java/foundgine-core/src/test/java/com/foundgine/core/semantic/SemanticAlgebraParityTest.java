...
class SemanticAlgebraParityTest {
	...
	void logicalNormalizationFlattensSortsAndDeduplicates() {
		var a = new SemanticExpression.Binary("eq", new SemanticExpression.FieldReference(new FieldId(1), INT32),
				new SemanticExpression.Literal(SemanticValue.from(1), INT32),
				SemanticExpression.SemanticExpressionTypes.BOOLEAN);
		var b = new SemanticExpression.Binary("eq", new SemanticExpression.FieldReference(new FieldId(2), INT32),
				new SemanticExpression.Literal(SemanticValue.from(2), INT32),
				SemanticExpression.SemanticExpressionTypes.BOOLEAN);

		var expression = new SemanticExpression.Logical(SemanticExpression.LogicalOperator.AND, java.util.List.of(
				new SemanticExpression.Logical(SemanticExpression.LogicalOperator.AND, java.util.List.of(b, a)), a));
		...
		assertEquals(SemanticExpressionNormalizer.canonicalize(normalized.operands().get(0)), normalized.operands()
				.stream().map(SemanticExpressionNormalizer::canonicalize).sorted().findFirst().orElseThrow());
	}
	...
	void countDoesNotRequireAFieldArgument() {
		var source = new SemanticExpression.FieldReference(new FieldId(7),
				new SemanticType.CollectionType(new SemanticType.ObjectType("Order")));

		var expression = new SemanticExpression.Aggregate(SemanticExpression.AggregateExpressionKind.COUNT, source,
				null);
