...
class MutationPlanningBoundaryParityTest {
    private static MutationSchema schema() {
        var entity = new MutationEntitySchema(new EntityId(1), "Customer",
                Set.of(new ColumnId(1), new ColumnId(2)),
                Map.of(new FieldId(1), new ColumnId(1), new FieldId(2), new ColumnId(2)),
                new ColumnId(1));
        return new MutationSchema() {
            public MutationEntitySchema getEntity(EntityId id){return entity;}
            public MutationRelationshipSchema getRelationship(RelationshipId id){throw new NoSuchElementException(id.toString());}
        };
    }

    @Test void plannerConsumesNarrowMutationSchema() {
        var intent = new MutationIntent(new EntityId(1), MutationKind.CREATE,
                List.of(new MutationFieldValue(new ColumnId(2), "Ada")), null,
    ...
    @Test void plannerRejectsUnfilteredUpdate() {
    ...
    @Test void plannerRejectsUnknownReturnField() {
        var intent = new MutationIntent(new EntityId(1), MutationKind.CREATE,
