...
class SecurityCapabilityCompositionParityTest {
    ...
    private static SecurityWarrant warrant() {
        var now = Instant.now();
        return SecurityWarrant.ofDefaults("w1", "issuer", "agent", "audience",
                List.of(new CapabilityGrant("orders.read", "read", List.of("orders/*"))),
                new SecurityWarrantConstraints(List.of("tenant-a"), List.of(), List.of("orders/*"), List.of("read"), null, null),
    ...
    void duplicateCapabilitiesDoNotBroadenComposedAuthority() {
        var result = SecurityCapabilityComposition.validate(
                List.of(capability("orders.read", "read"), capability("orders.read", "read")),
                warrant(), "agent", "audience", "tenant-a", "orders/*");
    ...
    void crossTenantCompositionFailsClosed() {
        var result = SecurityCapabilityComposition.validate(
                List.of(capability("orders.read", "read")),
                warrant(), "agent", "audience", "tenant-b", "orders/*");
    ...
    void writeCapabilityCannotBeSmuggledIntoReadOnlyWarrant() {
        var result = SecurityCapabilityComposition.validate(
                List.of(capability("orders.write", "write")),
                warrant(), "agent", "audience", "tenant-a", "orders/*");
