...
class SemanticApproximateRetrievalParityTest {
	...
	private SemanticModel model() {
		return new SemanticModelBuilder()
				.entity(PRODUCT, "Product", e -> e.identity("Id").field(NAME, "Name", String.class)).build().freeze();
	}

	@Test
	void relationalRetrievalDoesNotInvokeApproximateSource() {
		...
		var result = new EntityResolver(model(), source)
				.retrieve(new SemanticRetrievalRequest(PRODUCT, NAME, "widget", RetrievalStrategy.RELATIONAL));
		assertTrue(result.isEmpty());
	}

	@Test
	void approximateRetrievalIsEntityBoundedSortedAndLimited() {
		var source = (IApproximateCandidateSource) request -> List.of(new RetrievalCandidate(PRODUCT, "2", .70),
				new RetrievalCandidate(EntityId.create("Other"), "x", .99), new RetrievalCandidate(PRODUCT, "1", .90),
				new RetrievalCandidate(PRODUCT, "3", .80));
		var result = new EntityResolver(model(), source)
				.retrieve(new SemanticRetrievalRequest(PRODUCT, NAME, "widget", RetrievalStrategy.SEARCH, 2));
		assertEquals(List.of("1", "3"), result.stream().map(RetrievalCandidate::recordId).toList());
	}

	@Test
	void retrievalRejectsInvalidLimits() {
