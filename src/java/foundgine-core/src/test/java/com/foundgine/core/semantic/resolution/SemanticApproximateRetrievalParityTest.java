...
class SemanticApproximateRetrievalParityTest {
    ...
    void relationalRetrievalDoesNotInvokeApproximateSource() {
        var source =
                new IApproximateCandidateSource() {
                ...
                new EntityResolver(model(), source)
                        .retrieve(
                                new SemanticRetrievalRequest(
                                        PRODUCT, NAME, "widget", RetrievalStrategy.RELATIONAL));
    ...
    void approximateRetrievalIsEntityBoundedSortedAndLimited() {
        ...
        var result =
                new EntityResolver(model(), source)
                        .retrieve(
                                new SemanticRetrievalRequest(
                                        PRODUCT, NAME, "widget", RetrievalStrategy.SEARCH, 2));
    ...
    void retrievalRejectsInvalidLimits() {
        assertThrows(
                ...
                () ->
                        new SemanticRetrievalRequest(
                                PRODUCT, NAME, "widget", RetrievalStrategy.VECTOR, 0));
        assertThrows(
                IllegalArgumentException.class,
                () ->
                        new SemanticRetrievalRequest(
                                PRODUCT, NAME, "widget", RetrievalStrategy.VECTOR, 1001));
    }
}
