...
class ResultMaterializationParityE2ETest {
    ...
    private SemanticModel model() {
        return new SemanticModelBuilder()
            ...
            .entity(ORDER, "Order", e -> e.identity(ORDER_ID, "Id")
                .field(ORDER_ID, "Id", Integer.class)
    ...
    void materializerCollapsesRepeatedProviderRowsIntoOneRoot() {
        var child = new SemanticPlanNode(2, ExecutionOperation.TRAVERSE, ORDER,
            List.of(ORDER_ID, ORDER_NUMBER), ORDERS, null, List.of());
        var root = new SemanticPlanNode(1, ExecutionOperation.SCAN, CUSTOMER,
            List.of(CUSTOMER_ID, CUSTOMER_NAME), null, null, List.of(child));
    ...
    void materializerRejectsNullRootIdentityButIgnoresNullChildIdentity() {
        var child = new SemanticPlanNode(2, ExecutionOperation.TRAVERSE, ORDER,
            List.of(ORDER_ID), ORDERS, null, List.of());
        var root = new SemanticPlanNode(1, ExecutionOperation.SCAN, CUSTOMER,
            List.of(CUSTOMER_ID), null, null, List.of(child));
        var plan = new SemanticPlan(root);
        ...
        assertThrows(IllegalStateException.class, () -> new ResultMaterializer(model()).materialize(
            plan, new ExecutionResult(List.of(new ExecutionRow(Map.of(), missingRoot)))));
    }
}
