    ...
    void materializerCollapsesRepeatedProviderRowsIntoOneRoot() {
        ...
        var result =
                new ResultMaterializer(model())
                        .materialize(plan, new ExecutionResult(List.of(row1, row2)));
        ...
        assertEquals(
                "PO-101",
                result.roots().get(0).children().get(ORDERS).get(0).values().get(ORDER_NUMBER));
    }
        ...
        var materialized =
                new ResultMaterializer(model())
                        .materialize(
                                plan,
                                new ExecutionResult(
                                        List.of(new ExecutionRow(Map.of(), rootCells))));
        assertEquals(1, materialized.roots().size());
        assertTrue(
                materialized.roots().get(0).children().getOrDefault(ORDERS, List.of()).isEmpty());
                ...
                () ->
                        new ResultMaterializer(model())
                                .materialize(
                                        plan,
                                        new ExecutionResult(
                                                List.of(new ExecutionRow(Map.of(), missingRoot)))));
    }
}
