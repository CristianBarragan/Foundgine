...
class SecurityWarrantDelegationStateMachineParityTest {
    private static SecurityWarrant warrant() {
        ...
        return SecurityWarrant.ofDefaults("w-state", "issuer", "agent", "audience", List.of(),
                ...
                "nonce-state", "key-state", null, new byte[0]);
    }

    @Test void registrationCreatesActiveSnapshot() {
        var w = warrant();
    ...
    @Test void revokedWarrantCannotDelegate() {
        var w = warrant();
        var machine = new SecurityWarrantDelegationStateMachine();
    ...
    @Test void compromisedWarrantCannotReturnToActiveThroughKeyRotation() {
        var w = warrant();
        var machine = new SecurityWarrantDelegationStateMachine();
        machine.register(w, "key-1");
    ...
    @Test void keyRotationAdvancesSequenceWithoutChangingAuthorityState() {
        var w = warrant();
        var machine = new SecurityWarrantDelegationStateMachine();
        machine.register(w, "key-1");
