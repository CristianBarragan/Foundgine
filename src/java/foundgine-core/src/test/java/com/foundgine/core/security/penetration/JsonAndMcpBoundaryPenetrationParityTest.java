...
class JsonAndMcpBoundaryPenetrationParityTest {
    @Test
    void agentCannotSupplyAuthorityProviderOrSqlControls() {
        String[] forbidden = {
            "tenantId", "userId", "authorization", "provider", "connectionString", "sql"
        };
    ...
    @Test
    void relaxedUnknownPropertyModeStillDiscardsSecurityControls() {
        var a =
        ...
        assertNull(i.security());
    }

    @Test
    void oversizedSelectionFanoutIsRejectedAtTransportBoundary() {
        StringBuilder b = new StringBuilder("{\"rootEntity\":\"Customer\",\"selections\":[");
        for (int i = 1; i <= 101; i++) {
            if (i > 1) b.append(',');
            b.append("{\"field\":\"F").append(i).append("\"}");
        }
        b.append("]}");
        assertThrows(
                IllegalStateException.class,
                () ->
                        new JsonReadIntentAdapter(
                                        new JsonReadIntentAdapterOptions(
                                                32, 100, 32, 256, 16, true))
                                .parse(b.toString()));
    }
}
