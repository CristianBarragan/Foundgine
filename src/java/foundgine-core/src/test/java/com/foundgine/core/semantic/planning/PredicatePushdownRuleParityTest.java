...
class PredicatePushdownRuleParityTest {
    ...
    void distributesConjunctIntoOrBranches() {
        ...
        assertTrue(rewritten.expressions().stream().allMatch(expression ->
            expression instanceof SemanticAndFilter and && and.expressions().contains(c)));
    }
    ...
    void rewritePreservesSecurityInvariants() {
        ...
        var before = new SemanticPlan(planNode(filter), List.of(
            SecurityInvariantIds.AUTHORIZATION_REQUIRED,
            SecurityInvariantIds.RUNTIME_AUTHORIZATION), null);
        var after = new PredicatePushdownRule().apply(before);
    ...
    private static SemanticPlanNode planNode(SemanticFilterExpression filter) {
        return new SemanticPlanNode(1, ExecutionOperation.SCAN, new EntityId(1),
            List.of(new FieldId(1), new FieldId(2), new FieldId(3)),
            null, null, List.of(), new SemanticQueryOptions(filter, List.of(), null, null, null), null,
