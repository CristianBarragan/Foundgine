...
class PredicatePushdownRuleParityTest {
    ...
    void distributesConjunctIntoOrBranches() {
        ...
        var rewritten =
                assertInstanceOf(SemanticOrFilter.class, result.root().queryOptions().filter());
        assertEquals(2, rewritten.expressions().size());
        assertTrue(
                rewritten.expressions().stream()
                        .allMatch(
                                expression ->
                                        expression instanceof SemanticAndFilter and
                                                && and.expressions().contains(c)));
    }
                                ...
                                new SemanticOrFilter(
                                        List.of(
                                                ...
                                                new SemanticFieldFilter(
                                                        ...
                                                        "B"))),
                                new SemanticFieldFilter(
                                        new FieldId(3), SemanticFilterOperator.EQ, "C")));
        ...
        var filter =
                new SemanticAndFilter(
                        List.of(
                                ...
                                new SemanticFieldFilter(
                                        new FieldId(100), SemanticFilterOperator.EQ, 100)));
