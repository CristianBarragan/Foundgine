...
class MutationResultMaterializationParityTest {
	...
	private SemanticModel model() {
		return new SemanticModelBuilder()
				.entity(CUSTOMER, "Customer",
						e -> e.identity(CUSTOMER_ID, "Id").field(CUSTOMER_ID, "Id", Integer.class).relationship(ORDERS,
								"orders", ORDER, RelationshipCardinality.MANY))
				.entity(ORDER, "Order", e -> e.identity(ORDER_ID, "Id").field(ORDER_ID, "Id", Integer.class)).build();
	}

	@Test
	void wrongOperationCountFailsClosed() {
		var intent = new NestedMutationIntent(new MutationIntent(CUSTOMER, MutationKind.CREATE, List.of()), List.of());
		var materializer = new MutationResultMaterializer(model());
		assertThrows(IllegalStateException.class,
				() -> materializer.materialize(intent, new MutationBatchResult(List.of())));
	}
	...
	void batchMaterializationRejectsUnaccountedResults() {
		...
		assertThrows(IllegalStateException.class,
				() -> materializer.materializeBatch(List.of(item),
						new MutationBatchResult(List.of(new MutationResult(1, Map.of(CUSTOMER_ID, 1)),
