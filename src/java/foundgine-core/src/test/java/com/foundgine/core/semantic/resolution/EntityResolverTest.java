...
class EntityResolverTest {
                ...
                .entity(
                        ...
                        e ->
                                e.identity(CUSTOMER_ID, "Id")
                                        .field(CUSTOMER_ID, "Id", String.class)
                                        .relationship(
                                                ORDERS,
                                                "orders",
                                                ORDER,
                ...
                new ICandidateSource() {
                    public List<IdentityCandidate> findByIdentity(EntityId type, String value) {
                        return type.equals(CUSTOMER)
                                ? List.of(new IdentityCandidate(value, value))
                                : List.of();
                    }
    ...
    void multipleCandidatesAreAmbiguous() {
        var source =
                new ICandidateSource() {
                    public List<IdentityCandidate> findByIdentity(EntityId type, String value) {
                        return List.of(
                                new IdentityCandidate("1", "one"),
                                new IdentityCandidate("2", "two"));
                    }
    ...
    void contractLexiconProjectsCanonicalNamesAndAliases() {
        ...
        assertTrue(
                candidates.stream().anyMatch(c -> c.kind() == SemanticLexicalCandidateKind.ENTITY));
    }
}
