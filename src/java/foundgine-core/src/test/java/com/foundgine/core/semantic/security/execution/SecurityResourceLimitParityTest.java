...
/** Protocol-neutral resource-limit parity: limits apply before planning regardless of transport. */
class SecurityResourceLimitParityTest {
    ...
    @Test void filterNodeCountIsBounded() {
    ...
    private static SecurityResourceLimits withPageSize(SecurityResourceLimits d, int v) { return copy(d, v, d.maxOffset(), d.maxCursorLength(), d.maxFilterNodes(), d.maxOrderPathDepth(), d.maxSelectionDepth()); }
    private static SecurityResourceLimits withOrderPathDepth(SecurityResourceLimits d, int v) { return copy(d, d.maxPageSize(), d.maxOffset(), d.maxCursorLength(), d.maxFilterNodes(), v, d.maxSelectionDepth()); }
    private static SecurityResourceLimits withFilterNodes(SecurityResourceLimits d, int v) { return copy(d, d.maxPageSize(), d.maxOffset(), d.maxCursorLength(), v, d.maxOrderPathDepth(), d.maxSelectionDepth()); }
    private static SecurityResourceLimits withCursorLength(SecurityResourceLimits d, int v) { return copy(d, d.maxPageSize(), d.maxOffset(), v, d.maxFilterNodes(), d.maxOrderPathDepth(), d.maxSelectionDepth()); }
    private static SecurityResourceLimits copy(SecurityResourceLimits d, int page, int offset, int cursor, int filter, int orderDepth, int selectionDepth) {
        return limits(selectionDepth, d.maxOperationGraphNodes(), d.maxOperationGraphDepth(), d.maxOperationGraphEdges(), d.maxOperationGraphFields(),
                d.maxSelectionNodes(), d.maxFilterDepth(), filter, d.maxOrderTerms(), orderDepth, page, offset, cursor,
                d.maxMutationOperations(), d.maxMutationFieldsPerOperation(), d.maxMutationReturnFieldsPerOperation(), d.maxMutationDependencies(), d.maxMutationEffects());
    }
    private static SecurityResourceLimits limits(int selectionDepth, int graphNodes, int graphDepth, int graphEdges, int graphFields,
                                                   int selectionNodes, int filterDepth, int filterNodes, int orderTerms, int orderPathDepth,
                                                   int pageSize, int offset, int cursorLength, int mutationOps, int mutationFields,
                                                   int mutationReturns, int mutationDeps, int mutationEffects) {
        return new SecurityResourceLimits(selectionDepth, graphNodes, graphDepth, graphEdges, graphFields, selectionNodes,
                filterDepth, filterNodes, orderTerms, orderPathDepth, pageSize, offset, cursorLength, mutationOps,
                mutationFields, mutationReturns, mutationDeps, mutationEffects);
    }
}
