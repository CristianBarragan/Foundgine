...
class MutationExecutionSecurityGateParityTest {
    @Test void certificateIsBoundToExactMutationIrAndProviderInstance() {
        ...
        var irEx = assertThrows(IllegalStateException.class, () -> MutationExecutionSecurityGate.ensureExecutable(cloned, provider, certificate));
        assertTrue(irEx.getMessage().toLowerCase().contains("exact mutation ir"));

        var providerEx = assertThrows(IllegalStateException.class, () -> MutationExecutionSecurityGate.ensureExecutable(ir, new HonestProvider(), certificate));
        assertTrue(providerEx.getMessage().toLowerCase().contains("exact mutation ir and provider"));
    }

    @Test void providerWithoutConcreteEvaluatorCannotSatisfyProviderOwnedInvariant() {
        var ir = testIr(SecurityInvariantIds.PARAMETERIZED_VALUES);
        var ex = assertThrows(IllegalStateException.class, () ->
                MutationExecutionSecurityGate.certify(ir, new UncertifiedProvider(), UncertifiedProvider.class.getName(), List.of()));
        assertTrue(ex.getMessage().toLowerCase().contains("no concrete security conformance evaluator"));
    }

    @Test void providerConformanceCombinesWithUpstreamEvidence() {
    ...
    private static ExecutionMutationIR testIr(String... required) {
        var entity = new MutationEntitySchema(new EntityId(1), "Entity",
                java.util.Set.of(new ColumnId(3)), java.util.Map.of(new FieldId(2), new ColumnId(3)), new ColumnId(3));
