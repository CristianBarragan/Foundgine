...
class MutationExecutionSecurityGateParityTest {
    ...
    void certificateIsBoundToExactMutationIrAndProviderInstance() {
        ...
        var cloned =
                new ExecutionMutationIR(
                        ir.operations(), ir.dependencies(), ir.requiredSecurityInvariants());
        var irEx =
                assertThrows(
                        IllegalStateException.class,
                        () ->
                                MutationExecutionSecurityGate.ensureExecutable(
                                        cloned, provider, certificate));
        ...
        var providerEx =
                assertThrows(
                        ...
                        () ->
                                MutationExecutionSecurityGate.ensureExecutable(
                                        ir, new HonestProvider(), certificate));
        assertTrue(
                providerEx.getMessage().toLowerCase().contains("exact mutation ir and provider"));
    }
        ...
        var ex =
                assertThrows(
                        ...
                        () ->
                                MutationExecutionSecurityGate.certify(
                                        ir,
                                        new UncertifiedProvider(),
                                        UncertifiedProvider.class.getName(),
                                        List.of()));
        assertTrue(
                ex.getMessage()
                        .toLowerCase()
    ...
    private static ExecutionMutationIR testIr(String... required) {
        var entity =
                new MutationEntitySchema(
                        new EntityId(1),
                        "Entity",
                        java.util.Set.of(new ColumnId(3)),
                        java.util.Map.of(new FieldId(2), new ColumnId(3)),
                        new ColumnId(3));
        var operation =
                new MutationOperation(
                        entity,
                        MutationKind.CREATE,
                        List.of(new MutationFieldValue(new ColumnId(3), "value")),
                        null,
                        null,
                        List.of(new FieldId(2)));
        return ExecutionMutationIR.from(
                new MutationBatchPlan(List.of(operation), List.of()), List.of(required));
    }
