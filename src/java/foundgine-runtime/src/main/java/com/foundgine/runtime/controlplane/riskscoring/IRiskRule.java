 ...
 * destructive", "caller has no prior successful calls this session"). Unlike
 * {@code IRoutingRule} and {@code IPolicyRule}, risk rules don't abstain —
 * every rule contributes a signal (possibly zero-weight), so scoring is a pure
 * sum, not a resolution order.
 */
