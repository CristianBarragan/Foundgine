...
/**
 * Framework-neutral Java counterpart of the C# {@code AddFoundgine} extension
 * methods.
 *
 * <p>
 * The C# API targets {@code IServiceCollection}. Java deliberately does not
 * introduce a dependency on one particular DI framework. This adapter performs
 * the same startup trust-boundary work and registers the resulting objects in
 * {@link FoundgineServiceRegistry}. Applications can then bridge that registry
 * into their DI container.
 */
public final class FoundgineServiceCollectionExtensions {
	private FoundgineServiceCollectionExtensions() {
	}

	public static FoundgineServiceRegistry addFoundgine(FoundgineServiceRegistry services, SemanticModel model,
	...
	public static FoundgineServiceRegistry addFoundgine(FoundgineServiceRegistry services, SemanticModel model,
			ISemanticAuthorizationPolicy authorizationPolicy, IProviderPlanCompiler compiler,
		...
		FoundgineOptions options = new FoundgineOptions().model(model).authorizationPolicy(authorizationPolicy);

		SemanticContractSnapshot contract = model.freeze().createSnapshot();
		SecurityInvariantContractValidator
				.ensureContractValid(com.foundgine.core.semantic.capabilities.SemanticCapabilityContractDiscovery
		...
		services.addSingleton(FoundgineEngine.class, () -> new FoundgineEngine(options, contract, compiler, provider));
		...
		if (options.mutationSchema() != null && options.mutationProvider() != null) {
			services.addSingleton(IFoundgineMutations.class,
					() -> new FoundgineMutationEngine(options.mutationSchema(), options.authorizationPolicy(),
							options.mutationProvider(), options.model(), options.warrantKeyResolver(),
							options.expectedWarrantIssuer(), options.warrantReplayStore(),
							options.securityResourceLimits()));
		}
		return services;
	}

	public static FoundgineServiceRegistry addFoundgine(FoundgineServiceRegistry services, FoundgineOptions options,
			IProviderPlanCompiler compiler, IExecutionProvider provider) {
		...
		if (options.authorizationPolicy() == null) {
			if (options.authorizationConfiguration() != null)
				options.authorizationPolicy(
						new com.foundgine.core.semantic.authorization.ConfiguredSemanticAuthorizationPolicy(
