...
/**
 * Framework-neutral Java counterpart of the C# {@code AddFoundgine} extension methods.
 *
 * <p>The C# API targets {@code IServiceCollection}. Java deliberately does not introduce a
 * dependency on one particular DI framework. This adapter performs the same startup trust-boundary
 * work and registers the resulting objects in {@link FoundgineServiceRegistry}. Applications can
 * then bridge that registry into their DI container.
 */
public final class FoundgineServiceCollectionExtensions {
    ...
    public static FoundgineServiceRegistry addFoundgine(
        ...
        return addFoundgine(
                services, model, new AllowAllSemanticAuthorizationPolicy(), compiler, provider);
    }
    ...
    public static FoundgineServiceRegistry addFoundgine(
        ...
        FoundgineOptions options =
                new FoundgineOptions().model(model).authorizationPolicy(authorizationPolicy);
        ...
        services.addSingleton(
                FoundgineEngine.class,
                () -> new FoundgineEngine(options, contract, compiler, provider));
        services.addSingleton(
                IFoundgine.class, () -> services.getRequiredService(FoundgineEngine.class));
        services.addSingleton(
                IFoundgineExecutor.class, () -> services.getRequiredService(FoundgineEngine.class));

        if (options.mutationSchema() != null && options.mutationProvider() != null) {
            services.addSingleton(
                    IFoundgineMutations.class,
                    () ->
    ...
    public static FoundgineServiceRegistry addFoundgine(
        ...
        if (options.authorizationPolicy() == null) {
            if (options.authorizationConfiguration() != null)
                options.authorizationPolicy(
                        new com.foundgine.core.semantic.authorization
                                .ConfiguredSemanticAuthorizationPolicy(
                                options.authorizationConfiguration(),
                                options.authorizationContext()));
            else
                throw new IllegalStateException(
                        "Foundgine requires an ISemanticAuthorizationPolicy or configured"
                                + " authorization.");
        }
        Objects.requireNonNull(services, "services");
        Objects.requireNonNull(compiler, "compiler");
