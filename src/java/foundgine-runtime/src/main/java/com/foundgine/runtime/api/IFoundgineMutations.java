...
public interface IFoundgineMutations {
    ...
    CompletableFuture<MutationExecutionResult> executeAsync(
            SemanticMutationRequest request, ExecutionContext context);

    MutationPlanApproval approve(SemanticMutationRequest request, String approvedBy);

    CompletableFuture<MutationExecutionResult> executeApprovedAsync(
            MutationPlanApproval approval, ExecutionContext context);

    record SemanticMutationRequest(
            SemanticMutationOperationGraph graph, SecurityExecutionContext security) {
        public SemanticMutationRequest(SemanticMutationOperationGraph graph) {
            this(graph, null);
        }
    }

    record MutationDryRunResult(
            String planFingerprint, List<MutationPlanOperation> operations, List<String> effects) {}

    record MutationPlanOperation(
            int index,
            String entity,
            String kind,
            List<String> fields,
    ...
    record MutationExecutionResult(
