...
/**
 * Port of {@code Foundgine.Runtime.ControlPlane.RiskScoring.CompositeRiskScorer},
 * declared alongside {@code IRiskRule} in the same C# file.
 *
 * <p>Evaluates every registered {@link IRiskRule} and aggregates the
 * resulting signals into a single {@link RiskScore}.
 *
 * <p><b>Porting decision:</b> the C# constructor accepts a nullable
 * {@code IEnumerable<IRiskRule>?} defaulting to an empty sequence. Java has
 * no nullable-with-default parameter, so it is ported as two constructors:
 * a no-arg constructor equivalent to the C# default, and one taking an
 * explicit rule list.
 */
