...
/**
 * Port of
 * {@code Foundgine.Runtime.ControlPlane.PolicyGateway.DefaultPolicyGateway},
 ...
 * otherwise the call is allowed only if at least one rule explicitly allowed
 * it. An empty rule set denies by default — governance with no configured
 * policy must not silently permit everything.
 *
 * <p>
 ...
 * unwrapped here via {@code Optional::isPresent} / {@code Optional::get} rather
 * than a null filter.
 */
public final class DefaultPolicyGateway implements IPolicyGateway {
	...
	public PolicyDecision evaluate(ToolDescriptor tool, SecurityExecutionContext security, RiskScore riskScore) {
		...
		var decisions = rules.stream().map(rule -> rule.evaluate(tool, security, riskScore))
		...
		return PolicyDecision.deny("control-plane.default", "No policy rule explicitly allowed this tool call.");
	}
}
