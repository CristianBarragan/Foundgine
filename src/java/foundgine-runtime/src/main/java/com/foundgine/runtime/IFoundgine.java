 ...
 *
 * <p>Full application-facing Foundgine surface. Most application code can
 * depend on {@link IFoundgineExecutor} and use only {@code executeAsync}.
 *
 * <p>C#'s {@code ExecutionContext? context = null, CancellationToken
 * cancellationToken = default} optional parameters on {@code
 * ExecuteApprovedAsync} are ported as the same overload pair used elsewhere
 * in this port.
 */
public interface IFoundgine extends IFoundgineExecutor {
    ...
    /**
     * Returns the capability contract visible to a verified warrant-backed
     * caller. Discovery never consumes replay state; execution still
     * re-authorizes.
     */
