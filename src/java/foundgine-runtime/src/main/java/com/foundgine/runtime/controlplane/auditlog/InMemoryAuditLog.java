 ...
 *
 * <p>
 * Process-local, append-only audit log. Entries are never mutated or removed
 * once recorded. A deployment that needs durable or tamper-evident audit
 * history should back {@link IAuditLog} with external storage — the append-only
 * contract is what callers can rely on either way.
 *
 * <p>
 * <b>Porting decision:</b> as with {@code InMemoryApprovalStore}, the C#
 * {@code System.Threading.Lock} is ported as a plain {@code Object} monitor
 * guarded with {@code synchronized}. The C# {@code Where(...).ToArray()} filter
 * chain is ported as an explicit loop over a defensive copy, avoiding a stream
 * pipeline for a simple two-predicate filter while keeping the
