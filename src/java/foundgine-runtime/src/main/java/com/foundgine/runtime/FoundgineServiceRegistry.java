...
/**
 * Small framework-neutral service registry used by the Java runtime capability surface. It
 * deliberately avoids coupling Foundgine.Runtime to Spring/CDI; applications can adapt these
 * registrations into their preferred container.
 */
public final class FoundgineServiceRegistry {
    private final Map<Class<?>, Supplier<?>> suppliers = new LinkedHashMap<>();

    public <T> FoundgineServiceRegistry addSingleton(
            Class<T> type, Supplier<? extends T> supplier) {
    ...
    public <T> T getRequiredService(Class<T> type) {
        ...
        if (supplier == null)
            throw new IllegalStateException(
                    "Required service '" + type.getName() + "' is not registered.");
        Object value = supplier.get();
        if (!type.isInstance(value))
            throw new IllegalStateException(
                    "Registered service for '" + type.getName() + "' has incompatible type.");
