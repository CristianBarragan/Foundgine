...
/**
 * Port of {@code Foundgine.Runtime.ControlPlane.RiskScoring.RiskScore}, together with the {@code
 * RiskTier} enum and {@code RiskSignal} record declared in the same C# file.
 *
 * <p>Never constructed with a bare number — {@link #aggregate} is the only path that assembles one
 * from signals, so a score can't exist without its explanation.
 */
public record RiskScore(RiskTier tier, double value, List<RiskSignal> signals) {
    ...
    /**
     * One contributing factor to a risk score. Signals are the unit of explanation: a score is
     * always traceable back to the signals that produced it, never an opaque number.
     */
