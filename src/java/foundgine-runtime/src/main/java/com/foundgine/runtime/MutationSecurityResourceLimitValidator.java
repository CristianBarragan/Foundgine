 ...
 *
 * <p>Canonical mutation-side resource guard. It runs before mutation planning, authorization,
 * provider lowering, or replay consumption.
 */
public final class MutationSecurityResourceLimitValidator {
    ...
    public static void validate(SemanticMutationRequest request, SecurityResourceLimits limits) {
        ...
        if (operations.size() > limits.maxMutationOperations())
            reject(
                    "Mutation operation count exceeds the configured maximum of "
                            + limits.maxMutationOperations()
                            + ".");
        ...
        if (dependencyCount > limits.maxMutationDependencies())
            reject(
                    "Mutation dependency count exceeds the configured maximum of "
                            + limits.maxMutationDependencies()
                            + ".");

        int effectCount = operations.stream().mapToInt(x -> x.effects().size()).sum();
        if (effectCount > limits.maxMutationEffects())
            reject(
                    "Mutation effect count exceeds the configured maximum of "
                            + limits.maxMutationEffects()
                            + ".");
        ...
        for (int i = 0; i < operations.size(); i++) {
            ...
            if (operation.fields().size() > limits.maxMutationFieldsPerOperation())
                reject(
                        "Mutation field count for operation "
                                + i
            ...
            if (operation.returnFields().size() > limits.maxMutationReturnFieldsPerOperation())
                reject(
                        "Mutation return-field count for operation "
                                + i
                                + " exceeds the configured maximum of "
