 ...
 *
 * <p>Canonical mutation-side resource guard. It runs before mutation
 * planning, authorization, provider lowering, or replay consumption.
 */
public final class MutationSecurityResourceLimitValidator {
    ...
    public static void validate(SemanticMutationRequest request, SecurityResourceLimits limits) {
        ...
        if (operations.size() > limits.maxMutationOperations())
            reject("Mutation operation count exceeds the configured maximum of "
        ...
        if (dependencyCount > limits.maxMutationDependencies())
            reject("Mutation dependency count exceeds the configured maximum of "
