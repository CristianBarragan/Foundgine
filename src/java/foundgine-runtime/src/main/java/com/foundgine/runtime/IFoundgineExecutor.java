 ...
 *
 * <p>Stable application-facing entry point for semantic execution.
 *
 * <p>C#'s {@code ExecutionContext? context = null, CancellationToken cancellationToken = default}
 * optional parameters are ported as a four-overload pair per method, matching this port's
...
public interface IFoundgineExecutor {
    ...
    default CompletionStage<ExecutionResult> executeAsync(
            SemanticRequest request, ExecutionContext context) {
    ...
    /**
     * Executes external, provider-neutral read intent after compiling it into the canonical
     * semantic request. This overload is intended for adapters such as JSON APIs and AI tools.
     */
    CompletionStage<ExecutionResult> executeAsync(
            ReadIntent intent, ExecutionContext context, CancellationToken cancellationToken);

    default CompletionStage<ExecutionResult> executeAsync(
            ReadIntent intent, ExecutionContext context) {
