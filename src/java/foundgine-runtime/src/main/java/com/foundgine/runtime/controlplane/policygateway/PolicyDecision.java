...
/**
 * Port of {@code Foundgine.Runtime.ControlPlane.PolicyGateway.PolicyDecision}, together with the
 * {@code PolicyOutcome} enum declared in the same C# file.
 *
 * <p>The result of evaluating policy for a tool call. Always carries the policy that produced it
 * and a human-readable reason — a bare {@link PolicyOutcome} is never surfaced on its own, matching
 * the explainability requirement already established by {@code
 * com.foundgine.runtime.controlplane.riskscoring.RiskScore.RiskSignal}.
 */
public record PolicyDecision(
        PolicyOutcome outcome, String policyId, String reason, List<String> obligationTags) {
    ...
    public static PolicyDecision requireApproval(
            String policyId, String reason, List<String> obligationTags) {
