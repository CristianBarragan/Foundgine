 ...
 *
 * <p>Application-facing facade over the Foundgine semantic execution pipeline. Applications
 * normally obtain this through {@link FoundgineOptions} / {@code FoundgineBuilder} rather than
 ...
 *       ValueTask<ExecutionAuthorizationAuthorityState?>>} authority resolver is ported as a
 *       synchronous {@link BiFunction} returning a nullable {@link
 *       ExecutionAuthorizationAuthorityState}, matching this port's existing decision (see {@link
 *       IExecutionAuthorizationRevalidator}) to make execution-time authority revalidation a
 *       synchronous, fail-closed check rather than an awaited call. A caller whose authority source
 *       is genuinely remote/async should resolve it before calling {@code executeAsync} and supply
 *       it via a resolver that reads a pre-fetched value.
 *   <li>The C# {@code async}/{@code await} pipeline is ported as ordinary synchronous Java methods
 *       (resolution, compilation, authorization, planning and caching are all CPU-bound here) that
 *       only return a {@link CompletionStage} at the single true asynchronous boundary: the call to
 *       {@link IExecutionProvider#executeAsync}. Evidence/receipt enrichment, which in C# happens
 *       after the {@code await}, is ported as a {@code thenApply} continuation on that stage.
 *   <li>C#'s {@code JsonSerializer.Serialize(request)} (System.Text.Json) is ported using the
 *       Jackson {@link ObjectMapper} already used elsewhere in this port (see {@code
 *       JsonReadIntentAdapter}); the exact JSON text differs from the C# serializer; it is only
 *       ever hashed for a same-process intent fingerprint, and hashing does not need to be stable
 *       across languages.
 * </ul>
 */
public final class FoundgineEngine implements IFoundgine {
    ...
    FoundgineEngine(
        ...
        this.planCache =
                options.planCache() != null ? options.planCache() : new MemoryProviderPlanCache();
        this.versions = SemanticVersionSet.forModel(this.model);
        this.securityContract =
                SemanticCapabilityContractDiscovery.describe(this.model, this.authorizationPolicy);
        ...
        this.securityResourceLimits =
                options.securityResourceLimits() != null
                        ? options.securityResourceLimits()
                        : new SecurityResourceLimits();
        this.executionAuthorizationRevalidator =
                options.executionAuthorizationRevalidator() != null
                        ? options.executionAuthorizationRevalidator()
                        : new SemanticExecutionAuthorizationRevalidator();
        this.executionAuthorizationAuthorityResolver =
                options.executionAuthorizationAuthorityResolver();
        this.securityResourceLimits.validate();
    }

    /**
     * Convenience overload for callers (largely tests) that build {@link FoundgineOptions} directly
     * and do not already hold a frozen {@link SemanticContractSnapshot}. The snapshot is derived
     * from {@link FoundgineOptions#model()} the same way the Hosting builder does at startup.
     */
    FoundgineEngine(
            FoundgineOptions options, IProviderPlanCompiler compiler, IExecutionProvider provider) {
    ...
    /**
     * Internal-compatible constructor for adapters/tests that intentionally provide the
     * orchestration components themselves.
     */
    FoundgineEngine(
            SemanticModel model,
    ...
    public SemanticCapabilityContract describeCapabilityContract(
        ...
        SemanticCapabilityContract contract =
                SemanticCapabilityContractDiscovery.describe(model, authorizationPolicy);
        List<SemanticCapability> visible =
                contract.capabilities().stream()
                        .filter(
                                capability ->
                                        com.foundgine.core.semantic.security.warrants
                                                .SecurityWarrantAuthorization.allows(
                                                security.warrant(),
                                                security.subject(),
                                                security.audience(),
                                                capability.id(),
    ...
    public DryRunResult dryRun(SemanticRequest request) {
        ...
        SemanticAuthorizationResult authorization =
                new SemanticAuthorizer(authorizationPolicy)
    ...
    public CompletionStage<ExecutionResult> executeApprovedAsync(
        ...
        if (!versions.semanticModelVersion().equals(approval.semanticModelVersion())
            ...
            throw new IllegalStateException(
                    "The approval was created against an incompatible semantic version set. Re-run"
                            + " dry-run and obtain a new approval.");
        }
        ...
        if (!currentFingerprint.equals(approval.planFingerprint())) {
            throw new IllegalStateException(
                    "The approved plan no longer matches the current authorized plan. Re-run"
                            + " dry-run and obtain a new approval.");
        }
        ...
        ProviderPlan providerPlan =
                ProviderPlanCacheExtensions.getOrAdd(
                        planCache,
                        cacheKey,
                        () ->
                                SecurityInvariantProofGate.attachAndValidate(
                                        compiler.compile(executionIr), executionIr, compiler));
    ...
    private void validateWarrant(
            SemanticRequest request, SemanticOperation operation, boolean consumeReplay) {
        ...
        for (var node : traverseDepthFirst(operation.root())) {
            securityContract.capabilities().stream()
                    .filter(
                            c ->
                                    c.targetEntityId().equals(node.entityId())
        ...
        if (capabilities.isEmpty())
            throw new IllegalStateException(
                    "No security capability contract exists for root entity '"
                            + operation.root().entityId()
                            + "' and its semantic composition.");
        ...
        SecurityCapabilityCompositionResult composition =
                SecurityCapabilityComposition.validate(
                        capabilities,
                        security.warrant(),
                        security.subject(),
                        security.audience(),
        ...
        if (!composition.isSatisfied()) throw new SecurityException(composition.failureReason());
        ...
        if (consumeReplay) {
            ...
            SecurityWarrantReplayGuard.consume(
                    security.warrant(), warrantReplayStore, Instant.now());
        }
    }
    ...
    private static List<com.foundgine.core.semantic.ir.SemanticReadNode> traverseDepthFirst(
        ...
        result.add(node);
        for (var child : node.children()) result.addAll(traverseDepthFirst(child));
        return result;
    }
    ...
    private SemanticPlan buildSecuredPlan(SemanticAuthorizationResult authorization) {
        ...
        if (!optimized.securityProof().isSatisfied())
            throw new IllegalStateException(
                    "The optimized semantic plan does not carry a satisfied security-preservation"
                            + " proof.");
                        ...
                        .orElseThrow(
                                () ->
                                        new IllegalStateException(
                                                "No security capability contract exists for root"
                                                        + " entity '"
                                                        + authorization
                                                                .operation()
                                                                .root()
        ...
        SemanticPlan plan =
                SecurityInvariantPlanRequirements.attach(
                        optimized.plan(), capability.effectiveSecurityInvariants());
        if (plan.effectiveSecurityInvariants().isEmpty())
            throw new IllegalStateException(
                    "The semantic execution contract is empty; no executable plan may be"
                            + " produced.");
    ...
    private CompletionStage<ExecutionResult> executeAndEnrichEvidenceAsync(
        ...
        if (authorizationEvidence == null)
            throw new IllegalStateException(
                    "Executable semantic plans require authorization evidence bound to the same"
                            + " semantic contract.");
        ...
        ExecutionAuthorizationAuthorityState currentAuthority =
                executionAuthorizationAuthorityResolver == null
                        ? null
                        : executionAuthorizationAuthorityResolver.apply(
                                authorizationEvidence, cancellationToken);
        executionAuthorizationRevalidator.validate(
                contract, authorizationEvidence, currentAuthority, cancellationToken);

        context.ensureWithinDeadline();
        Instant startedAt = Instant.now();
        var deadlineCts = context.createDeadlineCancellationSource(cancellationToken);
        return provider.executeAsync(providerPlan, context, deadlineCts.token())
                .thenApply(
                        result -> {
                            context.ensureWithinDeadline();
                            Instant completedAt = Instant.now();
                            if (result.evidence() == null) return result;
                            ...
                            try {
                                ...
                                throw new IllegalStateException(
                                        "Unable to serialize semantic request for intent"
                                                + " fingerprinting.",
                                        e);
                            }
                            ...
                            List<String> effects =
                                    plan.root().operation()
                                                    == com.foundgine.core.semantic.planning
                                                            .ExecutionOperation.SCAN
                                            ? List.of("read")
                                            : List.of("read", "relationship-traversal");
                            List<Integer> affectedNodeIds =
                                    enumerateNodes(plan.root()).stream()
                                            .map(SemanticPlanNode::id)
                                            .toList();
                            ExecutionReceipt receipt =
                                    ExecutionReceiptFactory.create(
                                            UUID.randomUUID().toString().replace("-", ""),
                                            evidence,
                            ...
                            return new ExecutionResult(
                                    result.rows(), result.pageInfo(), evidence, receipt);
                        });
    }
    ...
    private static List<SemanticPlanNode> enumerateNodes(SemanticPlanNode node) {
        ...
        result.add(node);
        for (var child : node.children()) result.addAll(enumerateNodes(child));
        return result;
    }

    private static ExecutionContext attachPaginationContext(
            SemanticPlan plan, ExecutionContext context) {
