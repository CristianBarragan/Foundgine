 ...
 * Applications normally obtain this through {@link FoundgineOptions} /
 * {@code FoundgineBuilder} rather than constructing it directly.
 *
 * <p>
 ...
 * {@link ExecutionAuthorizationAuthorityState}, matching this port's existing
 * decision (see {@link IExecutionAuthorizationRevalidator}) to make
 * execution-time authority revalidation a synchronous, fail-closed check rather
 * than an awaited call. A caller whose authority source is genuinely
 * remote/async should resolve it before calling {@code
 *       executeAsync} and supply it via a resolver that reads a pre-fetched
 * value.</li>
 * <li>The C# {@code async}/{@code await} pipeline is ported as ordinary
 * synchronous Java methods (resolution, compilation, authorization, planning
 * and caching are all CPU-bound here) that only return a
 * {@link CompletionStage} at the single true asynchronous boundary: the call to
 * {@link IExecutionProvider#executeAsync}. Evidence/receipt enrichment, which
 * in C# happens after the {@code await}, is ported as a {@code thenApply}
 * continuation on that stage.</li>
 * <li>C#'s {@code JsonSerializer.Serialize(request)} (System.Text.Json) is
 * ported using the Jackson {@link ObjectMapper} already used elsewhere in this
 * port (see {@code JsonReadIntentAdapter}); the exact JSON text differs from
 * the C# serializer; it is only ever hashed for a same-process intent
 * fingerprint, and hashing does not need to be stable across languages.</li>
 * </ul>
 */
public final class FoundgineEngine implements IFoundgine {
	...
	private final BiFunction<SemanticAuthorizationEvidence, CancellationToken, ExecutionAuthorizationAuthorityState> executionAuthorizationAuthorityResolver;
	private final String cacheNamespace = UUID.randomUUID().toString().replace("-", "");

	FoundgineEngine(FoundgineOptions options, SemanticContractSnapshot contract, IProviderPlanCompiler compiler,
		...
		this.securityResourceLimits = options.securityResourceLimits() != null ? options.securityResourceLimits()
	 ...
	 * {@link FoundgineOptions} directly and do not already hold a frozen
	 * {@link SemanticContractSnapshot}. The snapshot is derived from
	 * {@link FoundgineOptions#model()} the same way the Hosting builder does at
	 * startup.
	 */
	...
	/**
	 * Internal-compatible constructor for adapters/tests that intentionally provide
	 * the orchestration components themselves.
	 */
	FoundgineEngine(SemanticModel model, ISemanticAuthorizationPolicy authorizationPolicy, IPlanner planner,
			IProviderPlanCompiler compiler, IExecutionProvider provider, IProviderPlanCache planCache) {
	...
	FoundgineEngine(SemanticModel model, ISemanticAuthorizationPolicy authorizationPolicy, IPlanner planner,
			IProviderPlanCompiler compiler, IExecutionProvider provider) {
	...
	public SemanticCapabilityContract describeCapabilityContract(SecurityExecutionContext security) {
		...
		List<SemanticCapability> visible = contract.capabilities().stream()
				.filter(capability -> com.foundgine.core.semantic.security.warrants.SecurityWarrantAuthorization.allows(
						security.warrant(), security.subject(), security.audience(), capability.id(),
						capability.operation(), security.tenant(), security.resourceScope()))
				.toList();
	...
	public DryRunResult dryRun(SemanticRequest request) {
		...
		SemanticAuthorizationResult authorization = new SemanticAuthorizer(authorizationPolicy)
				.authorizeWithEvidence(contract, semanticOperation);
	...
	public PlanApproval approvePlan(SemanticRequest request, String approvedBy) {
		...
		return new PlanApproval(request, UUID.randomUUID().toString().replace("-", ""),
				dryRun.inspection().planFingerprint(), versions.semanticModelVersion(),
				versions.capabilityContractVersion(), versions.capabilityVersion(), versions.intentVersion(),
				versions.planVersion(), approvedBy, Instant.now());
	}

	@Override
	public CompletionStage<ExecutionResult> executeApprovedAsync(PlanApproval approval, ExecutionContext context,
		...
		ProviderPlan providerPlan = ProviderPlanCacheExtensions.getOrAdd(planCache, cacheKey,
				() -> SecurityInvariantProofGate.attachAndValidate(compiler.compile(executionIr), executionIr,
						compiler));

		ExecutionContext executionContext = attachPaginationContext(plan,
				context != null ? context : ExecutionContext.EMPTY);
		return executeAndEnrichEvidenceAsync(approval.request(), plan, providerPlan, executionContext, executionIr,
				cancellationToken, approval, authorization.evidence());
	}

	@Override
	public CompletionStage<ExecutionResult> executeAsync(ReadIntent intent, ExecutionContext context,
			CancellationToken cancellationToken) {
	...
	@Override
	public CompletionStage<ExecutionResult> executeAsync(SemanticRequest request, ExecutionContext context,
			CancellationToken cancellationToken) {
		Objects.requireNonNull(request, "request");
		SecurityResourceLimitValidator.validate(request, securityResourceLimits);
		...
		return executeAndEnrichEvidenceAsync(request, plan, providerPlan, executionContext, executionIr,
				cancellationToken, null, authorization.evidence());
	}
	...
	private String buildProviderPlanCacheKey(SemanticPlan plan, SecurityExecutionContext security) {
		...
		return security == null ? cacheNamespace + ":" + shape
	...
	private void validateWarrant(SemanticRequest request, SemanticOperation operation, boolean consumeReplay) {
		...
		for (var node : traverseDepthFirst(operation.root())) {
			securityContract.capabilities().stream()
					.filter(c -> c.targetEntityId().equals(node.entityId()) && "read".equals(c.operation())).findFirst()
		...
		for (var node : traverseDepthFirst(operation.root())) {
			for (var fieldId : node.fields()) {
				model.tryGet(node.entityId(), entity -> entity.fields().stream().filter(f -> f.id().equals(fieldId))
						.findFirst().ifPresent(f -> requestedFields.add(f.name())));
			}
		}

		SecurityCapabilityCompositionResult composition = SecurityCapabilityComposition.validate(capabilities,
				security.warrant(), security.subject(), security.audience(), security.tenant(),
				security.resourceScope(), requestedFields,
				request.options() != null && request.options().limit() != null ? Long.valueOf(request.options().limit())
						: null,
				null);
	...
	private SemanticPlan buildSecuredPlan(SemanticAuthorizationResult authorization) {
		...
		SemanticPlan plan = SecurityInvariantPlanRequirements.attach(optimized.plan(),
	...
	private CompletionStage<ExecutionResult> executeAndEnrichEvidenceAsync(SemanticRequest request, SemanticPlan plan,
			ProviderPlan providerPlan, ExecutionContext context, ExecutionIR executionIr,
			CancellationToken cancellationToken, PlanApproval approval,
		...
		ExecutionAuthorizationAuthorityState currentAuthority = executionAuthorizationAuthorityResolver == null ? null
				: executionAuthorizationAuthorityResolver.apply(authorizationEvidence, cancellationToken);
		executionAuthorizationRevalidator.validate(contract, authorizationEvidence, currentAuthority,
				cancellationToken);
		...
		return provider.executeAsync(providerPlan, context, deadlineCts.token()).thenApply(result -> {
			...
			try {
				intentFingerprint = ExecutionEvidenceFactory
						.hash("intent-v" + versions.intentVersion() + "|" + MAPPER.writeValueAsString(request));
			} catch (Exception e) {
			...
			ExecutionEvidence evidence = new ExecutionEvidence(baseEvidence.provider(),
					SemanticPlanFingerprint.create(plan), baseEvidence.authorizedNodeIds(), baseEvidence.rowsReturned(),
					baseEvidence.elapsedMilliseconds(), baseEvidence.providerOperationFingerprint(), intentFingerprint,
					authorizationFingerprint, baseEvidence.warrantId(), baseEvidence.warrantDigest(),
					baseEvidence.securityInvariantDigest(), baseEvidence.authorizationVersion());

			List<String> effects = plan.root()
					.operation() == com.foundgine.core.semantic.planning.ExecutionOperation.SCAN ? List.of("read")
			...
			ExecutionReceipt receipt = ExecutionReceiptFactory.create(UUID.randomUUID().toString().replace("-", ""),
					evidence, ExecutionReceiptFactory.fingerprintResult(result), affectedNodeIds, effects, startedAt,
					completedAt, versions.capabilityContractVersion(), versions.capabilityVersion(),
					versions.intentVersion(), versions.planVersion(), versions.semanticModelVersion(),
					approval != null ? approval.approvalId() : null, approval != null ? approval.approvedBy() : null,
