...
/**
 * Port of {@code Foundgine.Runtime.ControlPlane.AuthorizationContextIntegrityKeyRingManager}.
 *
 * <p>Process-local atomic owner of the current {@link AuthorizationContextIntegrityKeyRing}
 * snapshot. A rotation or retirement publishes a complete immutable snapshot under one lock, so
 * readers never observe a partially rotated configuration.
 *
 * <p><b>Porting decision:</b> C#'s {@code lock (_gate)} over a plain {@code object} is ported as a
 * {@code synchronized} block over a private final {@code Object} monitor — the direct Java analogue
 * with the same mutual-exclusion semantics, rather than introducing {@code java.util.concurrent}
 * lock machinery the original didn't use.
 */
