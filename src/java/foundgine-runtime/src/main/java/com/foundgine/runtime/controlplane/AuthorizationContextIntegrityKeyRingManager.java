 ...
 * snapshot. A rotation or retirement publishes a complete immutable
 * snapshot under one lock, so readers never observe a partially rotated
 * configuration.
 *
 * <p><b>Porting decision:</b> C#'s {@code lock (_gate)} over a plain
 * {@code object} is ported as a {@code synchronized} block over a private
 * final {@code Object} monitor — the direct Java analogue with the same
