 ...
 *
 * <p><b>Porting decision:</b> the C# {@code Query} method takes two optional
 * ({@code string?} with default {@code null}) filter parameters. Java has no
 * default parameter values, so it is ported as three overloads narrowing to
 * the fully-specified {@link #query(String, String)}, mirroring the call
 * shapes the C# defaults allow ({@code Query()}, {@code Query(toolName)},
