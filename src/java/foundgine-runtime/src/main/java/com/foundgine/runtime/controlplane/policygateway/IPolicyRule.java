 ...
 *
 * <p>A single policy concern. Rules abstain (return empty) rather than
 * allow by default, so silence never grants access — only an explicit
 * {@link PolicyDecision#allow} from some rule, or the gateway's own
 * no-rules-registered default, does.
 *
 * <p><b>Porting decision:</b> the C# method returns {@code PolicyDecision?}
 * (a nullable abstention). Ported as {@code Optional<PolicyDecision>} for
 * the same reason {@code IApprovalStore.TryGet} was: Java idiom prefers
 * {@link Optional} over a nullable return type to make abstention explicit
 * at every call site.
 */
