 ...
 *
 * <p>The generated key material is process-local and therefore intentionally unsuitable for
 * production. Production applications should configure a durable/trusted key resolver and replay
 * store before enabling this capability.
 */
public final class HighAssurance implements IFoundgineCapability {
    ...
    public void configure(FoundgineCapabilityContext context) {
        ...
        if (options.expectedWarrantIssuer() == null) options.expectedWarrantIssuer(DEV_ISSUER);
        if (options.warrantReplayStore() == null)
            options.warrantReplayStore(
                    new FileSecurityWarrantReplayStore(
                            Path.of(
                                            System.getProperty("java.io.tmpdir"),
                                            "foundgine-dev-warrant-replay.log")
                                    .toString()));
    }
