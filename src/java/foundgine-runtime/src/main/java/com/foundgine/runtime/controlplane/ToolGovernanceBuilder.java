 ...
 *
 * <p>
 * <b>Porting decision:</b> the C# source wires the governance pipeline through
 * {@code Microsoft.Extensions.DependencyInjection}'s {@code IServiceCollection}
 * (singleton registrations, with the last registration winning so a host can
 * override any default). This has no DI container dependency, so the same
 * fluent shape is preserved but retargeted at a plain builder that assembles
 * the object graph directly: {@link #addRiskRule}, {@link #addPolicyRule},
 * {@link #addRoutingRule}, and {@link #registerTool} correspond to the C#
 * builder's generic {@code AddRiskRule<TRule>()} etc. (instantiated rule
 * objects here, rather than DI-resolved types), and
 * {@link #approvalStore}/{@link #auditLog} correspond to a host overriding the
 * default {@code IApprovalStore}/ {@code IAuditLog} singleton registration.
 * Calling {@link #build()} corresponds to resolving {@code ToolCallGovernor}
 * from the container.
 */
public final class ToolGovernanceBuilder {
	 ...
	 * {@link InMemoryToolRegistry}'s constructor at {@link #build()} time, so
	...
	/**
	 * Overrides the default {@link InMemoryApprovalStore} with a durable
	 * implementation.
	 */
