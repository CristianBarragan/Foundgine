...
/**
 * Port of {@code Foundgine.Runtime.ControlPlane.ToolGovernanceBuilder} and {@code
 * ToolGovernanceServiceCollectionExtensions.AddFoundgineToolGovernance}, merged into a single type.
 *
 * <p><b>Porting decision:</b> the C# source wires the governance pipeline through {@code
 * Microsoft.Extensions.DependencyInjection}'s {@code IServiceCollection} (singleton registrations,
 * with the last registration winning so a host can override any default). This has no DI container
 * dependency, so the same fluent shape is preserved but retargeted at a plain builder that
 * assembles the object graph directly: {@link #addRiskRule}, {@link #addPolicyRule}, {@link
 * #addRoutingRule}, and {@link #registerTool} correspond to the C# builder's generic {@code
 * AddRiskRule<TRule>()} etc. (instantiated rule objects here, rather than DI-resolved types), and
 * {@link #approvalStore}/{@link #auditLog} correspond to a host overriding the default {@code
 * IApprovalStore}/ {@code IAuditLog} singleton registration. Calling {@link #build()} corresponds
 * to resolving {@code ToolCallGovernor} from the container.
 */
public final class ToolGovernanceBuilder {
    ...
    /**
     * Seeds the tool registry with a descriptor at startup. Collected into {@link
     * InMemoryToolRegistry}'s constructor at {@link #build()} time, so multiple calls accumulate
    ...
    public ToolCallGovernor build() {
        ...
        return new ToolCallGovernor(
                registry, riskScorer, policyGateway, approvals, routing, auditLog);
    }
}
