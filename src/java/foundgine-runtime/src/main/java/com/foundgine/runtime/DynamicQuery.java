 ...
 *
 * <p>Open, fluent, string-keyed query authoring surface for entities not
 * known at compile time. Compiles to the same {@link ReadIntent} as {@link
 * TypedQuery}.
 *
 * <p><b>Porting decision:</b> C#'s {@code Action<DynamicQuery> configure}
 * callback parameters ({@code Include}, {@code WhereRelated}) are ported as
 * {@link Consumer}, and {@code params string[] relationshipPath} is ported
 * as a Java varargs {@code String...} parameter, both direct equivalents.
 */
public final class DynamicQuery {
    ...
    public DynamicQuery whereRelated(
            String relationship, SemanticRelationshipQuantifier quantifier, Consumer<DynamicQuery> configure) {
        if (relationship == null || relationship.isBlank())
            throw new IllegalArgumentException("relationship must not be null or blank");
        Objects.requireNonNull(configure, "configure");
        DynamicQuery child = new DynamicQuery(foundgine, relationship);
