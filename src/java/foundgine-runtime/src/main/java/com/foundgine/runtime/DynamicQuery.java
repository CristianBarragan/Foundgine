 ...
 *
 * <p>Open, fluent, string-keyed query authoring surface for entities not known at compile time.
 * Compiles to the same {@link ReadIntent} as {@link TypedQuery}.
 *
 * <p><b>Porting decision:</b> C#'s {@code Action<DynamicQuery> configure} callback parameters
 * ({@code Include}, {@code WhereRelated}) are ported as {@link Consumer}, and {@code params
 * string[] relationshipPath} is ported as a Java varargs {@code String...} parameter, both direct
 * equivalents.
 */
    ...
    public DynamicQuery whereRelated(
        ...
        if (child.filter == null)
            throw new IllegalArgumentException(
                    "A related filter requires a child where() expression.");
