...
/**
 * Port of
 * {@code Foundgine.Runtime.ControlPlane.Approvals.InMemoryApprovalStore},
 * declared alongside {@code IApprovalStore} in the same C# file.
 *
 * <p>
 * Process-local approval store. A production deployment with real
 * human-in-the-loop approvers should back {@link IApprovalStore} with durable
 * storage — pending approvals must survive a process restart — but the
 * interface and workflow shape stay the same.
 *
 * <p>
 * <b>Porting decision:</b> the C# type synchronizes on a
 * {@code System.Threading.Lock} (.NET 9's dedicated lock type). Java has no
 * equivalent lock type, so a private {@code final Object} monitor is used with
 * {@code synchronized} blocks instead, which is the standard Java substitute
 * for a plain mutual-exclusion lock. {@code Guid.NewGuid().ToString("n")} (32
 * lowercase hex digits, no dashes) is ported as a dash-stripped, lowercased
