 ...
 *
 * <p>
 * Governs a single tool call end to end: looks the tool up in the registry,
 * scores its risk, evaluates policy, and either denies it, opens a human
 * approval request, or routes it to a {@code TaskContract} — auditing every
 * step along the way. This sits in front of
 ...
 *
 * <p>
 * <b>Porting decision:</b> C#'s {@code $"..."} interpolated audit summaries
 * (e.g. {@code $"Risk tier {riskScore.Tier} (value {riskScore.Value:F2})..."})
...
public final class ToolCallGovernor {
	...
	public ToolCallGovernor(IToolRegistry registry, CompositeRiskScorer riskScorer, IPolicyGateway policyGateway,
			IApprovalStore approvals, IRoutingEngine routing, IAuditLog auditLog) {
	...
	public ToolCallGovernanceResult govern(String toolName, SecurityExecutionContext security) {
		...
		audit(AuditEvent.AuditCategory.RISK_SCORED, toolName, security, security.authorityCachePartition(),
				"Risk tier " + riskScore.tier() + " (value " + String.format("%.2f", riskScore.value()) + ") from "
						+ riskScore.signals().size() + " signal(s).");
		...
		case REQUIRE_APPROVAL -> {
			...
			audit(AuditEvent.AuditCategory.APPROVAL_REQUESTED, toolName, security, security.authorityCachePartition(),
					"Approval '" + approval.approvalId() + "' opened (" + approval.requiredApprovals() + " required).");
			yield new ToolCallGovernanceResult(PolicyDecision.PolicyOutcome.REQUIRE_APPROVAL, decision.reason(), null,
					approval);
		}
		case ALLOW -> {
			...
			audit(AuditEvent.AuditCategory.ROUTED, toolName, security, security.authorityCachePartition(),
					"Routed as " + contract.mode() + "/" + contract.runtime() + "/" + contract.worker() + " (task '"
							+ contract.taskId() + "').");
			yield new ToolCallGovernanceResult(PolicyDecision.PolicyOutcome.ALLOW, decision.reason(), contract, null);
		}
		};
	}
	 ...
	 * {@link ApprovalRequest} reaches
	 * {@link ApprovalRequest.ApprovalStatus#GRANTED}. Routing only happens here,
	 * after a human has signed off — never during the initial request-approval
	 * step.
	 */
	public ToolCallGovernanceResult resumeAfterApproval(String toolName, SecurityExecutionContext security,
		...
		return new ToolCallGovernanceResult(PolicyDecision.PolicyOutcome.ALLOW, "Approved by human reviewer.", contract,
				approval);
	}
	...
	private void audit(AuditEvent.AuditCategory category, String toolName, SecurityExecutionContext security,
		...
		auditLog.record(
				AuditEvent.create(category, toolName, security.subject(), security.tenant(), fingerprint, summary));
	}
}
