 ...
 *
 * <p>Governs a single tool call end to end: looks the tool up in the registry, scores its risk,
 * evaluates policy, and either denies it, opens a human approval request, or routes it to a {@code
 * TaskContract} — auditing every step along the way. This sits in front of {@code
 * Foundgine.Providers.Tools.MCP} (not yet ported); it does not call {@code FoundgineEngine} or
 * participate in the compile/authorize/execute pipeline. An unknown or non-active tool is always
 * denied.
 *
 * <p><b>Porting decision:</b> C#'s {@code $"..."} interpolated audit summaries (e.g. {@code $"Risk
 * tier {riskScore.Tier} (value {riskScore.Value:F2})..."}) are ported with {@link String#format}
 * using {@code %.2f} for the C# {@code F2} numeric format specifier.
 */
public final class ToolCallGovernor {
    ...
    public ToolCallGovernanceResult govern(String toolName, SecurityExecutionContext security) {
        ...
        if (tool == null || tool.status() != ToolDescriptor.ToolStatus.ACTIVE) {
            ...
            return new ToolCallGovernanceResult(
                    PolicyDecision.PolicyOutcome.DENY, reason, null, null);
        }

        var riskScore = riskScorer.score(toolName, security);
        audit(
                AuditEvent.AuditCategory.RISK_SCORED,
                toolName,
                security,
                ...
                "Risk tier "
                        + riskScore.tier()
                        + " (value "
                        + String.format("%.2f", riskScore.value())
                        + ") from "
                        + riskScore.signals().size()
                        + " signal(s).");

        var decision = policyGateway.evaluate(tool, security, riskScore);
        audit(
                AuditEvent.AuditCategory.POLICY_EVALUATED,
                toolName,
                security,
                security.authorityCachePartition(),
                "Policy '"
        ...
        return switch (decision.outcome()) {
            case DENY -> {
                audit(
                        AuditEvent.AuditCategory.DENIED,
                        toolName,
                        security,
                        security.authorityCachePartition(),
                        decision.reason());
                yield new ToolCallGovernanceResult(
                        PolicyDecision.PolicyOutcome.DENY, decision.reason(), null, null);
            }
            case REQUIRE_APPROVAL -> {
                var approval = approvals.create(security.authorityCachePartition());
                audit(
                        AuditEvent.AuditCategory.APPROVAL_REQUESTED,
                        toolName,
                        security,
                        security.authorityCachePartition(),
                        "Approval '"
            ...
            case ALLOW -> {
                ...
                audit(
                        AuditEvent.AuditCategory.ROUTED,
                        toolName,
                        security,
                        security.authorityCachePartition(),
                        "Routed as "
                                ...
                                + "').");
                yield new ToolCallGovernanceResult(
                        PolicyDecision.PolicyOutcome.ALLOW, decision.reason(), contract, null);
            }
        };
    }

    /**
     * Resumes a call that was previously placed in {@link
     * PolicyDecision.PolicyOutcome#REQUIRE_APPROVAL} once its {@link ApprovalRequest} reaches
     * {@link ApprovalRequest.ApprovalStatus#GRANTED}. Routing only happens here, after a human has
     * signed off — never during the initial request-approval step.
     */
    public ToolCallGovernanceResult resumeAfterApproval(
            String toolName, SecurityExecutionContext security, String approvalId) {
        if (toolName == null || toolName.isBlank()) {
            throw new IllegalArgumentException("toolName is required.");
        }
        Objects.requireNonNull(security, "security");
        ...
        audit(
                AuditEvent.AuditCategory.APPROVAL_DECIDED,
                toolName,
                security,
                security.authorityCachePartition(),
        ...
        if (approval.status() != ApprovalRequest.ApprovalStatus.GRANTED) {
            var reason =
                    "Approval '" + approvalId + "' is '" + approval.status() + "', not granted.";
            return new ToolCallGovernanceResult(
                    PolicyDecision.PolicyOutcome.DENY, reason, null, approval);
        }
