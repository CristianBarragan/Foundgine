 ...
 * exact plan fingerprint at the moment of execution; this type is the upstream
 * human workflow that decides whether the call may proceed at all.
 */
public final class ApprovalRequest {
	...
	public ApprovalRequest(String approvalId, String requestFingerprint, int requiredApprovals, ApprovalStatus status,
	...
	public ApprovalRequest withGrant(ApprovalGrant grant) {
		...
		if (status != ApprovalStatus.PENDING) {
			throw new IllegalStateException("Approval request '" + approvalId + "' is already '" + status
		...
		if (!grant.granted()) {
			return new ApprovalRequest(approvalId, requestFingerprint, requiredApprovals, ApprovalStatus.DENIED,
					updatedGrants);
		}
