...
/**
 * Port of {@code Foundgine.Runtime.ControlPlane.Approvals.ApprovalRequest} (and {@code
 * ApprovalStatus}, {@code ApprovalGrant} from the same C# file).
 *
 * <p>A pending human sign-off for a tool call that policy flagged with {@code
 * PolicyOutcome.RequireApproval}. This is intentionally distinct from {@code
 * Foundgine.Runtime.PlanApproval}: that type binds an execution to an exact plan fingerprint at the
 * moment of execution; this type is the upstream human workflow that decides whether the call may
 * proceed at all.
 */
public final class ApprovalRequest {
    ...
    public record ApprovalGrant(
            String approverId, boolean granted, String comment, Instant decidedAt) {}
    ...
    public static ApprovalRequest create(
            String approvalId, String requestFingerprint, int requiredApprovals) {
    ...
    public ApprovalRequest withGrant(ApprovalGrant grant) {
        ...
        if (status != ApprovalStatus.PENDING) {
            throw new IllegalStateException(
                    "Approval request '"
                            ...
                            + "' is already '"
                            + status
                            + "' and cannot accept further decisions.");
        }
        ...
        var newStatus =
                grantedCount >= requiredApprovals ? ApprovalStatus.GRANTED : ApprovalStatus.PENDING;
        return new ApprovalRequest(
                approvalId, requestFingerprint, requiredApprovals, newStatus, updatedGrants);
    }
}
