 ...
 *
 * <p>Mutations use the same fail-closed sequence as the C# implementation:
...
public final class FoundgineMutationEngine implements IFoundgineMutations {
    ...
    public FoundgineMutationEngine(
        ...
        this.securityContract = model == null
        ...
        this.securityResourceLimits = securityResourceLimits != null
                ? securityResourceLimits : new SecurityResourceLimits();
    ...
    public CompletionStage<MutationExecutionResult> executeAsync(
            SemanticMutationRequest request, ExecutionContext context, CancellationToken cancellationToken) {
    ...
    public MutationPlanApproval approve(SemanticMutationRequest request, String approvedBy) {
        ...
        return new MutationPlanApproval(
                request, UUID.randomUUID().toString().replace("-", ""),
    ...
    private CompletionStage<MutationExecutionResult> executePlan(
        ...
        try (var deadlineSource = executionContext.createDeadlineCancellationSource(
            ...
            MutationExecutionSecurityCertificate certificate = MutationExecutionSecurityGate.certify(
                    ir, provider, provider.getClass().getName(), plan.requiredSecurityInvariants().stream()
            ...
            executionContext.ensureWithinDeadline();
            return CompletableFuture.completedFuture(new MutationExecutionResult(
                    result, fingerprint(plan), fingerprintResult(result),
        ...
        for (var operation : request.graph().operations()) {
            var capability = securityContract.capabilities().stream()
                    ...
                    .orElseThrow(() -> new SecurityException(
                            "No security capability contract exists for entity '" + operation.entity()
                                    + "' and operation '" + operation.kind() + "'."));

            if (!SecurityWarrantAuthorization.allows(
                    security.warrant(), security.subject(), security.audience(),
                    capability.id(), capability.operation(), security.tenant(), security.resourceScope()))
                throw new SecurityException(
                        "Security warrant does not authorize capability '" + capability.id()
    ...
    private List<String> requiredSecurityInvariantsFor(SemanticMutationPlan plan) {
        ...
        if (required.isEmpty())
            throw new IllegalStateException("A mutation execution cannot proceed without explicit security invariants.");
    ...
    private static boolean isEnginePreservedInvariant(String id) {
        return switch (id) {
            case SecurityInvariantIds.AUTHORIZATION_REQUIRED,
                 ...
                 SecurityInvariantIds.RELATIONSHIP_VISIBILITY -> true;
            default -> false;
        };
    }
    ...
    private MutationDryRunResult describe(SemanticMutationPlan plan) {
        ...
        for (int i = 0; i < plan.operations().size(); i++) {
            ...
            operations.add(new MutationPlanOperation(
                    i, entity.name(), x.kind().name(),
                    x.fields().stream().map(f -> Long.toString(f.field().value())).toList(),
                    x.returnFields().stream().map(f -> Long.toString(f.value())).toList()));
        }
        List<String> effects = plan.operations().stream().flatMap(x -> x.effects().stream())
                .map(this::formatEffect).toList();
        ...
        try {
            ...
            for (var x : result.results()) {
                ...
                if (x.returnedValues() != null) {
                    row.put("returned", x.returnedValues().entrySet().stream()
                            .sorted(Map.Entry.comparingByKey(Comparator.comparingLong(FieldId::value)))
                            .map(e -> Map.of("field", e.getKey().value(), "value", e.getValue()))
                            .toList());
                }
                rows.add(row);
            }
