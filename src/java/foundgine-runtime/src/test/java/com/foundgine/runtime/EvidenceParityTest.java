 ...
 *
 * <p>The SQL-execution-based cases in the C# original ({@code
 * Sql_execution_returns_provider_neutral_evidence}, {@code
 * Equivalent_sql_plans_produce_the_same_plan_fingerprint}) are ported separately in {@code
 * com.foundgine.providers.storage.sql.SqlExecutionEvidenceParityTest}, following the same opt-in
 * real-PostgreSQL pattern already used by {@code SqlExecutionPostgresE2ETest} / {@code
 * UpsertParityTest} (Java has no embedded-database test dependency), since this test only needs the
                ...
                new SemanticRequest(
                        ...
                        List.of(
                                new SemanticSelection(
                                        FieldId.create("Customer", "Id"), null, List.of())),
                        null,
                        null);
    ...
    private static final class CapturingEvidenceProvider implements IExecutionProvider {
        ...
        public CompletionStage<ExecutionResult> executeAsync(
            ...
            return CompletableFuture.completedFuture(
                    new ExecutionResult(List.of(), null, evidence, null));
        }
    }
