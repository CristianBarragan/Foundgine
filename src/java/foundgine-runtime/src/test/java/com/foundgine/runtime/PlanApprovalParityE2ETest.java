...
class PlanApprovalParityE2ETest {
    ...
    void approvalRejectsWhenPlanFingerprintChanges() {
        ...
        var engine = createEngine(provider);
        var approval = engine.approvePlan(request(), "human@example");
        var tampered = new PlanApproval(approval.request(), approval.approvalId(), approval.planFingerprint() + "-tampered",
                approval.semanticModelVersion(), approval.capabilityContractVersion(), approval.capabilityVersion(),
                approval.intentVersion(), approval.planVersion(), approval.approvedBy(), approval.approvedAt());
    ...
    private static FoundgineEngine createEngine(CountingProvider provider) {
        var model = new SemanticModelBuilder()
                ...
                .build();
        return new FoundgineEngine(model, new AllowAllSemanticAuthorizationPolicy(), new com.foundgine.core.semantic.planning.Planner(),
                new TestCompiler(), provider);
    }
    ...
    private static final class TestCompiler implements IProviderPlanCompiler, IProviderSecurityConformanceEvaluator,
        ...
        @Override public ProviderPlan compile(ExecutionIR ir) { return new TestPlan(); }
        @Override public java.util.Collection<String> preservedSecurityInvariants() { return SecurityInvariantRegistry.allInvariants().stream().map(x -> x.id()).toList(); }
    ...
    private static final class TestPlan extends ProviderPlan { TestPlan() { super("test"); } }
