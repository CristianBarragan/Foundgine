...
/**
 * Port of C# {@code Foundgine.E2E.Tests.ContextSafePlanCacheTests}: security invariants for
 * provider-plan caching. The cache may reuse a compiled plan across runtime contexts only when the
 * plan itself contains the runtime context lookup as a provider-independent predicate.
 */
class ContextSafePlanCacheParityTest {
    ...
    void sameAuthorizedShapeReusesPlanAcrossRuntimeContexts() {
        ...
        engine.executeAsync(request, new ExecutionContext(Map.of("user.TenantId", 7)))
                .toCompletableFuture()
                .join();
        engine.executeAsync(request, new ExecutionContext(Map.of("user.TenantId", 42)))
                .toCompletableFuture()
                .join();
    ...
    void runtimeContextValuesAreNotPartOfThePlanCacheKey() {
        ...
        engine.executeAsync(request, new ExecutionContext(Map.of("user.TenantId", 7)))
                .toCompletableFuture()
                .join();
        engine.executeAsync(request, new ExecutionContext(Map.of("user.TenantId", 8)))
                .toCompletableFuture()
                .join();
    ...
    void deniedRequestsNeverCompileOrReadACachedProviderPlan() {
        ...
        assertThrows(
                ...
                () -> engine.executeAsync(createCustomerRequest()));
    ...
    private static FoundgineEngine createEngine(
        ...
        return new FoundgineEngine(
                new FoundgineOptions()
                        ...
                        .planCache(cache != null ? cache : new MemoryProviderPlanCache()),
                compiler,
                new TestExecutionProvider());
    }
    ...
    private static SemanticRequest createCustomerRequest() {
        return new SemanticRequest(
                CUSTOMER, List.of(new SemanticSelection(CUSTOMER_ID, null, List.of())), null, null);
    }
            ...
            return operation == AuthorizationOperation.READ && entityId.equals(CUSTOMER)
                    ? AuthorizationPredicate.equal(
                            AuthorizationPredicate.member(
                                    ...
                                    "RegionId"),
                            AuthorizationPredicate.member(
                                    AuthorizationPredicate.contextParameter("user"), "RegionId"))
                    : null;
        }
    }
    ...
    private static final class DenyCustomerPolicy extends AllowAllSemanticAuthorizationPolicy {
        ...
        public AuthorizationDecision getEntityAccess(
                EntityId entityId, AuthorizationOperation operation) {
            return entityId.equals(CUSTOMER) && operation == AuthorizationOperation.READ
