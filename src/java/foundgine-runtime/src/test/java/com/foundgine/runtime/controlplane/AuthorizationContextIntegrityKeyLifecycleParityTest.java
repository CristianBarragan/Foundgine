...
class AuthorizationContextIntegrityKeyLifecycleParityTest {
	private static AuthorizationContextIntegrityKey key(String id, String material) {
		try {
			return new AuthorizationContextIntegrityKey(id, MessageDigest.getInstance("SHA-256")
	...
	void activeKeyCannotBeRetired() {
		var ring = new AuthorizationContextIntegrityKeyRing(key("key-v1", "old"));
		var ex = assertThrows(IllegalStateException.class, () -> ring.retire("key-v1", provenance(1), new HashSet<>()));
		assertTrue(ex.getMessage().toLowerCase().contains("active"));
	}

	@Test
	void persistedEvidencePreventsRetirement() {
		var ring = new AuthorizationContextIntegrityKeyRing(key("key-v1", "old")).rotate(key("key-v2", "new"),
				provenance(1));
	...
	void rotationSequenceIsMonotonic() {
		var ring = new AuthorizationContextIntegrityKeyRing(key("key-v1", "old")).rotate(key("key-v2", "new"),
				provenance(7));
		var ex = assertThrows(IllegalStateException.class, () -> ring.rotate(key("key-v3", "newer"), provenance(7)));
	...
	void retiredKeyCannotBeReactivated() {
		var ring = new AuthorizationContextIntegrityKeyRing(key("key-v1", "old"))
				.rotate(key("key-v2", "new"), provenance(1)).retire("key-v1", provenance(2), new HashSet<>());
		var ex = assertThrows(IllegalStateException.class, () -> ring.rotate(key("key-v1", "old"), provenance(3)));
	...
	void unauthorizedOperatorCannotRotate() {
		...
		assertThrows(SecurityException.class, () -> manager.rotate(key("key-v2", "new"),
				new AuthorizationKeyRotationProvenance(operator, 1, "forged-credential")));
		assertThrows(SecurityException.class, () -> manager.rotate(key("key-v2", "new"),
				new AuthorizationKeyRotationProvenance(UUID.randomUUID(), 1, "real-credential")));
	}
	...
	void invalidRotationProvenanceFailsClosed() {
		...
		assertThrows(SecurityException.class,
				() -> ring.rotate(key("key-v2", "new"), new AuthorizationKeyRotationProvenance(
						AuthorizationKeyRotationProvenance.EMPTY_OPERATOR_ID, 1, "credential")));
