...
class AuthorizationContextIntegrityKeyLifecycleParityTest {
    ...
    private static AuthorizationKeyRotationProvenance provenance(long sequence) {
        return new AuthorizationKeyRotationProvenance(
                UUID.randomUUID(), sequence, "rotation-credential");
    }
    ...
    void unauthorizedOperatorCannotRotate() {
        ...
        var manager =
                new AuthorizationContextIntegrityKeyRingManager(
                        new AuthorizationContextIntegrityKeyRing(key("key-v1", "old")),
                        new AuthorizationKeyRotationAuthorizer(
                                Map.of(operator, "real-credential")));
        assertThrows(
                SecurityException.class,
                () ->
                        manager.rotate(
                                key("key-v2", "new"),
                                new AuthorizationKeyRotationProvenance(
                                        operator, 1, "forged-credential")));
        assertThrows(
                SecurityException.class,
                () ->
                        manager.rotate(
                                key("key-v2", "new"),
                                new AuthorizationKeyRotationProvenance(
                                        UUID.randomUUID(), 1, "real-credential")));
    }
    ...
    void concurrentRotationWithSameSequenceAllowsOnlyOneCommit() throws InterruptedException {
        ...
        var manager =
                new AuthorizationContextIntegrityKeyRingManager(
                        new AuthorizationContextIntegrityKeyRing(key("key-v1", "old")),
                        new AuthorizationKeyRotationAuthorizer(
                                Map.of(operator, "rotation-credential")));
        ...
        assertThrows(
                ...
                () ->
                        ring.rotate(
                                key("key-v2", "new"),
                                new AuthorizationKeyRotationProvenance(
                                        AuthorizationKeyRotationProvenance.EMPTY_OPERATOR_ID,
                                        1,
                                        "credential")));
        assertThrows(
