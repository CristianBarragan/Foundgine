...
/**
 * Port of C# {@code Foundgine.E2E.Tests.PlanCacheTests}: proves that provider compilation may be
 * cached without bypassing semantic authorization or removing runtime authorization predicates.
 */
class PlanCacheParityTest {
                ...
                tenantId == null
                        ...
                        : new SemanticQueryOptions(
                                new SemanticFieldFilter(
                                        CUSTOMER_TENANT_ID, SemanticFilterOperator.EQ, tenantId),
                                List.of(),
                                null,
                                null,
                                null);
    ...
    private static final class CountingPolicy extends AllowAllSemanticAuthorizationPolicy {
        ...
        public AuthorizationDecision getEntityAccess(
                EntityId entityId, AuthorizationOperation operation) {
            if (operation == AuthorizationOperation.READ) entityReadChecks.incrementAndGet();
