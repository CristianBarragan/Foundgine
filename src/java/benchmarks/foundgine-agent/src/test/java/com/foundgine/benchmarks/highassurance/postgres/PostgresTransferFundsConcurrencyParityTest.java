...
/**
 * PostgreSQL concurrency/visibility parity for the C# HighAssurance fixture. These tests are
 * intentionally opt-in because they require a live PostgreSQL server.
 */
class PostgresTransferFundsConcurrencyParityTest {
    ...
    void sameIdempotencyKeyFaultRollbackAllowsWaitingRequestToExecuteOnce() throws Exception {
        ...
        var firstExecutor =
                new PostgresTransferFundsExecutor(
                        ...
                        new Properties(),
                        (id, a, b) ->
                                new PostgresAuthorizationDecision(true, 1, "authorization-v1"),
                        (point, connection) -> {
                            if (point
                                            != PostgresTransferFundsExecutor.FaultPoint
                                                    .AFTER_MUTATION_BEFORE_COMMIT
                                    || !inject.compareAndSet(true, false)) return;
                            entered.countDown();
                            try {
                            ...
                            throw new IllegalStateException(
                                    "Injected concurrency fault before commit.");
                        });
        var waiting =
                service(
                        url,
                        (id, a, b) ->
                                new PostgresAuthorizationDecision(
                                        id.equals(a.ownerId()) && id.equals(b.ownerId()),
                                        1,
                                        "authorization-v1"));
        var first = new PostgresTransferFundsService(firstExecutor);
        var command =
                new TransferFunds.Command(
                        source, destination, bd("100"), "fault-concurrency-same-key");
        var failed =
                CompletableFuture.supplyAsync(() -> executeUnchecked(first, actor, 207, command));
        assertTrue(entered.await(5, TimeUnit.SECONDS));
        var replayCandidate =
                CompletableFuture.supplyAsync(() -> executeUnchecked(waiting, actor, 207, command));
        Thread.sleep(250);
        assertFalse(
                replayCandidate.isDone(),
                "The second request should remain blocked by the first transaction's advisory"
                        + " lock.");
        release.countDown();
        ...
        var firstExecutor =
                new PostgresTransferFundsExecutor(
                        ...
                        (point, connection) -> {
                            ...
                            throw new IllegalStateException(
                                    "Injected opposing-transfer fault before commit.");
                        });
        var first = new PostgresTransferFundsService(firstExecutor);
        var second =
                service(
                        ...
                        (id, x, y) ->
                                new PostgresAuthorizationDecision(
                                        id.equals(x.ownerId()) && id.equals(y.ownerId()),
                                        1,
                                        "authorization-v1"));
        var failed =
                ...
                CompletableFuture.supplyAsync(
                        () ->
                                executeUnchecked(
                                        ...
                                        actor,
                                        208,
                                        new TransferFunds.Command(b, a, bd("200"), "wait-b-to-a")));
        Thread.sleep(250);
        assertFalse(
                waiting.isDone(), "The opposing transfer should wait for the locked account rows.");
        release.countDown();
        assertThrows(CompletionException.class, failed::join);
    ...
    void authorizationRevokedWhileTransferWaitsIsObservedAfterLockAcquisition() throws Exception {
        ...
        try (var blocker = DriverManager.getConnection(url)) {
            ...
            var executor =
                    new PostgresTransferFundsExecutor(
                            ...
                            (id, a, b) -> {
                                ...
                                return new PostgresAuthorizationDecision(
                                        !revoked.get(),
                                        revoked.get() ? 2 : 1,
                                        revoked.get() ? "authorization-v2" : "authorization-v1");
                            });
                            ...
                            () ->
                                    executeUnchecked(
                                            ...
                                            new TransferFunds.Command(
                                                    ...
                                                    destination,
                                                    bd("100"),
                                                    "visibility-authorization-context-race")));
            Thread.sleep(100);
            ...
            assertTrue(
                    observed.get(),
                    "Authorization must be evaluated after the transfer acquires authoritative"
                            + " locks.");
        }
                ...
                service(
                        ...
                        (id, a, b) ->
                                new PostgresAuthorizationDecision(
                                        ...
                                        "authorization-v1"));
        var command =
                new TransferFunds.Command(source, destination, bd("100"), "same-key-concurrency");
        var pool = Executors.newFixedThreadPool(8);
        try {
            var futures = new ArrayList<Future<PostgresTransferFundsService.Receipt>>();
            for (int i = 0; i < 8; i++)
                futures.add(pool.submit(() -> service.execute(actor, 200, command)));
            ...
            assertEquals(
                    1,
                    receipts.stream()
                            .map(PostgresTransferFundsService.Receipt::transferId)
                            .distinct()
                            .count());
            assertEquals(1, receipts.stream().filter(r -> !r.replay()).count());
            assertEquals(
                    7,
                    receipts.stream().filter(PostgresTransferFundsService.Receipt::replay).count());
            assertState(url, source, destination, "900.0000", "100.0000", 1, 1);
        } finally {
            pool.shutdownNow();
        }
                    ...
                    pool.submit(
                            () ->
                                    service.execute(
                                            actor,
                                            201,
                                            new TransferFunds.Command(a, b, bd("100"), "a-to-b")));
            var second =
                    pool.submit(
                            () ->
                                    service.execute(
    ...
    void ownershipChangeQueuedBeforeTransferLockIsVisibleAfterLockAcquisition() throws Exception {
            ...
            try (var ownerConnection = DriverManager.getConnection(url)) {
                ...
                var ownerUpdate =
                        CompletableFuture.runAsync(
                                () -> changeOwner(ownerConnection, source, newOwner));
                Thread.sleep(100);
                var service =
                        service(
                                url,
                                ...
                                () ->
                                        executeUnchecked(
                                                ...
                                                new TransferFunds.Command(
                                                        ...
                                                        destination,
                                                        bd("100"),
                                                        "visibility-owner-race")));
                Thread.sleep(100);
                assertFalse(
                        transfer.isDone(), "Transfer should wait for authoritative account locks");
                blocker.commit();
        ...
        assertEquals(
                newOwner,
                readUuid(url, "SELECT owner_id FROM banking.bank_account WHERE id=?", source));
    }
    ...
    void tenantReassignmentCommittedBeforeTransferLockBlocksCrossTenantTransfer() throws Exception {
        ...
        assertState(url, source, destination, "1000.0000", "1000.0000", 0, 0);
        assertEquals(
                205, readInt(url, "SELECT tenant_id FROM banking.bank_account WHERE id=?", source));
    }
    ...
    private static PostgresTransferFundsService service(
            String url, PostgresTransferFundsExecutor.Authorization auth) {
        return new PostgresTransferFundsService(
                new PostgresTransferFundsExecutor(url, new Properties(), auth));
    }

    private static PostgresTransferFundsService.Receipt executeUnchecked(
            PostgresTransferFundsService s, UUID actor, int tenant, TransferFunds.Command c) {
        try {
            return s.execute(actor, tenant, c);
        } catch (Exception e) {
    ...
    private static void prepare(
        ...
        try (var c = DriverManager.getConnection(url);
            ...
            var schema =
                    PostgresTransferFundsConcurrencyParityTest.class.getResourceAsStream(
                            "/high-assurance-postgres-schema.sql");
            assertNotNull(schema);
            st.execute(new String(schema.readAllBytes(), StandardCharsets.UTF_8));
            st.executeUpdate(
                    "TRUNCATE banking.transfer_audit, banking.transfer_idempotency,"
                            + " banking.authorization_context, banking.bank_account CASCADE");
            try (var ps =
                    c.prepareStatement(
                            "INSERT INTO"
                                + " banking.bank_account(id,tenant_id,owner_id,balance,pending_transactions,regulatory_hold,daily_limit,is_frozen)"
                                + " VALUES (?,?,?,?,?,?,?,?)")) {
                account(
                        ps,
                        source,
            ...
            try (var ps =
                    c.prepareStatement(
                            "INSERT INTO"
                                ...
                                + " VALUES (?,?,?,?,?,'HMAC-SHA256/v1','test',repeat('0',64))")) {
                ps.setObject(1, actor);
                ps.setInt(2, tenant);
                ps.setBoolean(3, true);
                ps.setLong(4, 1);
                ps.setString(5, "authorization-v1");
                ps.executeUpdate();
            }
        }
    }
    ...
    private static void account(
        ...
        ps.setBigDecimal(4, bd(Double.toString(balance)));
        ps.setBigDecimal(5, bd(Double.toString(pending)));
        ps.setBigDecimal(6, bd(Double.toString(hold)));
        ps.setBigDecimal(7, bd(Double.toString(limit)));
        ps.setBoolean(8, frozen);
        ps.executeUpdate();
    }
    ...
    private static void lockAccounts(Connection c, UUID source, UUID destination) {
        try (var ps =
                c.prepareStatement(
                        "SELECT id FROM banking.bank_account WHERE id IN (?,?) ORDER BY id FOR"
                                + " UPDATE")) {
            ps.setObject(1, source);
            ps.setObject(2, destination);
    ...
    private static void changeOwner(Connection c, UUID id, UUID owner) {
        try (var ps = c.prepareStatement("UPDATE banking.bank_account SET owner_id=? WHERE id=?")) {
    ...
    private static void setFrozen(Connection c, UUID id) {
        try (var ps =
                c.prepareStatement("UPDATE banking.bank_account SET is_frozen=true WHERE id=?")) {
            ps.setObject(1, id);
            ps.executeUpdate();
        } catch (SQLException e) {
            throw new CompletionException(e);
    ...
    private static void changeTenant(Connection c, UUID id, int tenant) {
        try (var ps =
                c.prepareStatement("UPDATE banking.bank_account SET tenant_id=? WHERE id=?")) {
            ps.setInt(1, tenant);
            ps.setObject(2, id);
            ps.executeUpdate();
        } catch (SQLException e) {
    ...
    private static void deleteAccount(Connection c, UUID id) {
        try (var ps = c.prepareStatement("DELETE FROM banking.bank_account WHERE id=?")) {
            ps.setObject(1, id);
            ps.executeUpdate();
        } catch (SQLException e) {
            throw new CompletionException(e);
    ...
    private static void assertState(
        ...
        assertEquals(
                bd(sourceBalance),
                readDecimal(url, "SELECT balance FROM banking.bank_account WHERE id=?", source));
        assertEquals(
                bd(destinationBalance),
                readDecimal(
                        url, "SELECT balance FROM banking.bank_account WHERE id=?", destination));
        assertEquals(idempotency, count(url, "transfer_idempotency"));
        assertEquals(audit, count(url, "transfer_audit"));
    }

    private static UUID readUuid(String url, String sql, UUID id) throws Exception {
        try (var c = DriverManager.getConnection(url);
    ...
    private static int readInt(String url, String sql, UUID id) throws Exception {
        try (var c = DriverManager.getConnection(url);
                var ps = c.prepareStatement(sql)) {
            ps.setObject(1, id);
            try (var rs = ps.executeQuery()) {
    ...
    private static BigDecimal readDecimal(String url, String sql, UUID id) throws Exception {
        try (var c = DriverManager.getConnection(url);
                var ps = c.prepareStatement(sql)) {
            ps.setObject(1, id);
            try (var rs = ps.executeQuery()) {
    ...
    private static int count(String url, String table) throws Exception {
        try (var c = DriverManager.getConnection(url);
                var st = c.createStatement();
                var rs = st.executeQuery("SELECT count(*) FROM banking." + table)) {
            assertTrue(rs.next());
            return rs.getInt(1);
        }
    }

    private static int count(String url, String table, UUID id) throws Exception {
        try (var c = DriverManager.getConnection(url);
                var ps =
                        c.prepareStatement(
                                "SELECT count(*) FROM banking." + table + " WHERE id=?")) {
            ps.setObject(1, id);
            try (var rs = ps.executeQuery()) {
                assertTrue(rs.next());
                return rs.getInt(1);
