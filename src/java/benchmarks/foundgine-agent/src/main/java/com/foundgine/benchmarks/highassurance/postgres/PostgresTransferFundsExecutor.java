...
/**
 * JDBC/PostgreSQL execution adapter for high-assurance TransferFunds. The adapter owns the physical
 * transaction and never treats a semantic plan as sufficient authority: authorization is loaded,
 * locked, bound to the request, re-evaluated after mutation and checked again immediately before
 * commit.
 */
public final class PostgresTransferFundsExecutor {
    public enum FaultPoint {
        AFTER_MUTATION_BEFORE_COMMIT,
        BEFORE_AUTHORIZATION_COMMIT_CHECK
    }
    ...
    public record Account(
            ...
            BigDecimal regulatoryHold,
            BigDecimal dailyTransferred,
            BigDecimal dailyLimit,
            boolean frozen) {}

    public record AuthorizationContext(
            UUID actorId, int tenantId, boolean allowed, long version, String fingerprint) {}
    ...
    public PostgresTransferFundsExecutor(
            String jdbcUrl, Properties properties, Authorization authorization) {
    ...
    public Result execute(UUID actorId, int tenantId, TransferFunds.Command command)
            throws SQLException {
        ...
        try (Connection connection = DriverManager.getConnection(jdbcUrl, properties)) {
            ...
            try {
                ...
                var binding =
                        AuthorizationExecutionBinding.create(actorId, tenantId, command, decision);
                ...
                var currentContext =
                        loadAuthorizationContextForUpdate(connection, actorId, tenantId);
    ...
    private static void validateExecution(
            UUID actorId, int tenantId, Account source, Account destination, BigDecimal amount) {
        if (source.tenantId() != tenantId || destination.tenantId() != tenantId)
            throw new IllegalStateException("Tenant boundary violation.");
        if (!source.ownerId().equals(actorId) || !destination.ownerId().equals(actorId))
            throw new SemanticAuthorizationException(
                    "Transfer capability requires ownership of both accounts.");
        if (source.frozen() || destination.frozen())
            throw new IllegalStateException("Frozen accounts cannot participate in transfers.");
        if (source.dailyTransferred().add(amount).compareTo(source.dailyLimit()) > 0)
            throw new IllegalStateException("Transfer exceeds the source account daily limit.");
        var available =
                source.balance()
                        .subtract(source.pendingTransactions())
                        .subtract(source.regulatoryHold());
    ...
    private static void ensureAuthorizationMatches(
            PostgresAuthorizationDecision expected, PostgresAuthorizationDecision current) {
        ensureAuthorized(current);
        if (expected.version() != current.version()
                || !expected.fingerprint().equals(current.fingerprint()))
            throw new IllegalStateException(
                    "Authorization context changed during transfer execution; the authorization"
                            + " evidence is stale.");
    }

    private static void ensureAuthorizationMatchesContext(
            PostgresAuthorizationDecision decision, AuthorizationContext context) {
        if (context == null)
            throw new IllegalStateException(
                    "Authoritative authorization context is missing; authorization fails closed.");
        if (!context.allowed())
            throw new SemanticAuthorizationException(
                    "Authoritative authorization context is not allowed.");
        if (decision.version() != context.version()
                || !decision.fingerprint().equals(context.fingerprint()))
            throw new IllegalStateException(
                    "Authorization decision does not match authoritative PostgreSQL authorization"
                            + " context.");
    }

    private static Map<UUID, Account> loadAccountsForUpdate(
            Connection c, UUID sourceId, UUID destinationId) throws SQLException {
        var sql =
                "SELECT"
    ...
    private static IdempotencyRow findIdempotency(Connection c, String key) throws SQLException {
        var sql =
                "SELECT"
                    + " actor_id,tenant_id,source_account_id,destination_account_id,amount,transfer_id,source_balance,destination_balance"
    ...
    private static AuthorizationContext loadAuthorizationContextForUpdate(
            Connection c, UUID actorId, int tenantId) throws SQLException {
        var sql =
                "SELECT actor_id,tenant_id,allowed,version,fingerprint FROM"
                    + " banking.authorization_context WHERE actor_id=? AND tenant_id=? FOR UPDATE";
        try (var ps = c.prepareStatement(sql)) {
    ...
    private static void applyMutation(
        ...
        try (var ps = c.prepareStatement(sql)) {
            int i = 1;
            ps.setBigDecimal(i++, command.amount());
            ps.setBigDecimal(i++, command.amount());
            ps.setObject(i++, command.sourceAccountId());
            ps.setBigDecimal(i++, command.amount());
            ps.setObject(i++, command.destinationAccountId());
            ps.setString(i++, command.idempotencyKey());
            ps.setObject(i++, actorId);
            ps.setInt(i++, tenantId);
            ps.setObject(i++, command.sourceAccountId());
            ps.setObject(i++, command.destinationAccountId());
            ps.setBigDecimal(i++, command.amount());
            ps.setObject(i++, transferId);
            ps.setBigDecimal(i++, sourceBalance);
            ps.setBigDecimal(i++, destinationBalance);
            ps.setObject(i++, actorId);
            ps.setInt(i++, tenantId);
            ps.setObject(i++, command.sourceAccountId());
            ps.setObject(i++, command.destinationAccountId());
            ps.setBigDecimal(i++, command.amount());
            try (var rs = ps.executeQuery()) {
                if (!rs.next() || !transferId.equals(rs.getObject(1, UUID.class)))
                    throw new IllegalStateException(
                            "PostgreSQL mutation did not return expected transfer identity.");
            }
        }
    }

    private static void ensureReplayMatches(
            IdempotencyRow row, UUID actorId, int tenantId, TransferFunds.Command c) {
        if (!row.actorId().equals(actorId)
                || row.tenantId() != tenantId
                || !row.source().equals(c.sourceAccountId())
                || !row.destination().equals(c.destinationAccountId())
                || row.amount().compareTo(c.amount()) != 0)
            throw new IllegalStateException(
