...
public final class TransferFunds {
    ...
    public interface Authorization {
        boolean canTransfer(UUID actorId, BankAccount source, BankAccount destination);
    }
    ...
    public static final class OwnershipAuthorization implements Authorization {
        public boolean canTransfer(UUID actorId, BankAccount source, BankAccount destination) {
            return source.ownerId().equals(actorId)
                    && destination.ownerId().equals(actorId)
    ...
    public static final class Store {
        ...
        public BankAccount get(UUID id) {
            ...
            if (a == null) throw new IllegalStateException("Account '" + id + "' was not found.");
            return a;
        }

        public void replace(BankAccount account) {
            accounts.put(account.id(), account);
        }

        Object lockFor(UUID id) {
            return locks.computeIfAbsent(id, x -> new Object());
        }

        Idempotency idempotency(String key) {
            return idempotency.get(key);
        }

        void storeIdempotency(String key, Idempotency record) {
    ...
    public Receipt execute(UUID actorId, int tenantId, Command command) {
        ...
        if (command.sourceAccountId().equals(command.destinationAccountId()))
            throw new IllegalStateException("Source and destination accounts must differ.");
        var first =
                command.sourceAccountId().compareTo(command.destinationAccountId()) < 0
                        ? command.sourceAccountId()
                        : command.destinationAccountId();
        var second =
                first.equals(command.sourceAccountId())
                        ? command.destinationAccountId()
                        : command.sourceAccountId();
        synchronized (store.lockFor(first)) {
            synchronized (store.lockFor(second)) {
                var source = store.get(command.sourceAccountId());
                var destination = store.get(command.destinationAccountId());
                validate(actorId, tenantId, source, destination, command.amount());
                var replay = store.idempotency(command.idempotencyKey());
                if (replay != null) {
                    ensureReplayMatches(replay, actorId, tenantId, command);
                    return receipt(replay.result(), command, true);
                }
                ...
                var result =
                        new Result(
                                ...
                                destination.balance().add(command.amount()));
                store.replace(
                        new BankAccount(
                                source.id(),
                                source.tenantId(),
                                ...
                                source.regulatoryHold(),
                                source.dailyTransferred().add(command.amount()),
                                source.dailyLimit(),
                                source.frozen()));
                store.replace(
                        new BankAccount(
                                ...
                                destination.regulatoryHold(),
                                destination.dailyTransferred(),
                                destination.dailyLimit(),
                                destination.frozen()));
    ...
    private void validate(
        ...
        if (!authorization.canTransfer(actorId, source, destination))
    ...
    private static void ensureReplayMatches(Idempotency r, UUID actor, int tenant, Command c) {
        if (!r.actorId().equals(actor)
                || r.tenantId() != tenant
                || !r.source().equals(c.sourceAccountId())
                || !r.destination().equals(c.destinationAccountId())
                || r.amount().compareTo(c.amount()) != 0)
    ...
    private static Receipt receipt(Result r, Command c, boolean replay) {
        return new Receipt(
                ...
                "succeeded",
                replay
                        ? List.of("transferFunds.replay")
                        : List.of(
                                "transferFunds.debit",
                                "transferFunds.credit",
                                "transferFunds.audit"),
                Instant.now());
    }
}
