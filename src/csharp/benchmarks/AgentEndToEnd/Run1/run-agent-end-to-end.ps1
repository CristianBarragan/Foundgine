...
$BenchmarkProject = Join-Path $RepoRoot 'src/csharp/benchmarks/AgentEndToEnd/Run1/Foundgine.AgentEndToEnd.Run1.csproj'
...
$ComposeProjectName = 'foundgine-agent-e2e-run1'
...
Write-Host "Runs per tier:           $($RunsPerTier -join ', ')"
    ...
    for ($index = 0; $index -lt $CustomerCounts.Count; $index++) {
        ...
        foreach ($concurrency in $Concurrency) {
            # Each worker in this harness owns one distinct customer end-to-end
            # (see ResetCustomersAsync/customerIds in Program.cs), so concurrency
            # cannot exceed the customer count for the current tier. Skip rather
            # than let the benchmark process crash with an unhandled exception.
            if ($concurrency -gt $customerCount) {
                Write-Host ''
                Write-Host ">>> Concurrency tier: $concurrency (skipped: exceeds customer count $customerCount)"
                continue
            }
