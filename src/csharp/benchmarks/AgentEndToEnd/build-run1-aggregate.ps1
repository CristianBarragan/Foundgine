...
# Mirrors build-run2-aggregate.ps1: Run1's raw agent-benchmark.json reports
...
#
# Run1 only ever measured 4 of the 16 (customers x concurrency) cells - the
...
if ([string]::IsNullOrWhiteSpace($Run1Root)) { $Run1Root = Join-Path $BenchmarkRoot 'Run1\artifacts' }
if ([string]::IsNullOrWhiteSpace($Destination)) { $Destination = Join-Path $RepoRoot 'docs-site\assets\agent-benchmark\run1-aggregate.json' }

if (-not (Test-Path -LiteralPath $Run1Root -PathType Container)) { throw "Run 1 artifact directory not found: $Run1Root" }

$reportFiles = @(Get-ChildItem -LiteralPath $Run1Root -Recurse -Filter 'agent-benchmark.json' -File)
if ($reportFiles.Count -eq 0) { throw "No agent-benchmark.json files found under: $Run1Root" }
...
$result = [ordered]@{ run = 'Run1'; aggregate = $aggregate }

$dir = Split-Path -Parent $Destination
New-Item -ItemType Directory -Force -Path $dir | Out-Null
$result | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $Destination -Encoding UTF8
Write-Host "Built Run1 aggregate: $Destination ($($aggregate.Count) rows)" -ForegroundColor Green
