...
input: **claims** — extra context the _caller_ asserts on top of that
...
> caller-asserted context that can only ever _narrow_ what the policy already
...
1. **Reserved-identity-key check first, and it's a whole-request failure.**
   ...
   `permissions`, …) _before_ anything else runs. If any reserved key is
   ...
   about which other claims _would_ have been honored, and "a client that
...
2. **Per-key format validation.** Every _recognized_, non-identity key
   ...
   match `^CHG-\d{4,}$`. A malformed value is rejected _individually_; it
...
_any_ `ClaimSchema`, and this file is the only place SupplyChain-specific
claim rules live.
...
| Field rule           | Can this role see/write this _specific field_?                                       | `InventoryLot.Quarantined` is readable only by `WarehouseOperator`/`SupplyChainManager`; `Supplier.RiskScore` only by `Analyst`/`SupplyChainManager` — see `SensitiveFieldAuthorizationTests.cs` for the exhaustive matrix. |
...
| Predicate rule       | What _row-level_ filter applies even when the entity/field/relationship checks pass? | Every `Supplier`/`Warehouse` read gets a `TenantId == context.TenantId` predicate; if the caller's `warehouse` claim was accepted, an additional `WarehouseId == <claimed warehouse>` predicate is **AND**ed on top.        |
| Named-operation rule | Is this specific _named write operation_ (not just "any write") allowed?             | `"inventory.reconcile"` requires the `SupplyChainManager` role **and** both a `reason` and `change_ticket` claim to have survived validation — an ordinary `"update"` on the same entity does not require either.           |
...
`Supplier`/`Warehouse` reads, _and then_, only if the caller supplied a valid
`warehouse` claim, a second predicate is ANDed on top narrowing to that one
warehouse. There's no path where a claim can _replace_ the tenant predicate
...
wrapper _used to_ have — and which we replaced there with compiler-checked
...
sample's own `Infrastructure/README.md` states explicitly: _"there is
...
changes to semantic configuration."_ `SupplyChainSemanticModel.Build()` also
...
project and specifically want compile-time-checked ids for a _structural_
