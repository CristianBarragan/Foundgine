...
Do not add a new sample just to demonstrate another capability. If a capability belongs to the advanced Supply Chain story, add it there. If it is framework-wide behavior, add a test under `tests/`. Benchmark-only applications and compatibility fixtures belong under `src/csharp/benchmarks/`.
